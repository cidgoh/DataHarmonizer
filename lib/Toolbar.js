...
class Toolbar {
  ...
  async saveFile() {
    ...
    const Container = Object.entries(
    ...
    ).reduce((acc, [cls_key, container]) => {

      const fields = this.context.dhs[cls_key].getFields();
      ...
      if (typeof container.range !== 'undefined') {
        return Object.assign(acc, {
          [cls_key]: MultiEntityJSON[container.range].map((obj) =>
            ...
            entry = Object.fromEntries(Object.entries(entry).map(([k,v]) => {
              ...
              if (field.sources && !isEmptyUnitVal(v)) {
                ...
                if (field.multivalued === true) {
                  nv = v.split(MULTIVALUED_DELIMITER).map(_v => titleOverText(merged_permissible_values[_v])).join(MULTIVALUED_DELIMITER);
                } else {
                  nv = titleOverText(merged_permissible_values[v]);
                }
              };
              return [k, nv]
            }))
            console.log('saveFile:Container', entry);
            return entry;
          })
        });
      } else {
  ...
  showExportModal() {
    console.log(
      'showExportModal',
      this.context,
      this.context.getCurrentDataHarmonizer()
    );
    if (
