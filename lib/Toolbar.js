...
class Toolbar {
  ...
  bindEvents() {
    ...
    $('#file-ext-save-as-select').on('change', this.toggleJsonOptions.bind(this));
  ...
  async openFile() {
    ...
    if (!acceptedExts.includes(ext)) {
      ...
      if (ext === 'json') {
        ...
        if (this.context.template.schema.id !== schema || in_language !== previous_language) {
          ...
          await this.context.reload(template_path, locale)
            .then(context => {
              
              for (const dh in context.dhs) {
                const list_data = context.dhs[dh].loadDataObjects(jsonData.Container[dh], locale);
              ...
              // re-render the UI elements given a template change
              // - template name at top
              // - dh sections
              // - export formats
              // - modals

              const template_name = context.template.localized.schema.name;
              ...
              Object.entries(context.exportFormats).forEach(([option, details]) => {
                if (
                  !details.pertains_to ||
              ...
              const schema = context.template.localized.schema;
  ...
  async saveFile() {
    ...
    //   this.context.template.current.schema.classes, 
    //   this.context.template.localized.schema.classes, 
    //   // this.context.template.schema.classes.Container.attributes 
    )
    try {
  ...
  async loadSelectedTemplate(file = null) {
    ...
    const updateGUI = (
      ...
      translationSelect.on('input', async () => {
        ...
        if (supportsLocale) {
          console.log(`${i18next.language} is a supported locale.`);

          function translatableFields(data_harmonizer_fields) {
          ...
          function translateData(is_translatable, data_harmonizer_data) {
                ...
                arr.map((obj, index) => {
                  return obj !== null && is_translatable[index]
                    ? obj.split(MULTIVALUED_DELIMITER).length > 0
                      ? (() => {
                          // console.group();
                          // console.log(i18next.language, i18next.getResource(i18next.language, i18next.language === 'default'? 'reverse' : 'translation'))
                          // console.log(obj);
          ...
          try { 
            ...
            for (const dh in this.context.dhs) {
              ...
              _dh_data[dh] = translateData(translatableFields(fields), data);
            };

            // reload the context with the new locale
            await this.context.reload(current_template, language_update)
              .then(context => {
                for (const dh in context.dhs) {
                  if (!!_dh_data[dh]) {
                    context.dhs[dh].hot.loadData(_dh_data[dh]);
                  }
                }
              });
    ...
    $(document).on('dhCurrentChange', (event, extraData) => {
      console.log('dhCurrentChange', extraData.data);
  ...
  setupSectionMenu(dh) {
    ...
    dh.sections.forEach((section, index) => {
      sectionMenu.append(
        `<div id="show-section-${index}" class="dropdown-item show-section-dropdown-item" data-i18n="${section.title}">${i18next.t(section.title)}</div>`
      );
    });
