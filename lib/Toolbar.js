...
class Toolbar {
  ...
  constructor(root, dh, menu, options = {}) {
    ...
    $fileInput.on('change', function () {
      ...
      if (!acceptedExts.includes(ext)) {
        ...
        dh.invalid_cells = {};
        this.context.runBehindLoadingScreen(dh.openFile, [file]);
    ...
    $('#save-as-confirm-btn').click(() => {
      try {
        ...
        if (ext == 'json') {
          ...
          this.context.runBehindLoadingScreen(exportJsonFile, [data, baseName, ext]);
        } else {
          let matrix = [...dh.getFlatHeaders(), ...dh.getTrimmedData()];
          this.context.runBehindLoadingScreen(exportFile, [matrix, baseName, ext]);
        }
    ...
    $('#export-to-confirm-btn').click(() => {
      ...
      if (exportFormat in dh.export_formats) {
        ...
        this.context.runBehindLoadingScreen(exportFile, [
          outputMatrix,
          baseName,
          format.fileType,
        ]);
  ...
  async validate() {
    await this.context.runBehindLoadingScreen(this.dh.validate);
  ...
  // NOTE: (Ken) see dh refactor transcript for a refactored version of loadSelectedTemplate
  ...
  async loadSelectedTemplate(file = null) {
    ...
    const loadFromMenu = async () => {
      ...
      return { template_name, schema, exportFormats, languages, schemaClass };
    };
  
    let loadResult = file ? await loadFromFile(file) : await loadFromMenu();
    if (!loadResult) return;
    let { template_name, schema, exportFormats, languages, schemaClass, columnCoordinates } = loadResult;
    
    // Use the previously coupled dh methods
    this.dh.useSchema(schema, exportFormats, template_name);
    if (!languages) languages = await this.dh.getLanguages(template_name);
    if (!schemaClass) schemaClass = schema.classes[template_name];
    if (!columnCoordinates) columnCoordinates = this.dh.getColumnCoordinates();
  
    const updateGUI = ({ template_name, schema, exportFormats, languages }) => {
      ...
      this.dh.template.forEach((section, index) => {
        sectionMenu.append(
    ...
    updateGUI({ template_name, schemaClass, exportFormats, languages, columnCoordinates });
  }
  ...
  async loadSelectedTemplate(file = null) {
    ...
    // send the schema to the data harmonizer
    this.dh.useSchema(schema, exportFormats, template_name);
 
    // Enable template folder's export.js export options to be loaded dynamically.
    const selectExportFormat = $('#export-to-format-select');
    selectExportFormat.children(':not(:first)').remove();
    Object.entries(this.dh.export_formats).forEach(([option, details]) => {
      if (!details.pertains_to || details.pertains_to.includes(template_name)) {
        selectExportFormat.append(new Option(option, option));
      }
    });
    ...
    // Update SOP
    const helpSop = $('#help_sop');
    if (schemaClass.see_also) {
      helpSop.attr('href', schemaClass.see_also).show();
    } else {
      helpSop.hide();
    }
