...
*/
  ...
  showSectionColumns(event) {
    ...
    if (checkedTitles.length === 0) {
      ...
      Object.keys(this.context.dhs).forEach((dh) => {
        this.context.dhs[dh].showAllColumns();
      });
    } else {
      ...
      $('#show-all-cols-dropdown-item, #show-required-cols-dropdown-item, #show-recommended-cols-dropdown-item')
        .removeClass('disabled');
      Object.keys(this.context.dhs).forEach((dh) => {
        this.context.dhs[dh].showColumnsBySectionTitles(checkedTitles);
      });
    }
  }
  ...
  setupSectionMenu(dh) {
    ...
    dh.sections.forEach((section, index) => {
      sectionMenu.append(
        `<div id="show-section-${index}" class="dropdown-item show-section-dropdown-item" data-section-title="${section.title}" style="display:flex;justify-content:space-between;align-items:center;gap:0.5rem">` +
