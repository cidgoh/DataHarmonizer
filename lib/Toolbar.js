    ...
    const Container = Object.entries(
      ...
      if (typeof container.range !== 'undefined') {
        return Object.assign(acc, {
          ...
          .map((obj) => {
            const fields = this.context.dhs[key].getFields();
            ...
            obj = Object.fromEntries(Object.entries(obj).map(([k,v]) => {
              ...
              let nv = v;
              if (typeof field.sources !== 'undefined') {
                console.log(k, v, field.sources, field);
              }
              if (field.flatVocabulary?.length > 0) {
                nv = field.vocabTextToTitle[v];
              }
              return [k, nv]
            }))
            return obj;
