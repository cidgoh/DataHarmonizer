...
class Toolbar {
  ...
  initialize(options) {
    ...
    translationSelect.on('input', async () => {
      ...
      const supportsLocale =
        targetLocale !== null;
      const translationNamespace = previousLocale != targetLocale ? `${previousLocale}_to_${targetLocale}` : 'translation';
      ...
      const translateData = (isTranslatable, dataHarmonizerData, namespace='translation') => {
        // Helper function to translate a single value based on the current language and namespace
        const translateValue = (value) => {
          console.log("executing translateValue for", value, i18next.options.resources);
          console.log(value, previousLocale, targetLocale, i18next.t(value, { lng: i18next.language, ns: 'reverse' }), i18next.t(value, { lng: i18next.language, ns: 'translation' }));
        ...
        return dataHarmonizerData.map((row, row_index) => {
          return row.map((value, index) => {
            if (value != null && value != "" && isTranslatable[index]) {
              console.log(`translating ${value} at ${index} (${this.context.getCurrentDataHarmonizer().hot.getDataAtCell(row_index, index)})`)
            }
      ...
      if (supportsLocale) {
        try {
          ...
          for (const dh in this.context.dhs) {
            ...
            const translatableFields = findTranslatableFields(fields);
            _dh_data_cache[dh] = translateData(translatableFields, data, translationNamespace);
          }
          ...
          await this.context
            ...
            .then((context) => {
              for (const dh in context.dhs) {
                ...
                this.setupJumpToModal(context.dhs[dh]);
                this.setupFillModal(context.dhs[dh]);
              }
            });
  ...
  async saveFile() {
    ...
    if (ext === 'json') {
      ...
      const slotNamesForTitles = this.context.getCurrentDataHarmonizer().getFieldTitleMap(fields);

      const filterEmptyKeys = (obj, keyCallback = id => id) => Object.keys(obj).reduce((acc, itemKey) => {
        return itemKey in obj && !(itemKey in acc) && (obj[itemKey] != "") ? {
          ...acc,
          [keyCallback(itemKey)]: obj[itemKey],
        } : acc;
      }, {});
      
      const filterEmptyKeysForObjectsInList = lst => lst.map(el => filterEmptyKeys(el, em => slotNamesForTitles[em]));

      for (let concept in JSONFormat.Container) {
        JSONFormat.Container[concept] = filterEmptyKeysForObjectsInList(JSONFormat.Container[concept]);
      };
  ...
  updateGUI(dh, template_name, exportFormats) {
  ...
  setupFillModal(dh) {
    ...
    // clear options before providing them to update when active data harmonizer changes
    fillColumnInput[0].selectize.clearOptions();
    fillColumnInput[0].selectize.addOption(dh.getFields());
  ...
  setupJumpToModal(dh) {
    console.log('running jump to modal', dh.class_assignment);
    const columnCoordinates = dh.getColumnCoordinates();

    let jumpToInput = $('#jump-to-input')
      .selectize({
        openOnFocus: true,
      })

    // clear options before providing them to update when active data harmonizer changes
    jumpToInput[0].selectize.clearOptions();
    jumpToInput[0].selectize.addOption(Object.keys(columnCoordinates).map(col => ({
      text: col,
      value: col,
    })));

    $('#jump-to-modal').on('change', (e) => {
