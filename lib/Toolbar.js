...
class Toolbar {
  ...
  bindEvents() {
    ...
    // Slot tab will pay attention to selected schema and table (if any) by default, but it is possible to select from all schemas, all tables, and/or slot groups.
    $('#tab_report_select_type').on('change', (event) => {
      let dh = this.context.getCurrentDataHarmonizer();
      let report_type = $('#tab_report_select_type').val();
      // Visibility of "focused table" menu depends on selection of slot_usage or attribute report.
      $('#slot_report_class').toggle(['slot_usage','attribute'].includes(report_type));

      // Refresh #slot_report_schema select options (after +/- schema)
      this.regenerate_select(dh, 'Schema', '#slot_report_schema', 'name');
      // Refresh #slot_report_class select options (after +/- table)
      this.regenerate_select(dh, 'Class', '#slot_report_class', 'name');//title
      // Refresh #slot_report_slot select options
      this.regenerate_select(dh, 'Slot', '#slot_report_slot', 'name'); //title
      // Refresh #slot_report_slot_group select options 
      this.regenerate_select(dh, 'Slot', '#slot_report_slot_group', 'slot_group');

      // This function has special code for DH_LinkML Slot tab.
  ...
  async openDataFile() {
    ...
    // Here we're loading tsv, csv, xls, xlsx
      ...
      for (const dh of Object.values(this.context.dhs)) {
        ...
        dh.current_selection = [null, null, null, null];
        dh.clearValidationResults();
        await dh.openDataFileForTable(file);
      }
    }
  ...
  async saveDataFile() {
    ...
    const JSONFormat = {
      ...
      Container: this.getContainerData(schema_container, ext)
    };
   ...
   * 
   * @param {Object} schema_container - A schema's defined Container class where details of which classes to include in one or one-to-many tabular data saved format are contained.
