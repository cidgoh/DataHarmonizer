          ...
          return obj !== null && is_translatable[index]
            ? obj.split(MULTIVALUED_DELIMITER).length > 0
              ...
              : i18next.t(obj, {
                  lng: i18next.language,
                  ns:
                    i18next.language === 'default' ? 'reverse' : 'translation',
                })
            : obj;
        })
      );
        ...
        try {
          ...
          await this.context
            ...
            .then((context) => {
              console.log('context.dhs', context.dhs);
              for (const dh in context.dhs) {
                console.log('context.dhs data cache '+dh, _dh_data_cache[dh]);
