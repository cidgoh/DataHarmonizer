...
import {isEmpty, deleteEmptyKeyVals, nullValuesToString, isEmptyUnitVal} from '../lib/utils/general';
...
//            alert(`Unable to fetch table data from JSON file {template_path} for {dh}`);
  ...
  async openTabularDataFile(file) {
    ...
    const fieldMapper = new FieldMapper(this.context);
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      this.clearDH(dh.template.name);  // add ,false to do away with dh settings.

      let sheet_name = dh.getSpreadsheetName(workbook);
      if (!sheet_name) {
        alert(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO corresponding table in the uploaded data file.  The data file's tabs or tables are: "${workbook.SheetNames}".  Consequently, no data will be loaded.  Was the right schema loaded?`);
      }
      ...
      if (header_row > 0 && schema_slot_count == matches) { 
        ...
        // an empty table.
        if (matches == 0) {
          alert(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO matching fields with the corresponding data file tab or table. This DH tab will be left with no data.`);
        }
        else {
          // Have user try to correct any missing mappings. This doesn't handle
          // rarity of data file having duplicate column names.
          // If user cancels at this point, some tables are simply left without
          // data. Force mapping form to be completed?
          // Note matrix might not have section headers, so don't make code
          // dependent on them.
          const data_fields = matrix[header_row-1];
          fieldMapper.field_mapping_html += dh.appendFieldMappingModal(data_fields);
    ...
    if (fieldMapper.field_mapping_html) {
      ...
      fieldMapper.initFieldMappingModal(file.name, fieldMapper.field_mapping_html);
