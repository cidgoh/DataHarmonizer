...
import { findBestLocaleMatch, templatePathForSchemaURI } from '@/lib/utils/templates';
...
class Toolbar {
    ...
    if (!acceptedExts.includes(ext)) {
      ...
      if (ext === 'json') {
        ...
        if (
          ...
          if (!template_path) {
            alert(
              `The schema ${schema} cannot be found within the manifest.`
            );
            return false;
          }  
          ...
          await this.context
            ...
            .then((context) => {
              ...
              const rangeToContainerClass  = (cls_key) => {
                return Object.values(context.template.default.schema.classes.Container.attributes).filter(v => 'range' in v).filter(v => v.range === cls_key)[0].name;
              };

              for (const dh in context.dhs) {
                const list_data = context.dhs[dh].loadDataObjects(
                  jsonData.Container[rangeToContainerClass(dh)],
                  locale
                );
  ...
  async saveFile() {
    ...
    function titleOverText(key, enm) {
      return typeof enm !== 'undefined' && typeof enm.title !== 'undefined'
        ? enm.title
        : key;
    }
    ...
    const Container = Object.entries(
      ...
      if (typeof range !== 'undefined') {
        ...
        return Object.assign(acc, {
          [rangeToContainerClass(cls_key)]: MultiEntityJSON[range]
                  ...
                  if (field.sources && !isEmptyUnitVal(v)) {
                    ...
                    if (field.multivalued === true) {
                      nv = v
                        .split(MULTIVALUED_DELIMITER)
                        .map((_v) =>
                          titleOverText(_v, merged_permissible_values[_v])
                        )
                        .join(MULTIVALUED_DELIMITER);
                    } else {
