...
class Toolbar {
  ...
  bindEvents() {
    ...
    */
    ...
    /**************************** Field Mapping Modal ************************
     * See also: initSpecifyHeadersModal(), showSpecifyHeadersModal()
     */
    ...
    // "Forgets" selected user-specified mapping profile by erasing it in 
    // localStorage.
    // This doesn't affect maping rendered in form.
    $('#field-mapping-forget-btn').click(() => {
      const profile_name = $('#field-mapping-select').val();
      if (profile_name) {
    ...
    $('#field-mapping-store-btn').click(() => {
      const profile_name = $('#field-mapping-name').val();
      if (profile_name) {
  ...
  // Provides alot of initializations if not yet accessed.
  ...
  updateProfileMapping(profile_name) {
    const [dh_settings, profile] = this.getProfile(profile_name);
    let mapping = [];
    $('table#field-mapping-table td:first-child.field-mismatch')
    .each((index, slot_field) => {
      const data_field = $(slot_field).next('td');
      const slot_label = $(slot_field).text().split(' ')[1]; // get past column id
      const data_label = $(data_field).text().split(' ')[1];
      if (data_label) {
        mapping.push({'from': slot_label, 'to': data_label});
      }
    })
  ...
  saveBrowserDHSettings(dh_settings) {
    console.log("SAVING", dh_settings)
...
//            alert(`Unable to fetch table data from JSON file {template_path} for {dh}`);
  ...
  async openTabularDataFile(file) {
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      if (header_row > 0 && schema_slot_count == matches) { 
        ...
        header_mapping_html += dh.appendSpecifyHeadersModal(data_fields);
        // Insert all of the above section html before end of mapping table.
      }
    }

    if (header_mapping_html) {
      $('#field-mapping-table thead').after(header_mapping_html);
      this.showSpecifyHeadersModal();
    }

  }

  initSpecifyHeadersModal(file_name) {
    // Indicates uploaded data file name in field mapping modal.
    $('#field-mapping-data-file-name').text(file_name);
    // Keep first option instructions and 2nd option [no template]
    $('#field-mapping-select options').slice(2).remove(); 

    const dh_settings = this.readBrowserDHSettings();
    Object.entries(dh_settings.mapping_profiles || {}).forEach(([name, value]) => {
  ...
  showSpecifyHeadersModal() {

    // To enable drag-drop on newly-created elements in specify-headers-modal
