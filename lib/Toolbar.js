...
class Toolbar {
  ...
  updateProfileMapping(profile_name) {
    ...
    $('table#field-mapping-table tbody').each((t_index, tbody) => {
      ...
      const section_name =  $(tbody).attr('data-section');
      $(tbody).find('td:first-child.field-mismatch').each((index, slot_field) => {
        const data_field = $(slot_field).next('td');
        // Retrieve labels, get past column id;
        const slot_label = $(slot_field).text().split(' ',1)[1]; 
        const data_label = $(data_field).text().split(' ',1)[1];
        if (data_label) {
          mapping.push({
...
//            alert(`Unable to fetch table data from JSON file {template_path} for {dh}`);
  ...
  initFieldMappingModal(file_name, field_mapping_html) {
    ...
    // Populate schema mapping profiles. Keep first option instructions.
  ...
  renderFieldMappingModal(profile_name = null) {
    ...
    if (profile_name) {
      const [dh_settings, profile] = this.getProfile(profile_name);
      Object.entries(profile.mapping).forEach((mapping) => {
        // Find mapping.from text. mapping.from td will be accompanied
        // by an empty td.

        console.log('MAPPING',mapping)
        const tbody = $(`table#field-mapping-table tbody[data-template="${mapping.template_name}"][data-section="${mapping.section_name}"]`)
        // Slot td will have empty data field td.

        const schema_data_td = $(tbody)
          .find('td:eq(1).field-mismatch')
          .filter(function() {return $(this).text().endsWith(') ' + mapping.to)});
        const data_td_text = schema_data_td.text();
        schema_data_td.text = '';

        const schema_slot_data_td = $(tbody)
          .find('td:first-child.field-mismatch')
          .filter(function() {return $(this).text().endsWith(') ' + mapping.from).next('td')});
        schema_slot_data_td.text(data_td_text);
