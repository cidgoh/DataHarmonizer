...
//            alert(`Unable to fetch table data from JSON file {template_path} for {dh}`);
  ...
  async openTabularDataFile(file) {
    ...
    const fieldMapper = new FieldMapper(this.context, file.name);
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      if (!sheet_name) {
        ...
        if (header_row > 0 && schema_slot_count == matches) { 
          ...
          // an empty table.
          if (matches == 0) {
            alert(`The LinkML schema "${this.context.getSchemaRef().name}" template "${dh.template.name}" has NO matching fields with the corresponding data file tab or table. This DH tab will be empty.`);
          }
          else {
            ...
            // Matrix might not have section headers, so don't make code
            // dependent on them.
            const data_fields = matrix[header_row-1];
            fieldMapper.appendFieldMappingModal(dh, data_fields);
          }
        }
      }
    }
