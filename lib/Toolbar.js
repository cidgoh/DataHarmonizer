...
import { exportFile, exportJsonFile, readFileAsync, createWorkbookFromJSON, exportWorkbook } from '@/lib/utils/files';
...
class Toolbar {
  ...
  constructor(root, context, menu, options = {}) {
    ...
    $('#save-as-confirm-btn').on('click', () => {
      // TODO: multiple sheet aggregation
      // const dh = self.context.getCurrentDataHarmonizer();

      const MultiEntityJSON = Object.values(self.context.dhs).reduce((acc, dh) => {
      ...
      try {
        ...
        if (ext == 'json') {
          ...
          const MultiEntityWorkbook = createWorkbookFromJSON(MultiEntityJSON);
          console.log(MultiEntityWorkbook)
