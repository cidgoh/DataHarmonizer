...
import { MULTIVALUED_DELIMITER, titleOverText, rangeToContainerClass } from '@/lib/utils/fields';
        ...
        if (
          ...
          await this.context
            ...
            .then((context) => {
              ...
              for (const dh in context.dhs) {
                const container_class = rangeToContainerClass(context.template.default.schema.classes.Container, dh);
                  ...
                  if (field.sources && !isEmptyUnitVal(v)) {
                    ...
                    if (field.multivalued === true) {
                      nv = v
                        .split(MULTIVALUED_DELIMITER)
                        .map((_v) =>
                          _v in merged_permissible_values ? titleOverText(merged_permissible_values[_v]) : _v
                        )
                        .join(MULTIVALUED_DELIMITER);
                    } else {
                      nv = v in merged_permissible_values ? titleOverText(merged_permissible_values[v]) : v;
                    }
                  }
                  return [k, nv];
                })
