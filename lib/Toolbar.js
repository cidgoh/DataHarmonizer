...
import { translateObject, findLocalesForLangcodes, interface_translation } from './utils/i18n';
    ...
    const updateGUI = ({
      ...
      translationSelect.on('input', () => {
        ...
        for (const dh in this.context.dhs) {
          ...
          // translate old data into new language target
          const _i = data.map(arr => arr.map(obj => 
            obj !== null ? 
