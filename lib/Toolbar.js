...
import {isEmpty} from '../lib/utils/general';
import {
  exportFile,
  exportJsonFile,
  readFileAsync,
...
class Toolbar {
  ...
  bindEvents() {
    ...
    $('#translation-save').on('click', this.translationSave.bind(this));
    ...
    $('#save-template-button').on('click', (event) => {
      let dh = this.context.getCurrentDataHarmonizer().saveSchema();
    });
    ...
    // If user double-clicks search field, this advances to next hit.
  ...
  createNewFile(e) {
    ...
    if (e.target.id === 'new-dropdown-item' && isNotEmpty) {
      ...
      //dh.newHotFile(); // Only called from here.
    }
  }
  ...
  // Triggers only for loaded top-level template, rather than any dependent.
  ...
  async openFile() {
    ...
    if (ext === 'json') {
      ...
      if (schema_uri != this.context.template.default.schema.id) {
        alert(
          `The current json file's schema "${schema_uri}" needs to be loaded first, either by selecting it in the template menu, or by using the "Upload Template" option.`
        );
        return false;
      }
    ...
    // Here we're loading tsv, csv, xls, xlsx
    else {
      ...
      for (const dh of Object.values(this.context.dhs)) {
        ...
        dh.current_selection = [null, null, null, null];
        dh.clearValidationResults();
        // Issue: this is flashing for each tab/dh instance.
        //await this.context.runBehindLoadingScreen(dh.openFile.bind(dh), [file,]);
        await dh.openFile(file);
      }
    }

    $('#file_name_display').text(file.name);
      
    return false; // Is this doing anything?
  }
  ...
  async saveFile() {
    ...
    const Container = this.getContainerData(schema_container, ext);
    ...
    if (ext === 'json') {
      ...
      // Future: simplify these three below into one.
      ...
      // Unlike tabular formats, JSON format doesn't need empty slots.
      const processEntryKeys = (lst) => lst.map(filterEmptyKeys);

      for (let class_name in JSONFormat.Container) {
        JSONFormat.Container[class_name] = processEntryKeys(
  ...
  /** Look through a schema Container's attributes and see if one has a range
   ...
   * but may be more arbitrary.  Used directly in JSON output.
   * @param {Object} schema_container - part of a json file data format.
   * @param {String} ext - file type controls how column headers should be named.
   */
  getContainerData(schema_container, ext) {

    // Return dictionary of {[dh.template_name]: [row 0...n: alphabetical array of slot values]
    const ClassDataRowSlotValues = Object.values(this.context.dhs).reduce(
      (acc, dh) => {
    ...
    return Object.entries(schema_container.attributes).reduce(
      (acc, [cls_key, { name, range }]) => {

        if (typeof range !== 'undefined' && this.context.dhs[range]) {
          const dh = this.context.dhs[range]

          // Given container class [name], fetch slots 
          const processedClass = {
            // April 1 2025: CHANGED TO [range] from [name] 
            [name]: ClassDataRowSlotValues[range]
              ...
              .map((row_dict) => {                
                row_dict = Object.fromEntries(
                  Object.entries(row_dict).map(([slot_name, value]) => {
                    ...
                    let nv = value;
                    if (slot.sources && !isEmptyUnitVal(value)) {
                      ...
                      if (slot.multivalued === true) {
                        ...
                        if (!(value in merged_permissible_values)) {
                          console.warn(
                            `${value} not in merged_permissible_values ${Object.keys(
                              merged_permissible_values
                            )}`
                          );
                        }
                        nv =
                          value in merged_permissible_values
                            ? titleOverText(merged_permissible_values[value])
                            : value;
                      }
                    }
                    /* HANDLE conversion of numbers and booleans by slot datatype.
                    Number(row[i]); // Convert to number
                    */
  ...
  translationSave() {
    ...
    let class_name = this.context.current_data_harmonizer_name; // eg. Slot
  ...
  async loadSelectedTemplate(file = null) {
    ...
    $(document).on('dhCurrentChange', (event, extraData) => {
      ...
      extraData.dh.clearValidationResults();
    });
