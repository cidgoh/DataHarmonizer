...
class Toolbar {
  ...
  async saveFile() {
    ...
    // Look through each container and see if attribute class's range is in active
    // data harmonizer list of instances.
  ...
  async loadSelectedTemplate(file = null) {
    ...
    console.log(template_name, schema.classes)
    if (!(template_name in schema.classes)) {
      alert("The schema template menu lists a template name that is not present in the schema.  The menu likely needs to be updated by a DataHarmonizer schema maintainer.");
      return;
    }
