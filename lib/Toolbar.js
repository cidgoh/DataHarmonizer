...
import gettingStarted from './toolbarGettingStarted';
...
class Toolbar {
  ...
  constructor(root, dh, menu) {
    ...
    $('#getting-started-carousel-container').html(gettingStarted);

    // Select menu for available templates
    this.templateOptions();
    this.template_path = $('#select-template').val();
    this.loadSelectedTemplate();
    this.refresh();

    // Enable template to be loaded dynamically
    $('#select-template-load').on('click', async () => {
      this.template_path = $('#select-template').val();
      this.loadSelectedTemplate();
    });
    // Triggers show/hide of draft templates
    $('#view-template-drafts').on('change', this.templateOptions());
    ...
    $('#export-to-confirm-btn').click(() => {
      ...
      if (exportFormat in dh.export_formats) {
        ...
        dh.runBehindLoadingScreen(dh.exportFile, [
          outputMatrix,
          baseName,
          format.fileType,
        ]);
    ...
    $(this.root).on('click', showColsSelectors.join(','), function (e) {
    ...
    $fillColumnInput.bind(
      'focus',
      () => void $fillColumnInput.autocomplete('search')
    );
  ...
  refresh() {
    ...
    let path_array = this.template_path.split('/');
    ...
    // TODO: this uses the autocomplete from jquery-ui. Can we get
    // away with a less heavy dependency?
    // // Settings -> Jump to...
  ...
  loadSelectedTemplate() {
    const [folder, name] = this.template_path.split('/');
    this.dh.schema = this.menu[folder].schema;
    this.dh.processTemplate(name);
  }
}
