...
import './toolbar.css';
import { callIfFunction } from './utils/general';
...
class Toolbar {
  ...
  async loadSelectedTemplate() {
    ...
    const [template_folder, template_name] = template_path.split('/');

    // Update DataHarmonizer
    const menuItem = this.menu[template_folder];
    const schema = await callIfFunction(menuItem.schema);
    const exportFormats = await callIfFunction(menuItem.exportFormats);
    ...
    // Update SOP.
    $('#help_sop').attr('href', `${this.staticAssetPath}${template_folder}/SOP.pdf`);
