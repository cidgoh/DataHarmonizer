...
class Toolbar {
  ...
  initialize(options) {
    ...
    // If a schema was supplied directly (e.g. preview popup), register it in
    // the local menu so it appears in the template dropdown, matching the
    // behaviour of URL / file loading.
    if (options.forcedSchema) {
      this._buildMenuEntryFromSchema(options.forcedSchema, 'preview://');
    }
  ...
  bindEvents() {
    ...
    $('#load-data-manager-button').on('click', () => {
    ...
    $('#preview-schema-button').on('click', () => {
      this.context.schemaEditor.previewSchema();
    });
