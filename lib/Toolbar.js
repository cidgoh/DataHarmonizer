...
class Toolbar {
  ...
  async saveFile() {
    ...
    if (ext === 'json') {
      ...
      const slotNamesForTitles = this.context
        .getCurrentDataHarmonizer()
        .getFieldTitleMap(fields);

      const filterEmptyKeys = (obj, keyCallback = (id) => id) =>
      ...
      const filterEmptyKeysForObjectsInList = (lst) =>
        lst.map((el) => filterEmptyKeys(el, (em) => slotNamesForTitles[em]));

      for (let concept in JSONFormat.Container) {
        JSONFormat.Container[concept] = filterEmptyKeysForObjectsInList(
  ...
  setupFillModal(dh) {
    ...
    $('#fill-modal').on('shown.bs.modal', () => {
      ...
      selectizeInstance.addOption(this.context.getCurrentDataHarmonizer().getFields());
    ...
    $('#fill-button').on('click', () => {
      ...
      this.context.getCurrentDataHarmonizer().fillColumn(column, value);
  ...
  setupJumpToModal(dh) {
    const columnCoordinates = this.context.getCurrentDataHarmonizer().getColumnCoordinates();
