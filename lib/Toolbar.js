      ...
      render: {
        item: function (data, escape) {
          ...
          const label = data.text.replace(
            data.text.replace(' . . ', ''),
            i18next.t(data.text.replace(' . . ', '').replace(/ /g, '_'), {
              defaultValue: data.text,
            })
          );
          // TODO: data-i18n use translation?
          return (
            `<div data-i18n='${
              data.text.startsWith(' . . ')
                ? data.text.replace(' . . ', '').replace(/ /g, '_')
                : data.text
            }'>` +
            escape(label) +
            `</div>`
        ...
        option: (data, escape) => {
          ...
          const label = data.text.replace(
            data.text.replace(' . . ', ''),
            i18next.t(data.text.replace(' . . ', '').replace(/ /g, ' '), {
              defaultValue: data.text,
            })
          );
    ...
    $fillColumnInput.selectize({
      ...
      render: {
        item: function (data, escape) {
          // Some `data.text` input is already prepended with spaces, so we replace the translation
          // ISSUE: a more consistent approach might use a formatter but this is simple. ;[prepend]indent
          const label = i18next.t(data.alias, {
            defaultValue: data.title,
          });
          // TODO: not rerendering
          return `<div data-i18n='${data.alias}'>` + escape(label) + `</div>`;
        },
        option: (data, escape) => {
          // Some `data.text` input is already prepended with spaces, so we replace the translation
          // ISSUE: a more consistent approach might use a formatter but this is simple. ;[prepend]indent
          ...
          return `<div style="padding-left:${indentation}px" class="option ${
            value === '' ? 'selectize-dropdown-emptyoptionlabel' : ''
          }" data-i18n='${data.alias}'>${escape(label)}</div>`;
        },
      },
    });
