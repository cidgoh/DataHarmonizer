...
import { isOcaBundle, ocaToLinkml } from './OcaToLinkml';
import {
  exportFile,
  exportJsonFile,
  readFileAsync,
...
import { dhAlert, dhConfirm } from '../lib/utils/dialog';
...
const SCHEMA_EDITOR_EXPERT_TABS = '#tab-bar-Prefix, #tab-bar-UniqueKey, #tab-bar-Annotation, #tab-bar-EnumSource, #tab-bar-Extension, #tab-bar-Setting';

/**
 * Attempt to interpret a JSON text as a schema.
 *
 * Returns one of:
 *   { yaml: string }              — OCA bundle successfully converted to LinkML YAML
 *   { error: string, title: string } — problem; show to user as a dialog
 */
function _processSchemaJson(text) {
  let obj;
  try {
    obj = JSON.parse(text);
  } catch (e) {
    return { error: 'The file could not be parsed as JSON:\n' + e.message,
             title: 'Invalid JSON' };
  }
  if (isOcaBundle(obj)) {
    return { yaml: YAML.stringify(ocaToLinkml(obj)) };
  }
  // Detect older DataHarmonizer / LinkML schema serialised as JSON (deprecated format).
  if (obj && typeof obj === 'object' && typeof obj.name === 'string' &&
      (obj.classes !== undefined || obj.slots !== undefined)) {
    return {
      error: 'This appears to be a DataHarmonizer schema in the older deprecated JSON format.\n\n' +
             'Please use the YAML version of this schema instead.',
      title: 'Deprecated schema format'
    };
  }
  return {
    error: 'The selected file is not a recognised schema format.\n\n' +
           'Supported formats: YAML (LinkML), JSON (OCA bundle).',
    title: 'Unrecognised schema format'
  };
}

/** Show the OCA-loaded toast for 5 s then fade it out. */
let _ocaMessageTimer = null;
function _showOcaLoadedMessage() {
  if (_ocaMessageTimer) clearTimeout(_ocaMessageTimer);
  $('#oca-loaded-message').stop(true, true).show();
  _ocaMessageTimer = setTimeout(() => {
    $('#oca-loaded-message').fadeOut(400);
    _ocaMessageTimer = null;
  }, 5000);
}
...
class Toolbar {
  ...
  initialize(options) {
    ...
    // If a schema was supplied directly (e.g. demo popup), register it in
    // the local menu so it appears in the template dropdown, matching the
    // behaviour of URL / file loading.
    if (options.forcedSchema) {
      this._buildMenuEntryFromSchema(options.forcedSchema, 'demo://');
    }
    ...
    // In demo mode hide template-loading controls (no switching allowed).
    if (options.forcedSchema) {
      $('#load-template-section').hide();
    }

    // The template was already loaded by AppContext.reload() in web/index.js
    // before Toolbar was constructed. Just set up the GUI state; no second
    // network fetch or context.reload() needed.
    this._loadAppSettings(); // restore persisted user preferences before first render
    this.restartInterface(this.context);
  ...
  initializeLocale() {
    ...
    translationSelect.on('input', async () => {
      ...
      Object.values(this.context.dhs).forEach((d) => this._buildSectionMenuHTML(d));
      this.applySectionMenu(initialDh);
      this.setupJumpToModal();
      this.setupFillModal();
    });
  }
  ...
  bindEvents() {
    ...
    $('#help_contact').on('click', async () => {
      const lines = [
        i18next.t('template-contact-blurb', { defaultValue: '' }),
        i18next.t('dataharmonizer-contact-blurb', { defaultValue: '' }),
      ].filter(Boolean);
      await dhAlert(lines.join('\n\n'), { title: i18next.t('help_contact', { defaultValue: 'Contact Us' }) });
    });
    ...
    $('#schema_expert').on('click', () => {
      ...
      // Re-render the Field/Slot tab so cells() re-evaluates readOnly state.
      this.context.dhs?.['Slot']?.hot?.render();
      this._saveAppSetting('expert_user', expert_user);
    });

    $('#schema_linkml_terms').on('click', () => {
      this.context.use_linkml_terms = $('#schema_linkml_terms').is(':checked');
      // Re-label tabs: data-label-template stores the class name; look up in DH_TERMS.
      const _terms = this.context.getDhTermVars();
      $('[data-label-template]').each((_, el) => {
        const cname = el.getAttribute('data-label-template');
        el.textContent = _terms[cname] ?? cname;
      });
      // Refresh column headers in all Schema Editor DH instances.
      // getNestedHeaders() applies applyDhTerms only when schemaEditor is set,
      // so this is a no-op in regular DataHarmonizer mode.
      if (this.context.schemaEditor) {
        for (const dh of Object.values(this.context.dhs)) {
          if (dh.hot) {
            dh.hot.updateSettings({ nestedHeaders: dh.getNestedHeaders() });
          }
        }
      }
      this._saveAppSetting('use_linkml_terms', this.context.use_linkml_terms);
    });

    // Block the file picker immediately if the focused Schema row already has
    // data — prevents the user having to cancel a file dialog before seeing the
    // error.  The right-click context menu path (DataHarmonizer.js load_schema
    // callback) is intentionally left unchecked so reload-in-place still works.
    $('#upload-template-yaml-dropdown-item').on('click', (event) => {
      if (self.context.schemaEditor && !self.context.schemaEditor.schemaRowFreeForLoad()) {
    ...
    $('#schema_upload').on('change', (event) => {
      ...
      reader.addEventListener('load', (event2) => {
        ...
        if (file.name.toLowerCase().endsWith('.json')) {
          // JSON file: attempt OCA conversion; reject anything else with a clear message.
          const result = _processSchemaJson(event2.target.result);
          if (result.error) {
            dhAlert(result.error, { title: result.title });
            return;
          }
          self.context.schemaEditor.loadSchemaYAML(result.yaml);
          _showOcaLoadedMessage();
        } else {
    ...
    $('#load-schema-url-confirm-btn').on('click', async () => {
      ...
      try {
        ...
        // Detect JSON by URL suffix (Content-Type is unreliable for raw file hosts).
        if (url.split('?')[0].toLowerCase().endsWith('.json')) {
          const result = _processSchemaJson(text);
          if (result.error) {
            dhAlert(result.error, { title: result.title });
            return;
          }
    ...
    $('#demo-schema-button').on('click', () => {
      this.context.schemaEditor.demoSchema();
    });

    $('#compare-schemas-button').on('click', () => {
      this.context.schemaEditor.openCompareModal();
    });

    $('#compare-schemas-run-btn').on('click', () => {
      const schemaA = $('#compare-schema-a-select').val();
      const schemaB = $('#compare-schema-b-select').val();
      if (!schemaA || !schemaB) return;
      if (schemaA === schemaB) {
        $('#compare-schemas-status').text('Please select two different schemas.').show();
        $('#compare-schemas-results').hide();
        return;
      }
      $('#compare-schemas-status').text('Building report…').show();
      $('#compare-schemas-results').hide();
      setTimeout(() => {
        try {
          const results = this.context.schemaEditor.compareSchemas(schemaA, schemaB);
          const html = this.context.schemaEditor.renderCompareReport(results, schemaA, schemaB);
          $('#compare-schemas-results').html(html).show();
          $('#compare-schemas-status').hide();
        } catch (e) {
          $('#compare-schemas-status').text('Error: ' + e.message).show();
        }
      }, 0);
    });

    $('#copy-to-schema-menu-button').on('click', () => {
      const dh = this.context.getCurrentDataHarmonizer();
      this.context.schemaEditor.openCopyToSchemaModal(dh);
    });

    $('#copy-to-schema-confirm-btn').on('click', () => {
      this.context.schemaEditor.executeCopyToSchema();
    });
    ...
    $('#reset-col-widths-dropdown-item').on('click', () => {
      for (const dh of Object.values(this.context.dhs)) {
        dh.resetColWidths();
      }
    });
    // stopPropagation keeps the Settings dropdown open when toggling the section list.
    $('#section-label').on('click', (e) => e.stopPropagation());
    ...
    $('#search-field').on('keyup search', (event) => {
      ...
      // search.query() returns all matching cells including rows hidden by the
      // Display filter.  Restrict navigation results to visible rows only so
      // that hidden rows are neither navigated to nor revealed by scrollTo().
      const allResults = search.query(event.target.value); // Array of (row, col, data)
      const hiddenRowsPlugin = dh.hot.getPlugin('hiddenRows');
      const hiddenSet = hiddenRowsPlugin
        ? new Set(hiddenRowsPlugin.getHiddenRows())
        : new Set();
      dh.queryResult = allResults.filter(r => !hiddenSet.has(r.row));
      dh.hot.render();
      const hasVisible  = dh.queryResult.length > 0;
      const hasHidden   = allResults.length > dh.queryResult.length;
      const hasQuery    = event.target.value.length > 0;
      $('#previous-search-button, #next-search-button').toggle(hasVisible);
      $('#search-hidden-hint').toggle(hasHidden && hasQuery);
    });
    ...
    // Display dropdown menu: radio buttons for main type, checkboxes for
    // Fields-by-type (Slot tab only).  Stop propagation on input clicks so
    // the dropdown stays open while the user makes multiple selections.
    $('#display-dropdown-btn-group input').on('click', (e) => {
      e.stopPropagation();
    });
    $('#display-dropdown-btn-group input').on('change', () => {
      // Sync concise-view checkbox FIRST so refreshTabDisplay → _applyConciseView
      // reads the already-updated state and applies or removes FK column hiding.
      this._syncConciseViewToDisplayMode();
      this.context.refreshTabDisplay();
      // Re-evaluate any active search against the new hidden-row set so the
      // nav buttons and hidden-results hint reflect the updated Display filter.
      if ($('#search-field').val()) {
        $('#search-field').trigger('keyup');
      }
    });

    // Concise view: hide FK columns when in key-display mode.
    $('#concise-view-checkbox').on('change', () => {
      this._applyConciseView(this.context.getCurrentDataHarmonizer());
      this._saveAppSetting('concise_view', $('#concise-view-checkbox').is(':checked'));
    });

    // "Validate by column" now only controls Next Error / search direction.
    // tabMoves/enterMoves are fixed at { row:0, col:1 } (always right) in
    // DataHarmonizer.js so there is nothing to sync here on checkbox change.
    $('#validate_by_column').on('change', () => {
      this._saveAppSetting('validate_by_column', $('#validate_by_column').is(':checked'));
    });
  ...
  searchNavigation(event) {
    ...
    // When "Validate & search by column" is checked, navigate column-first
    // (top-to-bottom within each column, then advance to the next column).
    // Otherwise use the default row-first order returned by HOT's search plugin.
    const byColumn = $('#validate_by_column').is(':checked');
    const results = byColumn
      ? [...dh.queryResult].sort((a, b) => a.col - b.col || a.row - b.row)
      : dh.queryResult;
    ...
    if (current[0] === null)
      ...
      ptr = results.findIndex((e) => e.row == current[0] && e.col == current[1]);
      if (ptr < 0) {
        // Current cell is not itself a search result.  Find the nearest result
        // in the direction of travel from the user's current position so that
        // pressing > or < starts from where they are, not from result[0].
        const [curRow, curCol] = current;
        const isAfter = byColumn
          ? (r) => r.col > curCol || (r.col === curCol && r.row > curRow)
          : (r) => r.row > curRow || (r.row === curRow && r.col > curCol);
        if (direction === 1) {
          ptr = results.findIndex(isAfter);
          if (ptr < 0) ptr = 0; // nothing after current position → wrap to first
        } else {
          const revIdx = [...results].reverse().findIndex((r) => !isAfter(r));
          ptr = revIdx < 0 ? results.length - 1 : results.length - 1 - revIdx;
        }
      } else {
        ptr = ptr + direction;
        if (ptr < 0)
          ptr = results.length - 1;
        else if (ptr >= results.length)
          ptr = 0;
      }
    }
    let item = results[ptr];
    dh.scrollTo(item.row, item.col, true /* animated */);
  };
  ...
  updateTemplateOptions(template_path = null) {
    ...
    for (const [, schema_obj] of Object.entries(this.menu)) {
      ...
      // Skip schema entirely when every template has display:false.
      if (!Object.values(templates).some((t) => t.display === true)) continue;
    ...
    // The schema-editor-menu section is always reset to hidden here —
    // updateGUI() re-evaluates visibility after each template load.
    $('#schema-editor-menu').hide();
    $('#schema-editor-settings-section').hide();
  ...
  async createNewFile() {
    ...
    if (isNotEmpty) {
      const confirmed = await dhConfirm(
        i18next.t('clear-data-prompt', { defaultValue: 'Clear existing data?' }),
        {
          dangerous: true,
          okLabel: i18next.t('clear-data-confirm-btn', { defaultValue: 'Clear' }),
        }
      );
      if (!confirmed) return;
    }
    ...
    // Reset dependent-row state so the "Record(s) by selected key" breadcrumb
    // (Schema: … / Table: … / Field: …) does not show stale data from the
    // cleared workspace.  clearDH() empties the HOT tables but does not update
    // dependent_rows; without this, crudUpdateRecordPath() keeps the old trail.
    // Explicitly deselect every table first so getSelected() returns null and
    // has_selection is correctly set to false throughout the cascade.
    for (const tabDh of Object.values(this.context.dhs)) {
      tabDh.hot.deselectCell();
    }
    this.context.crudCalculateDependentKeys(dh.template_name);
    this.context.refreshTabDisplay();
  };

  /** Persist one app-level setting to localStorage. */
  _saveAppSetting(key, value) {
    try {
      const settings = JSON.parse(localStorage.getItem('dh_settings') || '{}');
      settings[key] = value;
  ...
  /**
   * Restore app-level settings from localStorage and apply them.
   * Called once from the constructor before the first restartInterface().
   */
  _loadAppSettings() {
    let settings;
    try {
    ...
    if ('concise_view' in settings) {
      // Store so _updateConciseViewVisibility (called inside restartInterface
      // after _loadAppSettings) uses the saved value instead of resetting to true.
      this._conciseViewSaved = settings.concise_view;
      $('#concise-view-checkbox').prop('checked', settings.concise_view);
    }

    if ('validate_by_column' in settings) {
      $('#validate_by_column').prop('checked', settings.validate_by_column);
    }

    if ('expert_user' in settings) {
      const expert_user = settings.expert_user;
      $('#schema_expert').prop('checked', expert_user);
      this.context.expert_user = expert_user;
      $(SCHEMA_EDITOR_EXPERT_TABS).toggle(expert_user);
      this.context.dhs?.['Slot']?.hot?.render();
    }

    if ('use_linkml_terms' in settings) {
      const use_linkml_terms = settings.use_linkml_terms;
      $('#schema_linkml_terms').prop('checked', use_linkml_terms);
      this.context.use_linkml_terms = use_linkml_terms;
      if (use_linkml_terms) {
        const _terms = this.context.getDhTermVars();
        $('[data-label-template]').each((_, el) => {
          const cname = el.getAttribute('data-label-template');
        ...
        if (this.context.schemaEditor) {
          for (const dh of Object.values(this.context.dhs)) {
            if (dh.hot) dh.hot.updateSettings({ nestedHeaders: dh.getNestedHeaders() });
          }
        }
      }
    }
  ...
  restartInterface(myContext) {
    $('#dh-mode-badge').text((myContext.schemaEditor ? 'Schema Editor' : 'DataHarmonizer') + ' v' + VERSION);
    this.updateGUI(
      myContext.getCurrentDataHarmonizer(),
      myContext.getTemplateName()
    );
    this._updateConciseViewVisibility();
    this._applyConciseView(myContext.getCurrentDataHarmonizer());
    // Re-apply export menu state here so it is correct even when the toolbar
    // was constructed after context.reload() ran (e.g. the demo popup path),
    // in which case the toggle in AppContext.reload() fired before the element
    // existed and had no effect.
    $('#export-to-dropdown-item').toggleClass('disabled', isEmpty(this.context.export_formats));
    // Commented out: selectCell(0,0)/deselectCell() was used to trigger
    // crudCalculateDependentKeys via afterSelectionEnd, but reload() already
    // calls crudCalculateDependentKeys directly, making this redundant.
    // Leaving here in case a HOT filter-related regression surfaces.
    //let hot = myContext.getCurrentDataHarmonizer().hot;
    //hot.selectCell(0, 0); // triggered deactivation of dependent tables with incomplete keys
  ...
  async openDataFile() {
    ...
    let loaded = false;
    if (ext === 'json') {
      // Await so that context.reload() inside openJSONDataFile completes and
      // creates the tab DOM elements BEFORE restartInterface runs updateGUI.
      // Without await, updateGUI fires while the tabs are being rebuilt, so
      // $(SCHEMA_EDITOR_EXPERT_TABS).toggle(expert_user) targets old/missing
      // elements and expert-only tabs appear visible after every workspace open.
      loaded = await this.openJSONDataFile(file);
  ...
  */
  async openJSONDataFile(file) {
    ...
    const metaDataFields = ['schema', 'in_language', 'container'];
    ...
    // Default version to 0 for schemas that omit the version field.
    if (!('version' in jsonData)) jsonData.version = 0;
    ...
    // Restore locales cell metadata on the Schema tab from Extension tab data.
    // hot.loadData() restores cell values but discards cell metadata; the full
    // locales object lives in Extension rows (name==='locales') so we rebuild it.
    this._reconstructLocalesMetadata();
    return true;
  }

  /**
   * After a JSON workspace load the Schema tab's HOT cell metadata (locales)
   * is lost because hot.loadData() only restores raw values.  The full locales
   * object is preserved in the Extension tab's "value" column, so we can
   * rebuild setCellMeta(row, 0, 'locales', ...) from there.
   */
  _reconstructLocalesMetadata() {
    const dh_schema = this.context.dhs['Schema'];
    const dh_ext    = this.context.dhs['Extension'];
    if (!dh_schema || !dh_ext) return;

    const hot_schema = dh_schema.hot;
    const hot_ext    = dh_ext.hot;

    // Column indices in the Extension tab
    const ext_schema_id_col = dh_ext.slot_name_to_column['schema_id'];
    const ext_name_col      = dh_ext.slot_name_to_column['name'];
    const ext_value_col     = dh_ext.slot_name_to_column['value'];
    // Schema tab uniquely identifies each row by its 'name' slot
    const schema_name_col   = dh_schema.slot_name_to_column['name'];

    if ([ext_schema_id_col, ext_name_col, ext_value_col, schema_name_col].some(c => c === undefined)) return;

    // Build schema_name → locales object from Extension tab source rows
    const localesMap = {};
    for (let phys = 0; phys < hot_ext.countSourceRows(); phys++) {
      if (hot_ext.getSourceDataAtCell(phys, ext_name_col) !== 'locales') continue;
      const schemaName = hot_ext.getSourceDataAtCell(phys, ext_schema_id_col);
      const rawValue   = hot_ext.getSourceDataAtCell(phys, ext_value_col);
      // Extension tab stores the full {tag:'locales', value:<locales dict>} object.
      // Mirror what loadSchemaYAML does: schema.extensions?.locales?.value
      const localesObj = rawValue?.value;
      if (schemaName && localesObj && typeof localesObj === 'object') {
        localesMap[schemaName] = localesObj;
      }
    }
    if (!Object.keys(localesMap).length) return;

    // Apply locales metadata to matching Schema tab rows (visual indices)
    for (let vr = 0; vr < hot_schema.countRows(); vr++) {
      const schemaName = hot_schema.getDataAtCell(vr, schema_name_col);
      if (schemaName && localesMap[schemaName]) {
        hot_schema.setCellMeta(vr, 0, 'locales', localesMap[schemaName]);
      }
    }
  }
  ...
  async openTabularDataFile(file) {
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      if (header_row < 1 || matches === 0) {
        // Check whether the file headers match a different locale's slot titles,
        // and if so hint that the user should switch language before loading.
        let localeHint = '';
        const candidateHeaders = new Set(
          [...(matrix[0] || []), ...(matrix[1] || [])].filter(Boolean)
        );
        let bestLocale = null, bestCount = 0;
        for (const [localeCode, localeData] of Object.entries(this.context.template.translations)) {
          const attrs = localeData.schema?.classes?.[template_name]?.attributes || {};
          const count = Object.values(attrs).filter(a => a.title && candidateHeaders.has(a.title)).length;
          if (count > bestCount) { bestCount = count; bestLocale = localeCode; }
        }
        if (bestLocale) {
          const optionVal = bestLocale === 'default' ? 'en' : bestLocale;
          const localeName = $(`#select-translation-localization option[value="${optionVal}"]`).text() || bestLocale;
          localeHint = i18next.t('no-matching-fields-locale-hint', { count: bestCount, locale: localeName });
        }
        messages.push(i18next.t('no-matching-fields-warning', {
          schema: this.context.getSchemaRef().name,
  ...
  getContainerData(ext, options = {}) {
    ...
    for (const [attribute_name, attribute] of Object.entries(container_attributes)) {
      ...
      for (const row of dh.hot.getSourceData()) {
        // Ignore empty rows.  getSourceData() returns minRows phantom rows as
        // null-filled arrays; isEmpty() misses those because arrays always have
        // numeric keys, so add an explicit all-empty array check.
        if (isEmpty(row) || (Array.isArray(row) && row.every(isEmptyUnitVal)))
          continue;
...
*/
  ...
  async showSaveAsModal() {
    ...
    if (!$.isEmptyObject(dh.invalid_cells)) {
      const confirmed = await dhConfirm(
        i18next.t('save-as-warning', { defaultValue: 'You have invalid cells. Continue saving?' }),
        { dangerous: true, okLabel: i18next.t('template-continue', { defaultValue: 'Continue' }) }
      );
      if (!confirmed) return;
    }
  ...
  async showExportModal() {
    if (!$.isEmptyObject(this.context.getCurrentDataHarmonizer().invalid_cells)) {
      const confirmed = await dhConfirm(
        i18next.t('export-to-invalid-warning', { defaultValue: 'You have invalid cells. Continue exporting?' }),
        { dangerous: true, okLabel: i18next.t('template-continue', { defaultValue: 'Continue' }) }
      );
      if (!confirmed) return;
    }
  ...
  showSectionColumns(event) {
    // Keep the dropdown open so multiple sections can be checked.
    event.stopPropagation();

    // Toggle this item's checkbox.
    const $item = $(event.currentTarget);
    const $cb = $item.find('.show-section-checkbox');
    $cb.prop('checked', !$cb.prop('checked'));

    // Collect all currently checked section titles.
    const checkedTitles = [];
    $('#section-menu .show-section-checkbox:checked').each(function () {
      checkedTitles.push(
        $(this).closest('.show-section-dropdown-item').data('section-title')
      );
    });
    ...
    if (checkedTitles.length === 0) {
      // Last checkbox cleared → restore full column view and activate Show All.
      $('#show-all-cols-dropdown-item, #show-required-cols-dropdown-item, #show-recommended-cols-dropdown-item')
        .removeClass('disabled');
      $('#show-all-cols-dropdown-item').addClass('disabled');
      dh.showAllColumns();
      dh._visState.colMode = 'all';
      dh._visState.sections = [];
    } else {
      // One or more sections checked → deactivate the other column controls.
      $('#show-all-cols-dropdown-item, #show-required-cols-dropdown-item, #show-recommended-cols-dropdown-item')
        .removeClass('disabled');
      dh.showColumnsBySectionTitles(checkedTitles);
      dh._visState.colMode = 'sections';
      dh._visState.sections = checkedTitles;
    }
    // Keep selected sections visible in the <summary> when the list is collapsed.
    $('#section-active-summary').text(checkedTitles.join(', '));
  }
  ...
  toggleColumnVisibility(method, arg) {
    ...
    // Clear section checkboxes — another column mode is now active.
    $('.show-section-checkbox').prop('checked', false);
    dh[method](arg);
    // Record the new column-filter mode for this tab.
    dh._visState ??= { colMode: 'all', sections: [], rowMode: 'all' };
    dh._visState.colMode = { showAllColumns: 'all', showRequiredColumns: 'required',
      showRecommendedColumns: 'recommended' }[method] ?? 'all';
    dh._visState.sections = [];
  }
  ...
  toggleRowVisibility(event) {
    ...
    // Track row-filter mode for this tab and update button active state.
    dh._visState ??= { colMode: 'all', sections: [], rowMode: 'all' };
    dh._visState.rowMode = {
      'show-all-rows-dropdown-item': 'all',
      'show-valid-rows-dropdown-item': 'valid',
      'show-invalid-rows-dropdown-item': 'invalid',
    }[event.target.id] ?? 'all';
    $('#show-all-rows-dropdown-item, #show-valid-rows-dropdown-item, #show-invalid-rows-dropdown-item')
      .removeClass('disabled');
    $(event.currentTarget).addClass('disabled');
  }

  // Restore the Settings menu highlight state to match dh's recorded visibility
  // mode. Called by tabChange() each time the user switches tabs, and by
  // updateGUI() on initial template load. applySectionMenu() handles restoring
  // the section checkboxes; this method handles the three column-mode buttons
  // and the three row-mode buttons.
  _restoreVisibilityUI(dh) {
    const s = (dh._visState ??= { colMode: 'all', sections: [], rowMode: 'all' });

    // Column-mode buttons
    $('#show-all-cols-dropdown-item, #show-required-cols-dropdown-item, #show-recommended-cols-dropdown-item')
      .removeClass('disabled');
    if (s.colMode === 'all')         $('#show-all-cols-dropdown-item').addClass('disabled');
    if (s.colMode === 'required')    $('#show-required-cols-dropdown-item').addClass('disabled');
    if (s.colMode === 'recommended') $('#show-recommended-cols-dropdown-item').addClass('disabled');
    // colMode === 'sections': no button active; checkboxes show state via applySectionMenu().

    // Row-mode buttons
    $('#show-all-rows-dropdown-item, #show-valid-rows-dropdown-item, #show-invalid-rows-dropdown-item')
      .removeClass('disabled');
    if (s.rowMode === 'all')     $('#show-all-rows-dropdown-item').addClass('disabled');
    if (s.rowMode === 'valid')   $('#show-valid-rows-dropdown-item').addClass('disabled');
    if (s.rowMode === 'invalid') $('#show-invalid-rows-dropdown-item').addClass('disabled');
  }

  // Show/hide the Concise view checkbox and reset it to checked.
  // Called each time a template is loaded.
  _updateConciseViewVisibility() {
    const isMultiTab = Object.keys(this.context.dhs).length > 1;
    $('#concise-view-item').toggle(isMultiTab);
    if (isMultiTab) {
      // Use the user's saved preference; fall back to true (on) for first-time users.
      $('#concise-view-checkbox').prop('checked', this._conciseViewSaved ?? true);
    }
    $('#record-hierarchy-div').toggle(isMultiTab);
    this._syncConciseViewToDisplayMode();
  }

  // Disable + uncheck Concise view when "All records" is selected (all key
  // columns must be visible in that mode or related records are ambiguous).
  // Saves the prior checked state so it can be restored when leaving "All".
  // Visibility of the item itself is governed by _updateConciseViewVisibility.
  _syncConciseViewToDisplayMode() {
    const isAllMode = $('input[name="display-main-type"]:checked').val() === 'all';
    const $checkbox = $('#concise-view-checkbox');
    const $item     = $('#concise-view-item');
    if (isAllMode) {
      if ($checkbox.is(':checked')) this._conciseWasChecked = true;
      $checkbox.prop('checked', false).prop('disabled', true);
      $item.addClass('disabled');
    } else {
      $checkbox.prop('disabled', false);
      $item.removeClass('disabled');
      // Restore checked state if it was programmatically unchecked for "All".
      if (this._conciseWasChecked) {
        $checkbox.prop('checked', true);
        this._conciseWasChecked = false;
      }
    }
  }

  // Hide FK (foreign-key) columns on the given DH tab when Concise view is
  // active (checkbox checked) and the display mode is "Record(s) by selected
  // key".  Restores all columns when either condition is false, but only if
  // we previously hid them (avoids clobbering the user's column-filter choice).
  _applyConciseView(dh) {
    if (!dh) return;
    // UniqueKey (Table key) and Annotation tabs always show all columns regardless
    // of concise-view state: UniqueKey's class_id and schema_id context are essential
    // for understanding which class each unique key belongs to; Annotation context
    // columns (annotation_type, class_name, slot_name) are equally essential.
    if (this.context.schemaEditor &&
        (dh.template_name === 'UniqueKey' || dh.template_name === 'Annotation')) return;
    const fkSlotNames = Object.keys(
      this.context.relations?.[dh.template_name]?.dependent_slots || {}
    );
    if (fkSlotNames.length === 0) return;

    // On the Schema Editor's Field (Slot) tab, keep class_id visible — schema-level
    // slot rows have no class_id, so hiding it would silently mask that distinction.
    const keepVisible = new Set(
      (this.context.schemaEditor && dh.template_name === 'Slot')
        ? ['class_id']
        : []
    );
    const fkCols = fkSlotNames
      .filter((s) => !keepVisible.has(s))
      .map((s) => dh.slot_name_to_column?.[s])
      .filter((c) => c !== undefined);
    if (fkCols.length === 0) return;

    const concise = $('#concise-view-checkbox').is(':checked');
    const isKeyMode = $('input[name="display-main-type"]:checked').val() === '';

    if (concise && isKeyMode) {
      dh.showAllColumns();
      dh.hideColumns(fkCols);
      dh._conciseViewActive = true;
    } else if (dh._conciseViewActive) {
      dh.showAllColumns();
      dh._conciseViewActive = false;
    }

    // HOT's fixedColumnsLeft counts physical columns (hidden ones included).
    // Walk left-to-right until 1 visible column has been passed, then set
    // fixedColumnsLeft to that physical index so exactly 1 column stays frozen.
    const hiddenColsPlugin = dh.hot.getPlugin('hiddenColumns');
    const hidden = new Set(hiddenColsPlugin.getHiddenColumns());
    const totalCols = dh.hot.countCols();
    let physIdx = 0;
    let visibleSeen = 0;
    while (physIdx < totalCols && visibleSeen < 1) {
      if (!hidden.has(physIdx)) visibleSeen++;
      physIdx++;
    }
    dh.hot.updateSettings({ fixedColumnsLeft: physIdx });
  }
  ...
  clearAllTabFilters() {
    $('#display-all-records-radio').prop('checked', true);
    this.context.refreshTabDisplay();
  }
  ...
  locateNextError() {
    ...
    // Suppress the afterSelectionEnd handler from clearing _navigatedViaNextError
    // during the programmatic selectCell() that scrollTo() triggers internally.
    // After scrollTo returns (afterSelectionEnd has already fired synchronously),
    // we set the flag so afterChange and beforeKeyDown know the user arrived here
    // via Next Error and should jump to the following error when they Tab out.
    dh._suppressClearNavFlag = true;
    dh.scrollTo(focus_row, focus_col);
    dh._suppressClearNavFlag = false;
    dh._navigatedViaNextError = true;
  }
  ...
  async validate() {
    ...
    // Restore the current tab's display state (sort order + row filter).
    // changeRowVisibility() called above for any DH with errors on hidden rows
    // un-hides all rows without re-applying the active FK filter or sort, so
    // the display can be left in an incorrect state after validation completes.
    context.refreshTabDisplay();
  }
  ...
  translationUpdate() {
    ...
    for (let input of $("#translate-modal.show #translate-modal-content textarea")) {
      ...
      if (value) {
        ...
        // Remove empty-string entries so they are not saved in the YAML.
        // Also handles the "clear a previously-set translation" case.
        const parts  = path.split('.');
        const leaf   = parts.pop();
        const parent = parts.length ? nestedProperty.get(locales, parts.join('.')) : locales;
  ...
  updateGUI(dh, template_name) {
    ...
    $('#schema-editor-settings-section').toggle(schema_editor);
    $('#data-manager-menu').toggle(!schema_editor);
    $(SCHEMA_EDITOR_EXPERT_TABS).toggle(expert_user);
    $('#slot_report_control').hide();

    Object.values(this.context.dhs).forEach((d) => this._buildSectionMenuHTML(d));
    this.applySectionMenu(dh);
    this._restoreVisibilityUI(dh);
    this.setupJumpToModal();
    this.setupFillModal();
    dh.clearValidationResults();
  }

  // Called once at template load. Handlers read getCurrentDataHarmonizer()
  // lazily so they always target the active tab without needing a re-bind.
  setupFillModal() {
    ...
    $('#fill-modal').off('shown.bs.modal').on('shown.bs.modal', () => {
      const dh = this.context.getCurrentDataHarmonizer();
      const selectizeInstance = fillColumnInput[0].selectize;
      selectizeInstance.clear();
      selectizeInstance.clearOptions();
      selectizeInstance.clearCache();
      selectizeInstance.addOption(dh.slots);
      $('#fill-value-input').val('');
      selectizeInstance.open();
    });

    $('#fill-button').off('click').on('click', () => {
      const dh = this.context.getCurrentDataHarmonizer();
      dh.fillColumn(fillColumnInput.val(), $('#fill-value-input').val());
      $('#fill-modal').modal('hide');
    });
  }

  // Note: HandsonTable won't advance to column if table has empty rows; no workaround to this.
  // Called once at template load. Handlers read getCurrentDataHarmonizer() lazily.
  setupJumpToModal() {
    let jumpToInput = $('#jump-to-input').empty();
    jumpToInput.selectize({ openOnFocus: true });

    $('#jump-to-modal').off('shown.bs.modal').on('shown.bs.modal', () => {
      const dh = this.context.getCurrentDataHarmonizer();
      ...
      selectizeInstance.addOption(
        Object.keys(columnCoordinates).map((col) => ({ text: col, value: col }))
      );
      selectizeInstance.open();
    });

    $('#jump-to-input').off('change').on('change', (e) => {
      if (!e.target.value) return;
      const dh = this.context.getCurrentDataHarmonizer();
      const columnX = dh.getColumnCoordinates()[e.target.value];
      dh.hot.selectColumns(columnX, columnX);
      $('#jump-to-modal').modal('hide');
    });
  }

  // Build and cache the section-menu HTML for one DH instance. Called for
  // every DH at template load and locale change (i18n text may differ).
  _buildSectionMenuHTML(dh) {
    dh._sectionMenuHTML = dh.sections.map((section, index) =>
      `<div id="show-section-${index}" class="dropdown-item show-section-dropdown-item"` +
      ...
      `<input type="checkbox" class="show-section-checkbox" style="pointer-events:none;flex-shrink:0">` +
      `</div>`
    ).join('');
  }

  // Swap in the cached section-menu HTML for dh and restore its checkbox state.
  applySectionMenu(dh) {
    $('#section-menu').html(dh._sectionMenuHTML || '');
    $('#section-menu').localize();
    // Restore which sections were checked for this tab.
    const s = dh._visState;
    const hasSections = s?.colMode === 'sections' && s.sections.length;
    if (hasSections) {
      const titleSet = new Set(s.sections);
      $('#section-menu .show-section-dropdown-item').each(function () {
        $(this).find('.show-section-checkbox')
          .prop('checked', titleSet.has($(this).data('section-title')));
      });
    }
    // Keep selected sections visible in the <summary> when the list is collapsed.
    $('#section-active-summary').text(hasSections ? s.sections.join(', ') : '');
  }
