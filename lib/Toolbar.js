...
*/
  ...
  _applyConciseView(dh) {
    if (!dh) return;
    // SlotGroup (Table section) tab always shows all columns regardless of
    // concise-view state — it has only a few columns and no FK to hide.
    if (this.context.schemaEditor && dh.template_name === 'SlotGroup') return;
