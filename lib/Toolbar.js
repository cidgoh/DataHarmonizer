...
*/
  ...
  // Disable Concise view when "All records" is selected (irrelevant in that
  // mode) and re-enable it when switching back to "Record(s) by selected key".
  ...
  _applyConciseView(dh) {
    if (!dh) return;
    // SlotGroup (Table section), UniqueKey (Table key), and Annotation tabs always
    // show all columns regardless of concise-view state: SlotGroup has few columns
    // and no FK to hide; UniqueKey's class_id and schema_id context are essential
    ...
    if (this.context.schemaEditor &&
        (dh.template_name === 'SlotGroup' || dh.template_name === 'UniqueKey' ||
         dh.template_name === 'Annotation')) return;
