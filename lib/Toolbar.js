  ...
  async openTabularDataFile(file) {
    ...
    for (const dh of Object.values(this.context.dhs)) {
      ...
      if (header_row > 0 && schema_slot_count == matches) { 
        ...
        // A DH template/tab/class had NO fields matching to respective data table!
        if (matches == 0) {
          alert(`The LinkML schema "${this.getSchemaRef().name}" template "${dh.template.name}" had NO matching fields to the given data file. Perhaps a new table has to be included in the data file?`);
          return false;
        }
        else {
