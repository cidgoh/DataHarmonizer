...
import { processYamlSchema } from '../lib/utils/schema_induction';
  ...
  initializeLocale() {
    ...
    translationSelect.on('input', async () => {
      ...
      const translateData = (
        ...
        // OUT OF LOOP.
        return dataHarmonizerData.map((row) => {
  ...
  */
  async openJSONDataFile(file) {
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      const data_matrix = data.map((row) => header.map((field) => row[field] ?? null));
  ...
  getContainerData(ext) {
    ...
    for (const [attribute_name, attribute] of Object.entries(container_attributes)) {
      ...
      for (const row of dh.hot.getSourceData()) {
        ...
        // Make this an ordered dict via map object?
        let row_dict = {}; 
        if (ext !== 'json') {
          Object.values(dh.slots).forEach((slot) => {row_dict[slot.title] = ''});
        }
...
*/
  ...
  async loadSelectedTemplate(file = null) {
          ...
          Object.entries(schema.classes.Container.attributes).forEach(
            ([, class_obj]) => {
              if (class_obj.range in schema.classes) {
                containerRanges.push(class_obj.range);
              }
            }
          );
          // Root class has no foreign_key annotations in any induced attribute.
          // Child classes carry a foreign_key slot pointing back to their parent.
          template_name = containerRanges.find((range) =>
    ...
    const loadFromMenu = async () => {
      ...
      if (!template_path) {
        alert("Error: loadFromFile() was unable to load a default schema because no schemas or templates exist or have a display:true setting (in /web/templates/menu.json)");
        return false;
      }
    ...
    if (!(template_name in schema.classes)) {
      alert(`Warning: The requested schema template ${template_path} could not be loaded. If present in the menu.json file, its entry likely needs to be updated by a DataHarmonizer schema maintainer.`);
      return;
    }
