...
class Toolbar {
  ...
  setupFillModal(dh) {
    ...
    // Use off() before on() to prevent accumulating stale handlers across template switches.
    $('#fill-modal').off('shown.bs.modal.fillModal').on('shown.bs.modal.fillModal', () => {
    ...
    // Use off() before on() to prevent accumulating stale handlers across template switches.
    $('#fill-button').off('click.fillModal').on('click.fillModal', () => {
  ...
  setupJumpToModal(dh) {
    ...
    // Set up the modal opening event to clear and refresh the options dynamically
    // Use off() before on() to prevent accumulating stale handlers across template switches.
    $('#jump-to-modal').off('shown.bs.modal.jumpTo').on('shown.bs.modal.jumpTo', () => {
    ...
    // Use off() before on() to prevent accumulating stale handlers across template switches.
    $('#jump-to-input').off('change.jumpTo').on('change.jumpTo', (e) => {
