    ...
    $('#save-as-confirm-btn').on('click', () => {
      ...
      try {
        ...
        const Container = Object.entries(
          self.context.template.schema.classes.Container.attributes
      ...
      translationSelect.on('input', () => {
        ...
        if (supportsLocale) {
          ...
          for (const dh in this.context.dhs) {
            // 0: Cache the existing data before changing language
            this.context.dhs[dh].context = this.context;
            const data = this.context.dhs[dh].hot.getData();

            // 1: Reset the DataHarmonizer and Handsontable
            this.context.dhs[dh].useSchema(
              this.context.template.localized.schema,
              exportFormats,
              template_name
            );
            ...
            // 2A: Compute filter that classifies translatable fields (should not include free text fields or non-textual fields)
            const key_in = (obj, keys) =>
              keys.some((key) => typeof obj[key] !== 'undefined'); // some key in the lit matches
            const is_translatable = this.context.dhs[dh].fields.reduce(
              (acc, field_spec, index) => {
                return Object.assign(acc, {
                  [index]: key_in(field_spec, ['flatVocabulary']), // flatVocabulary characterizes controlled vocab fields
                });
              },
              {}
            );

            console.log(this.context.dhs[dh].fields, is_translatable);

            // 2B: Translate old data into new language target
            const _i = data.map((arr) =>
              arr.map((obj, index) =>
                // null can't be translatedsupportsLocale(maybe_locale) {
                obj !== null && is_translatable[index] === true
                  ? // multivalued values need to be translated independently
                    obj.split(MULTIVALUED_DELIMITER).length > 0
                    ? obj
                        .split(MULTIVALUED_DELIMITER)
                        .map(i18next.t)
                        .join(MULTIVALUED_DELIMITER)
                    : // non-multivalued controlled vocabulary
                      i18next.t(obj)
