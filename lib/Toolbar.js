...
class Toolbar {
            ...
            .then((context) => {
              for (const dh in context.dhs) {
                if (typeof _dh_data_cache[dh] !== 'undefined') {
                    ...
                    // DEPRECATE: translate back and forth for multiselect
                    // .map(row => {         
                    // const fields = context.dhs[dh].getFields();           
                    //   return row.map((el, index) => {
                    //     const field = fields[index];
                    //     if (typeof field.flatVocabulary !== 'undefined'){
                    //       return field.flatVocabulary[el];
                    //     }
                    //     return el;
                    //   });
                    // })
  ...
  async openFile() {
    ...
    if (!acceptedExts.includes(ext)) {
      ...
      if (ext === 'json') {
        ...
        if (!location) {
          alert("The file you have requested is not in a DataHarmonizer JSON format.")
          return false;
        }
          ...
          await this.context
            ...
            .then((context) => {
              if (!jsonData.Container) {
                alert("Error: JSON data file does not have Container dictionary.")
              }
  ...
  async saveFile() {
    ...
    function titleOverText (key, enm) {
      return typeof enm !== 'undefined' && typeof enm.title !== 'undefined' ? enm.title : key;
    }
    ...
    ).reduce((acc, [cls_key, { range }]) => {
      // TODO: check if container attributes always have ranges?
      if (typeof range !== 'undefined') {

        return Object.assign(acc, {
          [cls_key]: MultiEntityJSON[range].map((obj) =>
          ...
          .map((entry) => {
            ...
            const findField = (colKey) => fields.filter(field => field.title === colKey)[0];
            entry = Object.fromEntries(Object.entries(entry).map(([k,v]) => {
              ...
              if (field.sources && !isEmptyUnitVal(v)) {
                ...
                if (field.multivalued === true) {
                  nv = v.split(MULTIVALUED_DELIMITER).map(_v => titleOverText(_v,merged_permissible_values[_v])).join(MULTIVALUED_DELIMITER);
                } else {
                  nv = titleOverText(v, merged_permissible_values[v]);
                }
              };
              return [k, nv]
            }))
  ...
  updateGUI(dh, template_name, exportFormats, columnCoordinates) {
    ...
    const template_classes = this.context.template.schema.classes[template_name];
    if (template_classes && template_classes.see_also) {
      helpSop
        .attr(
          'href',
          template_classes.see_also
        )
        .show();
    } else {
      helpSop.hide();
