...
class Toolbar {
  ...
  bindEvents() {
    ...
    $('#load-schema-editor-button').on('click', async () => {
      const firstDh = Object.values(this.context.dhs)[0];
      if (firstDh?.hot && firstDh.hot.countRows() > firstDh.hot.countEmptyRows(true)) {
        const confirmed = await dhConfirm(
          'The loaded data file may have unsaved changes. Switch to the Schema Editor without saving?',
    ...
    $('#load-data-manager-button').on('click', async () => {
      const firstDh = Object.values(this.context.dhs)[0];
      if (firstDh?.hot && firstDh.hot.countRows() > firstDh.hot.countEmptyRows(true)) {
        const confirmed = await dhConfirm(
          'The current workspace may have unsaved changes. Switch to DataHarmonizer without saving the workspace?',
          { title: 'Switch to DataHarmonizer?' }
        );
        if (!confirmed) return;
      }
      // Find the first display:true template that isn't the schema editor.
      const firstOption = this.$selectTemplate
        .find('option')
        .filter(function () {
          return $(this).val() && !$(this).val().startsWith('schema_editor/');
        })
        .first();
      if (firstOption.length) {
        this.$selectTemplate.val(firstOption.val()).change();
        this.loadSelectedTemplate();
      }
    });
  ...
  updateTemplateOptions(template_path = null) {
    ...
    // Show the "Run schema editor" button only when schema_editor/Schema is
    // available.  The schema-editor-menu section is always reset to hidden
    // here — it becomes visible only when the user explicitly clicks the button.
    $('#load-schema-editor-button').toggle(found_schema_editor);
