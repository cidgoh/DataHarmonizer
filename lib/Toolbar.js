...
class Toolbar {
  ...
  constructor(root, context, menu, options = {}) {
    ...
    $('#save-as-confirm-btn').on('click', () => {
      ...
      const translateObject = obj => Object.entries(obj).reduce((acc, [key, val]) => {
        return Object.assign(acc, {
          [i18next.t(key.replace(/ /g, '_'), { defaultValue: key })]: val !== null ? i18next.t(val) : null
        })
      }, {});
      ...
      try {
        if (ext == 'json') {
          ...
          const Container = Object.entries(self.context.template.schema.classes.Container.attributes)
            .reduce((acc, [key, container]) => Object.assign(acc, {
              [key]: MultiEntityJSON[container.range].map(translateObject)
            }), {});
