  ...
  async openFile() {
    ...
    if (!acceptedExts.includes(ext)) {
      ...
      if (ext === 'json') {
        ...
        const location = await templatePathForSchemaURI(schema);

        if (!location) {
          alert(
            `The schema ${location} cannot be found within the manifest.`
          );
          return false;
        }
