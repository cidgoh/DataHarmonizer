...
class Toolbar extends EventEmitter {
  ...
  constructor(root, dh, menu, options = {}) {
    ...
    this.on('templateSelected', this.loadSelectedTemplate);
    this.on('fileUploaded', this.handleFileUpload);
    this.root = root;
    this.menu = menu;
    ...
    $('#show-all-cols-dropdown-item').on('click', function () {
      ...
      Object.keys(self.context.dhs).forEach(dh => self.context.dhs[dh].showAllColumns());
    });
    $('#show-required-cols-dropdown-item').on('click', function () {
      $(showColsSelectors.join(',')).removeClass('disabled');
      $(this).addClass('disabled');
      Object.keys(self.context.dhs).forEach(dh => self.context.dhs[dh].showRequiredColumns());
    });
    $('#show-recommended-cols-dropdown-item').on('click', function () {
      $(showColsSelectors.join(',')).removeClass('disabled');
      $(this).addClass('disabled');
      Object.keys(self.context.dhs).forEach(dh => self.context.dhs[dh].showRecommendedColumns());
    });
    $(this.root).on('click', '.show-section-dropdown-item', function () {
      ...
      Object.keys(self.context.dhs).forEach(dh => self.context.dhs[dh].showColumnsBySectionTitle($this.text()));
    });
    ...
    $('#help_reference').on('click', () => this.context.getCurrentDataHarmonizer().renderReference());
  }
  ...
  async validate() {
    ...
    if (Object.keys(this.context.getCurrentDataHarmonizer().invalid_cells).length > 0) {
  ...
  async loadSelectedTemplate(file = null) {
    let template_name, schema, exportFormats, languages;
    if (file) {
      exportFormats = {};
      try {
      ...
      const language_options = Object.values(languages).map((el) =>
        $('<option>').val(el.langcode).text(el.nativeName)
      );
