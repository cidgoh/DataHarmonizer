      ...
      if (supportsLocale) {
        try {
          ...
          for (const dh in this.context.dhs) {
            ...
            const translatableFields = findTranslatableFields(this.context.dhs[dh].slots);
...
//    $('#view-template-drafts').on('change', () => this.updateTemplateOptions());
  ...
  updateTemplateOptions() {
    ...
    for (const [schema_name, schema_obj] of Object.entries(this.menu)) {
      ...
      for (const [template_name, template_obj] of Object.entries(templates)) {
        ...
        let label = (Object.keys(templates).length == 1) ? template_name : schema_obj.folder + '/' + template_name;
  ...
  async openFile() {
    ...
    if (!file) { // Happens when user cancels open action.
      return
    }
    ...
    if (!acceptedExts.includes(ext)) {
      ...
      if (ext === 'json') {
        ...
        if (schema_uri != this.context.template.default.schema.id) {
          alert(`The current json file's schema "${schema_uri}" is required, but one must select this template from menu first, if available.  Online retrieval of schemas is not yet available.`);
          return false;
        }
            ...
            if (!jsonData.Container) {
              ...
              for (const dh in context.dhs) {
                ...
                const list_data = context.dhs[dh].loadDataObjects(
                  jsonData.Container[container_class], locale
                );
  ...
  async saveFile() {
    ...
    const Container = Object.entries(schema_container.attributes)
    ...
    if (ext === 'json') {
      ...
      const sectionCoordinatesByClass = Object.values(this.context.dhs).reduce(
        (acc, dh) => {
          ...
          return Object.assign(acc, {
            [rangeToContainerClass(schema_container, dh.template_name)]:
      ...
      const columnCoordinatesByClass = Object.values(this.context.dhs).reduce(
        (acc, dh) => {
          const columnIndexCoordinates = dh.slots
            .reduce(
  ...
  locateNextError() {
    ...
    if (focus_row === null) {
      ...
      if (focus_row == rows[0]) {
        ...
        if (inv_focus) //Not sure how this is
