...
export default class KeyValueListEditor extends Handsontable.editors
  ...
  prepare(row, col, prop, td, value, cellProperties) {
    ...
    // Wrap HOT's default afterOnCellMouseDown to track explicit user row clicks.
    // The flag is consumed in finishEditing() to preserve the original cell value
    // when the dropdown is closed without an explicit selection (click-away).
    const defaultMouseDown = this.htOptions.afterOnCellMouseDown;
    const kvEditor = this;
    this.htOptions.afterOnCellMouseDown = function(event, coords) {
      kvEditor._userMadeExplicitSelection = true;
      if (defaultMouseDown) defaultMouseDown.call(this, event, coords);
    };
  ...
  open() {
    const ROW_HEIGHT = 23;
    const MAX_ROWS = 10;
    // When the dropdown must open above the cell, cap at 9 rows so it fits
    // within the viewport even when the cell is near the top of the page.
    const MAX_ROWS_ABOVE = 9;

    // Reset per-open state.
    this._userMadeExplicitSelection = false;
    // Snapshot the TEXTAREA value as set by beginEditing() so finishEditing()
    // can tell whether the user typed new content (free-text entry) vs just
    // opened and closed the dropdown without interacting.
    this._initialTextareaValue = this.TEXTAREA ? this.TEXTAREA.value : '';
    ...
    if (this.TD && this.cellProperties) {
      ...
      const sourceLen = (this.cellProperties.source || []).length;
      // Use MAX_ROWS to decide whether to flip; use the tighter MAX_ROWS_ABOVE
      // cap when actually opening above so the dropdown stays within the viewport.
      const wantedBelow  = Math.min(sourceLen, MAX_ROWS) * ROW_HEIGHT;
      const willOpenAbove = spaceBelow < wantedBelow && spaceAbove > spaceBelow;
      const wantedH = Math.min(sourceLen, willOpenAbove ? MAX_ROWS_ABOVE : MAX_ROWS) * ROW_HEIGHT;
    ...
    if (this.TD && this.htContainer) {
      ...
      if (actualH > 0) {
        ...
        if (spaceBelow < actualH && cellRect.top > spaceBelow) {
          ...
          // Safety clamp: if the rendered height (which can exceed Phase 1's
          // estimate due to container borders / scrollbars) would push the
          // dropdown above the viewport, shrink it to fit.
          const MARGIN = 4;
          const maxFit = Math.max(ROW_HEIGHT, cellRect.top - MARGIN);
          let posH = Math.min(actualH, maxFit);
          if (posH < actualH && this.htEditor) {
            this.htEditor.updateSettings({ height: posH });
            this.htEditor.render();
          }

          this._dropdownMaxH = posH;
          ...
          this.htContainer.style.top = `${cellRect.top - posH}px`;
        }
      }
    }
    ...
    // mode), leaving the current row unhighlighted.  selectCell and scrollViewportTo
    // use physical column indices on the inner HOT; col 1 = label (physical).
    //
        ...
        if (idx !== -1) {
          requestAnimationFrame(() => {
            if (htEditor && !htEditor.isDestroyed) {
              // Col 1 = physical label column. hiddenColumns hides physical col 0
              // (_id); selectCell and scrollViewportTo use physical column indices
              // on the inner HOT, so col 1 (label) is correct here.
  ...
  /**
   * Preserves the original cell value when the dropdown is closed without an
   * explicit row click (e.g. user clicked away, clicked an adjacent cell, or
   * clicked outside the dropdown bounds).
   *
   * HOT's HandsontableEditor.finishEditing() updates TEXTAREA from the inner
   * HOT's selection (set by Phase 3) and then calls super.finishEditing() which
   * invokes getValue() to commit.  When the user never clicked a row we reset
   * TEXTAREA to originalValue first; HandsontableEditor may still overwrite it
   * with Phase 3's label, but since Phase 3 always selects the originalValue row,
   * getValue() translates that label back to the same _id — so the cell is
   * unchanged regardless.
   */
  finishEditing(restoreOriginalValue, ctrlDown, callback) {
    if (!restoreOriginalValue && !this._userMadeExplicitSelection) {
      // Only restore originalValue when the user hasn't typed new content.
      // If TEXTAREA differs from its value at open() time, the user entered
      // free text and we should commit that instead of discarding it.
      const currentVal = this.TEXTAREA ? this.TEXTAREA.value : '';
      const userTyped = currentVal !== this._initialTextareaValue;
      if (!userTyped && this.originalValue != null) {
        this.TEXTAREA.value = String(this.originalValue);
      }
    }
