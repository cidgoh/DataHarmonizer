...
export class SchemaEditor {
  ...
  */
	...
	hotSettingsMenuHooks(dh, hot_settings, tab_name, keyColumns = null) {
	  ...
	  // Intercept HOT's built-in Delete/Backspace = emptySelectedCells shortcut so
	  // that pressing either key removes the entire selected row(s) instead of
	  // clearing cell values.  HOT registers both keys for emptySelectedCells; on
	  // macOS the physical Delete key sends 'Backspace', so both must be handled.
	  dh.hot.addHook('beforeKeyDown', (e) => {
	    ...
	    // Close any open editor without committing its current value.
	    const editor = dh.hot.getActiveEditor();
	    if (editor?.isOpened()) editor.close();
	    // Remove selected rows, showing a cascade-confirm dialog when dependents exist.
