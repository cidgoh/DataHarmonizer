...
export class SchemaEditor {
	    ...
	    switch (enum_name) {
	      ...
	      case 'SchemaMenu':
	        ...
	        for (let row=0; row < schema_dh.hot.countSourceRows(); row ++) {
	        	const slot_group_text = schema_dh.hot.getSourceDataAtCell(row, schema_name_ptr);
  ...
  */
	...
	initSlotGroupTab (dh, hot_settings) {
	  ...
	  //hot_settings.filters = false;
		//hot_settings.dropDownMenu = false;
	  hot_settings.fixedColumnsLeft = 0;
		//hot_settings.filters = false; // WHY NOT WORKING? Causes error as though
		// filters were enabled on SlotGroup tab.

	  // Somehow putting this into multiColumnSorting.initConfig {} doesn't
	  // enable memory of it, as though init config is being wiped out.
	  dh.defaultMultiColumnSortConfig = [
	    {column: dh.slot_name_column, sortOrder: 'asc'},
		  {column: dh.slot_class_name_column, sortOrder: 'asc'},
			{column: dh.schema_name_column, sortOrder: 'asc'}
  	];
