  ...
  */
	...
	hotSettingsMenuHooks(dh, hot_settings, tab_name) {
	  ...
	  hot_settings.afterRemoveRow = (index, amount, physicalRows, source) => {
	    ...
	    // Clear the HOT selection so dependent keys are recalculated with nothing
	    // selected — this clears the breadcrumb entry for the deleted row's key.
	    dh.hot.deselectCell();
	    this.context.crudCalculateDependentKeys(tab_name);
	  };
	}
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  */
	  ...
	  //
	  // Default sort: schema → name → slot_type → class_name.
	  // slot_type uses a fixed domain order: slot < slot_usage < attribute.

	  const SLOT_TYPE_RANK = {slot: 0, slot_usage: 1, attribute: 2};
	  hot_settings.columns[dh.slot_type_column].columnSorting = {
	    compareFunctionFactory: (sortOrder) => (a, b) => {
	      const aRank = SLOT_TYPE_RANK[a] ?? 999;
	      const bRank = SLOT_TYPE_RANK[b] ?? 999;
	  ...
	  dh.defaultMultiColumnSortConfig = [
	    ...
	    {column: dh.slot_name_column,         sortOrder: 'asc'}, // name
	    {column: dh.slot_type_column,         sortOrder: 'asc'}, // slot → slot_usage → attribute
	    {column: dh.slot_class_name_column,   sortOrder: 'asc'}, // class_name (ties within type)
	  ];
