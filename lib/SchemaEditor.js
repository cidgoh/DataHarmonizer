	    ...
	    for (let row = dh.current_selection[0]; row <= dh.current_selection[2]; row++) {
	      ...
	      for (const [locale, locale_schema] of Object.entries(locales)) {
	        ...
	        for (let column_name of row_text_cols) {
	          ...
	          // (if any) as a collapsed hint so the user knows what they'd override.
	          let schema_hint = '';
	          if (sub_part && key2) {
              ...
              schema_derived = schema_derived.length && typeof schema_derived[0] === 'object'
	            ...
	            if (schema_derived) {
	              schema_hint = `<details style="font-size:0.8em;margin-top:3px">`
	                + `<summary style="color:#888;cursor:pointer;user-select:none">`
	                + `schema-derived default</summary>`
	                + `<div style="padding:2px 4px;color:#555;white-space:pre-wrap;word-break:break-word">`
	                + `${schema_derived}</div></details>`;
	            }
	          }
