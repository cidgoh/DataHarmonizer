...
export class SchemaEditor {
  ...
  initMenus() {
  	...
  	// populated later by refreshMenus().
		...
		// SchemaDataTypeMenu is built from the static DH_LinkML type list — it never
		...
		this.schema.enums['SchemaDataTypeMenu'] ??= { name: 'SchemaDataTypeMenu' };
		this.schema.enums['SchemaDataTypeMenu'].permissible_values = dtPermValues;
		// Update column settings for every slot that references SchemaDataTypeMenu so the
		...
		for (const tab_dh of Object.values(this.context.dhs || {})) {
			for (const slot_obj of Object.values(tab_dh.slots)) {
				if (Object.values(slot_obj.sources || {}).includes('SchemaDataTypeMenu')) {
