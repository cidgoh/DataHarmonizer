...
export class SchemaEditor {
  ...
  */
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  hot_settings.afterGetColHeader = (col, TH) => {
	  ...
	  */
	  ...
	  dh.defaultMultiColumnSortConfig = [
	    ...
	    {
	      column: dh.slot_class_id_column, // class_id (Table ID)
	      sortOrder: 'asc'
	    },
  	];
	    ...
	    for (const change of changes) {
	      ...
	      if ((col === dh.slot_name_column || col === dh.slot_title_column)
	        ...
	        if (slot_type === 'slot_usage' || slot_type === 'attribute') {
	          const class_id = dh.hot.getSourceDataAtCell(physRow, dh.slot_class_id_column);
	          if (!class_id) {
	            blockedVisualRow = visualRow;
	    ...
	    if (blockedVisualRow !== null) {
	      ...
	      if (blockReason === 'slot_type') {
	        ...
	        setTimeout(async () => {
	          ...
	          dh.hot.selectCell(targetVisualRow, dh.slot_class_id_column);
	        }, 0);
	      } else { // 'create_slot'
  ...
  addSlotRecord(dh, tables, schema_name, class_name, slot_type, slot_key, slot_obj) {
    ...
    let slot_record = {
      ...
      // [class_id:] MATCHES DHLinkML schema slot.class_id specification
      class_id:  class_name, 
      rank:        slot_obj.rank, 
