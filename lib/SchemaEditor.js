		  ...
		  for (const enum_name of enums) {
		    ...
		    switch (enum_name) {
		      ...
		      case 'SchemaSlotGroupMenu': {
		         ...
		         * In "records by selected key" mode: filter by the selected schema only —
		         * slot groups are schema-wide, not class-specific.
		         */
		        const show_all = ($('input[name="display-main-type"]:checked').val() ?? '') === 'all';
		        const filter = {};
		        if (!show_all) {
  ...
  */
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			...
			case 'SlotGroup':
				this.initSlotGroupTab(dh, hot_settings);
	...
	initSlotGroupTab(dh, hot_settings) {
	  ...
	  const sgNameCol   = dh.slot_name_to_column['name'];
	  const sgSchemaCol = dh.slot_name_to_column['schema_id'];
	  const sgClassCol  = dh.slot_name_to_column['class_id'];

	  dh.hot.addHook('afterRowMove', (movedRows, finalIndex, dropIndex, movePossible, orderChanged) => {
	    if (!orderChanged || !movePossible) return;

	    const sgHot   = dh.hot;
	    const sgTotal = sgHot.countRows(); // visible rows only

	    // Determine schema+class from the first moved row's final position.
	    const firstPR = sgHot.toPhysicalRow(finalIndex);
	    if (firstPR == null) return;
	    const schema  = sgHot.getSourceDataAtCell(firstPR, sgSchemaCol) ?? '';
	    const cls     = sgHot.getSourceDataAtCell(firstPR, sgClassCol)  ?? '';

	    // Collect section names for this schema+class in new visual order.
	    const sectionOrder = [];
	    for (let vr = 0; vr < sgTotal; vr++) {
	      const pr = sgHot.toPhysicalRow(vr);
	      if (pr == null) continue;
	      if ((sgHot.getSourceDataAtCell(pr, sgSchemaCol) ?? '') !== schema) continue;
	      if ((sgHot.getSourceDataAtCell(pr, sgClassCol)  ?? '') !== cls)    continue;
	      const name = sgHot.getSourceDataAtCell(pr, sgNameCol) ?? '';
	      if (name) sectionOrder.push(name);
	    }

	    // ── Update Field tab ranks ─────────────────────────────────────────
	    const slotDh = this.context.dhs['Slot'];
	    if (!slotDh) return;
	    const slotHot   = slotDh.hot;
	    const slotTotal = slotHot.countSourceRows();

	    // Bucket Field rows by section, preserving current rank within each bucket.
	    const sectionFields  = new Map();
	    const noSectionFields = [];
	    for (let pr = 0; pr < slotTotal; pr++) {
	      if ((slotHot.getSourceDataAtCell(pr, slotDh.schema_name_column)   ?? '') !== schema) continue;
	      // When cls is '' all slot groups are schema-wide; include slots from all classes.
	      if (cls !== '' && (slotHot.getSourceDataAtCell(pr, slotDh.slot_class_id_column) ?? '') !== cls) continue;
	      const group = slotHot.getSourceDataAtCell(pr, slotDh.slot_group_column) ?? '';
	      const rank  = Number(slotHot.getSourceDataAtCell(pr, slotDh.slot_rank_column)) || Infinity;
	      if (group && sectionOrder.includes(group)) {
	        if (!sectionFields.has(group)) sectionFields.set(group, []);
	        sectionFields.get(group).push({ pr, rank });
	      } else {
	        noSectionFields.push({ pr, rank });
	      }
	    }
	    for (const fields of sectionFields.values()) fields.sort((a, b) => a.rank - b.rank);
	    noSectionFields.sort((a, b) => a.rank - b.rank);

	    // Defer rank writes + Field-tab sort out of the hook so HOT finishes its
	    // own post-move housekeeping before we mutate source data and re-sort.
	    setTimeout(() => {
	      let rank = 1;
	      for (const section of sectionOrder) {
	        for (const { pr } of (sectionFields.get(section) ?? [])) {
	          slotHot.setSourceDataAtCell(pr, slotDh.slot_rank_column, rank++);
	        }
	      }
	      for (const { pr } of noSectionFields) {
	        slotHot.setSourceDataAtCell(pr, slotDh.slot_rank_column, rank++);
	      }

	      const sortPlugin = slotHot.getPlugin('multiColumnSorting');
	      if (sortPlugin && slotDh.defaultMultiColumnSortConfig)
	        sortPlugin.sort(slotDh.defaultMultiColumnSortConfig);
	      else
	        slotHot.render();
	      // sgHot visual order is already correct (manualRowMove handled it).
	    }, 0);
	  });
	}
  ...
  hot_settings.multiColumnSorting.compareFunctionFactory = (sortOrder, columnMeta) => {
    ...
    return (valueA, valueB) => {
	  ...
	  // Sort option 1 (default): schema → table → rank (sections visible but not a sort key)
	  dh.slotSortByTableRank = [
	    ...
	    {column: dh.slot_title_column,    sortOrder: 'asc'}, // title (alphabetical tiebreaker)
	  ];
  ...
  const KEY_COLUMNS = new Set([
	  ...
	  dh.hot.addHook('afterChange', (changes, source) => {
	    ...
	    for (const [visualRow, col, oldVal, newVal] of changes) {
	      ...
	      // ── Base slot (type=slot) edit: offer to propagate to all slot_usage rows ──
	      if (slotType === 'slot') {
	        ...
	        // Collect every slot_usage row for this slot+schema — they all inherit.
	        const inheriting = [];
	        ...
	        if (!inheriting.length) continue; // no slot_usage rows — nothing to propagate

	        const fieldTitle = dh.slots[col]?.title ?? dh.slots[col]?.name ?? `column ${col}`;
	        const tableNames = inheriting.map(su => `<em>${su.classId}</em>`).join(', ');
	        const isExpert   = !!dh.context.expert_user;
	        const expertWarning = isExpert ? '' :
	          `<p class="text-warning mt-2"><strong>Expert User mode is required</strong> ` +
	          `to propagate changes to inherited fields.</p>`;

	        const body =
	          `<p>You changed <strong>${fieldTitle}</strong> on the schema field library entry ` +
	          `for <strong>${slotName}</strong>.</p>` +
	          `<p>The following table(s) reuse this field and inherit this attribute: ${tableNames}.</p>` +
	          `<p>Propagate <strong>${newVal ?? '(cleared)'}</strong> to all of them?</p>` +
	          expertWarning;

	        _inheritedEditDialogOpen = true;
	        const capturedInheriting = inheriting.slice();
	        const capturedCol        = col;
	        const capturedNewVal     = newVal;
	        ...
	        setTimeout(() => {
	          dhChoose(
	            ...
	            {
	              title: 'Schema field updated',
	              html: true,
	              cancelLabel: 'Keep existing table values',
	              disabledIndices: isExpert ? [] : [0],
	            }
	          ).then(choice => {
	            _inheritedEditDialogOpen = false;
	            if (choice === 0) {
	              const updates = [];
	              for (const su of capturedInheriting) {
	                const vr = dh.hot.toVisualRow(su.physRow);
	                if (vr !== null) updates.push([vr, capturedCol, capturedNewVal]);
	              }
	              if (updates.length) dh.hot.setDataAtCell(updates, 'cascade_confirm');
	            }
	          });
	        }, 0);
	      ...
	      // Build the context note for the "Override for this table" option.
	      let overrideNote;
	      ...
	      const expertWarning = isExpert ? '' :
	        `<p class="text-warning mt-2"><strong>Expert User mode is required</strong> ` +
	        `for any of the above changes.</p>`;
	      ...
	      const body =
	        ...
	        `<li><strong>Override for this table</strong> — Store the new value in ` +
	        ...
	        `<li><strong>Convert to attribute</strong> — Convert this field from ` +
	        `an inherited slot reference to a standalone table attribute. All attributes of this ` +
	        `field are editable independently of the schema field specification.</li>` +
	        `</ul>` +
	        expertWarning;
	      ...
	      setTimeout(() => {
	        dhChoose(
	          body,
	          ['Update schema field', 'Override for this table', 'Convert to attribute'],
	          {
	            title: 'Inherited field changed',
	            html: true,
	            cancelLabel: 'Cancel (revert change)',
	            disabledIndices: isExpert ? [] : [0, 1, 2],
	          }
	        ).then((choice) => {
	          _inheritedEditDialogOpen = false;
	          if (choice === 0) {
	...
	showFieldKeyModal(dh, visualRow = null) {
	  ...
	  // Two optgroups:
	  //   • "schema fields Section" — slot_groups set on schema.slots (type='slot') rows
	  //   • "<ClassName>-specific Section" — slot_groups set on the class's own attribute
	  //     rows (type='attribute') that are not already in the schema group
	  ...
	  const rebuildSlotGroupDropdown = (schemaId, classId, preselect) => {
	    ...
	    // Group 1: slot_groups referenced by schema.slots (type='slot') rows —
	    // schema-wide sections available to every class.
	    const schemaGroups = new Set();
	    for (let r = 0; r < hot.countSourceRows(); r++) {
	      ...
	      if (sg) schemaGroups.add(sg);
	    }

	    // Group 2: slot_groups referenced by the selected class's attribute rows
	    // (type='attribute') not already listed in Group 1.
	    const classGroups = new Set();
	    if (classId) {
	      for (let r = 0; r < hot.countSourceRows(); r++) {
	        if (hot.getSourceDataAtCell(r, dh.slot_type_column)      !== 'attribute') continue;
	        if (hot.getSourceDataAtCell(r, dh.schema_name_column)    !== schemaId)    continue;
	        if ((hot.getSourceDataAtCell(r, dh.slot_class_id_column) ?? '') !== classId) continue;
	        const sg = hot.getSourceDataAtCell(r, dh.slot_group_column);
	        if (sg && !schemaGroups.has(sg)) classGroups.add(sg);
	      }
	    }

	    if ([...schemaGroups].length) {
	      const $grp = $('<optgroup>').attr('label',
	        this.context.applyDhTerms('{{schema.slots}} {{Section}}'));
	      for (const sg of [...schemaGroups].sort()) $grp.append($('<option>').val(sg).text(sg));
	      $sg.append($grp);
	    }

	    if ([...classGroups].length) {
	      const $grp = $('<optgroup>').attr('label',
	        `${classId}-specific ` + this.context.applyDhTerms('{{Section}}'));
	      for (const sg of [...classGroups].sort()) $grp.append($('<option>').val(sg).text(sg));
	      $sg.append($grp);
	    }

	    // Ensure the preselected value is selectable even if it doesn't appear in
	    // either group (e.g. a slot_usage row whose section isn't on any schema slot).
	    if (preselect) {
	  ...
	  const updateSlotTypeRow = () => {
	    ...
	    // Reset checkbox to unchecked and set label based on the current initSlotType.
	    // Changing schema/class/field resets the "change type" intent.
	    ...
	    if (initSlotType === 'slot_usage') {
	      ...
	      $('#fkm-change-type-label').text(this.context.applyDhTerms('change to {{class slot}} + {{schema slot}} (if needed)'));
	    }
	  ...
	  $('#fkm-change-type').off('change.fkm').on('change.fkm', function() {
	    // Show "Copy schema-inherited field attributes" only when switching slot_usage → attribute.
	    const classId = $('#fkm-class-id').val();
	    const showCopyRow = (initSlotType === 'slot_usage') &&
	                        $(this).is(':checked') &&
	                        !!classId;
	  ...
	  $('#field-key-modal-title').text(
	    this.context.applyDhTerms(visualRow !== null ? 'Edit {{Slot}}' : 'Add {{Slot}}')
	  );
	  ...
	  $('#fkm-header-desc').html(
	    ...
	    this.context.applyDhTerms('{{slots}} are highlighted.')
	  );
	  // Type dropdown option labels — update to match active terminology.
	  $('#fkm-field-type option[value="slot"]').text(this.context.applyDhTerms('{{schema slot}}'));
	  $('#fkm-field-type option[value="slot_usage"]').text(this.context.applyDhTerms('{{table slot}}'));
	  $('#fkm-field-type option[value="attribute"]').text(this.context.applyDhTerms('{{table attribute}}'));
	  ...
	  $('#fkm-slot-group-desc').text(this.context.applyDhTerms(
	    'Optional display {{slot_group}} for this {{slot}}. New {{slot_groups}} can be added on the "{{Slot_group}}" tab.'
	  ));
	  $('#fkm-class-id-desc').text(
	    this.context.applyDhTerms('Select a {{class}} that schema {{slot}} will appear in.')
	  );
	  $('#fkm-field-type-desc').text(
	    this.context.applyDhTerms(
	      '{{schema slot}}: located in the {{schema.slots}} list; ' +
	      '{{class slot}}: located in a {{class.slot_usage}} list, derived from a {{schema slot}}; ' +
	      '{{attribute slot}}: Located in {{class.attributes}}, not connected to a {{schema slot}}.'
	    )
	  );
	  $('#fkm-error').hide().text('');
	  $modal.modal('show');
	  ...
	  $('#fkm-confirm-btn').off('click.fkm').on('click.fkm', () => {
	    ...
	    const classId   = $('#fkm-class-id').val();
	    const fieldId   = $('#fkm-name').val().trim();
	    const slotGroup = $('#fkm-slot-group').val() || '';
	    ...
	    if (visualRow === null) {
	      ...
	      // If switching attribute → slot_usage and no base slot exists yet,
	      // insert a new schema slot row automatically (same logic as Add mode Case C).
	...
	_buildSchemaYaml() {
		...
		for (let [, tab_name] of components.entries()) {
		  ...
		  if (tab_name === 'Slot') {
		    ...
		    // group 0 = base slot definitions; 1 = slot_usage; 2 = attribute
		    const _grp = (t) => t === 'slot_usage' ? 1 : t === 'attribute' ? 2 : 0;
		    rows.sort((a, b) => {
  ...
  loadSchemaYAML(text) {
    ...
    // Tracks slot_group names already added to the SlotGroup tab for this schema
    // load.  Used to deduplicate across schema.slots and all class.slot_usage
    // entries so each section name appears exactly once per schema.
    const seenSlotGroupNames = new Set();
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Slot': {
            ...
            // Collect slot_group from schema-level slots for the SlotGroup tab.
            if (value.slot_group && !seenSlotGroupNames.has(value.slot_group)) {
              seenSlotGroupNames.add(value.slot_group);
              this.addRowRecord(dh_sg, tables, {
                schema_id: loaded_schema_name,
                class_id:  '',
                name:      value.slot_group
              });
            }
            break;
          }
          ...
          case 'Class': {
            ...
            // Deduplicates against names already collected from schema.slots so each
            // section name appears exactly once per schema (schema-wide concept).
            if (value.slot_usage) {
              const sorted_slots = Object.values(value.slot_usage).sort(
                (a, b) => (a.rank ?? Infinity) - (b.rank ?? Infinity)
              );
              for (let obj of sorted_slots) {
                if (obj.slot_group && !seenSlotGroupNames.has(obj.slot_group)) {
                  seenSlotGroupNames.add(obj.slot_group);
                  this.addRowRecord(dh_sg, tables, {
                    schema_id: loaded_schema_name,
                    class_id:  '',
                    name:      obj.slot_group
    ...
    // state and break tab row-filtering.
    {
      ...
      if (rootEntry) {
        ...
        const slotDh = this.context.dhs['Slot'];
        const classDh = this.context.dhs['Class'];
        const ukDh   = this.context.dhs['UniqueKey'];
        const sgDh   = this.context.dhs['SlotGroup'];
        if (slotDh)  tables['Slot']      = tables['Slot'].filter(
          row => row[slotDh.slot_class_id_column] !== rootClassName);
        if (classDh) tables['Class']     = tables['Class'].filter(
          row => row[classDh.slot_name_to_column['name']] !== rootClassName);
        if (ukDh)    tables['UniqueKey'] = tables['UniqueKey'].filter(
          row => row[ukDh.slot_name_to_column['class_id']] !== rootClassName);
        if (sgDh)    tables['SlotGroup'] = tables['SlotGroup'].filter(
          row => row[sgDh.slot_name_to_column['class_id']] !== rootClassName);
      }
    }
  ...
  openCopyToSchemaModal(sourceDh) {
    ...
    const updateClassSelect = (targetSchema) => {
      ...
      if (classNames.length > 0) {
        ...
        $('#copy-class-assign').prop('checked', true);
        $('#copy-to-class-select').show();
        $('#copy-class-assign-row').show();
        $('#copy-to-class-row').show();
      } else {
        // No classes in target schema; hide "Assign to:" and force preserve mode.
        $('#copy-class-preserve').prop('checked', true);
        $('#copy-class-assign-row').hide();
        $('#copy-to-class-row').show();
      }
    };
