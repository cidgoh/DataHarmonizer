...
export class SchemaEditor {
  ...
  */
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			...
			case 'Class':
				...
				break;
			case 'SlotGroup':
				hot_settings.fixedColumnsLeft = 0;
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  dh._slotTypeFilter = new Set(); // show all on first load
	  ...
	  dh.defaultMultiColumnSortConfig = [
	    {
	      column: dh.slot_name_column, // slot.name
	      sortOrder: 'asc'
	    },
	    /*
			{
	...
	previewSchema() {
		const result = this._buildSchemaYaml();
		if (!result) {
		  dhAlert("The currently selected schema needs to be named before previewing. If you have named your schema please make sure it is selected.");
		  return;
		}
		const { schema_name, yaml_string } = result;
