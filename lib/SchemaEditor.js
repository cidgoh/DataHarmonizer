...
export class SchemaEditor {
  ...
  */
	...
	hotSettingsMenuHooks(dh, hot_settings, tab_name, keyColumns = null) {
	  ...
	  // Intercept the Delete key so it performs a full row removal — matching
	  // the 'remove row(s)' context-menu item — rather than HOT's default
	  // cell-clear behaviour.  This prevents blank rows with empty read-only
	  // FK fields from being left behind in child tables.
	  dh.hot.addHook('beforeKeyDown', (e) => {
	    if (e.key !== 'Delete') return;
	    if (dh.hot.getActiveEditor()?.isOpened()) return;
	    // HOT's recorder checks event.isImmediatePropagationEnabled (not the
	    // native stopImmediatePropagation flag) to decide whether to continue
	    // processing the key as a shortcut.  Set it directly.
	    e.isImmediatePropagationEnabled = false;
	    dh.hot.getPlugin('contextMenu').executeCommand('remove_row');
	  });
	  hot_settings.afterRemoveRow = (index, amount, physicalRows, source) => {
	    ...
	    // Deferred so it runs after HOT finishes its internal post-deletion re-render,
	    // which would otherwise reset the hiddenRows state we just applied.
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  const slot_editable_keys = [dh.slot_type_column, dh.slot_class_id_column, dh.slot_name_column];
	  ...
	  */
	  ...
	  //
	  // Default sort: schema → name → slot_type → class_id.
