	  ...
	  dh.hot.addHook('afterChange', (changes, source) => {
	    ...
	    for (const [visualRow, col, oldVal, newVal] of changes) {
	      ...
	      if (slotType === 'slot') {
	        ...
	        // Cascade the new value to slot_usage rows whose current value was an
	        // explicit copy of the old base-slot value.  Rows with an empty value
	        // already inherit implicitly and need no update; rows with a different
	        // non-empty value are explicit overrides and must not be touched.
	        const cascadeCells = [];
	        for (const su of inheriting) {
	          if (_emptyVal(su.currentVal) || su.currentVal !== oldVal) continue;
    ...
    $('#fkm-warn').hide();
	        ...
	        if (!this.context.expert_user) {
	          $('#fkm-error').show().text(
	            this.context.applyDhTerms(
	              'Expert user mode is required to add a {{schema slot}}. '
	  ...
	  $('#fkm-field-type-desc').text(
	    this.context.applyDhTerms(
	      ...
	      '{{class attribute}}: A {{slot}} located in a {{class\'s}} {{class attributes}} list, not connected to a {{schema slot}}.'
	    )
	  );
	    ...
	    $('#fkm-error').show().text(
	      this.context.applyDhTerms(
	        visualRow !== null
	          ...
	          : 'Expert user mode is required to add a {{schema slot}}. Enable it via File menu → "Toggle expert user mode".'
	      )
	    );
	  } else {
	    $('#fkm-error').hide().text('');
	    ...
	    if (visualRow === null) {
	      ...
	      if (slot_type === 'slot') {
	        ...
	        if (groupChanged) {
	          ...
	          const newVRowS = hot.toVisualRow(physRow);
	          if (newVRowS !== null && newVRowS !== -1) {
	            hot.selectRows(newVRowS);
	            hot.scrollViewportTo(newVRowS, 0);
	          }
	        }
	      } else {
	        ...
	        if (slotGroup !== initSlotGroup && classId === initClassId &&
	            (this.context.expert_user || slot_type === 'attribute')) {
	          ...
	          const newVRow3 = hot.toVisualRow(physRow);
	          if (newVRow3 !== null && newVRow3 !== -1) {
	            hot.selectRows(newVRow3);
	            hot.scrollViewportTo(newVRow3, 0);
	          }
	        }
	          ...
	          if (defPhysRow !== undefined) {
	            ...
	            for (let col = 0; col < hot.countCols(); col++) {
	              ...
	              if (_empty2(attrVal) || String(attrVal) !== String(baseVal)) {
	                toCopy.push([visualRow, col, baseVal]);
	              }
	            }
	            if (toCopy.length) hot.setDataAtCell(toCopy, 'field_key_modal');
	          }
	        ...
	        // Re-rank after attribute → slot_usage conversion: the copy-base-values
	        // block above may have updated slot_group to match the base schema slot.
	        // Re-position the converted slot at the end of its effective section and
	        // re-sort + scroll to its new location.
	        if (initSlotType === 'attribute' && slot_type === 'slot_usage') {
	          ...
	          const newVRow4 = hot.toVisualRow(physRow);
	          if (newVRow4 !== null && newVRow4 !== -1) {
	            hot.selectRows(newVRow4);
	            hot.scrollViewportTo(newVRow4, 0);
	          }
	        }
