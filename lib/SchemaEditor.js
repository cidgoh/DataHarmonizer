	  ...
	  dh.hot.addHook('beforeChange', (changes, source) => {
	    ...
	    if (!dh.context.expert_user) {
	      ...
	      for (const [visualRow, col, oldVal, newVal] of changes) {
	        ...
	        if (slotType === 'slot') {
	          setTimeout(() => dhAlert(
	            `<strong>${slotName}</strong> is a schema field library entry. ` +
	            `Enable <strong>Expert User</strong> mode before editing it.`,
	            { title: 'Expert User mode required', html: true }
	          ), 0);
	          return false;
	        }
	        ...
	        if (slotType === 'slot_usage' && dh.slotDefinitionIndex) {
	          ...
	          if (defPhysRow !== undefined) {
	            ...
	            if (!_emptyVal(baseVal) && oldVal === baseVal) {
	              // The cell is currently inheriting from the schema field library.
	              setTimeout(() => dhAlert(
	                `<strong>${slotName}</strong> inherits this value from the schema field library. ` +
	                `Enable <strong>Expert User</strong> mode before editing inherited fields.`,
	                { title: 'Expert User mode required', html: true }
	              ), 0);
	              return false;
	            }
	    ...
	    for (const [visualRow, col, oldVal, newVal] of changes) {
	      ...
	      if (slotType === 'slot') {
	        ...
	        const listItems = inheriting
	          .map(su => {
	            const valPart = (su.currentVal != null && su.currentVal !== '')
	              ? `: ${su.currentVal}`
	              : '';
	            return `<li><em>${su.classId}</em>${valPart}</li>`;
	          })
	          .join('');
	        ...
	        setTimeout(() => {
	          dhAlert(body, { title: `${slotName} updated`, html: true })
