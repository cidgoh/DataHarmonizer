...
export class SchemaEditor {
  ...
  */
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  // Block row removal for non-expert users — field rows are structural and
	  // deleting them without understanding cascade consequences is risky.
	  hot_settings.contextMenu.items['remove_row'] = {
	    name: 'Remove row(s)',
	    async callback() {
	      if (!dh.context.expert_user) {
	        await dhAlert(
	          'Row removal on the Field tab is only available in Expert User mode.\n\n' +
	          'Enable Expert User mode via the Settings menu to remove rows.',
	          { title: 'Expert User Mode Required' }
	        );
	        return;
	      }
	      await dh.removeSelectedRows();
	    },
	  };
  ...
  // Render the slot_type column using the enum's human-readable titles
	  ...
	  dh.hot.addHook('afterChange', (changes, source) => {
	    ...
	    for (const [visualRow, col, oldVal, newVal] of changes) {
	      ...
	      const expertWarning = isExpert ? '' :
	        ...
	        `to apply changes to inherited fields. Enable it via the Display menu to ` +
	...
	showFieldKeyModal(dh, visualRow = null) {
	  ...
	  if (isAllRecords) {
	    ...
	    const activeSchema = this.getSchemaEditorSelectedSchema() || initSchemaId;
	    if (activeSchema) $schemaSelect.append($('<option>').val(activeSchema).text(activeSchema));
	  ...
	  $('#fkm-confirm-btn').off('click.fkm').on('click.fkm', () => {
	    ...
	    const buildCells = (row) => {
	      ...
	      if (title)     cells.push([row, dh.slot_title_column,    title]);
	      return cells;
	    };
	    ...
	    if (visualRow === null) {
	      ...
	      hot.setDataAtCell(buildCells(targetRow), 'field_key_modal');
