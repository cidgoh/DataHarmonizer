	...
	showFieldKeyModal(dh, visualRow = null) {
	  ...
	  $('#fkm-field-type-desc').text(
	    this.context.applyDhTerms(
	      ...
	      '{{class attribute}}: A {{slot}} located in {{class attributes}}, not connected to a {{schema slot}}.'
	    )
	  );
	  $('#fkm-error').hide().text('');
	  $modal.modal('show');
    ...
    if (!substantive.length && !enrichment.length && !cosmetic.length) {
      ...
      if (substantive.length) {
        html += `<div class="compare-section">
          <h6>SUBSTANTIVE CHANGES (${substantive.length})</h6>
          ${substantive.map(fmtEntry).join('')}
        </div>`;
      } else {
        html += `<p class="text-muted small">SUBSTANTIVE CHANGES: 0</p>`;
      }
      if (enrichment.length)
        html += `<p class="text-muted small mt-1">ENRICHMENT — permissible_value title additions: ${enrichment.length} (suppressed)</p>`;
      if (cosmetic.length)
        html += `<p class="text-muted small">COSMETIC — redundant name fields / list wrapping: ${cosmetic.length} (suppressed)</p>`;
