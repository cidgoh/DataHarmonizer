...
export class SchemaEditor {
		    ...
		    switch (enum_name) {
		      ...
		      case 'SchemaSlotGroupMenu':
		        ...
		        for (let row of sg_rows) {
		        	const sg_name_text = sg_dh.hot.getDataAtCell(row, sg_name_ptr);
  ...
  */
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			case 'Schema': 
				this.hotSettingsMenuHooks(dh, hot_settings, ['SchemaMenu','SchemaClassMenu'])
				break;
			case 'Class': 
				this.hotSettingsMenuHooks(dh, hot_settings, ['SchemaClassMenu'])
				break;
			case 'SlotGroup':
	  		hot_settings.fixedColumnsLeft = 0;
				this.hotSettingsMenuHooks (dh, hot_settings, ['SchemaSlotGroupMenu'])
				break;
			case 'Slot': 
		...
		afterChange
              if (self.context.schemaEditor) {
                ...
                case 'Slot':
                  ...
                  self.context.schemaEditor.refreshMenus(['SchemaClassMenu','SchemaSlotMenu','SchemaSlotGroupMenu',]);
                  break;
                // SlotGroup menu refresh is handled entirely in SchemaEditor.initSlotGroupTab hooks.
...
*/
	...
	hotSettingsMenuHooks (dh, hot_settings, menus) {

	  // All three mutating hooks are handled here so that 
	  // refresh logic for this tab is consolidated in one place.

	  // ISSUE: DataHarmonizer.js is setting controlling this!!!!!!!
	  dh.hot.addHook('afterChange', 
			(changes, source) => {
				if (changes) {
					this.refreshMenus(menus, source);
					//					if (source === 'upload')
					//alert('changed ' + source);
				}
			}
	  );
	  //hot_settings.afterChange    = (changes, source) => {
	  //	if (changes) this.refreshMenus(menus, source);
	  //};
	  /*
		This is an empty row so no menu change needed
	  hot_settings.afterCreateRow = (index, amount, source) => {
	  	this.refreshMenus(menus, source);
	  	alert(menus)
	  };
	  */
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  // work right away.
	  // ALSO: Drag and drop of rows seems to destroy className.
	  hot_settings.cells = function(row, col) { 
	    ...
	    let visual_row = dh.hot.toVisualRow(row);
	...
	saveSchema () {
		...
		*/
		...
		if (schema_name === null) {
		  alert("The currently selected schema needs to be named before saving. If you have named your schema please make sure it is selected before saving");
		  return;
		}
