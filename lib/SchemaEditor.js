...
export class SchemaEditor {
  ...
  initMenus() {
  	...
  	// populated later by refreshMenus().
		...
		// "Record(s) by selected schema" option is commented out in toolbar.html pending further work.
		// Restore both the HTML block and this line when ready.
	...
	*/
	...
	translationForm(dh) {
	    ...
	    // Prefer HOT's live selection (accurate after display-mode filter changes
	    // visual indices); fall back to cached current_selection.
	    const _schemaSel = schema.hot.getSelected();
	    const schema_row = _schemaSel?.length ? _schemaSel[0][0] : schema.current_selection[0];
	    if (schema_row == null || schema_row < 0 || schema_row >= schema.hot.countRows()) {
	    ...
	    // Guard: translating the schema itself requires Expert User mode.
	    if (dh.template_name === 'Schema' && !dh.context.expert_user) {
	      dhAlert('Translating a schema requires Expert User mode. Enable it via the Settings menu → "Toggle expert user mode".');
	      return false;
	    }
	    ...
	    if (slotTypeCol != null) {
	      ...
	      if (hasSlotUsage && !dh.context.expert_user) {
	        dhAlert('Translating class-specific field overrides (slot_usage) requires Expert User mode. Enable it via the Settings menu → "Toggle expert user mode".');
	        return false;
	      }
	    }
	    ...
	    for (let row = dh.current_selection[0]; row <= dh.current_selection[2]; row++) {
	      ...
	      if (slotTypeCol != null) {
	        const physRow = dh.hot.toPhysicalRow(row);
	        slotType = dh.hot.getSourceDataAtCell(physRow, slotTypeCol);
	      ...
	      const fieldTypeLabelRaw = this.context.applyDhTerms(
	        slotType === 'slot_usage' ? '{{class slot}}'
	        : slotType === 'attribute' ? '{{attribute slot}}'
	        : slotType === 'slot' ? '{{schema slot}}'
	        // Use capitalized DH_TERMS keys for tab-based types; singular for PermissibleValue.
	        : { Class: '{{Class}}', Enum: '{{Enum}}', PermissibleValue: '{{permissible value}}', Schema: '{{Schema}}' }[dh.template_name]
	          || `{{${dh.template_name.toLowerCase()}}}`
	      );
	      // Capitalise first letter so slot-type labels ('derived field' etc.) also lead with a capital.
	      const fieldTypeLabel = fieldTypeLabelRaw.charAt(0).toUpperCase() + fieldTypeLabelRaw.slice(1);
	      if (key2) {
	        translate_rows += `<tr class="translate translate_key"><td colspan="${column_count+2}"><strong>${fieldTypeLabel}:</strong> ${key2}</td></tr>`;
	      } else if (key) {
	        translate_rows += `<tr class="translate translate_key"><td colspan="${column_count+2}"><strong>${fieldTypeLabel}:</strong> ${key}</td></tr>`;
	      }

	      translate_rows += `<tr class="translate"><td nowrap>${default_language}</td>${default_row_text}<td></td></tr>`;

	      // DISPLAY locale for each schema in_language menu item (sorted by label)
	      const sortedLocales = Object.entries(locales).sort(([a], [b]) => {
	        const titleA = locale_map[a]?.title || a;
	        const titleB = locale_map[b]?.title || b;
	        return titleA.localeCompare(titleB);
	      });
	      for (const [locale, locale_schema] of sortedLocales) {
	        let translate_cells = '';
	        let path = '';
	        for (let column_name of row_text_cols) {
	          ...
	          // For slot_usage / attribute rows, show the global-slot locale value
	          // (if any) as a hint controlled by the "show defaults" checkbox.
	          let schema_hint = '';
	          if (sub_part && key2) {
	            let schema_derived = locale_schema.slots?.[key2]?.[column_name] || '';
	            if (Array.isArray(schema_derived)) {
              schema_derived = schema_derived.length && typeof schema_derived[0] === 'object'
                ? schema_derived.map(x => x.value ?? String(x)).join('; ')
                : schema_derived.join('; ');
            }
	            if (schema_derived) {
	              hasSchemaDefaults = true;
	              schema_hint = `<div class="schema-derived-hint">`
	                + `<span style="color:#888;font-size:0.85em">default: </span>`
	                + `<span style="color:#555;white-space:pre-wrap;word-break:break-word;font-size:0.85em">`
	                + `${schema_derived}</span></div>`;
	            }
	          }

	          translate_cells += `<td style="width:${colPct}%;max-width:${colPct}%"><textarea name="${column_name}" data-path="${path}">${value}</textarea>${schema_hint}</td>`;
	        }
	        ...
	        const trans_language = locale_map[locale]?.title || locale;
	    ...
	    $('#translate-modal-title').text(
	      this.context.applyDhTerms(`{{${dh.template_name}}}`) + ' Locale Translations'
	    );

	    // Show/hide the "show defaults" checkbox based on whether any derived defaults exist.
	    const $showDefaults = $('#translation-show-defaults');
	    const $showDefaultsCb = $('#translation-show-defaults-cb');
	    if (hasSchemaDefaults) {
	      $('#translation-show-defaults-label').text(
	        this.context.applyDhTerms('Show {{schema}} {{slot}} default translation')
	      );
	      $showDefaultsCb.prop('checked', false);
	      $showDefaults.show();
	    } else {
	      $showDefaults.hide();
	    }
	    // Bind checkbox toggle (rebind to avoid stacking handlers across form opens).
	    $showDefaultsCb.off('change').on('change', function () {
	      $('.schema-derived-hint').toggle(this.checked);
	    });

	    $('#translate-modal .modal-body').scrollTop(0);
  ...
  setLocales(changes, grid_changes, hot) {
    ...
    let old_langs = changes.locales.old_value
      ...
      : new Set();
    let new_langs_user = changes.locales.value
      ? new Set(changes.locales.value.split(';'))
      : new Set();

    // ── Strip any locale that duplicates the default language ─────────────────
    const in_lang_col  = dh_schema.slot_name_to_column['in_language'];
    const row          = grid_changes[0]?.[0] ?? 0;
    const default_lang = (in_lang_col !== undefined)
      ? (hot.getDataAtCell(row, in_lang_col) || '').trim()
      : '';
    const locale_map = this.schema.enums?.LanguagesMenu?.permissible_values || {};
    // newly_added_dup: user just tried to add the default language as a translation locale.
    const newly_added_dup = default_lang
      ? (new_langs_user.has(default_lang) && !old_langs.has(default_lang))
      : false;
    // Always remove the default language from the translation locale set.
    const new_langs = new Set(new_langs_user);
    if (default_lang) new_langs.delete(default_lang);

    let deleted = Array.from(old_langs.difference(new_langs).keys());
    let created = Array.from(new_langs.difference(old_langs).keys());

    if (deleted.length || created.length || newly_added_dup) {
      ...
      const filtered_value = Array.from(new_langs).join(';');

      // Only-duplicate case: alert and revert silently (no confirm needed).
      if (!deleted.length && !created.length && newly_added_dup) {
        const dup_title = locale_map[default_lang]?.title || default_lang;
        dhAlert(
          `"${dup_title}" cannot be added as a translation locale ` +
          `because it is already set as the default language.`,
          { title: 'Duplicate locale not allowed' }
        );
        hot.setDataAtCell(
          grid_changes.map(([r, col]) => [r, col, filtered_value]),
          'locale_confirm'
        );
        return false;
      }
      ...
      if (newly_added_dup) {
        const dup_title = locale_map[default_lang]?.title || default_lang;
        if (created.length || deleted.length) message += '\n\n';
        message += `NOTE: "${dup_title}" cannot be added as a translation locale ` +
          `because it is already set as the default language.`;
      }
  ...
  */
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			case 'Schema': {
				...
				dh.hot.addHook('afterSelection', (row, _col) => {
					if (row !== _lastSchemaRow) {
						...
						setTimeout(() => {
							// Only refresh when a schema row is actually selected.
							// restartInterface() calls selectCell(0,0) then immediately
							// deselectCell() — the setTimeout fires while HOT is deselected,
							// causing getSchemaEditorSelectedSchema() to return null and
							// refreshMenus() to fall back to DH_LinkML enums (wrong schema).
							// Skipping here is safe: afterDeselect (below) resets _lastSchemaRow
							// to null, so the next user click on any row triggers a fresh refresh.
							if (!dh.hot.getSelected()?.length) return;
				...
				// Reset _lastSchemaRow when HOT is programmatically deselected
				// (restartInterface calls selectCell(0,0) then deselectCell()).
				// Without this, _lastSchemaRow stays at 0 and a subsequent user click
				// on Schema row 0 is seen as 'same row — skip', so menus never refresh
				// and SchemaEnumMenu shows the previous schema's enums.
				...
				// Validate: translation locales must not duplicate the default language.
				const inLangCol  = dh.slot_name_to_column['in_language'];
				const localesCol = dh.slot_name_to_column['locales'];
				if (inLangCol !== undefined && localesCol !== undefined) {
					const origGetInvalidCells = dh.getInvalidCells.bind(dh);
					dh.getInvalidCells = (data) => {
						const errors = origGetInvalidCells(data);
						const result = { ...errors };
						const lm = this.schema.enums?.LanguagesMenu?.permissible_values || {};
						for (let vRow = 0; vRow < data.length; vRow++) {
							const row = data[vRow];
							if (!row) continue;
							const defaultLang = (row[inLangCol] || '').trim();
							const localesRaw  = (row[localesCol] || '').trim();
							if (!defaultLang || !localesRaw) continue;
							const locales = localesRaw.split(';').map(s => s.trim()).filter(Boolean);
							if (!locales.includes(defaultLang)) continue;
							const langTitle = lm[defaultLang]?.title || defaultLang;
							const vRowStr = String(vRow);
							const colStr  = String(localesCol);
							if (!result[vRowStr]) result[vRowStr] = {};
							if (!result[vRowStr][colStr]) {
								result[vRowStr][colStr] =
									`"${langTitle}" is also set as the default language — ` +
									`remove it from the translation languages`;
							}
						}
						return result;
					};
						...
						for (const [row, , oldName, newName] of nameChanges) {
							...
							for (const depTabName of ['Slot', 'UniqueKey']) {
								...
								if (physRows.length > 0) {
									// Write directly to source data instead of setDataAtCell.
									// HOT's post-setDataAtCell `done` callback calls getCell(),
									// which crashes when the tab is hidden (viewport not yet
									// initialised → startRow undefined).
									const sourceData = depDh.hot.getSourceData();
									for (const pr of physRows) {
										if (sourceData[pr]) sourceData[pr][depClassIdCol] = newName;
									}
									depDh.hot.render();
								}
							}
	 ...
	 * reliable approach.
	 */
	initUniqueKeyTab(dh, _hot_settings) {
	...
	filterInheritedSlotUsageErrors(dh, data, errors) {
	  ...
	  // Key/identity columns whose values in a slot_usage row are set by the system
	  // (not editable data).  SchemaClassMenu and SchemaMenu are schema-scoped, so
	  // their range validation produces false positives for slot_usage rows whose
	  // schema/class belongs to a non-selected schema.  Suppress all errors on these
	  // columns for every slot_usage row.
	  ...
	  for (const [vRowStr, colErrors] of Object.entries(errors)) {
	    ...
	    for (const [colStr, message] of Object.entries(colErrors)) {
	      ...
	      // Always suppress errors on identity/key columns: their values are
	      // correct by construction and menu-scoping causes false positives.
	      if (IDENTITY_COLS.has(col)) continue;
	...
	validateSlotUsageMismatch(dh, data, errors) {
	  ...
	  for (let vRow = 0; vRow < data.length; vRow++) {
	    ...
	    for (let col = 0; col < numCols; col++) {
	      ...
	      // Also collapse internal whitespace: doPreValidationRepairs() folds consecutive
	      // whitespace to a single space in WhitespaceMinimizedString columns before
	      // getInvalidCells is called, so the usage cell may have a single space where
	      // the base-slot source data still has the original double space.  Normalising
	      // both sides avoids false-positive mismatches on insignificant whitespace.
	      const _normalizeWS = (s) => s.replace(/\s+/g, ' ').trim();
	      const usageStr = _normalizeWS(String(usageVal));
	      const baseStr  = _empty(baseVal) ? '' : _normalizeWS(String(baseVal));
	...
	/**
	 * Flag slot_usage rows whose slot name has no corresponding base slot in
	 * schema.slots (i.e. not found in slotDefinitionIndex for that schema).
	 * This catches orphaned or mistyped slot_usage names that would be silently
	 * skipped by validateSlotUsageMismatch.
	 *
	 * The error is placed on the slot name cell (slot_name_column).
	 *
	 * @param {Object} dh     DataHarmonizer instance for the Slot tab.
	 * @param {Array}  data   Visual-row data from hot.getData().
	 * @param {Object} errors Existing error map {vRowStr: {colStr: msg}}.
	 * @returns {Object} Augmented error map.
	 */
	validateOrphanSlotUsage(dh, data, errors) {
	  if (!dh.slotDefinitionIndex) return errors;
	  ...
	  for (let vRow = 0; vRow < data.length; vRow++) {
	    ...
	    const slotName = row[dh.slot_name_column];
	    const schemaId = row[dh.schema_name_column];
	    if (!slotName || !schemaId) continue;

	    if (!dh.slotDefinitionIndex.has(`${schemaId}\0${slotName}`)) {
	      const vRowStr = String(vRow);
	      const colStr  = String(dh.slot_name_column);
	      if (!result[vRowStr]) result[vRowStr] = {};
	      if (!result[vRowStr][colStr]) {
	        result[vRowStr][colStr] =
	          `"${slotName}" is not defined in schema "${schemaId}" slots — slot_usage requires a base schema slot with this name`;
	      }
	    }
	  }
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  const removeSelectedFieldsWithConfirmation = async () => {
	    ...
	    const expertNote = (hasSchemaFields && !isExpert)
	      ? `<p class="text-warning mt-2"><strong>Expert User mode is required</strong> ` +
	        `to delete ${dh.context.applyDhTerms('{{Schema slot}}s')}. Enable it via the Settings menu &rarr; ` +
	  ...
	  dh.getInvalidCells = (data) => {
	    ...
	    const withMismatch = this.validateSlotUsageMismatch(dh, data, withAbsent);
	    return this.validateOrphanSlotUsage(dh, data, withMismatch);
	  };
    ...
    $('#fkm-type-expert-note').hide();
	      ...
	      if (effectiveType === 'slot') {
	        ...
	        if (isAddMode) {
	          ...
	          if (!hasDerivableClasses) {
	            // No tables in this schema yet — disable checkbox, dropdown, and explain.
	            $('#fkm-also-derive').prop('checked', false).prop('disabled', true);
	            $('#fkm-also-derive-class').show().val('').prop('disabled', true);
	            $('#fkm-also-derive-note').show();
	            derivRowInit = true; // suppress reset-on-first-show
	          } else {
	            $('#fkm-also-derive-note').hide();
	            const cbDisabled = !this.context.expert_user;
	            $('#fkm-also-derive').prop('disabled', cbDisabled);
	            ...
	            if (!derivRowInit) {
	              ...
	              const cbChecked = $('#fkm-also-derive').is(':checked');
	              $('#fkm-also-derive-class').show().prop('disabled', cbDisabled || !cbChecked);
	              if (!cbChecked) $('#fkm-also-derive-class').val('');
	            }
	          }
	        } else {
	    ...
	    if (!this.context.expert_user && visualRow !== null && initSlotType === 'slot_usage') {
	      ...
	      $('#fkm-info').show().html(
	        this.context.applyDhTerms(
	          ...
	          'via Settings menu to edit these fields.'
	        )
	      );
	    }
	  };
	  ...
	  $('#fkm-also-derive').off('change.fkm').on('change.fkm', () => {
	    if ($('#fkm-also-derive').is(':checked')) {
	      ...
	      $('#fkm-also-derive-class').prop('disabled', false);
	    } else {
	      $('#fkm-also-derive-class').prop('disabled', true).val('');
	    }
	  });
  ...
  $('#fkm-also-derive-note').text(this.context.applyDhTerms('No {{classes}} are defined in this schema yet. Add a {{class}} first.'));
  $('#fkm-type-expert-note').html(
    '<strong>Expert user mode is required</strong> ' + this.context.applyDhTerms(
      'to add a {{schema slot}}. Enable it via Settings menu → "Toggle expert user mode".')
  );
	  ...
	  if (_openAsSlot && !this.context.expert_user) {
	    ...
	    $('#fkm-warn').show().html(
	      '<strong>Expert user mode is required</strong> ' + this.context.applyDhTerms(
	        visualRow !== null
	          ? 'to edit a {{schema slot}}. Enable it via Settings menu → "Toggle expert user mode".'
	          : 'to add a {{schema slot}}. Enable it via Settings menu → "Toggle expert user mode".'
	      )
	    );
	  } else {
	  ...
	  $('#fkm-confirm-btn').off('click.fkm').on('click.fkm', () => {
	    ...
	    if (slot_type === 'slot_usage' && visualRow === null && !fieldInLibrary) {
	      const expertHint = this.context.expert_user
	        ...
	        : this.context.applyDhTerms('Enable expert user mode (Settings menu) to create the {{schema slot}} first, or choose "{{class attribute}}" type instead — it does not require a {{schema slot}}.');
	      $('#fkm-error').show().text(
	        ...
	        if (!this.context.expert_user && slot_type === 'slot_usage' && fieldId !== initName) {
	          $('#fkm-error').show().text(
	            this.context.applyDhTerms(
	              ...
	              'Enable it via Settings menu → “Toggle expert user mode”.'
	            )
	          );
	          return;
	        }
  ...
  loadSchemaYAML(text) {
    ...
    for (let [schema_part, class_name] of Object.entries(conversion)) {
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Enum': {
            let enum_id = value.name || item_name;
            this.addRowRecord(dh, tables, {
              schema_id:   loaded_schema_name,
              name:        enum_id,
              title:       value.title,
          ...
          case 'Slot': {
            ...
            let slot_name = value.name || item_name;
    ...
    // NOTE: tables[] rows are numeric arrays (created by addRowRecord); use
    // column indices from slot_name_to_column, not string property names.
    {
      const rootEntry = Object.entries(schema.classes || {}).find(([, c]) => c.tree_root === true);
      if (rootEntry) {
        const rootClassName  = rootEntry[0];
        const slotClassIdCol = dh_slot.slot_name_to_column['class_id'];
        const classNameCol   = this.context.dhs['Class'].slot_name_to_column['name'];
        const ukClassIdCol   = dh_uk.slot_name_to_column['class_id'];
        tables['Slot']      = tables['Slot'].filter(
          row => row[slotClassIdCol] !== rootClassName);
        tables['Class']     = tables['Class'].filter(
          row => row[classNameCol] !== rootClassName);
        if (tables['UniqueKey']) tables['UniqueKey'] = tables['UniqueKey'].filter(
          row => row[ukClassIdCol] !== rootClassName);
      }
    }
    ...
    // Restore any persisted column widths for the newly loaded schema.
    // Pass loaded_schema_name explicitly so _colWidthsSchemaKey() doesn't
    // try to read the Schema tab selection before it is committed.
    for (const dh of Object.values(this.context.dhs)) {
      dh.restoreColWidths(loaded_schema_name);
    }
  ...
  openCopyToSchemaModal(sourceDh) {
    ...
    $('#copy-parent-deps-label').text(
      `These ${this.context.applyDhTerms('{{slot}}')}(s) will be copied from ${schemaLabel}:`
    );
