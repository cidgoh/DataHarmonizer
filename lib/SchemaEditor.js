  ...
  */
	...
	initSlotGroupTab(dh, hot_settings) {
	  ...
	  dh.hot.addHook('afterRowMove', (movedRows, finalIndex, dropIndex, movePossible, orderChanged) => {
	    ...
	    for (let pr = 0; pr < slotTotal; pr++) {
	      ...
	      if ((slotHot.getSourceDataAtCell(pr, slotDh.slot_class_id_column) ?? '') !== cls)    continue;
  ...
  hot_settings.multiColumnSorting.compareFunctionFactory = (sortOrder, columnMeta) => {
    ...
    return (valueA, valueB) => {
	  ...
	  // Sort option 2: schema → table → rank (sections visible but not a sort key)
	  dh.slotSortByTableRank = [
	    {column: dh.schema_name_column,   sortOrder: 'asc'}, // schema_id
	    {column: dh.slot_class_id_column, sortOrder: 'asc'}, // class_id (table)
	    {column: dh.slot_rank_column,     sortOrder: 'asc'}, // rank
	  ];
	  // Sort option 3: field label → schema → table
	  dh.slotSortByLabel = [
	...
	showFieldKeyModal(dh, visualRow = null) {
	  ...
	  const rebuildSlotGroupDropdown = (schemaId, classId, preselect) => {
	    ...
	    if (sgDh && sgNameCol !== undefined) {
	      // When no class is selected show all unique section names across the
	      // schema; when a class is selected show only that class's sections.
	      const seen = new Set();
	      for (let r = 0; r < sgDh.hot.countSourceRows(); r++) {
	        ...
	        if (sgn && !seen.has(sgn)) {
	          seen.add(sgn);
	          $sg.append($('<option>').val(sgn).text(sgn));
	        }
	      }
	    }
	  ...
	  const updateSlotTypeRow = (preferred = 'slot_usage') => {
	    ...
	    // Section is visible at all times — even when no Table is selected.
	    $sectionRow.show();
	    if (!classId) {
	      $row.hide();
	      return;
