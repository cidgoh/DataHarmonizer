...
export class SchemaEditor {
	...
	*/
	...
	SCHEMAMENUS = ['SchemaDataTypeMenu','SchemaMenu','SchemaClassMenu','SchemaSlotMenu','SchemaSlotGroupMenu','SchemaEnumMenu'];
		  ...
		  for (const enum_name of enums) {
		    ...
		    switch (enum_name) {
		      ...
		      case 'SchemaSlotGroupMenu': {
		         ...
		         * Filtered by selected schema, and optionally by selected class.
		         */

		      	// Could generalize getDynamicMenu to accept filter.class_id too.
		        const filter = {
		          schema_id: this.getSchemaEditorSelectedSchema(),
		        }

		        // If user selected a class name on Table tab, filter down to that class.
  ...
  */
    ...
    dh.slot_class_name_column = dh.slot_name_to_column['class_name'];
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			case 'Schema':
				this.hotSettingsMenuHooks(dh, hot_settings, 'Schema');
				break;
			case 'Class':
				this.hotSettingsMenuHooks(dh, hot_settings, 'Class');
				break;
			case 'SlotGroup':
				this.hotSettingsMenuHooks(dh, hot_settings, 'SlotGroup');
				break;
			case 'Slot':
				this.initSlotTab(dh, hot_settings);
				this.hotSettingsMenuHooks(dh, hot_settings, 'Slot');
				break;
			case 'Enum':
				this.hotSettingsMenuHooks(dh, hot_settings, 'Enum');
				break;
		}
	};
	 ...
	 */
	hotSettingsMenuHooks(dh, hot_settings, tab_name) {
	  dh.hot.addHook('afterChange',
	    (changes, source) => {
	      if (changes) this.refreshMenusForTab(tab_name, source);
	    }
	  );
	  // afterCreateRow is omitted: a freshly-added empty row carries no name
	  // value yet so no menu entry would be added anyway.
	  hot_settings.afterRemoveRow = (index, amount, physicalRows, source) => {
	...
	initSlotTab (dh, hot_settings) {
	  ...
	  */
	  ...
	  //
	  // Default sort by Schema ID then Table ID only — no sort on Field ID.
	  // This preserves the manual ordering of fields within sections (SlotGroups),
	  // which are themselves ordered manually via rank.
	  dh.defaultMultiColumnSortConfig = [
	    {
	      column: dh.schema_name_column, // schema_id
	      sortOrder: 'asc'
	    },
	    {
	      column: dh.slot_class_name_column, // class_name (Table ID)
	      sortOrder: 'asc'
	    },
  	];

	  // ── Guards: enforce required fields before name/title can be entered ────
	  // HOT 15 beforeChange() receives visual row indices; toPhysicalRow() is
	  // needed to read source data for the guard checks.
	  //
	  // Guard 1: slot_type must be set before name or title.
	  // Guard 2: for slot_usage or attribute rows, class_id must be set first.
	  // Guard 3: for slot_usage name edits, prompt to create a base slot if none
	  //          with that name exists yet; insert it above if confirmed.
	  dh.hot.addHook('beforeChange', (changes, source) => {
	    if (!changes) return;
	    // Let programmatic / load operations through without the guards.
	    if (['loadData', 'updateData', 'batch_updates', 'cascade_confirm', 'upload'].includes(source)) return;

	    let blockedVisualRow = null;
	    let blockReason = null; // 'slot_type' | 'class_id' | 'create_slot'
	    let blockedNewVal = null;

	    for (const change of changes) {
	      const [visualRow, col, , newVal] = change;
	      // Only intercept non-empty writes to the 'name' or 'title' columns.
	      if ((col === dh.slot_name_column || col === dh.slot_title_column)
	          && newVal !== null && newVal !== '') {
	        const physRow = dh.hot.toPhysicalRow(visualRow);
	        const slot_type = dh.hot.getSourceDataAtCell(physRow, dh.slot_type_column);
	        if (!slot_type) {
	          blockedVisualRow = visualRow;
	          blockReason = 'slot_type';
	          break;
	        }
	        if (slot_type === 'slot_usage' || slot_type === 'attribute') {
	          const class_id = dh.hot.getSourceDataAtCell(physRow, dh.slot_class_name_column);
	          if (!class_id) {
	            blockedVisualRow = visualRow;
	            blockReason = 'class_id';
	            break;
	          }
	        }
	        // Guard 3 — name column only: if this is a slot_usage row, ensure a
	        // base slot ('slot') with the same name already exists in the table.
	        if (col === dh.slot_name_column && slot_type === 'slot_usage') {
	          const sourceData = dh.hot.getSourceData();
	          const baseSlotExists = sourceData.some(
	            (row) => row[dh.slot_type_column] === 'slot' && row[dh.slot_name_column] === newVal
	          );
	          if (!baseSlotExists) {
	            blockedVisualRow = visualRow;
	            blockReason = 'create_slot';
	            blockedNewVal = newVal;
	            break;
	          }
	        }
	      }
	    ...
	    if (blockedVisualRow !== null) {
	      // HOT beforeChange is synchronous — show modal and focus asynchronously
	      // after the cancel has been committed.
	      const targetVisualRow = blockedVisualRow;
	      if (blockReason === 'slot_type') {
	        setTimeout(async () => {
	          await dhAlert(
	            'Please set the Field type (slot_type) first, before entering a name or title.',
	            { title: 'Field type required' }
	          );
	          dh.hot.selectCell(targetVisualRow, dh.slot_type_column);
	        }, 0);
	      } else if (blockReason === 'class_id') {
	        setTimeout(async () => {
	          await dhAlert(
	            'Please select a table (class_id) from the "table reuse" menu before entering a name or title.',
	            { title: 'Table selection required' }
	          );
	          dh.hot.selectCell(targetVisualRow, dh.slot_class_name_column);
	        }, 0);
	      } else { // 'create_slot'
	        const newName = blockedNewVal;
	        setTimeout(async () => {
	          const confirmed = await dhConfirm(
	            `No base slot named "${newName}" exists. Would you like to create one now?`,
	            { title: 'Create base slot?', okLabel: 'Create' }
	          );
	          if (confirmed) {
	            // Read schema_id from the slot_usage row before we insert above it.
	            const origPhysRow = dh.hot.toPhysicalRow(targetVisualRow);
	            const schema_id = dh.hot.getSourceDataAtCell(origPhysRow, dh.schema_name_column);
	            // Insert a new empty row at the slot_usage row's visual position;
	            // the slot_usage row shifts down to targetVisualRow + 1.
	            dh.hot.alter('insert_row_above', targetVisualRow, 1, 'cascade_confirm');
	            // Populate the new base-slot row.
	            const newRowData = [
	              [targetVisualRow, dh.slot_type_column, 'slot'],
	              [targetVisualRow, dh.slot_name_column, newName],
	            ];
	            if (schema_id) newRowData.push([targetVisualRow, dh.schema_name_column, schema_id]);
	            dh.hot.setDataAtCell(newRowData, 'cascade_confirm');
	            // Re-apply the name the user typed to the slot_usage row.
	            dh.hot.setDataAtCell(targetVisualRow + 1, dh.slot_name_column, newName, 'cascade_confirm');
	          } else {
	            // User declined — return focus to the name cell so they can choose a different name.
	            dh.hot.selectCell(targetVisualRow, dh.slot_name_column);
	          }
	        }, 0);
	      }
		        ...
		        if (record.name) {
		          ...
		          if (['slot_usage','attribute'].includes(record.slot_type)) {
		            if (!record.class_name) {
		              console.warn(`Skipping ${record.slot_type} row "${slot_name}": missing class_name`);
		              break;
		            }
		          ...
		          if (record.slot_type === 'slot_usage') {
		            ...
		            if (baseSlot) {
		              const skipKeys = new Set(['name','slot_type','class_name','schema_id','rank','slot_group']);
	...
	previewSchema() {
		const result = this._buildSchemaYaml();
		if (!result) {
		  dhAlert("A schema needs to be named and selected before previewing. If you have named your schema please make sure it is selected.");
		  return;
		}
		const { schema_name, yaml_string } = result;
		...
		if (classNames.length === 0) {
		  dhAlert('Preview requires at least one table. Please add a table with at least one field before previewing.');
		  return;
		}
		...
		if (!hasField) {
		  dhAlert('Preview requires at least one field in a table. Please add a field to a table before previewing.');
		  return;
		}

		sessionStorage.setItem(`dh_preview_${schema_name}`, JSON.stringify(schema));
		...
		const previewUrl = `${location.origin}${location.pathname}?preview=${encodeURIComponent(schema_name)}&t=${Date.now()}`;
		...
		const windowName = `dh_preview_${schema_name.replace(/[^a-zA-Z0-9_-]/g, '_')}`;
		window.open(previewUrl, windowName, 'popup,width=1400,height=900,resizable=yes,scrollbars=yes');
	}
  ...
  loadSchemaYAML(text) {
    ...
    if (rows.length > 0) {
      ...
      if (rows[0] == focus_row) {
        ...
        // Some other schema is loaded in this row, so don't toast that.
          ...
          dhAlert("This schema row is occupied.  Select an empty schema row to upload to."); return;
        }
      }
    }
    ...
    // present in tables['Slot'] by this point.
    {
      ...
      const skipCols = new Set([schemCol, typeCol, n2c['class_name'], nameCol]);
  ...
  addSlotRecord(dh, tables, schema_name, class_name, slot_type, slot_key, slot_obj) {
    ...
    let slot_record = {
      ...
      // The Slot tab column is 'class_name', not 'class_id' (class_id is used by UniqueKey/SlotGroup tabs).
  ...
  openCopyToSchemaModal(sourceDh) {
    ...
    const updateDeps = () => {
      ...
      const allDeps = this._analyzeCopyDependencies(sourceDh, physRows, sourceSchemaId, targetSchema);
    ...
    $select.off('change.copySchema').on('change.copySchema', updateDeps);
    updateDeps();
   ...
   */
  _analyzeCopyDependencies(sourceDh, physRows, sourceSchemaId, targetSchema) {
    ...
    for (const physRow of physRows) {
      ...
      if (classIdCol !== undefined) {
        const classId = sourceDh.hot.getSourceDataAtCell(physRow, classIdCol);
  ...
  _appendRowsToTab(tabName, records) {
    ...
    const currentData = dh.hot.getSourceData().map(row => [...row]);
