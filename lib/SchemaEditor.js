  ...
  */
    ...
    dh.slot_group_column = dh.slot_name_to_column['slot_group'];
		...
		switch (class_name) {
			case 'Schema': {
				...
				// Refresh all dependent menus when the user selects a different schema row,
				// since SchemaClassMenu, SchemaSlotMenu, SchemaSlotGroupMenu, and SchemaEnumMenu
				// all filter by the currently selected schema.
	...
	hotSettingsMenuHooks(dh, hot_settings, tab_name, keyColumns = null) {
	  ...
	  hot_settings.afterRemoveRow = (index, amount, physicalRows, source) => {
	    ...
	    // Re-apply the active Display filter (all records / records by selected key)
	    // so the tab's hidden-row state is restored after HOT's row-removal re-render
	    // resets it — otherwise the tab reverts to an unfiltered view.
