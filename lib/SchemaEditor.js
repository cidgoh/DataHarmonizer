  ...
  */
  ...
  const KEY_COLUMNS = new Set([
	        ...
	        if (slotType === 'slot_usage' && dh.slotDefinitionIndex) {
	          ...
	          if (defPhysRow !== undefined) {
	            ...
	            if (!_emptyVal(baseVal) && oldVal === baseVal) {
	              ...
	              const msg = dh.context.applyDhTerms(
	                `The <strong>${slotName}</strong> {{slot}} property <strong>${colName}</strong> is linked to a {{schema slot}} by the same name. ` +
	      ...
	      if (slotType === 'slot') {
	        ...
	        for (let pr = 0; pr < dh.hot.countSourceRows(); pr++) {
	          ...
	          inheriting.push({
	            ...
	            currentVal: dh.hot.getSourceDataAtCell(pr, col),
	          });
	        }
	...
	showFieldKeyModal(dh, visualRow = null, preFillRow = null) {
	  ...
	  const updateSlotTypeRow = () => {
	    $('#fkm-info').hide();  // Reset info message; re-shown below if needed.
	    ...
	    if (isAddMode) {
	      ...
	      if (effectiveType === 'slot') {
	        ...
	        if (!this.context.expert_user) {
	          $('#fkm-error').show().text(
	            this.context.applyDhTerms(
	              'Expert user mode is required to save a {{schema slot}}. '
	        ...
	        // For slot_usage: if the selected schema slot already has a slot_group,
	        // pre-fill and lock the input — the class slot must stay in that section.
	    ...
	    if (!classId || initSlotType === 'slot') {
	      ...
	      // Rename-cascade info: when the Field ID is being changed, count the
	      // slot_usage rows that will be cascade-updated and inform the user.
	      ...
	      // Expert gate: disable Save if not expert user.
	      if (!this.context.expert_user) $('#fkm-confirm-btn').prop('disabled', true);
	      return;
	    }
	    if (!fieldId) {
	      $row.hide();
	  ...
	  $('#fkm-slot-group-desc').text(this.context.applyDhTerms(
	    'Optional display {{slot group}} for this {{slot}}. New {{slot groups}} can be added on the "{{Slot group}}" tab.'
	  ));
	  ...
	  $('#fkm-field-type-desc').text(
	    this.context.applyDhTerms(
	      ...
	      '{{class slot}}: located in a {{class slot usage}} list, derived from a {{schema slot}}; ' +
	      '{{class attribute}}: A {{slot}} located in a {{table}}\'s {{class attributes}} list, not connected to a {{schema slot}}.'
	    )
	  );
	  // Re-apply expert warning if opening a schema slot (updateSlotTypeRow set it
	  // earlier, but the above hide().text('') call would clear it).
	  if (initSlotType === 'slot' && !this.context.expert_user) {
	    $('#fkm-error').show().text(
	      this.context.applyDhTerms(
	        visualRow !== null
	          ? 'Expert user mode is required to edit a {{schema slot}}. Enable it via File menu → "Toggle expert user mode".'
	          : 'Expert user mode is required to save a {{schema slot}}. Enable it via File menu → "Toggle expert user mode".'
	      )
	    );
	  } else {
	    $('#fkm-error').hide().text('');
	  ...
	  $('#fkm-confirm-btn').off('click.fkm').on('click.fkm', () => {
	    ...
	    if (visualRow === null) {
	      if (slot_type === 'slot') {
	        ...
	        if (slotGroup) {
	          // Insert after the last slot belonging to this slot_group.
	        ...
	        // Collect existing class slots of the same type/schema/class, sorted by rank.
	        const rankCol    = dh.slot_rank_column;
	        const srcData2   = hot.getSourceData();
	        const classSlots = [];
	        for (let physIdx = 0; physIdx < srcData2.length; physIdx++) {
	          const r = srcData2[physIdx];
	          if (r[dh.slot_type_column]     !== slot_type) continue;
	          if (r[dh.schema_name_column]   !== schemaId)  continue;
	        ...
	        if (slotGroup) {
	          // Insert after the last slot in this slot_group.
	      ...
	      if (slot_type === 'slot') {
	        ...
	        if (!this.context.expert_user) {
	          $('#fkm-error').show().text(
	            this.context.applyDhTerms(
	              'Expert user mode is required to edit a {{schema slot}}. '
	              + 'Enable it via File menu → "Toggle expert user mode".')
	          );
	          return;
	        }
	        ...
	        // Attribute → slot_usage with existing base slot: copy non-empty base slot
	        // values into empty cells of the converted row so inherited values are preserved.
	        if (initSlotType === 'attribute' && slot_type === 'slot_usage' &&
	          ...
	          if (defPhysRow !== undefined) {
	            ...
	            const SKIP_COLS2 = new Set([
	              ...
	              dh.slot_group_column,  dh.slot_name_to_column?.['rank'],
	            ]);
	            const toCopy = [];
	            for (let col = 0; col < hot.countCols(); col++) {
	              if (SKIP_COLS2.has(col)) continue;
	              if (!_empty2(hot.getSourceDataAtCell(physRow, col))) continue;
	              const baseVal = hot.getSourceDataAtCell(defPhysRow, col);
	              if (!_empty2(baseVal)) toCopy.push([visualRow, col, baseVal]);
	            }
	            if (toCopy.length) hot.setDataAtCell(toCopy, 'field_key_modal');
	          }
	        }
  ...
  renderCompareReport(results, schemaA, schemaB) {
    ...
    const fmtEntry = ([path, oldVal, newVal, cap]) => {
      ...
      if (oldVal !== '[ADDED]')
        h += `<div class="compare-old">${esc(fmt(oldVal))}</div>`;
      if (newVal !== '[REMOVED]')
        h += `<div class="compare-new">${esc(fmt(newVal))}</div>`;
      h += `</div>`;
      return h;
    };
    ...
    if (!substantive.length && !enrichment.length && !cosmetic.length) {
      ...
      html += fmtSection('COSMETIC — redundant name fields / list wrapping', cosmetic);
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Class': {
            ...
            // Collect unique slot_group names from slot_usage and attributes (in rank order)
            // for the SlotGroup tab.  Deduplicates against names already collected from
            // schema.slots so each section name appears exactly once per schema (schema-wide concept).
            if (value.slot_usage) {
            ...
            if (value.attributes) {
              const sorted_attrs = Object.values(value.attributes).sort(
                (a, b) => (a.rank ?? Infinity) - (b.rank ?? Infinity)
              );
              for (const obj of sorted_attrs) {
                if (obj.slot_group && !seenSlotGroupNames.has(obj.slot_group)) {
                  seenSlotGroupNames.add(obj.slot_group);
                  this.addRowRecord(dh_sg, tables, {
                    schema_id: loaded_schema_name,
