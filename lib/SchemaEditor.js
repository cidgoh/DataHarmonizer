	...
	*/
	...
	translationForm(dh) {
	    ...
	    for (let row = dh.current_selection[0]; row <= dh.current_selection[2]; row++) {
	      ...
	      const fieldTypeLabel = this.context.applyDhTerms(
	      ...
	      if (key) {
	        translate_rows += `<tr class="translate translate_key"><td colspan="${column_count+2}">${key}${key2 ? ' ' + key2 : ''} <small style="color:#888;font-style:italic">(${fieldTypeLabel})</small></td></tr>`;
	      }
