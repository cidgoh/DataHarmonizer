...
export function initI18n(langChangeCallback = () => {}) {
    ...
    .init(
      {
        ...
        resources: {
          ...
          'multiselect': 'This is a multiselect value: {{ value }}'
        }, // initial resources
        interpolation: {
          escapeValue: false,
        }
  ...
  i18next.services.formatter.add('multiselect', (value, lng, options) => {
    const { delimiter = MULTIVALUED_DELIMITER, multiValue=value, lang = lng } = options;
    return multiValue.split(delimiter).map(i18next.t).apply(formatMultivaluedValue);
  });
