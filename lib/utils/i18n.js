...
import { MULTIVALUED_DELIMITER, formatMultivaluedValue } from '@/lib/utils/fields';
...
export function initI18n(langChangeCallback) {
  ...
  i18next.services.formatter.add('multiselectFormatter', ( val, lng, options ) => {
    console.log(val, lng, options);
    const { defaultValue, value } = options;
    let values = [];
    if (!!defaultValue) { values = defaultValue.replace(/▼/g, "").split(MULTIVALUED_DELIMITER); }
    else { values = value; };
