...
function induceClass(schema, className) {
  ...
  for (const ancestor of chain) {
    ...
    //
    // When cls.slots is absent (e.g. a schema round-tripped through the Schema
    // Editor where _buildSchemaYaml populates slot_usage but the slots list was
    // lost), fall back to using the slot_usage keys so those definitions are
    // still reflected in the induced attributes.
    const slotsToProcess = cls.slots?.length
      ? cls.slots
      : Object.keys(cls.slot_usage || []);
    for (const slotName of slotsToProcess) {
