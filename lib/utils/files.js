...
function writeWorkbook(workbook, baseName, ext, opt = {}) {
  if (ext === 'xlsx' || ext === 'xls') {
    ...
    workbook.SheetNames.forEach(sheetName => {
      ...
      XlsxUtils.book_append_sheet(workbook_for_sheet, workbook[sheetName], sheetName);
      writeFile(workbook, `${baseName}${sheetName !== DEFAULT_SHEETNAME ? `_${sheetName}` : ""}.${ext}`, opt);
    });
  }
}
