...
export function exportWorkbook(workbook, baseName, ext) {
  ...
  switch (ext) {
    ...
    case 'csv':
      ...
      break;
    case 'csv (UTF-16)':
      writeWorkbook(workbook, baseName, 'csv', {
        bookType: 'txt',
        FS: ',',
      });
      break;
    case 'tsv':
      ...
      break;
    case 'tsv (UTF-16)':
      writeWorkbook(workbook, baseName, 'tsv', {
        bookType: 'txt',
        FS: '\t',
      });
      break;
    case 'csv (UTF-8, no BOM)':
      for (const worksheet in workbook.SheetNames) {
        //Customization: skips BOM prefix '\uFEFF'
        csv = XlsxUtils.sheet_to_csv(workbook.Sheets[worksheet], { FS: ',' });
...
function processJsonBasedOnType(jsonData, handleObject, handleArray) {
  if (Array.isArray(jsonData)) {
      ...
      throw new Error("Unexpected JSON type: JSON is neither an object nor an array");
  }
}

// Function to import and process a JSON file
export function importJsonFile(jsonData, className = null, containerType = null) {

  return processJsonBasedOnType(
      jsonData,
      (data) => {
          // Check if the object has all required fields
          const requiredFields = ['schema', 'version', 'in_language', 'Container', 'location'];
          if (!requiredFields.every(field => data.hasOwnProperty(field))) {
              throw new Error("The JSON file's schema is not compatible for loading!");
          }
          return { 
