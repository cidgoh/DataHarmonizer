...
export const binaryToUtf8 = (binaryString) => {
...
export const arrayBufferToUtf8 = (arrayBuffer) => {
...
export function exportWorkbook(workbook, baseName, ext) {
  switch (ext) {
    ...
    case 'xls':
      ...
      break;
      case 'csv':
        processAndSave(workbook, baseName, 'csv', ',', 'text/plain;charset=UTF-8');
        break;
  
      case 'csv (UTF-16)':
        processAndSave(workbook, baseName, 'csv', ',', 'text/plain;charset=UTF-16LE');
        break;
  
      case 'tsv':
        processAndSave(workbook, baseName, 'tsv', '\t', 'text/plain;charset=UTF-8');
        break;
  
      case 'tsv (UTF-16)':
        processAndSave(workbook, baseName, 'tsv', '\t', 'text/plain;charset=UTF-16LE');
        break;
  
      case 'csv (UTF-8, no BOM)':
        processAndSave(workbook, baseName, 'csv', ',', 'text/plain;charset=UTF-8', false);
        break;
  
      case 'csv (ASCII)':
        processAndSave(workbook, baseName, 'csv', ',', 'text/plain;charset=us-ascii', false);
        break;
    }
  }
  
  function processAndSave(workbook, baseName, ext, delimiter, mimeType, includeBOM = true) {
    ...
    sheets.forEach(sheetName => {
      ...
      const formattedData = sheetData.map(row => row.join(delimiter)).join('\n');
      data += formattedData + '\n';
      ...
      saveAs(blob, `${baseName}${sheets.length > 1 ? `_${sheetName}`: ''}.${ext.split(' ')[0]}`);
    });
  }
