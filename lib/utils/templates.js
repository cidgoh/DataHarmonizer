...
export async function templatePathForSchemaURI(schemaURI) {
  ...
  // TRY MENU.JS if we add schemaURI for each template; OR dhs this.appConfig
  // Issue: schemaURI
  console.log
...
export async function accessTemplate(
  schema_folder // e.g. canada_covid19
) {
  //console.log(schema_folder)
  const template = template_manifest.children.find(

  //const template = menu.children.find(
  ...
  try {
    template.locales = locales.reduce((acc, localization) => {
      ...
      return Object.assign(acc, {
        [locale_label]: {
          schema: schemaMerge//,documentation: doc,
        },
      });
    }, {});
  } catch (err) {
...
class TemplateProxy {
  ...
  static async create(template_name, options = {}) {
    ...
    if (forced_schema !== null) {
      // returns {name:..., default: { schema, documentation: null }, locales: {[locale]:{ schema, documentation: null }}}
      template = buildTemplateFromUploadedSchema(forced_schema);
    } else {
      // access template from directory in the manifest
      const fetchedTemplateData = await accessTemplate(template_name);
