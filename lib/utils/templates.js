...
import { fetchAndProcessYaml, processYamlSchema, induceAllClasses } from './schema_induction';
...
export function getTemplatePathInScope() {
  ...
  if (templatePath === null || typeof templatePath === 'undefined') {
    // Find first schema+template with display:true
    for (const schema_entry of Object.values(menu)) {
      const schema_folder = schema_entry['folder'];
      const templates = schema_entry['templates'] || {};
    ...
    // Fallback: first entry regardless of display flag
    const schema_entry = Object.values(menu)[0];
    const schema_folder = schema_entry['folder'];
...
function buildTemplateFromUploadedSchema(schema) {
  ...
  // Compile slots + slot_usage → class.attributes so useTemplate() can read them.
  // This mirrors what processYamlSchema() does for YAML-file uploads and what the
  // Python build pipeline does for pre-compiled schema.json bundles.
