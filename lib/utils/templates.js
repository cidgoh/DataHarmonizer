...
async function processNode(node, fetchFileImpl = templateFetcher) {
  ...
  console.log('folded_template_spec', folded_template_spec);
  ...
  if (typeof locales_folder !== 'undefined') {
    try {
      if (Array.isArray(locales_folder) && locales_folder.length > 2) {
        ...
        throw new Error("locales_folder is not an array or does not have enough elements.");
      }
    } catch (err) {
...
class TemplateProxy {
  ...
  async _init(template_name, locale = null, { fetchFileImpl }) {
    ...
    const template = buildTemplate(fetchedTemplateData);
