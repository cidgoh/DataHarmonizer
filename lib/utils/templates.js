...
const schemaFromChildPath = childPath => childPath.replace(/\/templates\/(.+)\/schema.json/,'$1');
async function importSchema (schema_path) {
  const template = (await import(`../../web/templates/${schema_path}/schema.json`)).default;
  return template;
}
const importSchemaFromChildPath = childPath => importSchema(schemaFromChildPath(childPath))
