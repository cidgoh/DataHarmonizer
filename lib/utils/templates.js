...
export function buildTemplateFromUploadedSchema(schema) {
  ...
  if (schema.in_language) {
    const languages = Array.isArray(schema.in_language) ? schema.in_language : [schema.in_language];
...
class TemplateProxy {
  ...
  static async create(template_name, options = {}) {
    const { locale = null, forcedSchema = null, fetchFileImpl = templateFetcher } =
      options;
    console.log(options, forcedSchema);
    const instance = new TemplateProxy();
    
    let template;
    if (forcedSchema !== null) {
      template = buildTemplateFromUploadedSchema(forcedSchema);
      console.log('forced schema template', template);
    } else {
  ...
  async _init(template, locale = null) {
    ...
    console.log(template.default.schema.classes);
    ...
    const toTemplateLocationName = name => name.toLowerCase().replace(/\-/g, '');

    Object.assign(this, {
      _name: template.default.schema.name,                                              // TODO: compatibility with local loading
      _location: `/templates/${toTemplateLocationName(template.default.schema.name)}`,  // TODO: compatibility with local loading
