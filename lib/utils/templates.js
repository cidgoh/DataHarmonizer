...
function deepMerge(defaultObj, localizedObj) {
  ...
  for (let key in localizedObj) {
    if (
      ...
      typeof localizedObj[key] === 'object' &&
      !Array.isArray(localizedObj[key])
    ) {
      result[key] = deepMerge(defaultObj[key], localizedObj[key]); // Recursive merge
    } else {
...
const accessFileFromAbsPath = async (relative_path) =>
  await accessFile(`../../${relative_path}`);
