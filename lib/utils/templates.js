...
async function compileSchema(schema_folder) {
  ...
  for (const schema_obj of Object.values(menu)) {
    if (schema_obj.folder === schema_folder) {
      ...
      Object.entries(schema.extensions?.locales?.value ||  {}).forEach(([locale, locale_schema]) => {
...
/**
 * CONVERTED TO LOOKUP as dh.container_class
 * Given a DataHarmonizer class, look up the corresponding Container.
 * @param Container {Object} from a given dataHarmonizer

export const rangeToContainerClass = (Container, cls_key) => {
  try {
    return Object.values(Container.attributes)
//      .filter((v) => 'range' in v)
      .filter((v) => v.range && v.range === cls_key)[0].name;
  } catch {
    return cls_key;
  }
};
