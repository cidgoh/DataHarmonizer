...
async function buildTemplate(template_name) {
    const [name, [schemas, documentations], locale_nodes] = await accessTemplate(template_name);
    // TODO: reduce accessors to make more robust tree access
 ...
 */
function findBestLocaleMatch(availableLocales, preferredLocales) {
    for (const preferred of preferredLocales) {
        ...
        // If the preferred locale is a generic language code, try to find the first specific locale of that language
        if (!preferred.includes('-')) {
            const match = availableLocales.find(locale => locale.split('-')[0] === preferred);
            if (match) {
                return match;
            }
        }
...
class TemplateProxy {
    ...
    static async create(template_name, locale = null) {
        ...
        return new Proxy(instance, {
            get: function(target, property) {
                ...
                if (target._localizedData && target._localizedData[property]) {
                    return target._localizedData[property];
                } else {
    ...
    changeLocale(newLocale) {
        ...
        if (bestLocale !== null) {
            ...
            this._localizedData = this._defaultData.locales[this._locale];
        } else {
...
// Usage example:
// const proxyInstance = await TemplateProxyDraft.create('template_name', 'en-US');
