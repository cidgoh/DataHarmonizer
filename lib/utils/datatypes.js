...
export default class Datatypes {
  ...
  parseDate(value) {
    const parsed = parse(value, this.dateFormat, this.REF_DATE);
    if (isNaN(parsed)) {
      return undefined;
    }
    return parsed;
  ...
  parseDateTime(value) {
    const parsed = parse(value, this.datetimeFormat, this.REF_DATE);
    if (isNaN(parsed)) {
      return undefined;
    }
    return parsed;
  ...
  parseTime(value) {
    const parsed = parse(value, this.timeFormat, this.REF_DATE);
    if (isNaN(parsed)) {
      return undefined;
    }
    return parsed;
...
export function stringifyDateObjectForJsonSchema(dateObject, datatype) {
  const outputFormat = JSON_SCHEMA_FORMATS[datatype];
  if (!outputFormat && !isDate(dateObject)) {
    return undefined;
  }
