  ...
  return {
    ...
    afterRemoveRow: function (index, amount, physicalRows, source) {
      for (let i = 0; i < amount; i++) {
        ...
        contextMenuCallback(rowData, 'removeRow');
      }
    ...
    afterCreateRow: function (index, amount, source) {
      for (let i = 0; i < amount; i++) {
        ...
        let rowData = hot.getDataAtRow(rowIndex);
        // Custom logic using the rowData
        contextMenuCallback(rowData, 'insertRow');
      }
      // Clear the temporary storage
      rowDataBeforeAdd = {};
    }
    ...
    if (!node.tree_root) {
      ...
      if (isParent) {
        ...
        propagatesChangesOnTheseKeys.forEach((shared_key) => {
          ...
          const rowDataManager = createRowDataManager(
            ...
            (rowData, source) => {
              ...
              switch (source) {
                case 'removeRow':
                  dispatchHandsontableUpdate({
                    prop: data_harmonizers[node.name].getColumnIndexByFieldName(shared_key.name),
                  ...
                  // reselect(data_harmonizers[node.name], row, prop);
                  break;
          ...
          for (const callbackName in rowDataManager) {
            data_harmonizers[node.name].hot.addHook(callbackName, rowDataManager[callbackName]);
          };
    ...
    if (!node.tree_root) {
      ...
      if (isChild) {
        ...
        document.addEventListener('handsontableUpdate', function (event) {
          console.log(event);
          const { oldValue, newValue, sourceTable, sharedKey } =
            event.detail;
...
function bindSelectHandlerEvents(
  data_harmonizers,
  schema_tree
) {
