...
// Since the action handlers are parameterized by appContext/data harmonizers, their uniqueness of their closures prevents referencing them by their function symbol.
// The event tracker stores the listener in a map so we can guarantee their deletion.
class EventTracker {
  constructor() {
...
export class OneToManyEventTracker extends EventTracker {
        ...
        if (maybe_unique_keys.length > 0) {
          ...
          for (let key_name of maybe_unique_keys) {
            this.dispatchHandsontableUpdate({
              ...
              column,
              key_values:
                appContext[dh.class_assignment].unique_keys[key_name]
                  .key_values, // Send the key values to filter by
            });
          }
        }
      }
  ...
  bindParentBroadcastsChange() {
    ...
    nodes.forEach((nodeName) => {
      ...
      for (const [, uniqueKey] of Object.entries(
        node.unique_keys
      )) {
        if (uniqueKey.foreign_key) {
          ...
          dh.hot.addHook('afterChange', (changes, source) => {
            ...
            if (changes) {
              ...
              const columnIndex =
                dh.getColumnIndexByFieldName(foreign_key_slot);

              for (const [row, change] of Object.entries(rowChanges)) {
                const updateActionType = Object.values(change.newValues)
                  .map(isEmptyUnitVal)
                  .includes(true)
                  ? ACTION.DELETE
                  : ACTION.UPDATE;
                ...
                const detail = {
                  ...
                  target: nodeName,
                  row,
                  key_old_values: takeKeys([columnIndex])(change.oldValues),
                  key_new_values: takeKeys([columnIndex])(change.newValues),
                  old_changes: change.oldValues,
                  new_changes: change.newValues,
                ...
                if (
                  findActiveDataHarmonizer(this.appContext) ===
                  detail.emitted_by
                ) {
