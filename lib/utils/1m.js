...
function dispatchHandsontableUpdate(action, detail) {
  ...
  const event = new CustomEvent('classDataRowKeyChange', { action, detail: _detail });
...
// NOTE: Handsontable doesn't have a notion of listeners similar to the DOMs. Its event system tracks user input not element-to-element interaction.
  ...
  switch (action) {
    ...
    case ACTION.UPDATE:
      ...
      for (let col_index in shared_key_details.key_indexes) {
        updateRows(dh, [
          col_index,
          appContext[emitted_by].unique_keys[key_name].key_old_values[col_index],
    ...
    case ACTION.VALIDATE:
      ...
      dh.validate();
      break
    // DEPRECATE: sub-case of update?
    case ACTION.DELETE:
      // TODO: empty? appContext[emitted_by].unique_keys[key_name].key_values = appContext[emitted_by].unique_keys[key_name].key_indexes.map(() => null);
  ...
  (dhs) => {
    ...
    for (let dh in dhs) {
      if (dh in appContext) {
        ...
        createRowFocusTracker(dhs[dh], dispatchHandsontableUpdate(ACTION.VALIDATE, { emitted_by: dh, target: dh }));
