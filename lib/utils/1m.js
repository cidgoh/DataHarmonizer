...
const handsontableUpdateRouter = (appContext, dhs) => {
  document.addEventListener('classDataRowKeyChange', (event) => {
    ...
    if (classCanFire) {
      fireAction(action, emitted_by, target, dhs, event, appContext, propagates, unique_key);
    }
...
function fireAction(action, emitted_by, target, dhs, event, appContext, propagates) {
    ...
    for (let unique_key_name in node.unique_keys) {
      ...
      if (unique_key.foreign_key) {
        document.addEventListener('handsontableUpdate', function (event) {
          const { row, prop, oldValue, newValue, sourceTable, shared_key } = event.detail;
          console.warn('receiving change:', node, sourceTable, shared_key);
