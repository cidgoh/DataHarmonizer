  ...
  dh.hot.addHook('afterSelection', (row, column, row2, column2) => {
    ...
    if (multiRowSelected) {
      ...
      */
      ...
      // TODO: how does this interact with currentSelection?
      // Read doesn't always mean focus
...
const validateOnFocusChange = dh => (oldRow, newRow) => {
...
const bindKeyConstraintValidation = (appContext) => (dh) => {
    ...
    if (changes) {
      ...
      for (let row in rowChanges) {
        ...
        for (let keyName of maybeUniqueKeys) {
          if (hasDuplicates(getNonEmptyColumnValues(dh.hot, dh.getColumnIndexByFieldName(keyName)))) {
            // Undoing
            dh.hot.undo();
          }
        }
  ...
  //           const isValid = uniqueKeyConstraintValidation(...Array.from(newKeyValues));
  ...
  //         if (!uniqueKeyConstraintValidation(...Array.from(keyValues))) {
  //           console.warn(
...
//const uniqueKeyConstraintValidation = (...unique_key_column_values) => {
  // console.log('uniqueKeyConstraintValidation', unique_key_column_values)
  ...
  // const concatenated_keys = unique_key_column_values.map((_, rowIndex) =>
      ...
      if (maybe_unique_keys_in_range.length > 0) {
        ...
        for (let key_name in unique_keys_in_scope) {
          ...
          for (let row in row_changes) {
            ...
            dispatchHandsontableUpdate(ACTION.UPDATE, details);
...
const handleAction = (appContext) => (dh) => (action, details) => {
  ...
  console.info(action, emitted_by, '->', target);
  switch (action) {
    case ACTION.CREATE:
      console.warn(ACTION.CREATE)
      break;
    case ACTION.READ:
      console.warn(ACTION.READ)
      ...
      break;
    case ACTION.FILTER:
      console.warn(ACTION.FILTER, target, currentSelection)
      console.warn(appContext[target].unique_keys[key_name].key_indexes[0])
      // TODO: single key
      const columnToFilterIndex = appContext[target].unique_keys[key_name].key_indexes[0];
      dh.hideMatchingRows(columnToFilterIndex, currentSelection.valueToMatch);
      break;
    case ACTION.UPDATE:
      console.warn(ACTION.UPDATE)
      // TODO: are key_indexes and key_values guaranteed to be the same length?
      ...
      break;
    case ACTION.SELECT:
      console.warn(ACTION.SELECT)
      triggerCurrentSelectionChange(currentSelection);
      break;
    case ACTION.DELETE:
      console.warn(ACTION.DELETE);
      break;
    default:
...
const runHook = dh => (action, details) => {
  dh.hot.runHooks(
    `classDataRowKeyChange:${dh.class_assignment}`, action, details);
}
...
const dispatchChildHooks = (unique_key) => (action, details) => {
  ...
  if (classHasChildClasses) {
    for (let child_class of unique_key.child_classes) {
      console.warn('Dispatching Child Hook', child_class, action, details)
      details.emitted_by = child_class;
      details.target = child_class;
      dispatchHandsontableUpdate(action, details);
    }
  }
}
  ...
  document.addEventListener('handsontableUpdate', (event) => {
    ...
    if (classCanFire) {
      if (action === ACTION.READ && emitted_by === target) {
        ...
        setActiveDataHarmonizer(appContext)(event.detail.target);
        if ('child_classes' in unique_key) {
          console.error("run hook on child class for unique key", key_name, ": ", unique_key.child_classes[0]);
          runHook(dhs[unique_key.child_classes[0]])(ACTION.FILTER, {
            emitted_by,
            target: unique_key.child_classes[0],
            key_name,
            currentSelection: makeCurrentSelection(appContext)(dhs[findActiveDataHarmonizer(appContext)])
          });
        }
        // propagate filter actions to child tables if possible
        // dispatchChildHooks(unique_key)(ACTION.FILTER, event.detail);
      } else if (action === ACTION.SELECT) {
        console.info('SELECT Arguments:\n', 
          `\tActive Data Harmonizer: ${findActiveDataHarmonizer(appContext)}\n`, 
          `\tEvent Detail Row: ${event.detail.row}\n`, 
          `\tEvent Detail Column: ${event.detail.col} ${event.detail.column}\n`, 
          `\tmakeCurrentSelection: ${JSON.stringify(makeCurrentSelection(appContext)(dhs[findActiveDataHarmonizer(appContext)]))}\n`)
      ...
      } else if (action === ACTION.FILTER && emitted_by !== target) {
        console.info('FILTER Arguments:\n',
        `\tActive Data Harmonizer: ${findActiveDataHarmonizer(appContext)}\n`,
        `\tCurrent Selection: ${JSON.stringify(makeCurrentSelection(appContext)(dhs[findActiveDataHarmonizer(appContext)]))}\n`
        )
        const activeDataHarmonizer = dhs[findActiveDataHarmonizer(appContext)];
        event.detail.currentSelection = makeCurrentSelection(appContext)(activeDataHarmonizer);
        if (event.detail.currentSelection.parentRow === 0) console.error(event.detail.currentSelection.parentRow === 0);
...
const triggerCurrentSelectionChange = (currentSelection) => {
  console.warn('triggering current selection change', currentSelection.valueToMatch);
  $(document).trigger('dhCurrentSelectionChange', { currentSelection });
}

const makeCurrentSelection = appContext => dh => {
  let { row, column, unique_keys } = structuredClone(appContext[dh.class_assignment]);
  if (column <= -1) {
    column = 0;
  }
  ...
  (dhs) => {
    ...
    for (let dh in dhs) {
      if (dh in appContext) {
        ...
        createRowFocusTracker(dhs[dh], validateOnFocusChange(dhs[dh]));
