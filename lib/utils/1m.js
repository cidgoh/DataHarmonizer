...
// NOTE: Handsontable doesn't have a notion of listeners similar to the DOMs.
...
const handleAction = (appContext) => (dh) => (action, details) => {
  ...
  console.info(action, emitted_by, '->', target, old_changes, new_changes, currentSelection);
  switch (action) {
    case ACTION.CREATE:
      break;
    case ACTION.READ:
  ...
  hot.batch(() => {
    ...
    data.every((childRow, rowIndex) => {
      ...
      if (shouldDeleteChildRow) {
        console.info('should delete', dh.class_assignment, rowIndex, hot.getDataAtRow(rowIndex));
