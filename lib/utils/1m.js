...
function bindChildListensAndHandlesChange(appContext, dhs) {
  ...
  nodes.forEach(node_name => {
    ...
    for (let unique_key_name in node.unique_keys) {
      const unique_key = node.unique_keys[unique_key_name];
      if (unique_key.foreign_key) {
        document.addEventListener('handsontableUpdate', provideActionCallback(appContext, dhs));
      }
    };
  });
}
...
const routeAction = (appContext, dhs, action, event) => {
  ...
  if (action === ACTION.READ && emitted_by === target) {
    ...
    try {
      ...
      console.error(e);
      console.warn(emitted_by, appContext[emitted_by], unique_key, key_name, event.detail);
      debugger;
    }
