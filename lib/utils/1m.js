...
const makeCurrentSelection = (appContext, dh) => {
  ...
  console.warn(unique_keys, key_name);
        ...
        dh.hot
          .addHook('afterChange', (changes, source) => {
              ...
              if (changes) {
                ...
                //     detail: { row, prop, oldValue, newValue, sourceTable: foreign_key_class, shared_key: foreign_key_slot }
                //   });
                //   document.dispatchEvent(event);
                // });
                ...
                for (let row in row_changes) {
                  ...
                  if (propagates) {
                    // warn about deletion if it's the final key 
                    if (updateActionType === ACTION.DELETE) {
                      console.warn("PROPAGATION:", "Row collections with key will be deleted", detail);
                    }
      
                    // // warn if update will cause a duplicate that merges entries in child tables which share that same key
                    if (updateActionType === ACTION.UPDATE && getNonEmptyColumnValues(dh.hot, column_index).filter(el => el === currentValue).length + 1 > 1) {
                      console.warn("PROPAGATION:", "Row collections will merge with existing those of existing key value", detail);
                    }
                  }
                  ...
                  if (findActiveDataHarmonizer(appContext) === detail.emitted_by) {
                    // dispatchHandsontableUpdate(ACTION.SELECT, details);
                    // handleAction2(ACTION.SELECT, dhs[detail.emitted_by], detail);
                  }
    ...
    for (let unique_key_name in node.unique_keys) {
      ...
      if (unique_key.foreign_key) {
        document.addEventListener('handsontableUpdate', function (event) {
          const { action, emitted_by, key_name, key_new_values, key_old_values } = event.detail;
          console.warn('receiving change:', node, emitted_by, key_name);
          console.warn('executing handleAction2');
          // handleAction2(appContext, dhs[node_name], action, event.detail);

          // const childCol = dh.getColumnIndexByFieldName(data_harmonizers[node.name], sharedKey.name);
          // if (receivesEventsFrom.flatMap(el => el.related_concept).includes(sourceTable)) {
          //   handleChange(dhs[node.name].hot, [, childCol, oldValue, newValue]);
          // }
        });
      }
    };
...
// const handleAction2 = (appContext, dh, action, details) => {
//   let {
//     emitted_by,
//     key_name,
//     old_changes,
...
// const routeAction2 = (appContext, dhs, action, emitted_by, target, dhs, event, propagates, unique_key) => {
//   if (action === ACTION.READ && emitted_by === target) {
//     runHook(dhs[emitted_by], action, event.detail);
//     dispatchHandsontableUpdate(ACTION.SELECT, event.detail);
//   } else if (action === ACTION.SELECT) {
