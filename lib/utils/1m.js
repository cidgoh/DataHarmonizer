...
export function buildAppContext(schema) {
  ...
  add_child_classes_to_unique_keys_for_appContext(
    ...
    if (changes) {
      ...
      for (let row in rowChanges) {
        ...
        for (let keyName of maybeUniqueKeys) {
          if (
            hasDuplicates(
              getNonEmptyColumnValues(
                dh.hot,
                dh.getColumnIndexByFieldName(keyName)
              )
            )
          ) {
            // Undoing
...
const handsontableUpdateRouter = (appContext) => (dhs) => {
  document.addEventListener('classDataRowKeyChange', (event) => {
    const { emitted_by, target, key_name, action } = event.detail;
    ...
    if (classCanFire) {
      if (action === ACTION.READ && emitted_by === target) {
        ...
        event.detail.currentSelection = makeCurrentSelection(appContext)(
          dhs[findActiveDataHarmonizer(appContext)]
        );
...
const handleAction = (appContext) => (dh) => (action, details) => {
  ...
  switch (action) {
    ...
    case ACTION.DELETE:
      // TODO: should the empty update strategy be an array of nulls?
      // appContext[emitted_by].unique_keys[key_name].key_values = appContext[emitted_by].unique_keys[key_name].key_indexes.map(() => null);
