        ...
        for (const [, uniqueKey] of Object.entries(node.unique_keys)) {
            if (uniqueKey.foreign_key) {
                ...
                const computeImpactedRows = (rowChanges) => {
                    // Pre-compute impact
                    const impactedRows = Object.entries(rowChanges).map(([row, change]) => ({
                      row,
                ...
                const showModalBlocking = () => {
                  let result = false;
                  const modal = $("#primary-key-delete-modal");
                
                  modal.modal('show');
                
                  modal.find(".btn-warning").one("click", () => {
                    result = true;
                    modal.modal('hide');
                  });
                
                  modal.find(".btn-secondary").one("click", () => {
                    result = false;
                    modal.modal('hide');
                  });
                
                  // NOTE: This is a synchronous blocking call, but in a real browser, you may need
                  // to handle the flow differently since browser event loops don't "block" on modal calls.
                  return result;
                };
