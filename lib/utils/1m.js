    ...
    dh.hot.addHook('afterSelection', (row, col) => {
      ...
      if (!isEmptyUnitVal(valueToMatch)) {
        ...
        schema_tree[parent_name].children.forEach((child_name) => {
          ...
          const shared_key_name = getSharedKeyName(
            schema_tree,
            parent_name,
            child_name
          );
...
// const createContextMenuHandler = (data_harmonizers, shared_key_spec) => {
    ...
    if (!node.tree_root) {
      ...
      if (isParent) {
        ...
        propagatesChangesOnTheseKeys.forEach((shared_key) => {
          ...
          data_harmonizers[node.name].hot.addHook(
            'beforeRemoveRow',
            rowDataManager.beforeRemoveRow
          );
          data_harmonizers[node.name].hot.addHook(
            'afterRemoveRow',
            rowDataManager.afterRemoveRow
          );
      ...
      if (isChild) {
        ...
        document.addEventListener('handsontableUpdate', function (event) {
          const { row, oldValue, newValue, sourceTable, sharedKey } =
            event.detail;
          const childCol = getColumnIndexByFieldName(
            data_harmonizers[node.name],
            sharedKey.name
          );
          if (
            receivesEventsFrom
