  ...
  document.addEventListener('handsontableUpdate', (event) => {
    ...
    if (classCanFire) {
      if (action === ACTION.READ) {
        ...
        triggerCurrentSelection(makeCurrentSelection(appContext)(dhs[emitted_by]));
...
const handleAction = (appContext) => (dh) => (action, details) => {
  ...
  switch (action) {
    ...
    case ACTION.READ:
      ...
      break;
    case ACTION.FILTER:
      console.log(ACTION.FILTER, emitted_by, dh.class_assignment, target, key_name);
      const column = dh.getColumnIndexByFieldName(key_name);
      filterRowsByCondition(
        dh.hot,
        column,
        val => val === appContext[emitted_by].unique_keys[key_name],
      );
      break;
    case ACTION.UPDATE:
...
const makeCurrentSelection = appContext => dh => {
  ...
  console.log(key_name, unique_keys);
  const { key_values, key_indexes } = unique_keys[key_name];
  console.log('makeCurrentSelection', key_values, key_indexes, column);
