...
function getSharedKeyNameForParentAndChild(schemaTree, parentName, childName) {
...
function makeColumnReadOnly(data_harmonizer, shared_key_name) {
  ...
  const columnIndex = data_harmonizer.getColumnIndexByFieldName(shared_key_name);
...
const createRowDataManager = (hot, contex_menu_callback) => {
...
function setupSharedColumn(
  ...
  if (columnIndex === -1) {
    ...
    const rowDataManager = createRowDataManager(hot, contex_menu_callback);
  ...
  const createContextMenuHandler = (shared_key_spec) => {
    return (rowData, source) => {
      schema_tree_node.children.forEach((cls_key) => {
        ...
        const col = data_harmonizers[cls_key].getColumnIndexByFieldName(shared_key_spec.name );
...
function clearFiltersAtIndex(dh, columnIndex) {
  const plugin = dh.hot.getPlugin('filters');
...
function propagateFilter(
  ...
  visitSchemaTree(
    ...
    (schema_tree_node) => {
      const hot = data_harmonizers[schema_tree_node.name].hot;
      const columnIndex = data_harmonizers[schema_tree_node.name].getColumnIndexByFieldName(shared_key_name);
...
function propagateAdditionalFilter(
  data_harmonizers,
  shared_key_name,
  schema_tree,
  child_name,
...
export function attachSelectionHandlersToDataHarmonizers(
  data_harmonizers,
  schema_tree
) {
...
export const setup1M = (dhs, schema_tree) => {
  attachPropagationEventHandlersToDataHarmonizers(dhs, schema_tree);
  attachSelectionHandlersToDataHarmonizers(dhs, schema_tree);
};
