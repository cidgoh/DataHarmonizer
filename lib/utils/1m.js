...
const destroyHandsontableUpdateRouter = () => {
  document.removeEventListener('handsontableUpdate', provideActionCallback);
};
...
const dispatchHandsontableUpdate = (detail) => {
  const event = new CustomEvent('handsontableUpdate', { detail });
  document.dispatchEvent(event);
};

function bindParentBroadcastsChange(appContext, dhs) {
  // look for nodes
  ...
  nodes.forEach((node_name) => {
    ...
    for (let unique_key_name in node.unique_keys) {
      ...
      if (unique_key.foreign_key) {
        ...
        // dispatch change event with dispatchHandsontableUpdate, which will get a child dataharmonizer to fire 'routeAction'
        // TODO: namespace the event?
        dh.hot.addHook('afterChange', (changes, source) => {
          ...
          if (changes) {
            ...
            for (let row in row_changes) {
              ...
              const updateActionType = Object.values(row_changes[row].newValues)
                .map(isEmptyUnitVal)
                .includes(true)
                ? ACTION.DELETE
                : ACTION.UPDATE;
...
function provideActionCallback(appContext, dhs) {
  return function (event) {
    // TODO: check here if I need the listener to restrict routeAction based on:
    // - the foreign key and primary key relationship
    // - the emitted and target class
    routeAction(appContext, dhs, event.detail.action, event);
  };
}

function bindChildListensAndHandlesChange(appContext, dhs) {
  const nodes = Object.keys(appContext);
  nodes.forEach((node_name) => {
    const node = appContext[node_name];
    for (let unique_key_name in node.unique_keys) {
      ...
      if (unique_key.foreign_key) {
        document.addEventListener(
          'handsontableUpdate',
          provideActionCallback(appContext, dhs)
        );
      }
    }
  });
...
const routeAction = (appContext, dhs, action, event) => {
  ...
  if (action === ACTION.READ && emitted_by === target) {
    ...
    dispatchHandsontableUpdate(event.detail);
...
const handleAction = (appContext, dhs, dh, action, details) => {
  ...
  let shared_key_details = null;
  // key_name could be null on a READ action
  if (key_name in appContext[emitted_by].unique_keys) {
    shared_key_details = appContext[emitted_by].unique_keys;
  }
...
export const setup1M = ({ appContext }, dhs) => {
  ...
  for (let dh in dhs) {
    if (dh in appContext) {
      ...
      bindParentBroadcastsChange(appContext, dhs);
      bindChildListensAndHandlesChange(appContext, dhs);
    }
  }
