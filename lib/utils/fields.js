...
export function findFieldIndex(fields, key, translationMap = {}) {
  ...
  let index = fields.findIndex((f) => {
    // `f.name === undefined` and `f.alias === undefined` are true, but not a
    // direct match, so we need to check if both properties exist.
    const nameMatch = Object.hasOwn(f, 'name') && f.name === key;
    const aliasMatch = Object.hasOwn(f, 'alias') && f.alias === key;
  ...
  if (index < 0 && translationMap[key]) {
    ...
    index = fields.findIndex((f) => {
        const nameMatch =
          Object.hasOwn(f, 'name') && f.name === untranslatedKey;
        const aliasMatch =
          Object.hasOwn(f, 'alias') && f.alias === untranslatedKey;
        return nameMatch || aliasMatch;
      }
    );
  }
