...
/**
 * Returns { minimum_value, maximum_value } for a slot, checking the top-level
 * slot definition first (backward-compat), then falling back to the any_of
 * entry whose range is a numeric schema type.
 */
export function getNumericConstraints(slotDef, schema) {
  if (slotDef.minimum_value != null || slotDef.maximum_value != null) {
    return { minimum_value: slotDef.minimum_value, maximum_value: slotDef.maximum_value };
  }
  for (const entry of slotDef.any_of ?? []) {
    if (schema.types?.[entry.range] && (entry.minimum_value != null || entry.maximum_value != null)) {
      return { minimum_value: entry.minimum_value, maximum_value: entry.maximum_value };
    }
  }
  return {};
}
