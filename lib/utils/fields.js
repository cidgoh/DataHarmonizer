...
export function dataObjectToArray(dataObject, fields, options = {}) {
  ...
  for (const [key, value] of Object.entries(dataObject)) {
    const fieldIdx = findFieldIndex(fields, key, translationMap);
...
export function titleOverText(enm) {
  return typeof enm.title !== 'undefined'
    ? enm.title
    : enm.text;
}
...
export const rangeToContainerClass  = (Container, cls_key) => {
  console.log(Object.values(Container.attributes).filter(v => 'range' in v).filter(v => v.range === cls_key))
  return Object.values(Container.attributes).filter(v => 'range' in v).filter(v => v.range === cls_key)[0].name;
};