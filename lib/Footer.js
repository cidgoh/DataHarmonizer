...
class Footer {
  ...
  // Load a DH schema.yaml file into this slot.
    ...
    //alert("Note, If you load the same schema more than one time you may encounter some delay in loading.")

    $('#schema_upload').on('change', (event) => {
      alert("Note: currently loading larger schemas can take a longer time to load, or may crash application.")
      ...
      // Normally 
      //console.log(event.target.files[0], reader.readyState) // readyState is 0 here.
