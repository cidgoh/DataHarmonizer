...
class Validator {
  ...
  */
  getValidatorForSlot(slot, options = {}) {
    ...
    */
    ...
    const validate = (value) => {
      const valueIsAbsent = value === null || value === undefined || value === '';
      if (valueIsAbsent) {
      ...
      if (slotDefinition.multivalued) {
        if (Array.isArray(value)) {// Handsontable seems to allow arrays for cell values.
          splitValues = value;
          console.log(`Multivalued slot ${slotDefinition.name} cell data is an array; should it be loaded as a delimited string?  See arraygetValidatorForSlot()`, value);
        }
        else
      ...
      for (const value of splitValues) {
        ...
        if (slotType) {
          ...
          if (parsed === undefined) {
            // Only fall through to anyOfValidators when slotType was derived
            // from any_of[0] as a fallback (no direct 'range' on the slot).
            // In that case a parse failure is not conclusive because another
            // any_of branch (e.g. a NullValueMenu enum) may accept the value.
            // When there IS a direct range, a parse failure is definitive.
            const slotTypeIsDirect = !!slotDefinition.range;
            if (!anyOfValidators.length || slotTypeIsDirect) {
              return `Value does not match format for ${slotType.uri}`;
            }
            // parsed_ok stays false; fall through to anyOfValidators below.
          }
          // Value parsed successfully as the declared type.
          else {
            ...
            // Basic type constraints passed. Fall through to any_of/all_of/etc
            // checks below, which may impose additional structural constraints.
            parsed_ok = true;
          }
        ...
        // Enum check only for values that did not pass a basic type check.
        // (A typed value that passes its type check is not subject to enum restriction.)
        if (!parsed_ok && slotEnum && !slotPermissibleValues.includes(value)) {
        ...
        // Typed value passed all basic and structural constraint checks.
        if (parsed_ok) {
          continue;
        }
        ...
        if (
          ...
          // Passed validation via constraint validators; any parse error overridden.
        } else if (parse_error.length) {
          // No constraint validators present and basic type check failed.
          return parse_error;
        }
      }
    };
  ...
  /**
   * Re-validate a single row without running cross-row checks.
   * Covers: per-cell atomic constraints, dependent column comparisons, and
   * rules (pre/post/else conditions). Uniqueness / identifier duplicate checks
   * are intentionally skipped — they require the full dataset and are left
   * stale until the next full validate() call.
   *
   * @param {Array<Array>} data  Full grid data array (visual-row indexed, from getData()).
   * @param {Array<string>} header  Slot names aligned to data columns.
   * @param {number} rowIndex  Index into `data` for the row to validate.
   * @return {Object}  { colIndex: errorMessage } for invalid cells (empty if valid).
   */
  validateRow(data, header, rowIndex) {
    const row = data[rowIndex];
    if (!row || rowIsEmpty(row)) return {};

    const errors = {};

    // Per-cell atomic validation (reuses the cached slot validators).
    for (let col = 0; col < row.length; col++) {
      const slotName = header[col];
      if (!slotName) continue;
      const valueValidator = this.getValidatorForSlot(slotName, { cacheKey: slotName });
      const result = valueValidator(row[col]);
      if (result !== undefined) errors[col] = result;
    }

    // Dependent comparison validation (within this row only).
    const runDependentCheck = (dependantMap, compareFn, message) => {
      for (const [slotName, compareSlotName] of dependantMap.entries()) {
        const col = header.indexOf(slotName);
        const compareCol = header.indexOf(compareSlotName);
        if (col < 0 || compareCol < 0) continue;
        const slotDefinition = this.#targetClassInducedSlots[slotName];
        const slotRange = slotDefinition.range || slotDefinition.any_of?.[0]?.range;
        const slotType = this.#schema.types?.[slotRange];
        const compareValue = this.#parser.parse(row[compareCol], slotType.uri);
        if (!compareValue) continue;
        const value = this.#parser.parse(row[col], slotType.uri);
        if (!value) continue;
        if (!compareFn(value, compareValue)) {
          errors[col] = `Value ${message} value of ${compareSlotName} column`;
        }
      }
    };
    runDependentCheck(this.#dependantMinimumValuesMap, (a, b) => a >= b, 'is less than');
    runDependentCheck(this.#dependantMaximumValuesMap, (a, b) => a <= b, 'is greater than');

    // Rules — pre/post/else conditions evaluated for this row only.
    const rules = this.#targetClass.rules ?? [];
    for (let idx = 0; idx < rules.length; idx++) {
      ...
      const preConditions = this.#buildSlotConditionGettersAndValidators(
        rule.preconditions, header, `rule-${idx}-preconditions`
      );
      ...
      const applicableConditions = this.#buildSlotConditionGettersAndValidators(
        preConditionsMet ? rule.postconditions : rule.elseconditions,
        header,
        `rule-${idx}-${preConditionsMet ? 'postconditions' : 'elseconditions'}`
      );
      for (const [getter, validator] of applicableConditions) {
        const [col, value] = getter(row);
        const result = validator(value);
        if (result !== undefined) errors[col] = result;
      }
    }
