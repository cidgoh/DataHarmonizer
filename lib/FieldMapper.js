...
export class FieldMapper {
  constructor(context, file, data) {
    ...
    dh_settings.schema[schema.name].mapping ??= {};
    Object.entries(dh_settings.schema[schema.name].mapping).forEach(([name]) => {
    ...
    $('#field-mapping-select').off('change').on('change', function() {
      ...
      const has_profile = profile_name.length > 0;
      $('#field-mapping-delete-btn').prop('disabled', !has_profile);
      $('#field-mapping-apply-profile-btn').prop('disabled', !has_profile);
      if (has_profile) {
    ...
    // Applies the selected profile to the mapping form.
    $('#field-mapping-apply-profile-btn').off('click').on('click', function() {
      const profile_name = $('#field-mapping-select').val();
    ...
    // This doesn't affect mapping rendered in form.
    $('#field-mapping-delete-btn').off('click').on('click', function() {
      ...
      if (profile_name > '') { // we have something to delete.
        ...
        if (dh_settings.schema[schema.name]?.mapping?.[profile_name])
          delete(dh_settings.schema[schema.name].mapping[profile_name]);
        saveBrowserDHSettings(dh_settings);
        $(`#field-mapping-select option[value="${profile_name}"]`).remove();
        $('#field-mapping-select').val('');
        $('#field-mapping-display').text('');
        $('#field-mapping-name').val('');
        $('#field-mapping-apply-profile-btn').prop('disabled', true);
        $('#field-mapping-delete-btn').prop('disabled', true);
      }
    ...
    $('#field-mapping-save-btn').off('click').on('click', function (){
      ...
      if (profile_name) {
        ...
        dhAlert("Please provide a name for this mapping profile.");
      }
    });
  ...
  loadMappedData() {
    ...
    for (const [template_name, dh] of Object.entries(this.context.dhs)) {
      ...
      const new_table = [];
      const slot_to_data_col = this.data[template_name].slot_to_data_col_matches
      const data_field_to_col = this.data[dh.template.name].header_lookup;
      // Do mapping (in map_obj.map ) if any needed 
      ...
      dh.hot.loadData( dh.matrixFieldChangeRules(new_table) );
  ...
  appendFieldMappingModal(dh, append_html = true) {
    ...
    // Lookup dictionary
    const data_field_to_col = Object.fromEntries(
      data_fields.map((item, index) => {return [item, index]})
    );
    this.data[dh.template.name].header_lookup = data_field_to_col;
    ...
    dh.slot_names.forEach((slot_name, index) => {
      if (slot_name in data_field_to_col) { // JSON data matches on slot.name
        //console.log("FOUND NAME", slot_name, data_field_to_col[slot_name])
        const data_index = data_field_to_col[slot_name];
        ...
        if (title in data_field_to_col) { // Tabular tsv,csv,xls,xlsx matching
          //console.log("FOUND TITLE", title, data_field_to_rcol[title])
          const data_index = data_field_to_col[title]
          slot_matches[index] = data_index;
          data_matches[data_index] = index;
          //found_by_title = true;
        }
    ...
    // Store exact matches for each template
  ...
  /** Updates browser localStorage dataharmonizer_settings.schema.[schema name].mapping[profile name]
  ...
  updateProfileMapping(profile_name) {
    ...
    const schema = this.context.getSchemaRef();
    dh_settings.schema[schema.name].mapping ??= {};
    dh_settings.schema[schema.name].mapping[profile_name] = profile;
    saveBrowserDHSettings(dh_settings);
  }

  // Retrieves browser memory dataharmonizer_settings
  // .schema[schema name].mapping[profile] data structure and initializes if
  // not yet accessed.
  getProfile(profile_name) {
    ...
    if (!(schema.name in dh_settings.schema))
      dh_settings.schema[schema.name] = {};
    dh_settings.schema[schema.name].mapping ??= {};
    if (!(profile_name in dh_settings.schema[schema.name].mapping)) {
      dh_settings.schema[schema.name].mapping[profile_name] = {
        schema_version: schema.version,
        tables: {}
      }
    }
    return [dh_settings, dh_settings.schema[schema.name].mapping[profile_name]];
  };
  ...
  renderFieldMappingModal(profile_name = null) {
    ...
    // On a fresh render (reset button / initial open) clear concise mode.
    // When re-rendering with a profile (save/apply), preserve the current state.
    const concise = profile_name
      ? $('#field-mapping-concise-checkbox').is(':checked')
      : false;
    if (!profile_name) $('#field-mapping-concise-checkbox').prop('checked', false);
    this.conciseFieldMappingModal(concise);

    // Apply DH_TERM substitution to the "Template {{Slot}}" header so it
    // renders as "Template Field" (default) or "Template Slot" (LinkML mode).
    const $templateTh = $('#field-mapping-table thead th:first-child');
    $templateTh.text(this.context.applyDhTerms($templateTh.text()));
  ...
  applyFieldMapping(profile_name) {
    ...
    Object.entries(profile.tables || {}).forEach(([table_name, table_obj]) => {
      ...
      Object.entries(table_obj?.map || {}).forEach(([, mapping]) => {
        ...
        // Mirror the drop-handler colour feedback.
        $(schema_slot_td).next('td').css('background-color', 'lightskyblue');
        $(schema_data_td).css('background-color', 'lightblue');
      });
    });
  }
