...
class DataHarmonizer {
  ...
  createHot() {
    ...
    this.hot.addHook('afterRemoveCellMeta', function (row, column, key, value) {
      console.log('afterRemoveCellMeta', row, column);
    });
      ...
      function (changes, source) {
        if (source === 'CopyPaste.paste') {
          ...
          const { startRow, startCol } =
            self.clipboardCoordCache[previousAction];
        ...
        if (fields[col].flatVocabulary && fields[col].multivalued === true) {
          ...
          $('#field-description-text .multiselect')
            ...
            .on('change', function () {
              ...
              self.getCell(row, col).setAttribute(
                'data-i18n-options',
                JSON.stringify({
                  value: $('#field-description-text .multiselect').val(),
                })
