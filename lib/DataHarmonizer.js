    ...
    this.hot.addHook('afterBeginEditing', function (row, col) {
        ...
        if (meta.meta?.datatype === 'multivalued') {
          ...
          const selectizeCfg = {
            ...
            // Slide the dropdown in rather than revealing it instantly.
            // An instantaneous appearance can place menu items under the mouse
            // cursor, causing a mouseover/mousedown event to fire and
            // inadvertently add an item to the selection.  The brief animation
            // keeps the menu away from the pointer long enough that no such
            // event is triggered.
            onDropdownOpen: function($dropdown) {
              $dropdown.hide().slideDown(200);
            },
          };
          ...
          // Defer open() and focus until the modal animation has completed.
          // Calling open() on a hidden element silently fails (browsers ignore
          // focus on invisible nodes), leaving selectize unfocused so that
          // subsequent clicks on the input area also do nothing.
          $('#multiselect-modal').one('shown.bs.modal', () => {
            // Native focus called synchronously in shown.bs.modal — before
            // returning to the event loop — so HOT's rAF render cycles
            // (hot.listen → rootElement.focus) cannot steal the cursor.
            // Selectize's own onFocus handler calls open() automatically when
            // the input receives native focus; explicit s.open() is not needed
            // and creates a jQuery-trigger vs native-focus race.
