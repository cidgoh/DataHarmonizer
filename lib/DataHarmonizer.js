...
class DataHarmonizer {
  ...
  saveSchema () {
    ...
    let row = dh.current_selection[0];
    let schema_name = dh.hot.getDataAtCell(row, 0);
    let save_prompt = `Provide a name for the ${schema_name} schema file. This will save the following schema parts:\n`;

    let [save_report, confirm_message] = this.getChangeReport(this.template_name);

    // prompt() user for schema file name.
    let file_name = prompt(save_prompt + confirm_message, 'schema.json');

    if (file_name) {

      //let schema = new Map (Object.entries({ // Provide defaults here.
      ...
      for (let [ptr, class_name] of components.entries()) {
        ...
        // Schema | Prefix | Class | UniqueKey | Slot | SlotUsage | Enum | PermissableValues
        for (let dep_row in rows) {
          // Convert row slots into an object for easier reference.
          let record = {};

          for (let [dep_col, dep_slot] of Object.entries(dependent_dh.slots)) {
            // 'row_update' attribute may avoid triggering handsontable events
            let value = dependent_dh.hot.getDataAtCell(dep_row, dep_col);
          ...
          switch (class_name) {
            case 'Schema':
              this.copySlots(record, schema, ['name','description','id','version','default_prefix']);
              if (record.in_language)
                schema.in_language = record.in_language.split(';');
              break;
            ...
            case 'Class':
              ...
              this.copySlots(record, schema.classes[record.name], ['name','title','description','version','class_uri']);
              if (record.container) {
            ...
            case 'UniqueKey':
              ...
              this.copySlots(record, schema.classes[record.class_id].unique_keys[record.name], ['description','notes']);
              break;

            case 'Slot':
              if (record.name) {
                schema.slots[record.name] = {};
                this.copySlots(record, schema.slots[record.name], ['name','slot_group','slot_uri','title','range','required','recommended','description','aliases','identifier','foreign_key','multivalued','minimum_value','maximum_value','equals_expression','pattern','comments','examples'
                ]);
              }
              break;
            ...
            case 'SlotUsage':
              ...
              this.copySlots(record, schema.classes[record.class_id].slot_usage[record.slot_id], ['name','rank','slot_group','title','range','inlined','required','recommended','description','aliases','identifier','foreign_key','multivalued','minimum_value','maximum_value','minimum_cardinality','maximum_cardinality','pattern','comments','examples'
              ]); //title , 'equals_expression' ; don't really need rank here either.
              break;

            case 'Enum':
              schema.enums[record.name] = {};
              this.copySlots(record, schema.enums[record.name], ['name','title','enum_uri','description']);
              break;

            case 'PermissableValues':
              schema.enums[record.enum_id].permissible_values = {};
              this.copySlots(record, schema.enums[record.enum_id].permissible_values, ['name','text','description','meaning']); // is_a
              break;

            case 'Type':
              // Coming soon, saving all loaded data types.
              break;
          }
        }
      };
      ...
      a.href = URL.createObjectURL(new Blob([stringify(schema)], {
        type: "text/plain"
      }));
      a.setAttribute("download", "schema.yaml");
  ...
  loadSchemaYAML(text) {
    ...
    // Make efficient all load operations on new rows - no handsontable rendering.
    // IMPORTANT: Code below shouldn't exit before restoreRender();
    for (const dh of Object.values(this.context.dhs)) {
      dh.hot.suspendRender();
      //dh.hot.suspendExecution();
      // performance issue in handsontable adding new records when existing ones are hidden.
      //dh.hot.updateSettings({hiddenRows: false}); 
    ...
    // alter not needed?  Row added automatically via setDataAtCell() ?!
    // dh.hot.alter('insert_row_below', dh_row, 1);

    // 2nd pass, now start building up table records from core Schema prefixes, classes, slots, enums entries:
    let conversion = {
      prefixes: 'Prefix',
    ...
    for (let [schema_part, class_name] of Object.entries(conversion)) {
      ...
      // Handsontable ERRORS with "Assertion failed: Expecting an unsigned 
      // number." if alter() is surrounded by "suspendRender()"

      //let dh_row = dh.hot.countSourceRows();
      //dh.hot.alter('insert_row_below', dh_row, Object.keys(schema[schema_part]).length);
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part])) {
        ...
        switch (class_name) {
          ...
          case 'Class':
            ...
            if (value.slot_usage) {
              dh_su.hot.batchRender(() => { // TESTING
                for (let [key_name, obj] of Object.entries(value.slot_usage)) {
                  this.addSlotRecord(dh_su, load_name, value.name, key_name, obj);
                };
              });
            }
            break;
          ...
          case 'Slot':
            ...
            this.addSlotRecord(dh, load_name, null, value.name, value);
            break;
   ...
   */ 
  addSlotRecord(dh, schema_name, class_name, slot_name, slot_obj) {
...
*/
    ...
    this.addRowRecord(dh, slot_record);
  };

  getListAsDelimited(my_array, filter_attribute) {
    if (Array.isArray(my_array)) {
      if (filter_attribute) {
        return my_array.filter((item) => filter_attribute in item)
          .map((obj) => obj.value)
          .join(';');
      } 
   ...
   * Interestingly, just setDataAtCell, if row points to a non-existing row,
   * handsontable will create a new row.
   */ 
  addRowRecord(dh, record) {

    let dh_row = dh.hot.countSourceRows();
    for (let [slot_name, value] of Object.entries(record)) {
      if (slot_name in dh.slot_name_to_column) {
        let dh_col = dh.slot_name_to_column[slot_name];
        // This will trigger a beforeChange() event and there's no way to avoid that,
        // so beforeChange code has conditional to ignore the 'upload' source.
        dh.hot.setDataAtCell(dh_row, dh.slot_name_to_column[slot_name], value, 'upload');
      }
      else
  ...
  copySlots(record, target, slot_list) {
    for (let [ptr, slot_name] of Object.entries(slot_list)) {
      if (record[slot_name]) {
        target[slot_name] = record[slot_name];
      }
    }
  };
...
<body>
  ...
  getColumns() {
    ...
    for (let slot of this.slots) {
      ...
      if (slot.sources) {
        const options = slot.sources.flatMap((source) => {
          if (slot.permissible_values[source])
            ...
            if (! ['SchemaTypeMenu','SchemaClassMenu','SchemaEnumMenu'].includes(source))
              alert(`Schema Error: Slot range references enum ${source} but this was not found in enumeration dictionary, or it has no selections`);
            return [];
          }
        });
