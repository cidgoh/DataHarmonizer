...
import { findLocalesForLangcodes, interface_translation } from '@/lib/utils/i18n';
...
class DataHarmonizer {
  ...
  async openFile(file) {
    ...
    const container_for_range = (container, maybe_range) =>  {
      Object.entries(container.attributes)
        .forEach(([container_class, attributes]) => {
    ...
    const range_for_container = (container, container_class) => container.attributes[container_class].range;
    ...
    try {
      ...
      if (file.type === 'application/json') {
        ...
        console.log(schema, in_language)
        ...
        if (this.schema.id !== schema) {
          ...
          this.context = await this.context.setupDataHarmonizers({
            template_path: `${location.split('/').slice(-1)}/${schema.split('/').slice(-1)}`,
          })
          .then(async (context) => {
            const languages = await context.getLocaleData(context.template._name);
            await context.addTranslationResources(context.template, languages);
            return [context, languages];
          })
          .then(([context, languages]) => {
            ...
            // TODO
            translationSelect.val(in_language).trigger('change');
            ...
            if (typeof schema.classes[schema.name] !== 'undefined' && typeof schema.classes[schema.name].see_also !== 'undefined') {
              helpSop.attr('href', schema.classes[schema.name].see_also).show();
            } else {
              helpSop.hide();
            };
          ...
          .then((context) => {
            Object.values(context.dhs).forEach((dh) => {
              dh.invalid_cells = {};
              dh.openFile.bind(dh)(file);
              dh.current_selection = [null, null, null, null];
            });
            return context;
          });

          Object.keys(jsonData.Container)
            .forEach(container_class => {
              let dataObjects = jsonData.Container[container_class];
              let range = range_for_container(this.context.template.current.schema.classes.Container, container_class);
              let list_data = this.context.dhs[range].loadDataObjects(dataObjects, i18next.language);
              if (list_data != null) {
                console.log('loading list data in hot 1', list_data, dataObjects);
          ...
          const container_class = container_for_range(this.context.template.current.schema.classes.Container, this.class_assignment);
          if (container_class) {
            let dataObjects = jsonData.Container[container_class]
            list_data = this.loadDataObjects(dataObjects, i18next.language);
            console.log('loading list data in hot 2', list_data, dataObjects);
  ...
  loadDataObjects(data) {
    ...
    data.forEach((row) => {
      const dataObject = dataObjectToArray(row, fields, {
        ...
        datetimeFormat: this.datetimeFormat,
        timeFormat: this.timeFormat,
        translationMap: invert(i18next.getResourceBundle('fr', 'translation'))
      });
