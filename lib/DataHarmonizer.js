...
<body>
  ...
  // not having data?
  loadSpreadsheetData(workbook, file_name) {
    ...
    const [header_row, schema_slot_count, matches] = this.compareMatrixHeadersToGrid(matrix);
    ...
    if (header_row > 0 && schema_slot_count == matches) { 
      ...
      console.log("Loading spreadsheet tab", sheet_name, "into", this.template.name);
      return matrix.slice(header_row); // chops 1 or 2 header rows off
      //return data_table;
    } 
    else {
      ...
      // If 
      this.launchSpecifyHeadersModal(matrix, header_row, file_name);
      return null;
    }
  }
   ...
   */
  launchSpecifyHeadersModal(matrix, header_row, file_name) {

    const self = this;
    $('#field-mapping-description').html(`One or more template header field names do not exactly match those in the selected data file <strong>${file_name}</strong>.  Unmatched field(s) - shown in orange - can be remedied by dragging the appropriate data field to the matching template field row.<br>
      (Field ordering of matching fields is automatically handled.)`);

    // Note: this doesn't handle rare issue of source having duplicate column names.
    // matrix might not have section headers.
    const data_field_lookup = Object.fromEntries(
      matrix[header_row-1].map((item, index) => {return [item, index]})
    );

    let slot_row = 0;
    let data_row = 0;

    $('#field-mapping').append(`
      <tr class="field-mapping-template">
        <td colspan="3"><strong>${this.template_name}</strong></td>
      </tr>`);

    Object.entries(this.sections).forEach(([i, section]) => {
      $('#field-mapping').append(`
      <tr class="field-mapping-section">
        <td colspan="3">${section.title}</td>
      </tr>`);
      Object.entries(section.children).forEach(([i, slot]) => {
        const match = slot.title in data_field_lookup;
        if (match) {
          slot_row += 1;
          data_row += 1;
          $('#field-mapping').append(`
          <tr>
            <td>${slot_row}</td>
            <td>${slot.title}</td>
            <td class="field-match">${data_row}) ${slot.title}</td>
          </tr>`);
        }
        else {
          slot_row +=1;
          ...
          // Now show as many slot rows as it takes to catch up to next
          // slot-to-data match (or all) - but before doing that next match, 
          // show all intervening data fields.
          $('#field-mapping').append(`
          <tr>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td class="draggable-mapping-item field-mismatch">${data_row}) ${matrix[header_row-1][i]}</td>
          </tr>`);
        }
      });
    });

    // To enable drag-drop on specify-headers-modal
    ...
    $(".draggable-mapping-item").droppable({
      ...
      //console.log("RETRIEVED", JSON.parse(storedUsername)); // Output: JohnDoe
      // Store a variable
      localStorage.setItem('username', JSON.stringify(data_field_lookup));
...
*/
  ...
  */
  ...
  compareMatrixHeadersToGrid(matrix) {
    const schemaRow = this.getFlatHeaders()[1];
