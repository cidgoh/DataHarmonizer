...
class DataHarmonizer extends EventEmitter {
  ...
  constructor(root, options = {}) {
    ...
    this.validatorService = new ValidatorService(this.schema, MULTIVALUED_DELIMITER, {
  ...
  async useSchema(schemaName, templateName) {
    const { schema, exportFormats } = await this.schemaService.loadSchema(schemaName, templateName);
    this.validatorService.setSchema(schema);
    this.exportService.setExportFormats(exportFormats);
    this.emit('schemaLoaded', { schema, exportFormats, templateName });
  }
    this.schema = schema;
    this.validatorService = new ValidatorService(this.schema, MULTIVALUED_DELIMITER, {
      dateFormat: this.dateFormat,
    ...
    this.schemaService.useTemplate(template_name).then(() => {
      this.emit('templateLoaded', template_name);
    });
  }
  ...
  */
  ...
  showColumnsBySectionTitle(sectionTitle, showFirstColumn = true) {
    if (this.getFields().some(field => field.section_title === sectionTitle)) {
   ...
   */
  // This method should be moved to a new service class
  ...
  */
   ...
   */
  // This method should be moved to ValidatorService
