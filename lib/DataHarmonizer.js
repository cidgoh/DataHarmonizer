...
import { changeCase, fieldUnitBinTest } from './utils/fields';
import {
  checkProvenance,
  itemCompare,
  testNumericRange,
  ...
  getTransformedField(headerName, value, field, prefix) {
    if (field.sources) {
			...
			for (const source_index in field.sources) {
				const term = DataHarmonizer.schema.enums[field.sources[source_index]].permissible_values[value];
    ...
    for (let row = 0; row < this.hot.countRows(); row++) {
      ...
      for (let col = 0; col < fields.length; col++) {
        ...
        if (datatype === 'Provenance') {
          checkProvenance(provenanceChanges, VERSION_TEXT, cellVal, row, col);
        }
        ...
        // If not an empty field, check its contents against field datatype AND/OR other kind of range
          ...
          switch (datatype) {
            ...
            case 'xsd:double':
              ...
              valid &= !isNaN(cellVal) && testNumericRange(parsedFloat, field);
              break;
