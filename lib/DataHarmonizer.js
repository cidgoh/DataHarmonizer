      ...
      for (let range of range_array) {
        ...
        if (range in types || pascalToLowerWithSpaces(range) in types) {
          ...
          if (range in enums || pascalToLowerWithSpaces(range) in enums) {
            ...
            new_field.vocabTextToTitle = Object.values(
              range_obj.permissible_values
            ).reduce((acc, item) => {
            ...
            .selectize({
              ...
              render: {
                item: function (data, escape) {
                  // console.log('render item', fields[col].vocabTextToTitle[data.text], data.value)
                  // return `<div>` + escape(fields[col].vocabTextToTitle[data.text]) + `</div>`;
                ...
                option: (data, escape) => {
                  // console.log('render option', fields[col].vocabTextToTitle[data.text], data.value)
                  // return `<div style="padding-left:${indentation}px" class="option ${
                  //   value === '' ? 'selectize-dropdown-emptyoptionlabel' : ''
                  // }">${escape(fields[col].vocabTextToTitle[data.text])}</div>`;
                  // Some `data.text` input is already prepended with spaces, so we replace the translation
                  // ISSUE: a more consistent approach might use a formatter but this is simple. ;[prepend]indent
                  // const label = data.text.replace(
                  //   data.text.trim(),
