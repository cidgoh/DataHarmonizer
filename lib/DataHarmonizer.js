    ...
    for (let col = 0; col < fields.length; col++) {
      ...
      if (fields.length > col + 1) {
        ...
        // Rule: for any [x], [x unit], [x bin] series of fields
          ...
          // 
          if (fieldUnitBinTest(fields, col)) {
    ...
    for (let row in matrix) {
      ...
      if (value && value.length > 0) {
        ...
        if (number >= 0) {
          ...
          const bin_value = typeof matrixRow !== "undefined" ? matrixRow[hotRowBinCol] : null;
          ...
          if (binOffset == 2) {
            const unit_value = typeof matrix[hotRowPtr]!== "undefined" ? matrix[hotRowPtr][hotRowNextCol] : null;
