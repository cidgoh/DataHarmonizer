...
<body>
  ...
  scrollTo(row, column) {
    ...
    // Two-step scroll: (1) centre the target row vertically, then (2) bring
    // the target column into view without disturbing the vertical position.
    const _wrapper = this.hot.rootElement?.querySelector('.wtHolder');
    const _rowH    = (_wrapper && this.hot.getRowHeight(parseInt(row))) || 23;
    const _halfVis = _wrapper ? Math.floor(_wrapper.clientHeight / _rowH / 2) : 8;
    // Step 1 – vertical: scroll so anchorRow is at the top, making the target
    // row land near the middle.  Column 0 resets horizontal position so step 2
    // has a clean baseline.
    this.hot.scrollViewportTo(Math.max(0, parseInt(row) - _halfVis), 0);
    // Step 2 – horizontal: the target row is now visible; HOT only adjusts the
    // horizontal scroll to bring the target column into view.
    this.hot.scrollViewportTo(parseInt(row), parseInt(column));
