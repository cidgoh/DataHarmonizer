      ...
      const hot_settings = {
        ...
        beforeChange: function (grid_changes, action) {
          ...
          for (const row in row_changes) {
            ...
            let warning = self.context.crudGetDependentUpdates(self.template_name, row);
            // confirm(warning) presents user with report containing warning text.
            if (!warning || confirm (warning+ "HI!")) {
              if (warning)
                // "True" parameter triggers updates
                self.context.crudGetDependentUpdates(self.template_name, row, true);
            } 
