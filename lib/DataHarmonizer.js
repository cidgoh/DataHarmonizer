...
<body>
  ...
  loadSpreadsheetData(workbook) {
    ...
    if (!(sheet_name in workbook.Sheets)) {
      alert(`None of the data file table(s) "${sheet_names}" match to the loaded schema "${this.schema.name}" templates. \n\nFirtst select the schema template, either by selecting it in the template menu, or by using the "Upload Template" option.`);
      return null;
    }
    ...
    const [headerRow, schemaSlots, matches] = this.compareMatrixHeadersToGrid(matrix);
    //console.log("HEADERS", headerRow, schemaSlots, matches)

    // Here all DH tab schema slots have matched. 
    // A more powerful version would require only the primary key fields?
    if (headerRow > 0 && schemaSlots == matches) { 
      // Clean up loaded data
      console.log("Loading spreadsheet tab", sheet_name, "into", this.template.name);
      return matrix.slice(headerRow); // chops 1 or 2 header rows off
      return data_table;
    } 
    else {
  ...
  launchSpecifyHeadersModal(matrix) {
    ...
    console.log("HEADERS", flatHeaders) 
    const self = this;
    if (flatHeaders) {
