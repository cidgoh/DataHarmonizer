...
class DataHarmonizer {
  ...
  constructor(root, context, options = {}) {
    ...
    $(this.root).on('dblclick', '.secondary-header-text', (e) => {
    ...
    if (this.slots) {
      ...
      */
      const hot_settings = {
        ...
        multiColumnSorting: {
          sortEmptyCells: true, 
          // false = All empty rows are at end of the table regardless of asc/desc
          ...
          /* TRYING DISABLING 
          initialConfig: {
            column: this.slot_name_to_column['name'], // Guess
            sortOrder: 'asc' //          columnSorting.getSortConfig([column]) 
          }
          */
        },
/*
        // PROBLEM: If done here, row-highlight gets applied to wrong rows when
        // or after subsequent sorting is done.
        cells: function(row, col, prop) { // FUTURE: prop is name of column.

          const cellProperties = {};

          // fixed via afterColumnSort() below.
          // call to cellMetaData() causes stack crash since cells calls this function
          // See https://forum.handsontable.com/t/changing-cell-class-name-using-setcellmeta-not-having-any-effect/3547
          if (self.template_name == 'Slot' && col === 2) {// col - slot_type            
            if (self.hot.getDataAtCell(row, col) === 'slot')
              cellProperties.className = 'row-highlight';  
            else 
...
*/
        ...
        /*
          {
            ...
            name: 'Sort by column',
            hidden: function () {
              return (!(self.current_selection[1] >= 0));
            },
            callback: function (action, selection, event) {
              ...
              columnSorting.sort(
              {
                ...
                sortOrder: 'asc', // How to know which state?
              }
            );
            },
          },
          */
          ...
          },
          {
            ...
            callback: function () {self.translationForm()}
          },
      ...
      // Here is place to add settings based on tab, e.g. cell handling.
  ...
  /**
   * Ensures that Slot table content shows not only rows with keys for active
   * schema_id, and class_id, but also for each such item's name, the general
   * schema_id + name + null class.
   */ 
  schemaSlotClassView(dh) {
    // Ensure that for every slot that has a class_id/name expressed
   ...
   * is visible to css stylesheets, to 'row-highlight'
   */
  rowHighlightColValue(dh, column_name, value) {
    let col = dh.slot_name_to_column[column_name];      
    for (let row = 0; row < dh.hot.countRows(); row++){
      let className = (dh.hot.getDataAtCell(row, col) == value) ? 'row-highlight' : '';
      dh.hot.setCellMeta(row, col, 'className', className); // 
    }
  }
  ...
  translationForm() {
    ...
    if (!locales) {
      alert("In order to see the translation form, first select a schema which has locales.")
      return false;
    }
  ...
  loadSchemaYAML(text) {
    ...
    let focus_row = null;
    let focus_cell = dh_schema.hot.getSelected();
    // Find an empty row
    if (!focus_cell) {
    ...
    if (locales) {
      ...
      console.log(focus_row, this.slot_name_to_column['locales'])
    ...
    for (let class_name of Object.keys(this.context.relations['Schema'].child)) {
      ...
      //console.log(dh_table.template_name)
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Class':
            ...
            if (value.slot_usage) { // class.slot_usage holds slot_definitions 
            ...
            else if (value.attributes) { // class.attributes holds slot_definitions 
  ...
  saveSchema () {
    ...
    if (file_name) {
      ...
      let new_schema = { // Provide defaults here.
        name: '',
        description: '',
        id: '',
        version: '',
        ...
        for (let dep_row of rows) {
          ...
          switch (tab_name) {
            case 'Schema':
              ...
              this.copySlots(tab_name, record, new_schema, 
                ['name','description','id','version','in_language','default_prefix']);
            ...
            case 'Prefix':
              new_schema.prefixes[record.prefix] = record.reference;
              break;

            case 'Class':
              target_obj = new_schema.classes[record.name] ??= {};
              // ALL MULTISELECT FIELDS GET CONVERTED BACK INTO ';' delimited lists
              this.copySlots(tab_name, record, target_obj, 
                ['name','title','description','version','class_uri']
              );
              break;

            case 'UniqueKey':
              let class_record = new_schema.classes[record.class_name] ??= {};
              class_record.unique_keys ??= {};
              target_obj = class_record.unique_keys[record.name] = {
            ...
            case 'Slot':
              if (record.name) {
                ...
                this.copySlots(tab_name, record, target_obj, ['slot_group','inlined','inlined_as_list','slot_uri','title','unit','required','recommended','description','aliases','identifier','multivalued','minimum_value','maximum_value','minimum_cardinality','maximum_cardinality','todos', 'equals_expression','exact_mappings','comments','examples']);
              }
              break;
            ...
            case 'Annotation':
              ...
              case 'slot_usage':
                ...
                break;
              case 'attribute':
                target_obj = new_schema.classes[record.class_name].attributes[record.slot_name] ??= {};
              }
              // And we're just adding annotation:
            ...
            case 'Enum':
              let enum_obj = new_schema.enums[record.name] ??= {};
              this.copySlots(tab_name, record, enum_obj, ['name','title','enum_uri','description']);
              break;
            ...
            case 'EnumSource':
              ...
              if (record.criteria) { 
                ...
                let enum_target_obj = new_schema.enums[record.enum_id] ??= {};
                ...
                this.copySlots(tab_name, record, enum_target_obj, ['source_ontology','is_direct','source_nodes','include_self','relationship_types']);
              }
              break;
      ...
      if (metadata.locales) {
        new_schema.extensions = {locales: {tag: 'locales', value: metadata.locales}}
      }
      ...
      a.href = URL.createObjectURL(new Blob([stringify(new_schema)], {type: 'text/plain'}));
  ...
  async filterByKeys(dh, class_name, key_vals) {
    //THIS SHOULD NOT BE APPLIED TO SCHEMA EDITOR TAB!!!! 
    // ISSUE IS FILTER MOVES selected cell to same column but first row.
    // beginning Forces deselection of cell
    ...
    Object.entries(key_vals).forEach(([key_name, value]) => {
      ...
      if (column !== undefined) {
        ...
        filtersPlugin.addCondition(column, 'eq', [value], 'conjunction'); //
      }
      else
        console.log(`ERROR: unable to find filter column "${column}" in "${class_name}" table.  Check DH_linkML unique_key_slots for this class`);
    });
  ...
  updateSources(slot) {
    return slot.sources.flatMap((source) => {
      if (slot.permissible_values[source])
        ...
        if (! SCHEMAMENUS.includes(source))
