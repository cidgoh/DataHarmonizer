      ...
      const hot_settings = {
        ...
        contextMenu: {
          items: {
            ...
            // Issue is that this needs current column #. It doesn't freeze the 
            // column user is on.  It freezes the first unfrozen column to left.  
        ...
        beforeChange: function (grid_changes, action) {
          ...
          for (const row in row_changes) {
            ...
            const change_prelude = `Your key change on ${self.template_name} row ${
              parseInt(row) + 1
