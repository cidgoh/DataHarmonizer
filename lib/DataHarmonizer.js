...
class DataHarmonizer {
  ...
  addRowsToBottom(numRows) {
    ...
    function findNearestIndexAfterLastNonEmpty(hotInstance, column) {
        // Initialize baseCoord to the first index (0)
        let nearestIndex = 0;
        ...
        for (let i = 0; i < totalRows; i++) {
            const cellValue = hotInstance.getDataAtCell(i, column);
    ...
    // Get the nearest index after the last non-empty row in the specified column
    const baseColCoord = findNearestIndexAfterLastNonEmpty(this.hot, 0);

    // If there's a valid current selection, proceed to populate the new rows
    if (this.context.currentSelection) {
        ...
        if (valueToMatch !== null && sharedKeys.length > 0) {
            sharedKeys.forEach((shared_key_spec) => {
                ...
                const currentColumnName = stripDiv(this.hot.getColHeader()[col]);
                const isMatchingKey = currentColumnName === name && relation === 'parent' && source === related_concept;

                if (isMatchingKey) {
                    this.hot.batch(() => {
                        // Populate the new rows with the selected value
                        for (let i = 0; i < numRows; i++) {
                            ...
                            if (existingValue === null || existingValue === '') {
                                ...
                                console.warn(`Skipping row index ${targetRowIndex} as it already contains a value.`);
                            }
                        }
                    });
                }
