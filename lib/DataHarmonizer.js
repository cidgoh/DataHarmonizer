...
class DataHarmonizer {
      ...
      for (let range of range_array) {
        ...
        if (range in types || pascalToLowerWithSpaces(range) in types) {
          ...
          if (range in enums || pascalToLowerWithSpaces(range) in enums) {
            ...
            if (!('vocabTextToTitle' in new_field)) {
              new_field.vocabTextToTitle = {};
            }
            ...
            new_field.vocabTextToTitle = Object.assign(new_field.vocabTextToTitle, Object.values(
              range_obj.permissible_values
            ).reduce((acc, item) => {
              return Object.assign(acc, {
                [item.text]:
                  typeof item.title !== 'undefined' ? item.title : item.text,
              });
            }, {}));
  ...
  addRowsToBottom(numRows) {
    ...
    console.log('addRowsToBottom', 'currentSelection', this.context.currentSelection )
  ...
  populateNewRows(numRows, baseColCoord) {
    ...
    console.log('populateNewRows', this.context.currentSelection);
        ...
        const selectOptionsForFlatVocab = (slot_id, flat_vocabulary) =>
          flat_vocabulary.reduce((acc, item) => {
            const newAcc = acc.concat([
              {
                value: item,
                _id: typeof field.vocabTextToTitle[item] !== 'undefined' ? field.vocabTextToTitle[item] : item,
                label: typeof field.vocabTextToTitle[item] !== 'undefined' ? field.vocabTextToTitle[item] : item,
              },
            ]);
            return newAcc;
          }, []);
