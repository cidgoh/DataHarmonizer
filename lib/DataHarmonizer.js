...
class DataHarmonizer {
  ...
  useTemplate(template_name) {
    ...
    for (let name in attributes) {
      ...
      // console.log(
      //   this.context.template.default.schema.slots[name],
      //   attributes[name]
      // )
      let field = deepMerge(
        attributes[name],
      ...
      const cond = 'any_of' in new_field && 'any_of' in this.context.template.default.schema.slots[name];
      if (cond && new_field.any_of.length > 1) console.log(new_field.name, new_field.any_of)
      ...
      if ('any_of' in new_field) {
        for (let item of new_field.any_of) {
          // console.log(item.range in this.context.template.current.schema.enums ||
          //   item.range in this.context.template.current.schema.types ||
          //   pascalToLowerWithSpaces(item.range) in this.context.template.current.schema.enums ||
          //   pascalToLowerWithSpaces(item.range) in this.context.template.current.schema.types)
          if (
      ...
      for (let range of range_array) {
        ...
        // Range is a datatype?
        const types = this.context.template.current.schema.types;
        if (range in types || pascalToLowerWithSpaces(range) in types) {
          const range_obj = !!types[range] ? types[range] : types[pascalToLowerWithSpaces(range)];
          ...
          if (range in enums || pascalToLowerWithSpaces(range) in enums) {
            const range_obj = !!enums[range] ? enums[range] : enums[pascalToLowerWithSpaces(range)];
    ...
    if (fields.length) {
      ...
      const hot_settings = {
        ...
        beforeChange: function (changes) {
          console.log('beforeChange triggered_changes');
          if (!changes) {
            return;
          }
  ...
  filterAllEmpty() {
    ...
    filtersPlugin.addCondition((row /*col, value*/) => {
      return !this.hot.getDataAtRow(row).every((cell) => isEmptyUnitVal(cell));
    });
      ...
      afterBeginEditing: function (row, col) {
        ...
        if (fields[col].flatVocabulary && fields[col].multivalued === true) {
          ...
          if (value !== formattedValue) {
            console.log('enableMultiSelection: setDataAtCell')
  ...
  fillColumn(colname, value) {
    ...
    if (colname in fieldYCoordinates) {
      ...
      if (changes.length > 0) {
        console.log('fillColumn: setDataAtCell')
  ...
  doPreValidationRepairs(data) {
    return new Promise((resolve) => {
      ...
      if (cellChanges.length) {
        console.log('doPreValidationRepairs: setDataAtCell and afterChange Hook')
