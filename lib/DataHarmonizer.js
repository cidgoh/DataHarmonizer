  ...
  async openFile(file) {
    ...
    try {
      ...
      if (file.type === 'application/json') {
        ...
        let schema = jsonData.schema;

        // change the schema and language if applicable
        // TODO: these should fire off to the context as events 
        // instead of the current setup
        // HACK: this is brute forcing a bad solution. try to abstract this from context
        // TODO: schema value, is it the correct reference? [URI vs template name]

        if (this.schema.id !== schema) {
          // process depending on if it's an array or object matching the schema for data harmonizer files
          const { container, in_language = 'default', location } = importJsonFile(jsonData);

          this.context.clearInterface();
          this.context.clearContext();
          this.context.setupDataHarmonizers({
            template_path: `${location.split('/').slice(-1)}/${schema.split('/').slice(-1)}`,
          })
          .then(context => {
            const dh = context.getCurrentDataHarmonizer();
            dh.loadDataObjects(container[dh.class_assignment]);
            console.log(dh);
            return context
          })
          .then(context => {
            const languageSelect = $('#select-language-localization');
            i18next.changeLanguage(in_language);
            languageSelect.val(i18next.language).trigger('change');
            $(document).localize();
            return context
          });  
        ...
        list_data = this.loadDataObjects(jsonData.Container[this.class_assignment]);
      } else {
