...
import { findSlotNamesForClass } from "@/web/AppContext"
...
class DataHarmonizer {
  ...
  useSchema(schema, export_formats, template_name) {
    ...
    this.field_filters = findSlotNamesForClass(this.schema, this.class_assignment);
  ...
  useTemplate(template_name) {
    ...
    let attributes = Object.entries(self.context.template.current.schema.classes)
    ...
    this.template_unique_keys = Object.values(
      Object.entries(self.context.template.current.schema.classes)
        .filter(([cls_key]) => cls_key !== 'dh_interface')
        .reduce((acc, [, spec]) => {
          return [...acc].concat(
    ...
    for (let name in attributes) {
      ...
      let field = Object.assign({}, self.context.template.current.schema.slots[name], attributes[name]);
      ...
      if (!sectionIndex.has(section_title)) {
        ...
        const section = self.context.template.current.schema['slots'][section_title];
        if (section) {
          Object.assign(section_parts, section);
        }

        self.sections.push(section_parts);
      }

      let section = self.sections[sectionIndex.get(section_title)];
      ...
      if ('any_of' in new_field) {
        for (let item of new_field.any_of) {
          if (
            item.range in self.context.template.current.schema.enums ||
            item.range in self.context.template.current.schema.types
          ) {
      ...
      for (let range of range_array) {
        ...
        // Range is a datatype?
        if (range in self.context.template.current.schema.types) {
          const range_obj = self.context.template.current.schema.types[range];
          ...
          if (range in self.context.template.current.schema.enums) {
            const range_obj = self.context.template.current.schema.enums[range];
            ...
            if (range in self.context.template.current.schema.classes) {
  ...
  async openFile(file) {
        ...
        $('#' + menuId).on('change', function () {
          ...
          menuIds.forEach(function (otherMenuId) {
            if (otherMenuId !== menuId) {
              ...
              if (
                $otherMenu.find('option[value="' + selectedValue + '"]')
                  .length > 0
              ) {
    ...
    try {
      ...
      if (file.type === 'application/json') {
        ...
        // process depending on if it's an array or object matching the schema for data harmonizer files
        const { schema, in_language = null, location } = importJsonFile(jsonData);
        ...
        if (this.schema.id !== schema) {
          const _file = file;
          const _jsonData = jsonData;
          this.context.clearInterface();
          this.context.clearContext();
          this.context = await this.context
            .setupDataHarmonizers({
              template_path: `${location.split('/').slice(-1)}/${schema
                .split('/')
                .slice(-1)}`,
              locale: in_language
            })
            .then(async (context) => {
              const languages = await context.getLocaleData(
                context.template._name
              );
              await context.addTranslationResources(
                context.template,
                languages
              );
              const exportFormats = await context.getExportFormats(`${location.split('/').slice(-1)}`);
              return [context, languages, exportFormats];
            })
            .then(([context, languages, exportFormats]) => {
              ...
              const translationSelect = $(
                '#select-translation-localization'
              ).empty();
              ...
              // TODO
              translationSelect.val(in_language).trigger('input');
              ...
              const template_name = context.template.current.schema.name;
              $('#template_name_display').text(template_name);
              $('#file_name_display').text(_file.name);
              ...
              if (
                typeof schema.classes[schema.name] !== 'undefined' &&
                typeof schema.classes[schema.name].see_also !== 'undefined'
              ) {
                helpSop
                  .attr('href', schema.classes[schema.name].see_also)
                  .show();
              } else {
                helpSop.hide();
              }
            ...
            .then(context => {
              ...
              context.template.updateLocale(in_language);
              Object.keys(_jsonData.Container).forEach((container_class) => {
                let dataObjects = _jsonData.Container[container_class];
                let range = range_for_container(
                  context.template.current.schema.classes.Container,
                  container_class
                );
                let list_data = this.context.dhs[range].loadDataObjects(
                  dataObjects,
                  i18next.language
                );
                if (list_data != null) {
                  // this.context.dhs[range].hot.clear();
                  context.dhs[range].hot.loadData(list_data);
                  context.dhs[range].hot.render();
                }
              });
              return context;
            })
          ...
          const container_class = container_for_range(
            this.context.template.current.schema.classes.Container,
            this.class_assignment
          );
          if (container_class) {
            let dataObjects = jsonData.Container[container_class];
            list_data = this.loadDataObjects(dataObjects, i18next.language);
            // this.hot.clear();
        ...
        // console.log('loading list data in hot 3', list_data, dataObjects);
        // this.hot.clear();
        this.hot.loadData(list_data);
        this.hot.render(list_data);
      }
  ...
  createHot() {
    ...
    this.hot.addHook(
      ...
      function (changes, source) {
        // TODO: DEPRECATE
        // console.log('afterChange working', changes, source);
        // if (source === 'thisChange') {
        //   const [row, col, initial, next] = changes[0];
        //   const TD_target = self.hot.getCell(row, col);
        //   if (TD_target && TD_target.classList.contains('htAutocomplete')) {
        ...
        //     self.hot.render();
        //   } else {
        //     console.warn("not an autocomplete")
        //   }
        // }
  ...
  hideAllButEmpty() {
    ...
    const rowsToHide = allRows.filter((index) => !emptyRows.includes(index));
  ...
  hideAll() {
    ...
    const allRows = Array.from({ length: numberOfRows }, (_, index) => index);
    this.hideRows(allRows);
  }

  showAllRows() {
    // Update the Handsontable settings to remove all hidden rows
    this.hot.updateSettings({
      hiddenRows: {
        rows: [], // Clearing the array of hidden rows
        indicators: false, // Optionally, you can turn off the indicators
      },
    });

    // Re-render the table to apply the changes
    this.render();
  }

  hideAllButFirstNRows(n) {
    const numberOfRows = this.hot.countRows();
  ...
  hideAllButFirstNEmptyRows(n) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].every((cell) => cell === null || cell === '')) {
        if (emptyRows.length < n) {
          emptyRows.push(i); // Collect only up to 'n' empty rows to keep
        }
      }
    }
    ...
    // Compute the rows to hide (all rows minus the first 'n' empty rows)
    const rowsToHide = allRows.filter((index) => !emptyRows.includes(index));

    // Use the general hideRows function to hide the computed rows
    this.hideRows(rowsToHide);
  }
  ...
  loadDataObjects(data) {
    ...
    if (typeof data === 'object' && !Array.isArray(data) && data !== null) {
      ...
      if (inferredIndexSlot) {
        ...
        console.log(fields, inferredIndexSlot, dataKeys);
    ...
    let listData = [];
    data.forEach((row) => {
      const dataObject = dataObjectToArray(row, fields, {
        serializedDateFormat: this.dateExportBehavior,
        dateFormat: this.dateFormat,
        datetimeFormat: this.datetimeFormat,
        timeFormat: this.timeFormat,
        // translationMap: invert(i18next.getResourceBundle(i18next.language, 'translation')),
      });
