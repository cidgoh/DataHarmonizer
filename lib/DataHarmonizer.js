...
<body>
  ...
  launchSpecifyHeadersModal(matrix, header_row, file_name) {
    ...
    const matrix_fields = matrix[header_row-1];
    console.log('MATRIX',matrix_fields)
    // Lookup dictionary
    const data_field_to_row = Object.fromEntries(
      matrix_fields.map((item, index) => {return [item, index]})
    );
    //console.log("DATA_FIELD TO ROW", data_field_to_row)
    // Preliminary scan for all matches via ordered slot_names array
    let slot_matches = new Array(this.slot_names.length).fill(false);
    let data_matches = new Array(matrix_fields.length).fill(false);
    let found_by_title = false;
    this.slot_names.forEach((slot_name, index) => {
      if (slot_name in data_field_to_row) { // JSON data matches on slot.name
        ...
        let title = this.slots[this.slot_name_to_column[slot_name]].title;
        //console.log("TITLE", slot_name, title, title in data_field_to_row)
    ...
    // Indicates uploaded data file name in modal.
    ...
    let html = `
      <tbody class="field-mapping-template">
        <tr>
          <td colspan="2">${this.template_name}</td>
        </tr>
      </tbody>`;
    ...
    Object.entries(this.sections)
      ...
      html += `
        <tbody class="field-mapping-section">
          <tr>
      ...
      Object.entries(section.children).forEach(([section_slot_row, slot]) => {
        ...
        if (slot_matches[slot_row] !== false && slot_matches[slot_row] >= 0) {
          ...
          <tr class="field-match">
            ...
            <td class="field-match">${ordering}) ${matrix_fields[data_row]}</td>
          </tr>`;
        ...
        while (data_row < data_matches.length 
          ...
          <tr>
            <td>&nbsp;</td>
            <td class="draggable-mapping-item field-mismatch">${data_row}) ${matrix_fields[data_row]}</td>
          </tr>`;
          data_row += 1;
        }
    ...
    // Load all of the above html into the table in one go.
    $('#field-mapping-table tfoot').before(html);
    ...
    $("#field-mapping-mismatched-checkbox").change(function(){
      // FUTURE: try hiding section if it doesn't have a mismatch?
