...
class DataHarmonizer {
  ...
  constructor(root, options = {}) {
    ...
    // field settings – value, visibility, and other ways to operate on the table view
  ...
  useTemplate(template_name) {
    ...
    const attributes = Object.entries(self.schema.classes)
    ...
    // TODO: unique
    this.template_unique_keys = Object.values(Object.entries(self.schema.classes)
      .filter(([cls_key,]) => cls_key !== 'dh_interface')
      .reduce((acc, [, spec]) => {
        return [
          ...acc,
        ].concat(!!spec.unique_keys ? [spec.unique_keys] : []);
      }, []));
    ...
    this.validator.useTargetClass(template_name);
  ...
  */
  _updateVisibility(exclusionFunction) {
    this.showAllColumns();
    const hiddenColumns = this.getFields().map(exclusionFunction).reduce((acc, hide, index) => {
  ...
  showColumnsByNames(names) {
    if (names.length > 0) this._updateVisibility((field) => !names.includes(field.name));
  }
  ...
  showColumnsBySectionTitle(sectionTitle, showFirstColumn = true) {
    this._updateVisibility((field, index) => 
      !(showFirstColumn && index === 0) && field.section_title !== sectionTitle
    );
  }

  /**
    * Updates row visibility based on a provided filter function.
    *
    * @param {Function} filterFunction A function that determines if a row should be hidden.
    *                                  It receives the row index and returns
    *                                  a boolean where true means the row should be hidden.
    */
  // _updateRowVisibility(filterFunction) {
  //   const hiddenRowsPlugin = this.hot.getPlugin('hiddenRows');
  //   // Un-hide all currently hidden rows
  //   const hidden = hiddenRowsPlugin.getHiddenRows();
  //   if (hidden && hidden.length > 0) {
  //     hiddenRowsPlugin.showRows(hidden);
  //   }

  //   // Calculate rows to hide using filter function
  //   const totalRows = this.hot.countRows();
  //   const rowsToHide = [...Array(totalRows).keys()]
  ...
  //   // Determine the filtering logic based on the id
  //   let filterFunction;
  //   if (id === 'show-valid-rows-dropdown-item') {
  //     filterFunction = (row) => this.invalid_cells.hasOwnProperty(row) || this.hot.isEmptyRow(row);
  //   } else if (id === 'show-invalid-rows-dropdown-item') {
  //     const invalidRowsSet = new Set(Object.keys(this.invalid_cells).map(Number));
  //     filterFunction = (row) => !invalidRowsSet.has(row) || this.hot.isEmptyRow(row);
  //   }

  //   if (filterFunction) {
  //     this._updateRowVisibility(filterFunction);
  //   }
  ...
  getFields() {
    ...
    let fields = Array.prototype.concat.apply(
      ...
      this.template
        .map(section => 
          section.children
            .filter(child => this.class_assignment !== null && child.domain_of.length > 0 ? child.domain_of.includes(this.class_assignment) : true))
    );
    // const fields = Object.entries(this.schema.classes).filter(([cls_key,]) => cls_key !== 'dh_interface').flatMap(([, cls_obj]) => {
    //   return Object.values(cls_obj.attributes);
    // })
    let new_fields = this.field_filters.length > 0 ? fields.filter(field => this.field_filters.includes(field.name)) : fields;
    console.log(this.template)
    console.log("field_filters", this.field_filters, "this.class_assignment", this.class_assignment, fields.length != new_fields.length);
    return new_fields;
  }
