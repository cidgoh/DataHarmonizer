...
*/
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Class':
            ...
            // No annotations allowed directly on slots for now.
            break;
        }
      };
  ...
  addSlotRecord(dh, tables, schema_name, class_name, slot_type, slot_key, slot_obj) {
    ...
    let slot_record = {
      ...
      range:       slot_obj.range || this.getListAsDelimited(slot_obj.any_of, 'range'), // should only be 1 value???
      ...
      comments:    this.getListAsDelimited(slot_obj.comments),
      examples:    this.getListAsDelimited(slot_obj.examples, 'value'), //extract value from list objects
    ...
    if (slot_obj.annotations) {
      const dh_annotation = this.context.dhs.Annotation;
      for (let [annotation_name, obj] of Object.entries(slot_obj.annotations)) {
        ...
        this.addRowRecord(dh_annotation, tables, {
          ...
          // looked up as a SLOT rather than as an enumeration - because code
        ...
        for (let dep_row of rows) {
          ...
          switch (class_name) {
            ...
            case 'Class':
              ...
              // FUTURE: SORT ROOT CLASS TO FRONT OF CONTAINER
              /*
              if (record.container) { 
                ...
                schema.classes.container.attributes[class_name + 's'] = {
                  "name": class_name + 's',
                  "range": class_name,
                  "multivalued": true,
            ...
            */
            ...
            case 'PermissibleValue': // LOOP?????? 'text shouldn't be overwritten.
              ...
              this.copySlots(class_name, record, schema.enums[record.enum_id].permissible_values[record.text], ['text','title','description','meaning', 'is_a','notes']);
              break;
