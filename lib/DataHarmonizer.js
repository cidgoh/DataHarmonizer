...
*/
        ...
        // TESTING May 15, 2025          // 'make_read_only', '---------',
          ...
          switch (tab_name) {
              ...
              if (record.name) {
                ...
                switch (record.slot_type) {
                  ...
                  case 'attribute':
                    ...
                    const su_class_obj = new_schema.classes[record.class_name] ??= {};
            ...
            case 'Annotation':
              ...
              switch (record.annotation_type) {
                ...
                case 'slot_usage':
                  target_obj = new_schema.classes[record.class_name] ??= {};
                  target_obj = target_obj.slot_usage[record.slot_name] ??= {};
                  break;

                case 'attribute':
                  target_obj = new_schema.classes[record.class_name] ??= {};
                  target_obj = target_obj.attributes[record.slot_name] ??= {};
                  break;
              }
            ...
            case 'Setting':
              new_schema.settings[record.name] = record.value;
              break;
  ...
  async filterByKeys(dh, class_name, key_vals) {
      ...
      if (column !== undefined) {
        ...
        */
          ...
          if (focused_class_name > '') {
            filtersPlugin.addCondition(class_column, 'eq', [focused_class_name], 'disjunction'); // , 'disjunction'
            //filtersPlugin.addCondition(class_column, 'empty',[], 'disjunction');
          }
          // With no table selected, nly show rows that DONT have a table/class mentioned
          else {
    ...
    if (class_name === 'Slot') {
      ...
      const multiColumnSorting = dh.hot.getPlugin('multiColumnSorting');
    ...
    // NEED TO ADD TEST IF RENDER HAS CAPTURED UNDERLYING DATA OR NOT.  
    // E.G compare last record.

    // DON'T RESTORE CURSOR UNTIL WE KNOW THAT IT IS POINTING TO SAME KEY AS BEFORE?
  ...
  rowHighlightColValue(dh, column_name, value) {
    ...
    for (let row = 0; row < dh.hot.countRows(); row++){
      ...
      dh.hot.setCellMeta(row, col, 'className', className); // Toggles highlighting on or off
    }
  }
