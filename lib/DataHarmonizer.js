  ...
  enableMultiSelection() {
    ...
    this.hot.addHook('afterBeginEditing', function (row, col) {
        ...
        if (meta.source && meta.meta?.datatype === 'multivalued') {
          ...
          // When the slot has a WhitespaceMinimizedString or token range alongside
          // its enum sources (any_of/one_of mixing a string type with enumerations),
          // enable Selectize's `create` option for free-text entry.
          ...
          const slot = self.slots[col];
          const allowFreeText = slot?.datatype === 'xsd:token' || slot?.datatype === 'string';
          if (allowFreeText) {
            ...
            for (const v of (selectizeCfg.items || [])) {
              const norm = slot.datatype === 'xsd:token'
                ? v.trim().replace(/\s+/g, ' ')
                : v.trim();
            ...
            // Normalises whitespace to match the slot's declared datatype.
            selectizeCfg.create = (input) => {
              const v = slot.datatype === 'xsd:token'
          ...
          // Bootstrap focuses the modal element just before firing shown.bs.modal;
          // setTimeout(0) defers our focus call until after that chain finishes.
          $('#multiselect-modal').one('shown.bs.modal', () => {
            const s = $('#multiselect-text select')[0]?.selectize;
            if (!s) return;
            s.open();
            // Native DOM .focus() rather than selectize.focus() (which uses
            // jQuery trigger('focus') — jQuery skips the native call when the
            // element is already activeElement, leaving no visible cursor).
            setTimeout(() => s.$control_input[0]?.focus(), 0);
          });
        }
    });
  }
