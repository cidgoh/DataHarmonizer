...
class DataHarmonizer {
  ...
  createHot(data = []) {
    ...
    // Apply ifabsent defaults to sibling columns when a user begins editing
    // a completely empty row.  Registered via addHook (not updateSettings) so
    // it survives subsequent updateSettings calls such as enableMultiSelection().
    this.hot.addHook('afterBeginEditing', (row, col) => {
      ...
      for (let c = 0; c < this.slots.length; c++) {
        if (c === col) continue; // leave the cell being actively edited alone
        const val = this._evaluateIfAbsent(this.slots[c]?.ifabsent);
        if (val !== null) changes.push([row, c, val]);
      }
