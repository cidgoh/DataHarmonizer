...
import { KeyValueListEditor, keyValueListValidator, keyValueListRenderer } from '@/lib/editors';
...
class DataHarmonizer {
      ...
      if ('any_of' in new_field) {
        for (let item of new_field.any_of) {
          if (
            ...
            pascalToLowerWithSpaces(item.range) in this.context.template.default.schema.enums ||
            pascalToLowerWithSpaces(item.range) in this.context.template.default.schema.types 
      ...
      for (let range of range_array) {
        ...
        if (range in types || pascalToLowerWithSpaces(range) in types) {
          ...
          if (range in enums || pascalToLowerWithSpaces(range) in enums) {
            ...
            // console.log(range_obj.permissible_values);
            // to convert between flatVocab., and their title names, if they exist.
            new_field.vocabTextToTitle = Object.values(range_obj.permissible_values).reduce((acc, item) => {
              return Object.assign(acc, {
                [item.text]: typeof item.title !== 'undefined' ? item.title : item.text
              })
            }, {});
            console.log(new_field.name, new_field.vocabTextToTitle);
            ...
            const newAcc = acc.concat([
              {
                ...
                _id: item,
                label: field.vocabTextToTitle[item],
              },
            ]);
            return newAcc;
          }, []);
  ...
  getCommentDict(field) {
    ...
    field = deepMerge(deepMerge(
      field,
      this.context.template.default.schema.slots[field.name]
