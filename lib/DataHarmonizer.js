...
<body>
  ...
  scrollTo(row, column) {
    ...
    // Scroll so the target cell has at least one row of context above it and
    // one column of context to its left (when available).  HOT's
    // scrollViewportTo snaps the anchor cell to the top-left of the viewport,
    // so anchoring at (row-1, col-1) places the target at viewport position
    // (1, 1) — one row below the header and one column from the left edge.
    this.hot.scrollViewportTo(
      Math.max(0, parseInt(row)    - 1),
      Math.max(0, parseInt(column) - 1)
    );
