...
class DataHarmonizer {
        ...
        contextMenu: [
          {
             ...
             */
              ...
              if (
                ...
                for (let [dependent_name, dependent_obj] of change_report) { 
                ...
                //self.hot.alter('remove_row', selection); // covered above.
              }
            },
          },
          {
        ...
        beforeChange: function (grid_changes, action) {
          ...
          for (const row in row_changes) {
            ...
            if (!change_message || confirm(change_prelude + change_message)) {
              if (change_message) {
                //console.log("CHANGE", change_report, change_message)
                // User has seen the warning and has confirmed ok to proceed.
                self.hot.batchRender(() => {
                  for (let [dependent_name, dependent_obj] of change_report) { 
                    ...
                    if (dependent_name != self.template_name) {
                      if (dependent_obj.rows?.length) {
                        ...
                        for (let dep_row in dependent_obj.rows) {
                          Object.entries(dependent_obj.changed_slots).forEach(([dep_slot, dep_value]) => {
  ...
  getChangeReport(class_name, changes = {}) {
    let change_message = '';
    let dependent_rows = this.context.crudGetDependentRows(class_name, changes);
    for (let [dependent_name, dependent] of dependent_rows) {
      if (class_name != dependent_name && dependent.count) {
        let key_vals = Object.entries(dependent.slots)
        .join('\n')
        .replace(',', ': ');
        change_message += `\n - ${dependent.count} ${dependent_name} records with key:\n      ${key_vals}`;
      }
    }
    return [dependent_rows, change_message];
  }
