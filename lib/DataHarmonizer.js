...
class DataHarmonizer {
      ...
      const hot_settings = {
        ...
        contextMenu: [
          ...
          },
          {
            ...
            name: 'Translations',
            hidden: function () {
              // If in schema editor, then if no translation metadata for this
              // schema, or no translatable tab content, then hide option.
              const schema = self.context.dhs.Schema; 
              if (!schema || schema.schema.name != "DH_LinkML") return true;
              const locales = schema.hot.getCellMeta(schema.current_selection[0], 0).locales;
              if (!locales) return true;
              return !(self.template_name in TRANSLATABLE);
            },
            callback: function () {
              // FUTURE: could be WYSIWYG editor
              // FUTURE: could be translation table form for all visible rows.
              const row = self.current_selection[0];
              const schema = self.context.dhs.Schema;
              // On loading, each schema has locales object stored in its first
              // row cell metadata.
              const locales = schema.hot.getCellMeta(schema.current_selection[0], 0).locales;
              ...
              for (const locale_item of Object.values(locales)) {
                for (const [locale, locale_schema] of Object.entries(locale_item)) {
                  let translate_cells = '';
                  let translate_text = '';
                  for (let column_name of text_columns) {
                    ...
                    translate_cells += `<td><textarea name="${column_name}">${value}</textarea></td>`;
                  }
                  ...
                  translate_rows += `<tr class="translation-input"><td><b>${locale}</b></td>${translate_cells}<td>${translate}</td></tr>`;
                }
              }
              ...
              $('#translate-modal-content').html(
                `<div>
                  <table>
                    <thead>
                      <tr><th class="locale">locale</th><th>${TRANSLATABLE[self.template_name][4].join('</th><th>')}</th><th>translate</th></tr>
                    </thead>
                    <tbody>
                      <tr><td>${schema.schema.in_language}</td>${default_row}<td></td></tr>
                      ${translate_rows}
                    </tbody>
                  </table>
                </div>`
    ...
    if (file_name) {
      ...
      for (let [ptr, class_name] of components.entries()) {
        ...
        for (let dep_row in rows) {
          ...
          switch (class_name) {
            case 'Schema':
              this.copySlots(class_name, record, schema, ['name','description','id','version','default_prefix']);
            ...
            case 'PermissableValues':
              ...
              this.copySlots(class_name, record, schema.enums[record.enum_id].permissible_values, ['text','title','description','meaning', 'is_a']);
              //code should get converted to key of field
              break;
  ...
  // TRY SWITCHING TO hot.getData() + new records, then hot.updateData()
  // See https://handsontable.com/docs/javascript-data-grid/api/core/#updatedata 
  // emits beforeUpdateData, Hooks#event:afterUpdateData, Hooks#event:afterChange
  loadSchemaYAML(text) {
    let schema = null;
    try {
      schema = parse(text);
     ...
     */
    if (schema.extensions?.locales?.value) {
      this.hot.setCellMeta(focus_row, 0, 'locales', schema.extensions?.locales?.value);
      // Compile list of relevant fields for each 
    }
    /*
  locale: 
    tag: locale
    value:
    - fr:
        description: Un modèle DataHarmonizer pour éditer un schéma LinkML.
*/
    ...
    // Handsontable appears to get overloaded by inserting data via setDataAtCell() after
    // loading of subsequent schemas.
    // Notes on efficiency:
    // 
