...
class DataHarmonizer {
      ...
      if (self.schema.name === 'DH_LinkML') {
        ...
        if (self.template_name === 'Slot') {
          ...
          // separate reporting controls are at work.
            ...
            let slot_report = $('#slot_report_select_type').val() === 'slot';
  ...
  translationForm() {
    ...
    for (let row = tab_dh.current_selection[0]; row <= tab_dh.current_selection[2]; row++) {
      ...
      translate_rows += `<tr class="translate"><td nowrap>${language}</td>${default_row_text}<td></td></tr>`; 
...
//      let multiColumnSorting = dh.hot.getPlugin('multiColumnSorting');
      ...
      const mode = $('#slot_report_select_type').val();
...
*/
        ...
        case 'slot_usage':
          ...
          //filtersPlugin.addCondition(dh.slot_type_column, 'neq', ['attribute']); //slot_type
          //filtersPlugin.filter();
          
          this.slotFilter(dh.hot, ['slot','slot_usage']);
          ...
          this.render();  // Otherwise cells() implements meta and class stuff that isn't reflected
          break;
      }
  ...
  slotFilter(dh, show_type) {
    ...
    for (let row = 0; row < dh.countSourceRows(); row++){
      let slot_type = dh.getSourceDataAtCell(row, this.slot_type_column);
