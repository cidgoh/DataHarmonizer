...
import { invert, deepMerge } from '@/lib/utils/objects';
...
class DataHarmonizer {
  ...
  useTemplate(template_name) {
    ...
    let attributes = Object.entries(
      this.context.template.localized.schema.classes
    )
      .filter(
        ([cls_key]) =>
    ...
    for (let name in attributes) {
      ...
      let field = Object.assign(
        {},
        this.context.template.localized.schema.slots[name],
        attributes[name]
      );
      ...
      if ('any_of' in new_field) {
        for (let item of new_field.any_of) {
          if (
            item.range in this.context.template.current.schema.enums ||
            item.range in this.context.template.current.schema.types
          ) {
      ...
      for (let range of range_array) {
        ...
        // Range is a datatype?
        if (range in this.context.template.current.schema.types) {
          const range_obj = this.context.template.current.schema.types[range];
          ...
          if (range in this.context.template.current.schema.enums) {
            const range_obj = this.context.template.current.schema.enums[range];
  ...
  addRowsToBottom(numRows) {
    ...
    plugin.clearConditions(this.context.currentSelection.col); // change newVal per new selection in the column
    plugin.addCondition(this.context.currentSelection.col, 'eq', [this.context.currentSelection.valueMatch]);
    plugin.filter();
  ...
  hideAllButFirstNEmptyRows(n) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].every((cell) => cell === null || cell === '' || typeof cell === 'undefined')) {
  ...
  getCommentDict(field) {
    ...
    field = deepMerge(
      field,
      this.context.template.current.schema.slots[field.name]
    );
