...
class DataHarmonizer {
  ...
  createHot(data = []) {
    ...
    if (this.slots) {
      const hot_settings = {
        ...
        afterScrollVertically:   () => self._updateStatusBar(),
        ...
        afterOnCellMouseDown: (event, coords) => {
          ...
          if (!self.hot.getActiveEditor()?.isOpened()) {
            self.hot._getEditorManager()?.openEditor('', null);
          }
        },
  ...
  _updateStatusBar() {
    ...
    const cell = (selRow >= 0 && selCol >= 0)
      ? this.hot.getCell(selRow, selCol)
      : null;
    ...
    if (rawMsg && cell) {
      ...
      // Position below the cell; fall back to above if too close to viewport bottom.
      const cellRect = cell.getBoundingClientRect();
      const barRect  = bar.getBoundingClientRect();
      const gap = 4;
      let top  = cellRect.bottom + gap;
      if (top + barRect.height > window.innerHeight) top = cellRect.top - barRect.height - gap;
