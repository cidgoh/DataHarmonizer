  ...
  createHot() {
    ...
    if (fields.length) {
      ...
      this.hot.addHook('afterChange', function(changes, source) {
        if (source !== 'loadData') {  // Not triggered by the initial load
  ...
  getColumns() {
    ...
    for (let field of this.getFields()) {
      ...
      if (field.flatVocabulary) {
        ...
        const selectOptionsForFlatVocab = (slot_id, flat_vocabulary) => flat_vocabulary.reduce((acc, item, index) => {
          const newAcc = acc.concat([{ value: item, _id: `${slot_id.replace(':', '_')}_${index}`, label: item }]);
          return newAcc;
        }, []);
        // NOTE: is slot_uri guaranteed to exist?
        const options = selectOptionsForFlatVocab(field.slot_uri, field.flatVocabulary);
        col.source = options;
        ...
        if (field.multivalued === true) {
          ...
          //console.log("type multivalued", field, field.sources)
        } else {
          //console.log("type autocomplete", field, field.sources)
          // TODO: replace with custom type
          // col.type = 'dh.flatVocabulary';
          
          // Original Value:
          col.type = 'key-value-list'
          
          // col.editor = 'select';
          // col.selectOptions = options;
