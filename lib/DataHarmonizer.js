...
import { readFileAsync, updateSheetRange, importJsonFile } from '@/lib/utils/files';
  ...
  async openFile(file) {
    ...
    const translateObject = (obj, lng = 'default', delimiter='_') => Object.entries(obj).reduce((acc, [key, val]) => {
      return Object.assign(acc, {
        [i18next.t(key.replace(/ /g, delimiter), { lng })]: val !== null ? i18next.t(val, { lng }) : null
      })
    }, {});
    ...
    try {
      ...
      if (file.type === 'application/json') {
        ...
        const { schema, container, in_language = 'default', location } = importJsonFile(jsonData);
        ...
        if (this.schema.id !== schema) {
          ...
          //   $('#template_name_display').text(context.template._name);
          ...
          // Object.keys(jsonData.Container).forEach(dh_class => {
          //   let dataObjects = jsonData.Container[dh_class]
          ...
          let dataObjects = container[this.class_assignment].map(obj => translateObject(obj, 'default', ' '));
          // HACK, remove ASAP
          // there's a weird problem where the keys won't translate without the underscore... try to eliminate
          dataObjects = dataObjects.map(obj => Object.fromEntries(Object.entries(obj).map(([key, val]) => {
            return [key.replace(/_/g, ' '), val];
          })));
        ...
        if (in_language) {
          ...
          translationSelect.val(i18next.language === 'en' ? 'default': i18next.language).trigger('change');
          $(document).localize();
        };
