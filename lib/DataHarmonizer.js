...
class DataHarmonizer {
      ...
      const hot_settings = {
        ...
        contextMenu: [

          {
            ...
            callback: function () {
              ...
              let warning = self.context.crudGetDependentChanges(self.template_name);
              if (!warning || confirm ('WARNING: If you proceed, this will include deletion of one\n or more dependent records:\n' + warning)) {
                // User has seen the warning and has confirmed ok to proceed.
                if (warning) {
                  // "True" parameter triggers deletions
                  self.context.crudGetDependentChanges(self.template_name, 'delete');
        ...
        beforeChange: function (grid_changes, action) {
          ...
          for (const row in row_changes) {
            ...
            if (!warning || confirm (`Your key change on ${self.template_name} row ${parseInt(row)+1} would also change existing dependent table records! Do you want to continue?  Check:\n` + warning)) {
              if (warning) {
                // "True" parameter triggers updates
                self.context.crudGetDependentChanges(self.template_name, 'update');
              }
            }
            else return false;
  ...
  getRowChanges(grid_changes) { 
    ...
    console.log("row_changes", row_change)
    return row_change;
  }
  ...
  */
  addRows(row_where, numRows, startRowIndex = false) {
    ...
    // PROBLEM: THIS IS TRIGGERING CHANGE test in subordinate tables
    // GRDIIsolate setDataAtCell triggers AMRTest complaint
    this.hot.batchRender(() => {
      for (let row = startRowIndex; row < startRowIndex + numRows; row++) {
        Object.entries(required_selections).forEach(([slot_name, value]) => {
          ...
          // "add_row" is critical so we can trap that in before_change event
