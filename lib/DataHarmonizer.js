...
import { findSlotNamesForClass } from '@/lib/AppContext';

import { readFileAsync, updateSheetRange } from '@/lib/utils/files';
import { isValidHeaderRow, rowIsEmpty, wait, stripDiv } from '@/lib/utils/general';
...
class DataHarmonizer {
  ...
  constructor(root, context, options = {}) {
    ...
    this.known_physical_key = options.known_physical_key || null;
  ...
  fullData() {
    const filterPlugin = this.hot.plugin('filters');
    filterPlugin.disablePlugin();
    const fullHotData = this.hot.getData();
  ...
  useTemplate(template_name) {
    ...
    let attributes = Object.entries(
      self.context.template.localized.schema.classes
    )
      .filter(
        ([cls_key]) =>
    ...
    for (let name in attributes) {
      ...
      let field = Object.assign(
        {},
        self.context.template.localized.schema.slots[name],
        attributes[name]
      );
  ...
  countOccupiedRows() {
    const a = this.hot.countRows();
    console.log("countRows", a)
    const b = this.hot.getData().length;
    console.log("getData", b)
    const c = this.hot.getSettings().data.length;
    console.log("getSettings().data", c)
    const d = this.hot.countSourceRows();
    console.log("countSourceRows()", d)
  ...
  addRowsToBottom(numRows) {
    ...
    // Ensure currentSelection is set and valid
    if (this.context.currentSelection) {
      const { col, valueToMatch, source } = this.context.currentSelection;

      // Check if there are shared keys for the current class assignment
      // Optional chaining operator: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Optional_chaining
      const sharedKeys = this.context.schema_tree[this.class_assignment]?.shared_keys || [];
  
      if (valueToMatch !== null && sharedKeys.length > 0) {
        sharedKeys.forEach((shared_key_spec) => {
          const { name, relation, related_concept } = shared_key_spec;
  
          // Verify that the current column matches the shared key and the selection source matches the related concept
          if (stripDiv(this.hot.getColHeader()[col]) === name && relation === 'parent' && source === related_concept) {
            this.hot.batch(() => {
              // HoT adds padding every time the table expands.
              // The starting coordinate needs to take into account this padding.
              // The first iteration is good: it adds values to the beginning of the column of filled values.
              // The second iteration is padded with rows. So it needs to be compensate for by subtract 100 (padding constant)
              // From there the counts are okay since it fills in extra rows.
              const baseCoord = Math.min(Math.max(this.hot.countRows() - 200, 1), this.hot.countRows());
              // Populate the specified column for all new rows with the selected value
              for (let i = 0; i < numRows; i++) {
                this.hot.setDataAtCell(baseCoord + i, col, valueToMatch);
              }
            });
          }
        });
  ...
  */
  ...
  toJSON() {
    ...
    // "filters affected data exports. so disable filters temporarily before export then turn them back on after unfiltered data is reported."
    const handsontableInstance = this.hot;
    const tableData = this.fullData();
