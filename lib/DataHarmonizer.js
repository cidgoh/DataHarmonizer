...
<body>
  ...
  fieldChangeRules(change, fields, triggered_changes) {
    ...
    // Test field against capitalization change. Null value menu selections
    // (e.g. "Not Applicable [GENEPIO:0001619]") carry a bracketed ontology
    // code that Title/Upper/Lower casing would corrupt, so leave them as-is.
    if (
      field.capitalize &&
      change[3] &&
      change[3].length > 0 &&
      !this.schema.enums.NullValueMenu.permissible_values[change[3]]
    ) {
