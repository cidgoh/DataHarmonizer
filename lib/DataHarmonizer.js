...
import { KeyValueListEditor, keyValueListValidator } from '@/lib/editors';
...
class DataHarmonizer {
  ...
  useTemplate(template_name) {
    ...
    let attributes = Object.entries(
      self.context.template.current.schema.classes
    )
    ...
    for (let name in attributes) {
      ...
      let field = Object.assign(
        {},
        self.context.template.current.schema.slots[name],
        attributes[name]
      );
      ...
      if (!sectionIndex.has(section_title)) {
        ...
        const section =
          self.context.template.current.schema['slots'][section_title];
        if (section) {
  ...
  async openFile(file) {
    ...
    try {
      ...
      if (file.type === 'application/json') {
        ...
        if (in_language !== translationSelect.val()) {
        ...
        if (this.schema.id !== schema) {
          ...
          this.context = await this.context
            ...
            .then(async (context) => {
              ...
              const exportFormats = await context.getExportFormats(
            ...
            .then(([context, languages, exportFormats]) => {
              ...
              // Setup the value
              translationSelect.val(in_language);
          ...
          if (container_class) {
            ...
            console.log('list data', dataObjects, i18next.language, list_data);
