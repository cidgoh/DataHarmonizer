...
/**
 * Estimate the pixel width needed to display `text` as a HOT column header.
 * Used to size checkbox-type columns to their label width so they don't
 * waste horizontal space. Text columns are intentionally left to HOT's
 * content-based autoColumnSize.
 *
 * The font string should approximate HOT's header rendering (Bootstrap 4
 * default sans-serif at ~13px).  If results look too narrow or too wide,
 * adjust CHECKBOX_COL_PADDING or the font.
 */
const CHECKBOX_COL_FONT    = '13px sans-serif';
const CHECKBOX_COL_PADDING = 45; // px of horizontal padding (sort/filter controls + cell padding)

function checkboxColWidth(text) {
  const ctx = document.createElement('canvas').getContext('2d');
  ctx.font = CHECKBOX_COL_FONT;
  return Math.ceil(ctx.measureText(text).width) + CHECKBOX_COL_PADDING;
}
...
class DataHarmonizer {
  ...
  async validate() {
    ...
    // getInvalidCells returns errors keyed by data-array index, which equals the
    // visual row index (getData skips hidden rows, so index i == visual row i).
    // Convert to physical row indices so markings remain correct after rows are
    // shown or hidden without re-running validation.
    const visual_errors = this.getInvalidCells(data);
    this.invalid_cells = {};
    for (const [vRowStr, cols] of Object.entries(visual_errors)) {
      const physRow = this.hot.toPhysicalRow(Number(vRowStr));
      if (physRow != null) this.invalid_cells[physRow] = cols;
    }
    this.hot.render();
  }
  ...
  createHot(data = []) {
    ...
    // Track user-resized column widths so they survive updateSettings({columns})
    // calls (HOT 15 signature: newSize, column, isDoubleClick).
    this._userColWidths = null; // null = no user overrides; {} = at least one set
    this.hot.addHook('afterColumnResize', (newSize, column) => {
      if (!this._userColWidths) this._userColWidths = {};
      this._userColWidths[column] = newSize;
      this._persistColWidths();
    });

    // Apply ifabsent defaults to sibling columns when the user commits the
    // first value into a completely-empty row.  Registered via addHook (not
    // updateSettings) so it survives subsequent updateSettings calls such as
    // enableMultiSelection().
    //
    // Why afterChange instead of afterBeginEditing + setDataAtCell:
    // HOT closes the active text editor when setDataAtCell is called while it
    // is open (even on sibling cells), discarding whatever the user has typed.
    // Calling it synchronously in afterBeginEditing swallows the first
    // character; deferring it with setTimeout(0) still fires while the editor
    // is open and swallows 1-2 chars depending on Playwright CDP round-trip
    // timing.  Using a one-shot afterChange listener guarantees setDataAtCell
    // runs only after Tab/Enter has closed the editor and committed the value.
    this.hot.addHook('afterBeginEditing', (row, col) => {
      if (!this.slots?.length) return;
      const physRow = this.hot.toPhysicalRow(row);
      const isRowEmpty = this.slots.every((_, c) => {
        const v = this.hot.getSourceDataAtCell(physRow, c);
        return v === null || v === undefined || v === '';
      });
      if (!isRowEmpty) return;

      // One-shot afterChange listener: fires once the first user-driven edit
      // on this row is committed, then fills remaining empty siblings.
      const applyDefaults = (changes, source) => {
        if (!changes) return;
        if (['loadData', 'updateData', 'upload', 'add_row',
             'validation_revert', 'cascade_rename'].includes(source)) return;
        if (!changes.some(([r]) => r === row)) return;
        this.hot.removeHook('afterChange', applyDefaults);
        const defaults = [];
        for (let c = 0; c < this.slots.length; c++) {
          if (c === col) continue;
          const v = this.hot.getSourceDataAtCell(physRow, c);
          if (v !== null && v !== undefined && v !== '') continue;
          const val = this._evaluateIfAbsent(this.slots[c]?.ifabsent);
          if (val !== null) defaults.push([row, c, val]);
        }
        if (defaults.length) this.hot.setDataAtCell(defaults, 'add_row');
      };
      this.hot.addHook('afterChange', applyDefaults);
    });
    ...
    if (this.slots) {
      const hot_settings = {
        ...
        // Tab and Enter always move right regardless of the "Validate by column"
        // setting.  That setting controls only Next Error direction and search
        // order — not cell navigation during normal editing.  When the user
        // arrives at an invalid cell via the Next Error button and then fixes it,
        // the afterChange / beforeKeyDown hooks trigger Next Error programmatically,
        // using the Validate-by-column direction at that point.
        ...
        // Seeing if this fixes rapid scroll triggering error "Error: Could not find identifiable element"
        ...
        contextMenu: {
          items: {
            remove_row: {
              ...
              async callback() {
                await self.removeSelectedRows();
              },
            },
            row_above: {
              name() { return i18next.t('context-menu-insert-row-above'); },
              callback () {
            ...
            row_below: {
              name() { return i18next.t('context-menu-insert-row-below'); },
              callback() {
            ...
            'hsep1': '---------',
            load_schema: {
              name() { return i18next.t('context-menu-load-schema'); },
              hidden() {
                return self.template_name != 'Schema';
              },
              callback() {
                if (self.context.schemaEditor && !self.context.schemaEditor.schemaRowFreeForLoad()) return;
                $('#schema_upload').click();
              }
            },
            save_schema: {
              name() { return i18next.t('context-menu-save-schema'); },
              hidden() {
                return self.template_name != 'Schema';
              },
            ...
            preview_schema: {
              name() { return i18next.t('context-menu-demo'); },
              hidden() {
                return self.template_name != 'Schema';
              },
            ...
            copy_to_schema: {
              name() { return i18next.t('context-menu-copy-to-schema'); },
              hidden() {
            ...
            */
            ...
            reset_filter: {
              name() { return i18next.t('context-menu-reset-filter'); },
              disabled() {
            ...
            'hsep3': '---------',
            translations: {
              name() { return i18next.t('context-menu-translations'); },
              hidden() {
                const schema = self.context.dhs.Schema; 
              ...
              disabled() {
                ...
                // Hide if no locales.
                // Prefer HOT's live selection (accurate after display-mode filter
                // changes visual indices); fall back to cached current_selection.
                const _sel = schema.hot.getSelected();
                const current_row = _sel?.length ? _sel[0][0] : schema.current_selection[0];
                if (current_row === null || current_row === undefined || current_row < 0
                    || current_row >= schema.hot.countRows())
                  return true;
        ...
        beforeChange: function (grid_changes, action) {
          ...
          if (['add_row','upload','prevalidate','multiselect','batch_updates',
               'cascade_confirm','locale_confirm'].includes(action))
          ...
          for (const row in row_changes) {
            ...
            if (self.context.schemaEditor && self.template_name === 'Schema' 
              && 'locales' in changes) {
              return self.context.schemaEditor.setLocales(changes, grid_changes, self.hot);
            };
            ...
            // Show async confirm before cascading subordinate changes.
            // Block the change now; re-apply via setDataAtCell if confirmed.
            if (change_message) {
              dhConfirm(change_prelude + change_message).then((confirmed) => {
                if (confirmed) {
                  // Apply cascade to dependent tables first.
                  ...
                  // Re-apply original changes; cascade is done so beforeChange
                  // will skip the confirm (action === 'cascade_confirm').
                  self.hot.setDataAtCell(
                    grid_changes.map(([row, col, , newValue]) => [row, col, newValue]),
                    'cascade_confirm'
                  );
                }
              });
              return false; // block the change pending user response
            }
        ...
        afterChange: function (grid_changes, action) {
          if (['upload','updateData','batch_updates'].includes(action)) {
            ...
            // Note: 'CopyPaste.paste' is intentionally NOT skipped here —
            // paste may introduce duplicate keys, so crudCalculateDependentKeys
            // must still run. Users click Validate for full error highlighting.
            return;
          }
          ...
          if (grid_changes) {
            ...
            // Re-validate rows that already have errors so cell styling stays in
            // sync without a full re-validate.  If Tab was pressed, also
            // revalidate the edited row even if it had no prior errors — that
            // lets us catch a valid cell that just became invalid.
            // Note: uniqueness / identifier duplicate errors are NOT updated
            // here; they remain stale until the next full validate() call.
            const tabWasPressed = self._tabKeyPressed;
            self._tabKeyPressed = false;
            const rowsToRevalidate = new Set();
            const prevInvalidChangedCells = [];
            let tabChangedCell = null; // first changed cell when Tab was pressed
            for (const [vRow, col] of grid_changes) {
              const physRow = self.hot.toPhysicalRow(vRow);
              if (physRow == null) continue;
              if (tabWasPressed && tabChangedCell == null) {
                tabChangedCell = { physRow, col };
              }
              if (self.invalid_cells[physRow]) {
                rowsToRevalidate.add(physRow);
                if (self.invalid_cells[physRow][col] !== undefined) {
                  prevInvalidChangedCells.push({ physRow, col });
                }
              }
            }
            // If Tab was pressed on a row not already queued, add it so we can
            // detect a valid→invalid transition on the edited cell.
            if (tabWasPressed && tabChangedCell && !rowsToRevalidate.has(tabChangedCell.physRow)) {
              rowsToRevalidate.add(tabChangedCell.physRow);
            }
            if (rowsToRevalidate.size > 0) {
              const data = self.hot.getData();
              for (const physRow of rowsToRevalidate) {
                const vRow = self.hot.toVisualRow(physRow);
                if (vRow == null) continue;
                const rowErrors = self.validator.validateRow(data, self.slot_names, vRow);
                if (Object.keys(rowErrors).length === 0) {
                  delete self.invalid_cells[physRow];
                } else {
                  self.invalid_cells[physRow] = rowErrors;
                }
              }
              self.hot.render();
              // Refresh the floating status bar for the currently focused cell,
              // in case it was one of the re-validated cells.
              self._updateStatusBar();
              // If all errors on this tab are now resolved, restore tab styling and buttons.
              ...
              const firstFixed = prevInvalidChangedCells.find(
                ({ physRow, col }) => !self.invalid_cells[physRow]?.[col]
              );
              if (firstFixed) self._cellJustFixed = firstFixed;

              // Tab navigation after editing:
              //   cell now invalid (was valid or invalid) → return to it
              //   cell was invalid, now valid, other errors remain, AND user arrived
              //     here via the Next Error button → go to Next Error
              //   all other cases → HOT's tabMoves/enterMoves stand (move right)
              if (tabWasPressed && tabChangedCell) {
                const { physRow, col } = tabChangedCell;
                const nowInvalid = self.invalid_cells[physRow]?.[col] !== undefined;
                if (nowInvalid) {
                  setTimeout(() => {
                    const vRow = self.hot.toVisualRow(physRow);
                    if (vRow != null) self.hot.selectCell(vRow, col);
                  }, 0);
                } else if (prevInvalidChangedCells.length > 0
                    && Object.keys(self.invalid_cells).length > 0
                    && self._navigatedViaNextError) {
                  // Was invalid, now valid, more errors remain, and we got here via
                  // Next Error → jump to the next error (direction per validate_by_column).
                  self._navigatedViaNextError = false; // consumed; locateNextError re-sets it
                  setTimeout(() => {
                    const vRow = self.hot.toVisualRow(physRow);
                    if (vRow != null) {
                      self.hot.selectCell(vRow, col);
                      $('#next-error-button').trigger('click');
                    }
                  }, 0);
                }
                // else: normal edit (or manually-navigated invalid cell) — HOT's
                // tabMoves / enterMoves stand (move right).
              }
            }
          }
        },
        ...
        afterSelectionEnd: (row, column, row2, column2, _selectionLayerLevel, _action) => {

          //console.log("afterSelectionEnd",self.schema, row, column, row2, column2, _selectionLayerLevel)
          ...
          */
          if (row === self.current_selection[0]
            ...
            //console.log('Double event: afterSelectionEnd', row, column, row2, column2, _selectionLayerLevel, _action)
            return false
          }
          // A genuine user-initiated selection change: clear the "arrived via Next
          // Error" flag so that Tab on this cell uses normal right-movement rather
          // than triggering the next-error jump.  _suppressClearNavFlag is set by
          // locateNextError() around its own programmatic selectCell() call so that
          // its navigation does not clear the flag it is about to set.
          if (!self._suppressClearNavFlag) {
            self._navigatedViaNextError = false;
          }
          // If the user clicked away from a just-fixed picker/dropdown cell without
          // pressing Tab, discard the flag so it doesn't linger.
          if (self._cellJustFixed) {
            const physRow = self.hot.toPhysicalRow(row);
            if (physRow !== self._cellJustFixed.physRow || column !== self._cellJustFixed.col) {
              self._cellJustFixed = null;
            }
          }
          ...
          // Skip when "Display: All records" is active — no dependent filtering
          // should happen and tabs must remain enabled.
          if (row_change && $('input[name="display-main-type"]:checked').val() !== 'all') { // primary_key slot cell value change case handled in afterChange() event above.
            self.context.crudCalculateDependentKeys(self.template_name);
          }
          ...
          // Show a floating tooltip above the focused cell when it is invalid.
          // Deferred one animation frame so that:
          //   (a) HOT has finished re-rendering the cell's DOM element (getCell()
          //       returns null when the row hasn't been painted yet, e.g. row 0
          //       after scrolling up via Next Error), and
          //   (b) any afterScrollVertically/Horizontally hide-events have already
          //       fired synchronously, so we show last and win.
          requestAnimationFrame(() => self._updateStatusBar());

          // Horizontal-scroll correction for keyboard navigation (Enter, Tab, arrows).
          // HOT natively scrolls to keep the newly selected cell visible, but this
          // can cause jarring horizontal drift when the cell is already well within
          // the left portion of the unfrozen viewport.  The rule:
          //   • Cell in the LEFT  half of the unfrozen viewport → restore the
          //     pre-keypress scrollLeft (no horizontal movement).
          //   • Cell in the RIGHT half                          → centre the cell
          //     horizontally in the unfrozen viewport.
          if (self._preNavScrollLeft != null) {
            const _savedScroll = self._preNavScrollLeft;
            self._preNavScrollLeft = null;
            const _col = column, _row = row;
            requestAnimationFrame(() => {
              const wt = self.hot.rootElement?.querySelector('.wtHolder');
              if (!wt) return;
              const frozenPane   = self.hot.rootElement?.querySelector('.ht_clone_left');
              const frozenWidth  = frozenPane?.offsetWidth ?? 0;
              const viewportW    = wt.clientWidth - frozenWidth;
              const cellEl       = self.hot.getCell(_row, _col);
              if (!cellEl) return;
              const holderRect   = wt.getBoundingClientRect();
              const cellRect     = cellEl.getBoundingClientRect();
              const cellRelLeft  = cellRect.left - holderRect.left - frozenWidth;
              if (cellRelLeft < viewportW / 2) {
                // Left half — undo any horizontal drift caused by HOT's native scroll.
                wt.scrollLeft = _savedScroll;
              } else {
                // Right half — centre the cell horizontally.
                wt.scrollLeft = Math.max(
                  0,
                  wt.scrollLeft + cellRelLeft + cellRect.width / 2 - viewportW / 2
                );
              }
            });
          }
        ...
        // Detect Tab for Tab-to-next-error navigation. Two cases:
        //  1. Editor open (text/numeric/etc): set flag, afterChange handles navigation.
        //  2. Editor already closed (picker/dropdown committed its value, cursor still
        //     on the same cell): if that cell was just fixed, refocus + Next Error.
        beforeKeyDown: (e) => {
          // Save the horizontal scroll position before any navigation key is
          // processed.  afterSelectionEnd uses this to decide whether to undo
          // HOT's native scroll (cell in left half) or centre the cell (right half).
          if (['Enter','Tab','ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)) {
            const _wt = self.hot.rootElement?.querySelector('.wtHolder');
            self._preNavScrollLeft = _wt ? _wt.scrollLeft : null;
          }

          // Ctrl+N / Cmd+N — scroll right to the first column of the next section.
          // Shift+Ctrl+N / Shift+Cmd+N — scroll left to the first column of the previous section.
          if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'n') {
            e.preventDefault();
            e.stopImmediatePropagation();
            const sel = self.hot.getSelected()?.[0];
            if (sel) {
              const currentCol = sel[1];
              // Build an ordered list of each section's start column index.
              let colPtr = 0;
              const sectionStarts = self.sections.map((section) => {
                const start = colPtr;
                colPtr += section.children.length;
                return start;
              });
              const targetCol = e.shiftKey
                // Previous: largest section start that is strictly less than currentCol.
                ? sectionStarts.filter((s) => s < currentCol).at(-1)
                // Next: smallest section start that is strictly greater than currentCol.
                : sectionStarts.find((s) => s > currentCol);
              if (targetCol !== undefined) {
                const row = Math.max(sel[0], 0);
                // Select without auto-scroll (scrollToCell=false), then
                // scroll explicitly with snapToRight=false so the target
                // column lands at the left edge of the viewport.
                self.hot.selectCell(row, targetCol, row, targetCol, false);
                self.hot.scrollViewportTo(row, targetCol, false, false);
              }
            }
            return;
          }
          ...
          if (e.key !== 'Tab') return;
          if (self.hot.getActiveEditor()?.isOpened()) {
            // Case 1: afterChange will fire with the committed value and navigate.
            self._tabKeyPressed = true;
          } else if (self._cellJustFixed && Object.keys(self.invalid_cells).length > 0
              && self._navigatedViaNextError) {
            // Case 2: picker/dropdown already closed, the fixed cell was reached via
            // Next Error → re-select it and jump to the next error.
            const sel = self.hot.getSelected()?.[0];
            if (sel) {
              const physRow = self.hot.toPhysicalRow(sel[0]);
              if (physRow === self._cellJustFixed.physRow && sel[1] === self._cellJustFixed.col) {
                const { physRow: fPhysRow, col: fCol } = self._cellJustFixed;
                self._cellJustFixed = null;
                self._navigatedViaNextError = false; // consumed; locateNextError re-sets it
                setTimeout(() => {
                  const vRow = self.hot.toVisualRow(fPhysRow);
                  if (vRow != null) {
                    self.hot.selectCell(vRow, fCol);
                    $('#next-error-button').trigger('click');
                  }
                }, 0);
              }
        ...
        // HOT 15 passes movedRows as [] in afterRowMove, but populates it
        // correctly in beforeRowMove. Capture the count here for use below.
        beforeRowMove: (movedRows) => {
          self._pendingMoveCount = movedRows.length;
        },
        ...
        afterRowMove: (movedRows, finalIndex, dropIndex, movePossible, orderChanged) => {
          ...
          // value restores the same visual position after the reload.
          ...
          setTimeout(() => {
            ...
            // viewport (selectCell's 5th arg false = no scroll-to-cell).
            const lastCol = Math.max(0, self.hot.countCols() - 1);
            self.hot.selectCell(finalIndex, 0, finalIndex + numMoved - 1, lastCol, false);
          }, 0);
        },

        // Hide the validation status bar while scrolling — repositioning it
        // on every scroll event is unreliable near the header and footer edges.
        // It reappears on the next cell selection (afterSelectionEnd).
        afterScrollHorizontally: () => { const bar = document.getElementById('validation-status-bar'); if (bar) bar.style.display = 'none'; },
        afterScrollVertically:   () => { const bar = document.getElementById('validation-status-bar'); if (bar) bar.style.display = 'none'; },

        // Single-click-to-open for key-value-list (single-select dropdown) cells.
        // First click selects the cell; a click on the already-selected cell opens
        // the editor immediately, without requiring a double-click.
        // _singleClickPreCell captures the HOT selection *before* each mousedown so
        // we can tell whether the user clicked on a cell that was already focused.
        beforeOnCellMouseDown: (event, coords) => {
          if (coords.row < 0 || coords.col < 0 || event.button !== 0) {
            self._singleClickPreCell = null;
            return;
          }
          const sel = self.hot.getSelected();
          self._singleClickPreCell = sel ? { row: sel[0][0], col: sel[0][1] } : null;
        },

        afterOnCellMouseDown: (event, coords) => {
          if (coords.row < 0 || coords.col < 0 || event.button !== 0) return;
          if (event.shiftKey || event.ctrlKey || event.metaKey) return;
          const pre = self._singleClickPreCell;
          if (!pre || pre.row !== coords.row || pre.col !== coords.col) return;
          const meta = self.hot.getCellMeta(coords.row, coords.col);
          if (meta.type !== 'key-value-list'
              && meta.meta?.datatype !== 'multivalued'
              && meta.type !== 'dh.date'
              && meta.type !== 'dh.datetime'
              && meta.type !== 'dh.time') return;
          if (!self.hot.getActiveEditor()?.isOpened()) {
            // Pass null (not '') so BaseEditor.beginEditing() falls through to
            // stringify(originalValue) and sets the TEXTAREA to the current cell
            // value.  Passing '' satisfies typeof==='string' and resets the
            // TEXTAREA to empty, causing getValue() to return '' on click-away
            // and silently clearing the cell.
            self.hot._getEditorManager()?.openEditor(null, null);
          }
        },

        // Hide the validation status bar when the cell editor opens.
        afterBeginEditing: () => {
          const bar = document.getElementById('validation-status-bar');
        ...
        afterRenderer: (TD, row, col) => {
          // invalid_cells is keyed by physical row; convert visual row before lookup
          // so markings survive show/hide operations without re-validating.
          const physRow = self.hot.toPhysicalRow(row);
          if (physRow != null && Object.prototype.hasOwnProperty.call(self.invalid_cells, physRow)) {
            if (Object.prototype.hasOwnProperty.call(self.invalid_cells[physRow], col)) {
              const msg =
                self.invalid_cells[physRow][col] === 'This field is required';
      ...
      // Restore any column widths persisted from a previous session for this
      // schema + tab combination.  The schema key is already resolvable here
      // (getSchemaRef() returns the active schema for both DH and SchemaEditor
      // initial load).
      this.restoreColWidths();
    } else {
      console.warn(
  ...
  /**
   * Evaluate a LinkML `ifabsent` expression and return the string value to
   * place in a cell, or `null` if the expression is unrecognised.
   *
   * Supported syntax (https://linkml.io/linkml/schemas/slots.html#default-values):
   *   string(text)          → the literal text
   *   int(N)                → integer as string
   *   float(N.M)            → float as string
   *   True / False          → boolean literal
   *   date("YYYY-MM-DD")    → date string
   *   datetime("ISO8601")   → datetime string
   *   EnumName(choice)      → the enum choice text (CamelCase function name)
   *
   * @param {string|undefined} expr - The ifabsent expression from the slot definition.
   * @return {string|null} The default cell value, or null if not applicable.
   */
  _evaluateIfAbsent(expr) {
    if (!expr) return null;
    ...
    m = s.match(/^datetime\("(.+)"\)$/);
    if (m) return m[1];
    // EnumName(choice) — CamelCase enum reference
    m = s.match(/^[A-Z][A-Za-z0-9]*\((.+)\)$/);
    if (m) return m[1];
    return null;
  }
  ...
  clearValidationResults() {
    ...
    $('#validation-status-bar').hide();
    this.invalid_cells = {};
    this.hot.render();
  }

  /**
   * Update (or hide) the floating validation status bar for the currently
   * focused cell.  Safe to call any time; creates the DOM element on first use.
   */
  _updateStatusBar() {
    if (!document.getElementById('validation-status-bar')) {
      const el = document.createElement('div');
      el.id = 'validation-status-bar';
      document.body.appendChild(el);
    }
    const bar = document.getElementById('validation-status-bar');

    const [selRow, selCol] = this.current_selection;
    // Pass topmost=true so HOT returns the element from the highest-stacking
    // overlay — for frozen-left columns that is .ht_clone_left (the visible
    // frozen pane), not the .ht_master element which may be scrolled off-screen.
    const cell = (selRow >= 0 && selCol >= 0)
      ? this.hot.getCell(selRow, selCol, true)
      : null;
    const physRow = cell ? this.hot.toPhysicalRow(selRow) : null;
    const rawMsg = physRow != null ? this.invalid_cells[physRow]?.[selCol] : undefined;

    if (rawMsg && cell) {
      // Deduplicate lines (any_of sub-validators may repeat the same message).
      const unique = [...new Set(rawMsg.split('\n').filter(s => s.trim()))];
      bar.textContent = unique.join('; ');
      bar.style.display = 'block';

      // Position below the cell; fall back to above if it would enter the footer
      // or viewport bottom.  The footer (#dh-layout-footer) is in normal flow
      // with no z-index, so z-index cannot keep the bar "under" it — we must
      // clamp vertically instead.
      const cellRect  = cell.getBoundingClientRect();
      const barRect   = bar.getBoundingClientRect();
      const gap       = 4;
      const footerEl  = document.getElementById('dh-layout-footer');
      const maxBottom = footerEl
        ? footerEl.getBoundingClientRect().top - gap
        : window.innerHeight - gap;

      let top = cellRect.bottom + gap;
      if (top + barRect.height > maxBottom) top = cellRect.top - barRect.height - gap;
      top = Math.max(gap, Math.min(top, maxBottom - barRect.height));

      let left = cellRect.left;
      const overflow = left + barRect.width - window.innerWidth;
  ...
  /**
   * Remove every currently selected row, prompting for confirmation when the
   * deletion would cascade to dependent records.  Shared by the right-click
   * "Remove row(s)" context-menu item and the Delete-key handler.
   */
  async removeSelectedRows() {
    ...
    // Pre-compute the dependency report for every selected row.  selectCell is
    // called so that crudGetDependentKeyVals reads the correct primary-key values.
    const rowReports = visualRows.map((vRow, i) => {
      this.hot.selectCell(vRow, 0, vRow, 0, false);
      const [change_report, change_message] = this.getChangeReport(this.template_name);
      return { vRow, rowNum: i + 1, change_report, change_message };
    });

    // When multiple rows are selected and at least one has dependent records,
    // show a summary report and ask the user to delete affected rows individually
    // so they can review each dependency before confirming.
    if (visualRows.length > 1) {
      const withDeps = rowReports.filter(r => r.change_message.length);
      if (withDeps.length) {
        const summary = withDeps
          .map(({ rowNum, change_message }) => `Row ${rowNum}:\n${change_message}`)
          .join('\n\n');
        await dhAlert(
          `${withDeps.length} of the ${visualRows.length} selected rows have dependent records` +
          ` that would also be deleted:\n\n${summary}\n\n` +
          `Please delete rows with dependents one at a time to review each dependency report.`
        );
        return;
      }
    }

    // Single row, or multiple rows with no dependents: proceed with deletion.
    const rowsToDelete = [];
    for (const { vRow, change_report, change_message } of rowReports) {
      if (!change_message.length) {
        // No cascading dependents — safe to delete without prompting.
        rowsToDelete.push(vRow);
      } else {
        // Single row with dependents — show the cascade-confirm dialog.
        if (await dhConfirm(
          `WARNING: Deleting this row will also delete dependent records and cannot be undone:\n${change_message}`,
          { dangerous: true, okLabel: 'Delete' }
        )) {
          for (const [dependent_name, dependent_obj] of Object.entries(change_report)) {
            if (dependent_obj.rows?.length) {
              this.context.crudDeleteRowsByPhysical(dependent_name, dependent_obj.rows);
            }
          }
          rowsToDelete.push(vRow);
        }
    ...
    if (this.context?.schemaEditor) {
      // Use loadData (same as crudDeleteRowsByPhysical) rather than alter() so
      // hidden rows are handled correctly and the hiddenRows plugin state is
      // preserved.  loadData does not fire afterRemoveRow, so the post-removal
      // cleanup is done here explicitly.
      const physRows = new Set(rowsToDelete.map(vRow => this.hot.toPhysicalRow(vRow)));
      const newData = this.hot.getSourceData().filter((_, idx) => !physRows.has(idx));
      this.hot.loadData(newData);
      this.hot.deselectCell();
      this.context.schemaEditor.refreshMenusForTab(this.template_name);
      this.context.crudCalculateDependentKeys(this.template_name);
      setTimeout(() => this.context.refreshTabDisplay(), 0);
    } else {
      // Standard (non-SchemaEditor) path: alter fires afterRemoveRow and allows
      // HOT to manage undo/redo.  Delete in descending order so earlier visual
      // indices are not shifted by later removals.
      for (const vRow of rowsToDelete.sort((a, b) => b - a)) {
        this.hot.alter('remove_row', vRow);
      }
    }
  }
  ...
  updateColumnSettings(columnIndex, options) {
    ...
    // Preserve user-resized column widths by injecting them into the column
    // definitions before updateSettings rebuilds the column config.
    if (this._userColWidths) {
      for (const [col, width] of Object.entries(this._userColWidths)) {
        const idx = parseInt(col);
        if (currentColumns[idx]) currentColumns[idx] = { ...currentColumns[idx], width };
      }
    }

    // updateSettings({columns}) resets HOT's hidden-columns, hidden-rows, and
    // multiColumnSorting plugin state.  Capture all three before the call so
    // they can be restored after.
    const hiddenRowsPlugin = this.hot.getPlugin('hiddenRows');
    const prevHiddenRows = hiddenRowsPlugin?.getHiddenRows?.() ?? [];
    const hiddenColsPlugin = this.hot.getPlugin('hiddenColumns');
    const prevHiddenCols = hiddenColsPlugin?.getHiddenColumns?.() ?? [];
    const sortPlugin = this.hot.getPlugin('multiColumnSorting');
    const prevSortConfig = sortPlugin?.getSortConfig?.() ?? [];
    this.hot.updateSettings({ columns: currentColumns });
    // Restore sort config if a sort was active before updateSettings wiped it.
    if (prevSortConfig.length > 0) sortPlugin.sort(prevSortConfig);
    // Restore hidden columns, merging prev state with any permanently hidden columns
    // (e.g. tree_root on the Class tab).
    const colsToRestore = [...new Set([...prevHiddenCols, ...(this._permanentlyHiddenCols ?? [])])];
    if (colsToRestore.length > 0) hiddenColsPlugin.hideColumns(colsToRestore);
    // Restore hidden rows if any were active.
    if (prevHiddenRows.length > 0) {
      hiddenRowsPlugin.hideRows(prevHiddenRows);
      this.hot.render();
    }
    // Re-apply concise view (hidden columns).
    if (this._conciseViewActive) this.context.toolbar?._applyConciseView(this);
  }

  // ── Column-width persistence ────────────────────────────────────────────────

  /** localStorage key components for this DH's column widths. */
  _colWidthsSchemaKey() {
    // In SchemaEditor mode, use the currently-loaded schema name (the one the
    // user selected in the Schema tab), not the editor's own schema name.
    // In plain DH mode, use the context's schema name.
    return (
      this.context?.schemaEditor?.getSchemaEditorSelectedSchema?.() ??
      this.context?.getSchemaRef?.()?.name ??
      'default'
    );
  }

  /** Save current _userColWidths for this tab under its schema key.
   * Column keys are stored as slot names (not integer indices) so that widths
   * survive column reordering. */
  _persistColWidths() {
    if (!this._userColWidths) return;
    const schemaKey = this._colWidthsSchemaKey();
    const dh_settings = readBrowserDHSettings();
    dh_settings.schema ??= {};
    dh_settings.schema[schemaKey] ??= {};
    dh_settings.schema[schemaKey].colWidths ??= {};
    // Convert in-memory integer-keyed map to slot-name-keyed map for storage.
    const namedWidths = {};
    for (const [col, width] of Object.entries(this._userColWidths)) {
      const slotName = this.slot_names?.[parseInt(col)] ?? col;
      namedWidths[slotName] = width;
    }
    dh_settings.schema[schemaKey].colWidths[this.template_name] = namedWidths;
    saveBrowserDHSettings(dh_settings);
  }

  /**
   * Clear stale in-memory widths and load any persisted widths for `schemaKey`
   * (defaults to the currently active schema).  Called by SchemaEditor after a
   * schema is loaded so each tab picks up its remembered column widths.
   */
  restoreColWidths(schemaKey) {
    this._userColWidths = null;
    const key = schemaKey ?? this._colWidthsSchemaKey();
    const saved = readBrowserDHSettings().schema?.[key]?.colWidths?.[this.template_name];
    if (!saved || !Object.keys(saved).length) return;

    this._userColWidths = {};
    for (const [key, width] of Object.entries(saved)) {
      // Keys may be slot names (current format) or column indices (legacy format).
      let col;
      if (this.slot_name_to_column && key in this.slot_name_to_column) {
        col = this.slot_name_to_column[key];
      } else {
        col = parseInt(key);
        if (isNaN(col)) continue;
      }
      this._userColWidths[col] = parseInt(width);
    }

    // Inject saved widths into column definitions and apply via updateSettings.
    // At schema-load time hidden-rows/cols and sort are in their initial state,
    // so the simpler direct call is safe here.
    const currentColumns = [...(this.hot.getSettings().columns || [])];
    for (const [col, width] of Object.entries(this._userColWidths)) {
      const idx = parseInt(col);
      if (currentColumns[idx]) currentColumns[idx] = { ...currentColumns[idx], width };
  ...
  /**
   * Clear all user-set column widths for this tab: wipes the in-memory cache,
   * removes the persisted entry from localStorage, and strips explicit `width`
   * from column definitions so HOT reverts to auto-sizing.
   */
  resetColWidths() {
    this._userColWidths = null;
    ...
    const schemaKey = this._colWidthsSchemaKey();
    const dh_settings = readBrowserDHSettings();
    if (dh_settings.schema?.[schemaKey]?.colWidths?.[this.template_name]) {
      delete dh_settings.schema[schemaKey].colWidths[this.template_name];
      saveBrowserDHSettings(dh_settings);
    }

    // Strip explicit width from column defs and call updateSettings with the
    // same hidden-rows/cols/sort save-restore dance used by updateColumnSettings.
    const currentColumns = (this.hot.getSettings().columns || []).map(col => {
      const rest = { ...col };
      delete rest.width;
      return rest;
    });
  ...
  addRows(row_where, numRows, startRowIndex = false) {
    ...
    // Register a one-shot afterCreateRow listener that applies ifabsent defaults
    // to every newly created row.  afterCreateRow fires with the actual (index, amount)
    // where HOT placed the rows, so this works for both insert_row_above and
    // insert_row_below regardless of position.
    const applyIfAbsentDefaults = (index, amount) => {
      this.hot.removeHook('afterCreateRow', applyIfAbsentDefaults);
      if (!this.slots?.length) return;
      const changes = [];
      for (let row = index; row < index + amount; row++) {
        for (let col = 0; col < this.slots.length; col++) {
          const val = this._evaluateIfAbsent(this.slots[col]?.ifabsent);
          if (val !== null) changes.push([row, col, val]);
        }
      }
      if (changes.length) this.hot.setDataAtCell(changes, 'add_row');
    };
    this.hot.addHook('afterCreateRow', applyIfAbsentDefaults);
  ...
  showAllColumns() {
    ...
    // Re-hide columns that must remain permanently hidden (e.g. tree_root on Class tab).
    if (this._permanentlyHiddenCols?.size > 0) {
      hiddenColsPlugin.hideColumns([...this._permanentlyHiddenCols]);
    }
    this.render();
  }
  ...
  showColumnsBySectionTitles(sectionTitles, showFirstColumn = true) {
    const titleSet = new Set(sectionTitles);
    if (this.slots.some((field) => titleSet.has(field.section_title))) {
      this._updateVisibility(
        (field, index) =>
          !(showFirstColumn && index === 0) &&
          !titleSet.has(field.section_title)
      );
    }
  }
  ...
  changeRowVisibility(id) {
    ...
    // getHiddenRows() returns visual indices; showRows() expects visual — correct.
    const hiddenRowsPlugin = this.hot.getPlugin('hiddenRows');
    const hidden = hiddenRowsPlugin.getHiddenRows();
    hiddenRowsPlugin.showRows(hidden);

    // Work entirely in physical row space: invalid_cells keys are physical
    // (see validation code) and isEmptyRow() takes physical indices.
    const physRows = [...Array(this.hot.countSourceRows()).keys()];
    const emptyPhysRows = physRows.filter((row) => this.hot.isEmptyRow(row));
    let hiddenPhysRows = [];

    if (id === 'show-valid-rows-dropdown-item') {
      hiddenPhysRows = Object.keys(this.invalid_cells).map(Number);
      hiddenPhysRows = [...hiddenPhysRows, ...emptyPhysRows];
    } else if (id === 'show-invalid-rows-dropdown-item') {
      const invalidPhysSet = new Set(
        Object.keys(this.invalid_cells).map(Number)
      );
      hiddenPhysRows = physRows.filter((row) => !invalidPhysSet.has(row));
      hiddenPhysRows = [...hiddenPhysRows, ...emptyPhysRows];
    }

    // hideRows() expects visual indices; convert from physical before passing.
    if (hiddenPhysRows.length > 0) {
      hiddenRowsPlugin.hideRows(
        hiddenPhysRows.map((physRow) => this.hot.toVisualRow(physRow))
      );
    }
    this.render();
  }
  ...
  renderReference(mystyle = null) {
    ...
    let row_html = '';
    const applyTitle = this.context?.schemaEditor
      ? (s) => this.context.applyDhTermsToTitle(s)
      : (s) => s;
...
<head>
  ...
  <title>${i18next.t('reference_guide_title')} "${
      applyTitle(schema_template.title || schema_template.name)
    }" template</title>
...
<body>
  <div>
    ...
    <hr/>
    <p>${(this.context?.applyDhTerms
        ? this.context.applyDhTerms(schema_template.description || '')
        : schema_template.description) || ''}</p>
   ...
   */
  scrollTo(row, column, animated = false) {
    const hiddenColsPlugin = this.hot.getPlugin('hiddenColumns');
    const hidden = hiddenColsPlugin.getHiddenColumns();
    ...
    // If the target row is hidden (e.g. by a slot-type or FK row filter on
    // the Field tab), unhide it first so selectCell/scrollViewportTo can
    // actually navigate to it vertically.
    // getHiddenRows() and showRows() both use visual row indices in HOT 15.
    const hiddenRowsPlugin = this.hot.getPlugin('hiddenRows');
    if (hiddenRowsPlugin) {
      const hiddenRows = hiddenRowsPlugin.getHiddenRows();
      if (hiddenRows.includes(row)) {
        hiddenRowsPlugin.showRows([row]);
        this.hot.render();
      }
    }

    // scrollToCell=false: suppress HOT's built-in auto-scroll so the
    // scrollViewportTo call below has full control over the final position.
    // (HOT's auto-scroll always snaps to the top/start edge, which conflicts
    // with the 'bottom'/'end' snap used for backward navigation.)
    this.hot.selectCell(
      parseInt(row),
      parseInt(column),
      parseInt(row),
    ...
    // Only scroll when the target cell is outside (or on the boundary of)
    // the current fully-visible viewport.  If it is comfortably interior,
    // selecting it is sufficient and the user's scroll position is left
    // undisturbed.
    //
    // HOT 15 scrollViewportTo() accepts verticalSnap ('top'|'bottom') and
    // horizontalSnap ('start'|'end') so we can place the target one position
    // away from the leading or trailing edge without knowing the viewport size:
    //   • Forward (at/past last visible): snap target-1 to 'top'/'start'
    //     → target lands at position 1 from the top/left edge.
    //   • Backward (before first visible): snap target+1 to 'bottom'/'end'
    //     → target lands at position 1 from the bottom/right edge.
    // Only the out-of-range axis is included in the scroll call so the
    // in-range axis scroll position is left undisturbed.
    const firstVisRow = this.hot.getFirstFullyVisibleRow();
    const lastVisRow  = this.hot.getLastFullyVisibleRow();
    const firstVisCol = this.hot.getFirstFullyVisibleColumn();
    const lastVisCol  = this.hot.getLastFullyVisibleColumn();

    const rowBefore = row    < firstVisRow;
    const rowAfter  = row   >= lastVisRow;
    const colBefore = column < firstVisCol;
    const colAfter  = column >= lastVisCol;

    if (rowBefore || rowAfter || colBefore || colAfter) {
      const wtHolder = this.hot.rootElement?.querySelector('.wtHolder');

      if (animated && wtHolder) {
        // Snapshot where we are now.
        const origTop  = wtHolder.scrollTop;
        const origLeft = wtHolder.scrollLeft;

        // Let HOT jump to the target synchronously — no browser paint occurs
        // yet because we're still in the same JS task.
        if (rowAfter)  this.hot.scrollViewportTo({ row: Math.max(0, row - 1), verticalSnap:   'top'    });
        if (rowBefore) this.hot.scrollViewportTo({ row: row + 1,              verticalSnap:   'bottom' });
        if (colAfter)  this.hot.scrollViewportTo({ col: Math.max(0, column - 1), horizontalSnap: 'start'  });
        if (colBefore) this.hot.scrollViewportTo({ col: column + 1,              horizontalSnap: 'end'    });

        // scrollViewportTo() marks HOT's internal scroll state dirty but does
        // not always flush the result into wtHolder.scrollLeft/scrollTop
        // synchronously.  render() forces that flush.  We are still inside the
        // same JS task so no browser paint occurs before we restore the
        // original position two lines below.
        this.hot.render();

        // Read the resolved pixel target for both axes.
        const targetTop  = wtHolder.scrollTop;
        const targetLeft = wtHolder.scrollLeft;

        // Restore original position (still in the same JS task — still no paint).
        wtHolder.scrollTop  = origTop;
        wtHolder.scrollLeft = origLeft;

        // In the next animation frame the browser paints the original position,
        // then a single native scrollTo animates both axes together so neither
        // can cancel the other.
        requestAnimationFrame(() => {
          wtHolder.scrollTo({ top: targetTop, left: targetLeft, behavior: 'smooth' });
        });
      } else {
        if (rowAfter)  this.hot.scrollViewportTo({ row: Math.max(0, row - 1), verticalSnap:   'top'    });
        if (rowBefore) this.hot.scrollViewportTo({ row: row + 1,              verticalSnap:   'bottom' });
  ...
  getNestedHeaders() {
    ...
    const rows = [[], []];
    // applyTitle translates individual column (slot) titles via exact DH_TERMS
    // lookup (e.g. "Class ID" → "Table ID" in common-language mode).
    // Section group headers (parent.title) are plain class/section names and
    // are rendered as-is; they do not contain {{...}} placeholders.
    const applyTitle = this.context?.schemaEditor
      ? (s) => this.context.applyDhTermsToTitle(s)
      : (s) => s;
    for (const parent of sections) {
      ...
      for (const child of parent.children) {
        ...
        rows[1].push(
          `<div class="secondary-header-text${required}${recommended}">${applyTitle(child.title)}</div>`
        );
      }
    }
    return rows;
  ...
  getColumns() {
    ...
    for (let slot of this.slots) {
      ...
      } else if (slot.multivalued === true) {
        // Free-text multivalued: no enum source.  Mirror the enum-backed
        // multivalued setup so HOT treats the column consistently.
        // col.source is left null; enableMultiSelection() detects via
        // meta.meta.datatype rather than requiring a truthy source.
        col.type = 'autocomplete';
        col.editor = 'text'; // suppress the autocomplete dropdown
        col.renderer = 'multiKeyValueListRenderer';
        col.meta = {datatype: 'multivalued'};
      }
      ...
      switch (slot.datatype) {
        case 'xsd:boolean':
          ...
          if (!slot.sources) {
            ...
            col.width = checkboxColWidth(slot.title || slot.name || '');
          }
          break;
        case 'xsd:date':
          col.type = 'dh.date';
  ...
  updateSources(slot) {
    let options = [];
    const multiSource = slot.sources.length > 1;

    slot.sources.forEach((source) => {
      // Prepend a non-selectable group header for slots with multiple sources.
      if (multiSource) {
        const groupTitle = this.schema.enums?.[source]?.title || source;
        options.push({
          depth:         0,
          label:         groupTitle,
          value:         '\x00' + source,
      ...
      if (!(source in slot.permissible_values)) {
        dhAlert(`Schema Programming Error: Slot range references enumeration ${source} but this was not found in enumeration dictionary`);
      }
      // This case catches empty menus, which merits an error in end-user DH but is ok in Schema Editor.
        ...
        dhAlert(`Schema Programming Error: Slot range enumeration's ${source} has no selections!`);
      }
      else {
        const sortedItems = this._buildSortedSourceItems(
          slot.permissible_values[source] || {}
        );
        sortedItems.forEach(item => { item.optgroup = source; });
        options.push(...sortedItems);
      }
    });
  ...
  /**
   * Build a flat, alphabetically sorted list of options from a permissible_values
   * map.  Root items (no is_a) are sorted alphabetically; children of each parent
   * are also sorted alphabetically, preserving the is_a depth hierarchy.
   *
   * @param {Object} pvMap  permissible_values object for one enum source
   * @returns {Array<{depth,label,value,_id}>}
   */
  _buildSortedSourceItems(pvMap) {
    const items = Object.values(pvMap);

    // Group by parent code (null = root)
    const byParent = new Map();
    byParent.set(null, []);
    for (const pv of items) {
      const parent = pv.is_a ?? null;
      if (!byParent.has(parent)) byParent.set(parent, []);
      byParent.get(parent).push(pv);
    }

    // Sort each sibling group alphabetically by display label
    for (const siblings of byParent.values()) {
      siblings.sort((a, b) =>
        (a.title || a.text).localeCompare(b.title || b.text)
      );
    }

    // DFS traversal to produce a flat array that preserves depth
    const result = [];
    const traverse = (parentCode, depth) => {
      for (const pv of byParent.get(parentCode) || []) {
        result.push({
          depth,
          label: pv.title || pv.text,
          value: pv.text,
          _id:   pv.text,
        });
        traverse(pv.text, depth + 1);
      }
    };
    traverse(null, 0);
    return result;
  ...
  enableMultiSelection() {
    ...
    this.hot.addHook('afterBeginEditing', function (row, col) {
        ...
        if (meta.meta?.datatype === 'multivalued') {
          ...
          // Separate group-header items (inserted by updateSources for multi-source
          // slots) from the actual selectable options.  Selectize's native optgroups
          // API is used so headers are rendered but are never selectable.
          // meta.source is null for pure free-text multivalued columns.
          const allSource       = meta.source || [];
          const optgroups       = allSource
            .filter(o => o.isGroupHeader)
            .map(o => ({ value: o.optgroup, label: o.label }));
          // Always create a new array so the allowFreeText pre-population loop
          // cannot mutate meta.source and leak cell-specific free-text values
          // into subsequent cells in the same column.
          const selectableOpts  = optgroups.length > 0
            ? allSource.filter(o => !o.isGroupHeader)
            : allSource.slice();
          const hasGroups       = optgroups.length > 0;

          const selectizeCfg = {
            plugins:      ['remove_button'], // adds × to each chip for click-to-remove
            maxItems:     null,
            ...
            hideSelected: false,
            // Use plain substring matching so that regex special characters
            // like ( ) [ ] in the typed query are handled safely by sifter's
            // own escape_regex().  The default respect_word_boundaries:true
            // prepends \b to every token regex; \b\( never matches because (
            // is a non-word char preceded by a space (also non-word), so no
            // word boundary exists there.  Disabling word-boundary mode makes
            // the search a pure case-insensitive substring filter.
            respect_word_boundaries: false,
            // Do not open the dropdown automatically when the input receives
            // programmatic focus (shown.bs.modal step 1).  The dropdown is
            // opened explicitly in a setTimeout after all HOT/Bootstrap frame
            // cycles have settled (step 2), which avoids focus-stealing races.
            openOnFocus: false,
            // Keep the cursor in the input after a selection so the user can
            // continue typing or make additional picks without an extra click.
            onChange: function() {
              this.focus();
            },
            render: {
              // Label shown for a selected item in the input control.
              item: function (data, escape) {
                return '<div>' + escape(data.label) + '</div>';
              },
              // Each option row in the dropdown list.
              option: (data, escape) => {
            ...
            // Fade the dropdown in rather than revealing it instantly.
            // An instantaneous appearance can place menu items under the mouse
            // cursor, causing a mouseover/mousedown event to fire and
            // inadvertently add an item to the selection.  The brief fade
            // keeps the menu unclickable long enough that no such event fires.
            //
            // NOTE: the previous implementation used $dropdown.hide().slideDown(200).
            // The synchronous .hide() (display:none) caused browsers to silently
            // move keyboard focus from the Selectize <input> to document.body,
            // leaving the text cursor absent when the cell had pre-existing values.
            // Using opacity + pointer-events avoids any display change and therefore
            // does not disturb focus on the sibling .selectize-input element.
          ...
          if (hasGroups) {
            selectizeCfg.optgroups     = optgroups;
            selectizeCfg.optgroupField = 'optgroup';
            selectizeCfg.render.optgroup_header = (data, escape) =>
              `<div class="dh-selectize-group-header">${escape(data.label)}</div>`;
          }

          // Enable Selectize's `create` option for free-text entry when:
          //   (a) the column has no enum vocabulary (pure free-text multivalued), or
          //   (b) the slot mixes a string/token range with enum sources.
          // slot.datatype is 'xsd:token' for WhitespaceMinimizedString/token and
          // 'string' for the plain string type; other datatypes (integer, date, …)
          // must not enable free-text creation in a picklist context.
          // col is a visual column index; self.slots is indexed by physical column.
          // Convert so that hidden/reordered columns don't produce the wrong slot.
          const physCol = dh.toPhysicalColumn ? dh.toPhysicalColumn(col) : col;
          const slot = self.slots[physCol];
          const isFreeTextOnly = !meta.source?.length; // pure free-text: null or empty source
          const allowFreeText = isFreeTextOnly
            || slot?.datatype === 'xsd:token' || slot?.datatype === 'string';
          if (allowFreeText) {
            // Treat undefined datatype as token (trim + collapse spaces) for
            // pure free-text columns whose type may not resolve in meta-schema.
            const isToken = slot?.datatype === 'xsd:token' || !slot?.datatype;
            // Pre-populate options with any existing free-text values that are
            // not part of the enum vocabulary so Selectize displays them as
            // already-selected items when the modal opens.
            const existingOptValues = new Set(selectableOpts.map(o => o.value));
            const normalizedItems = [];
            for (const v of (selectizeCfg.items || [])) {
              const norm = isToken
                ? v.trim().replace(/\s+/g, ' ')
                : v.trim();
              if (norm && !existingOptValues.has(norm)) {
                selectableOpts.push({ value: norm, label: norm, _id: norm, depth: 0 });
                existingOptValues.add(norm);
              }
              if (norm) normalizedItems.push(norm);
            }
            // For pure free-text columns replace items with normalised values so
            // they match the option value fields added above.  Enum-backed items
            // stay as-is since their keys already match the vocabulary.
            if (isFreeTextOnly) selectizeCfg.items = normalizedItems;
            // create: function — called when the user types a value and presses Enter.
            selectizeCfg.create = (input) => {
              const v = isToken
                ? input.trim().replace(/\s+/g, ' ')
                : input.trim();
              return { value: v, label: v, _id: v, depth: 0 };
            };
          }

          $('#multiselect-text .multiselect')
            .selectize(selectizeCfg) // must be rendered when html is visible
          ...
          $('#multiselect-modal button[data-dismiss]').off().on('click', function () {
            // Scope to the clicked button's own modal so that in multi-DH contexts
            // (schema editor with 12 DH instances, each appending #multiselect-modal
            // to body) we read from the active modal rather than the first DOM match.
            const $thisModal = $(this).closest('[id="multiselect-modal"]');
            let newValCsv = formatMultivaluedValue(
              $thisModal.find('.multiselect').val()
            );
            // Only write back if the value actually changed.  An empty Selectize
            // produces '' while the HOT cell may hold null; treating both as ''
            // prevents spurious beforeChange events (which trigger the SchemaEditor
            // propagation dialog) when OK is pressed without making any edits.
            const currentVal = dh.getDataAtCell(row, col) ?? '';
            if (newValCsv !== currentVal) {
              dh.setDataAtCell(row, col, newValCsv, 'multiselect_change');
            }
          })

          // Cancel the HOT text editor BEFORE showing the modal.
          // modal('show') moves focus to the modal, which causes HOT's textarea
          // to fire focusout → finishEditing(false) → commits '' to the cell.
          // On the second visit to a non-empty cell (e.g. locales='sq'), this
          // triggers beforeChange(old='sq', new='') → setLocales → DELETE dialog.
          // finishEditing(true) cancels the editor without committing, preventing
          // the spurious beforeChange.
          if (dh.getActiveEditor()?.isOpened()) {
            dh.getActiveEditor().finishEditing(true); // cancel, no beforeChange
          }
          $('#multiselect-modal').modal('show');
          // finishEditing above causes HOT to schedule listen()/rootElement.focus()
          // via rAF/setTimeout.  Calling unlisten() here prevents HOT from fighting
          // Bootstrap's focus trap: without this, Bootstrap intercepts HOT's focus
          // attempt and redirects focus to the .modal element, leaving the Selectize
          // input without a text cursor.
          dh.unlisten();
          // Defer focus and dropdown-open until the modal animation is complete.
          $('#multiselect-modal').one('shown.bs.modal', () => {
            const input = document.querySelector('#multiselect-text .selectize-input input');
            const sz    = document.querySelector('#multiselect-text .multiselect')?.selectize;
            if (!input) return;
            // Step 1: give the input keyboard focus so the text cursor appears.
            // openOnFocus:false in the Selectize config means onFocus will NOT
            // call open(), so no onDropdownOpen animation fires on this focus()
            // call and focus remains stable.
            input.focus();
            // Step 2: open the dropdown after a brief delay, once all HOT/Bootstrap
            // rAF cycles triggered by finishEditing above have fully settled.
            // sz.open() internally calls self.focus() (a no-op if already focused)
            // then opens the dropdown cleanly — no focus contention at this point.
            if (sz) {
              setTimeout(() => {
                if (document.querySelector('[id="multiselect-modal"].show') && !sz.isOpen) {
                  sz.open();
                }
              }, 100);
            }
          });
          // Re-activate HOT keyboard handling when the modal is dismissed for
          // any reason (OK, Escape, or clicking outside).
          $('#multiselect-modal').one('hidden.bs.modal', () => {
            dh.listen();
          });
        }
    });
  ...
  getComment(field) {
    ...
    if (this.columnHelpEntries.includes('column')) {
      ...
      )}</strong>: ${applyTitle(slot_dict.title)}<br><span class="coding_name">(${slot_dict.name})</span></p>`;
    }
    ...
    if (slot_dict.description && this.columnHelpEntries.includes('description')) {
      ret += `<p><strong data-i18n="help-sidebar__description">${i18next.t(
        'help-sidebar__description'
      )}</strong>: ${slot_dict.description}</p>`;
    }
  ...
  getCommentDict(field) {
    ...
    let guide = {
      ...
      description: urlToClickableAnchor(
        field.description && this.context?.applyDhTerms
          ? this.context.applyDhTerms(field.description)
          : field.description
      ) || '',
      guidance: '',
      examples: '',
    ...
    if (field.comments && field.comments.length > 0) {
      const applyTerms = this.context?.applyDhTerms?.bind(this.context) ?? (s => s);
      const comments = Array.isArray(field.comments) ? field.comments : [field.comments];
      guidance = guidance.concat(comments.map(c => applyTerms(c)));
    }
    if (field.pattern) {
      guidance.push(
    ...
    const { minimum_value, maximum_value } = getNumericConstraints(field, this.schema);
    const hasMinValue = minimum_value != null;
    const hasMaxValue = maximum_value != null;
    ...
    return new Promise((resolve) => {
      ...
      for (let row = 0; row < data.length; row++) {
        ...
        for (let col = 0; col < data[row].length; col++) {
          ...
          // Coerce array-valued cells to semicolon-delimited strings for
          // non-multivalued fields (e.g. data saved when a field was multivalued
          // but the current schema defines it as a single-value field).
          if (Array.isArray(cellVal) && field.multivalued !== true) {
            cellVal = cellVal
              .filter((v) => v != null && v !== '')
              .map((v) => String(v).trim())
