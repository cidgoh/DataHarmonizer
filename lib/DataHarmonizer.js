...
*/
  ...
  translationForm() {
    ...
    if (!locales) {
      alert("You need to select a schema which has locales in order to see the translation form.")
      return false;
    }
      ...
      for (let [item_name, value] of Object.entries(schema[schema_part] || {})) {
        ...
        switch (class_name) {
          ...
          case 'Class':
            ...
            if (value.attributes) { // class.attributes holds slot_definitions 
      ...
      for (let [ptr, tab_name] of components.entries()) {
        ...
        for (let dep_row of rows) {
          ...
          switch (tab_name) {
            ...
            case 'UniqueKey':
              let class_record = new_schema.classes[record.class_id] ??= {};
            ...
            case 'Slot':
              if (record.name) {
                ...
                case 'slot_usage':
                case 'attribute':
                  // Error case if no record.class_id .. Should it be class_name s ??? 
                  const su_class_obj = new_schema.classes[record.class_id] ??= {};
                  console.log(su_class_obj, record.class_id)
            ...
            case 'Enum':
              target_obj = new_schema.enums[record.name] ??= {};
              this.copySlots(tab_name, record, target_obj, ['name','title','enum_uri','description']);
              break;
