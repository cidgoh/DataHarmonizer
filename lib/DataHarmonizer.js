    ...
    if (fields.length) {
      ...
      const hot_settings = {
        ...
        beforeChange: function (changes) {
          console.log('beforeChange triggered_changes', changes);
          if (!changes) {
            return;
          }
  ...
  getColumns() {
    ...
    for (let field of fields) {
      ...
      if (field.sources) {
        ...
        const titleOverText = enm => typeof enm !== 'undefined' && typeof enm.title !== 'undefined' ? enm.title : enm.text;
        const options = field.sources.flatMap(source => {
          return Object.values(field.permissible_values[source])
            .reduce((acc, item) => {
              console.log('options', item, titleOverText(item))
              acc.push({
      ...
      afterBeginEditing: function (row, col) {
        ...
        if (fields[col].flatVocabulary && fields[col].multivalued === true) {
          ...
          if (value !== formattedValue) {
            console.log('value is not formatted value');
          ...
          if (fields[col].sources) {
            fields[col].sources.forEach(source => {
              Object.values(fields[col].permissible_values[source]).forEach(permissible_value => {
                const field = permissible_value.text;
                const field_trim = field.trim();
                let selected = selections.includes(field_trim)
                  ? 'selected="selected"'
                  : '';
                content += `<option value="${field_trim}" ${selected}'>${titleOverText(permissible_value)}</option>`;
              })
            });
            ...
            .selectize({
              ...
              render: {
                item: function (data, escape) {
                  console.log('selectize:render:item', data);
                  return `<div>` + escape(data.text) + `</div>`;
                },
                option: (data, escape) => {
                  console.log('selectize:render:option', data);
