  ...
  getColumns() {
    ...
    for (let slot of this.slots) {
      ...
      if (slot.sources) {
        ...
        /* For multivalued slots that have menu enumerations specified in
         * slot.sources, we need a multiselect popup control.  This is
         * provided by the enableMultiSelection() call that adds a DH hook for
         * afterBeginEditing() event on a cell. The popup appears if cel's 
         * col.meta multivalued flag exists AND it has a picklist vocabulary 
         * in col.sources .
         */
        if (slot.multivalued === true) {
          col.editor = 'text';
          // This just provides the "label" overlay on cell's key content.
          col.renderer = multiKeyValueListRenderer(slot);
          col.meta = {datatype: 'multivalued'} // metadata
        } else {
          //{type: 'autocomplete', strict: true, filter: false}
          col.type = 'autocomplete'; //'key-value-list';
          col.source = col.source.map(function(val) {return val.label});
          // FUTURE, when HandsonTable 16.1.0+ is implemented, us this 
          // col.source; and remove trimDropdown conditional
          //col.source = col.source.map(function(val) {return {key: val._id, value: val.label}});
          if (
