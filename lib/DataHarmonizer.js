  ...
  createHot(data = []) {
    ...
    if (fields.length) {
      ...
      const hot_settings = {
        ...
        contextMenu: [

          {
            ...
            callback: function () {
              ...
              let warning = self.context.crudGetDependentDeletes(self.template_name);
              if (!warning || confirm (warning)) {
                if (warning)
                  // "True" parameter triggers deletions
                  self.context.crudGetDependentDeletes(self.template_name, true);
          ...
          },
          {
            ...
            callback: function (action, selection) {
              ...
              /* Primary key in read-only fields can be changed?
               * Simpler to just do this on master tables.
               */
              alert("Coming soon...")
         ...
         *
         * One assumption: no functioning key slot can have null values.
         */
        beforeChange: function (grid_changes, action) {
          console.log("beforeChange")
          if (!grid_changes) { // Is this ever the case?
            return;
          }          
           ...
           */
          let row_changes = self.getChanges(grid_changes); 
          ...
          for (const row in row_changes) {
             ...
             * If a class has a unique_key with >=1 slot then multiple changes 
             * to key must be tested for duplicates as well as multiple
            ...
            for (let key_name in self.template.unique_keys) {
              // If change doesn't pertain to any index slots then keyVals 
              // remains empty.
              let [found, keyVals, change_log]
                = self.context.crudHasUniqueKey(self, row, key_name, changes);
              if (found) {
                ...
                // own this is ok.  Below "dependent_name" test will prompt
                // user if any implications to dependent records.
            ...
            for (let dependent_name in self.context.crudGetChildren[self.template_name]) {
              // dependent_class = ...
              let [found, keyVals, change_log]
               = self.context.crudCheckDependency(self, row, dependent_name, changes);
              if (found && found == '') {
                // found status: 
                // 
                // Here we have complete unique_keys entry to test for duplication
                // There is a matching row so fail this action.
                if (duplicate_row && duplicate_row != row) {
                  alert(`Your key change on row ${parseInt(row)+1} would affect existing dependent records in the following tables! Do you want to continue with this change?  Check:\n\n ${change_log}`);
                  return false;
                };
              }
            }
                         */
            // IF PRIMARY KEY CHANGE ACCEPTED ON MAIN TABLE AFTER PROMPT, THEN
            // IMPLEMENT DATA CHANGE IN afterChange call...??? OR HERE TOO?
            // ISSUE - 
        ...
        afterChange: function (grid_changes, action) { // e.g. action=edit.
  ...
  */
  addRows(row_where, numRows, startRowIndex = false) {
    ...
    if (startRowIndex === false)
      startRowIndex = this.hot.countSourceRows(); // or .countSourceRows();???
    ...
    let [required_selections, errors] = this.context.crudGetForeignKeyValues(parents);
...
*/
  ...
  */
  ...
  showColumnsBySectionTitle(sectionTitle, showFirstColumn = true) {
    if (
      this.slots.some((field) => field.section_title === sectionTitle)
    ) {
...
<body>
  ...
  enableMultiSelection() {
    ...
    this.hot.updateSettings({
      afterBeginEditing: function (row, col) {
        ...
        if (self.slots[col].flatVocabulary && self.slots[col].multivalued === true) {
          ...
          let content = '';
          if (this.slots[col].sources) {
            this.slots[col].sources.forEach((source) => {
              Object.values(this.slots[col].permissible_values[source]).forEach(
                (permissible_value) => {
  ...
  fillColumn(colname, value) {
    ...
    if (colname in fieldYCoordinates) {
      let changes = [];
      for (let row = 0; row < this.hot.countRows(); row++) { // .countSourceRows()??
