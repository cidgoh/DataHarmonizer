  ...
  createHot(data = []) {
    ...
    if (this.slots) {
      const hot_settings = {
        ...
        // Tab direction mirrors the "Validate by column" setting:
        //   checked (default) → Tab moves down a row (column-first navigation)
        //   unchecked         → Tab moves right across columns (row-first, HOT default)
        tabMoves: $('#validate_by_column').is(':checked') // normally tab moves right a cell
          ? { row: 1, col: 0 }
          : { row: 0, col: 1 },
        //enterMoves: {row: 0, col:1}, // normally Enter moves down a cell.
        hiddenColumns: {
        ...
        afterChange: function (grid_changes, action) {
          ...
          if (grid_changes) {
            ...
            if (rowsToRevalidate.size > 0) {
              ...
              //   cell now valid, was invalid, errors remain → go to Next Error
              //   cell was valid and still valid → HOT's Tab movement stands
              if (tabWasPressed && tabChangedCell) {
                ...
                } else if (prevInvalidChangedCells.length > 0 && Object.keys(self.invalid_cells).length > 0) {
                  // Was invalid, now valid, other errors remain.
                  setTimeout(() => {
                    const vRow = self.hot.toVisualRow(physRow);
                    if (vRow != null) {
                ...
                // else: was valid, still valid (or no errors) — HOT's Tab movement stands.
              }
            }
          }
        },
        ...
        beforeKeyDown: (e) => {
          ...
          if (self.hot.getActiveEditor()?.isOpened()) {
            ...
            // Case 2: picker/dropdown already closed, check cursor is still on fixed cell.
            const sel = self.hot.getSelected()?.[0];
            if (sel) {
