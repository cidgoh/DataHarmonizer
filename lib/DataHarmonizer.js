...
*/
  ...
  translationForm() {
    ...
    for (let row = tab_dh.current_selection[0]; row <= tab_dh.current_selection[2]; row++) {
      ...
      const language = locale_map[schema.schema.in_language].title; //  (${schema.schema.in_language})
      translate_rows += `<tr class="translate"><td>${language}</td>${default_row_text}<td></td></tr>`;   
...
//strOptions.fold.defaultStringType = 'QUOTE_DOUBLE'
    ...
    for (let [ptr, attr_name] of Object.entries(attribute_list)) {
      if (attr_name in record) { //No need to create/save empty values
        if (target instanceof Map) {// Required for Map, preserves order.
          ...
          // console.log(`Error: Saving ${class_name}, saved template is missing ${attr_name} attribute.`)
        }
      }
    }
  };
   ...
   */
  async filterByKeys(dh, class_name, key_vals = null) {
    ...
    */
    ...
    // In this case we override key values based on 
    if (class_name === 'Slot') {
      ...
      let mode = $('#slot_report_select_type').val();
      switch (mode) {
        ...
        case 'attribute':
          ...
          filtersPlugin.addCondition(2, 'eq', [mode]); //slot type
          filtersPlugin.filter();
        ...
        case 'slot_usage':
          // THIS WILL show for all tables UNLESS we filter.
          // One issue - possible naming collision with schema.slot name will cause schema slot attributes to be imported into slot.attributes slot.
      ...
      /* We need to provide  slot_type = slot / slot_usage / attribute
      a) Just a list of schema fields 
         (sort by slot_group if any, then rank if any, then alphabetically)
      b) Table fields with their schema source fields 
         (filter out table-only fields and slot fields not in that table)
      c) Table-only fields. type = 
         - Filter out all fields slot fields
      /*
