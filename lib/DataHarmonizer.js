      ...
      const hot_settings = {
        ...
        beforeChange: function (grid_changes, action) {
          ...
          for (const row in row_change) {
             ...
             * Here a shift from a complete foreign key to:
             * - deleted key -> prompt user for deletion of dependents.
             * - changed key (part) -> prompt user to use "right click: Change primary key option"
             * - incomplete key (deleted key part) -> block, prmpt as above.
             *
             * Shift from incomplete foreign key to complete one:
             ...
             */
            let class_names = self.context.crudGetChildren(self.template_name);
            for (let dependent_name in class_names) {
              let [found, keyVals, change_log]
              ...
              if (found && found == '') {
                ...
                // There is a matching row so fail this action.
                if (duplicate_row && duplicate_row != row) {
                  alert(`Your change on row ${parseInt(row)+1} would impact records in the following tables! Check:\n\n ${change_log}`);
                  return false;
                };
              }
            }
        ...
        afterChange: function (changes, action) { // e.g. action=edit.
          // FUTURE: trigger only if change to primary key fields.
          console.log("changes",changes, action)
