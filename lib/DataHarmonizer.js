...
class DataHarmonizer {
  ...
  async openFile(file) {
    ...
    // Helper Functions
    const translateObject = (obj, lng = 'default', delimiter = '_') =>
    ...
    try {
      ...
      if (file.type === 'application/json') {
        let jsonData;
        try {
          jsonData = JSON.parse(contentBuffer);
        } catch (error) {
        ...
        if (this.schema.id !== schema) {
          ...
          const container = Object.fromEntries(
            Object.entries(jsonData.Container)
            .map(([container_key, entity_data]) => {
              console.log(this.context.template.current.schema.classes.Container.attributes[container_key].range)
              return [this.context.template.current.schema.classes.Container.attributes[container_key].range, entity_data]
            })
          );
          
          Object.keys(jsonData.Container)
            .map(container_key => this.context.template.current.schema.classes.Container.attributes[container_key].range)
            .forEach(dh_class => {
              let dataObjects = container[dh_class]
                .map(obj => translateObject(obj, 'default', '_'))
                .map(obj => Object.fromEntries(Object.entries(obj).map(([key, val]) => {
                  return [key.replace(/_/g, ' '), val];
                })));
              this.context.dhs[dh_class].loadDataObjects(dataObjects);
              this.context.dhs[dh_class].hot.render();
            });
          ...
          let dataObjects = container[this.class_assignment].map((obj) =>
            translateObject(obj, 'default', ' ')
          );
          // HACK, remove ASAP
          // only workable in TEST case?!?!? what's going on?
          // there's a weird problem where the keys won't translate without the underscore... try to eliminate
        ...
        if (in_language) {
          ...
          i18next.changeLanguage(jsonData.in_language);
          translationSelect
