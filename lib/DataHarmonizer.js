...
class DataHarmonizer {
      ...
      const hot_settings = {
        ...
        contextMenu: [
          ...
          },
          {
            ...
            callback: function () {self.loadSchema()}
          },
          {
            key: 'save_schema',
  ...
  loadSchema () {
    ...
    input.setAttribute("accept", "application/yaml");
    input.onchange = function (event) {
      ...
      reader.addEventListener('load', (event) => {
        if (reader.error) {
          ...
          self.processSchemaYAML(event.target.result);
        }
      });
  ...
  processSchemaYAML(text) {
    ...
    dh_uk.hot.suspendRender();
    dh_su.hot.suspendRender();
    dh_pv.hot.suspendRender();  
      
    // First process top-level Schema's slots
    ...
    let focus_row = parseInt(this.hot.getSelected()[0][0]);
    if (rows.length > 0) {
      ...
      if (rows[0] == focus_row) {
        ...
        if (!dh_schema.hot.getDataAtCell(focus_row, 0)) {
          load_name = schema.name + '_1';
        }
      ...
      // Overwrite whatever is on current row?  
      // Future: Safety check to not load in occupied row.
    ...
    dh_su.hot.resumeRender(); //SlotUsage
    dh_pv.hot.resumeRender(); //PermissibleValue

    console.table("LOADING SCHEMA", schema);
