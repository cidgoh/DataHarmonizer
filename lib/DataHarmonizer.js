...
class DataHarmonizer {
   ...
   *   - dateBehavior: `DATE_OBJECT` (default) | `JSON_SCHEMA_FORMAT` | `INPUT_FORMAT`
   *     controls how values from xsd:date, xsd:dateTime, and xsd:time columns
   *     are returned. By default they are returned as native `Date` objects. If this
   *     option is `JSON_SCHEMA_FORMAT` they are returned as a strings compatible with
   *     JSON Schema date and time format validation
   *     (see: https://json-schema.org/understanding-json-schema/reference/string.html#dates-and-times)
   *     If this option is they are returned as strings in the original format they were
   *     input into the DataHarmonizer interface
  ...
  getDataObjects(options = {}) {
    const { dateBehavior, parseFailureBehavior } = {
