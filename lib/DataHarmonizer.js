...
import { wait, isValidHeaderRow, rowIsEmpty } from '@/lib/utils/general';
...
class DataHarmonizer {
  ...
  createHot() {
      ...
      const hot_copy_paste_settings = {
        ...
        afterCut: function (changes, coords) {
          ...
          if (TD_source.getAttribute('data-i18n').includes('multiselect')) {
            ...
            TD_source.removeAttribute('data-i18n-options');
          }
        },
    ...
    //       const clipboardCacheCopy = JSON.parse(
    //         JSON.stringify(self.clipboardCache)
    //       );
    ...
    //         console.log(TD_target, self.hot.getCellMeta(endRow, endCol));
    ...
    //             'data-i18n',
    //             ';[append]multiselect.arrow'
    //           );
    //           TD_target.setAttribute(
    //             'data-i18n-options',
    //             JSON.stringify({
    //               value: clipboardCacheCopy
    ...
    //           //console.log("TD", TD, localizedText)
    ...
    this.hot.updateSettings({
      afterBeginEditing: function (row, col) {
        ...
        if (fields[col].flatVocabulary && fields[col].multivalued === true) {
          ...
          if (
            this.getCell(row, col).getAttribute('data-i18n-options') == null
          ) {
            this.getCell(row, col).setAttribute(
              'data-i18n-options',
          ...
          $('#field-description-text .multiselect')
            .selectize({
              ...
              render: {
                item: function (data, escape) {
                  ...
                  return (
                    `<div data-i18n='${data.text}'>` + escape(label) + `</div>`
                  );
                },
                option: (data, escape) => {
                  const label = i18next.t(data.text);
                  ...
                  }" data-i18n='${data.text}'>${escape(label)}</div>`;
                },
              },
            ...
            .on('change', function () {
              ...
              let newValCsv = formatMultivaluedValue($('#field-description-text .multiselect').val());
