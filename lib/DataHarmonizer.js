...
class DataHarmonizer {
  ...
  *
  * @param {Function} filterFunction A function that determines if a column should be shown.
  *                                  It receives the field object and index and returns
  *                                  a boolean where true means the column should be hidden.
  */
  _updateVisibility(filterFunction) {
    this.showAllColumns();
    const hiddenColumns = this.getFields().map(filterFunction).reduce((acc, hide, index) => {
