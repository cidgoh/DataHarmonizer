...
export default class AppContext {
  ...
  */
  ...
  crudFilterDependentRows(class_name, dependent_report) {
    ...
    const class_tab_href = $('#tab-data-harmonizer-grid-' + domId);
    // this class's foreign keys if any, are satisfied.
    if (dependent_report.fkey_status > 0) {
      $(class_tab_href).removeClass('disabled').parent().removeClass('disabled');
      ...
      // Here an error was triggered by incomplete foreign key slot.
      $(class_tab_href).addClass('disabled').parent().addClass('disabled');
    }
  }
  ...
  crudGetDependentRows(start_name, skippable=false, changes = {}) {
    ...
    let [fkey_vals, pkey_vals, changed_key_vals, fkey_status] = 
    ...
    this.dependent_rows.set(start_name, dependent_rows_obj);
  ...
  crudGetDependentKeyVals(class_name, changes) { 
    ...
    if (class_dh) {
      ...
      // focused table.
      // Merges unique_key_slot & target_slot list for lookup of values (i.e. 
      // no overwrites).
      ...
      for (let slot_name of search_slots) {
        // Foreign key slot value already inherited. 
        if (!(slot_name in dependent_slots)) {

          pkey_vals[slot_name] = this.crudGetDHCellValue(class_dh, slot_name, class_row);
    ...
    return [fkey_vals, pkey_vals, changed_key_vals, fkey_status];
  }
