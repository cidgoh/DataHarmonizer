      ...
      return {
        beforeRemoveRow: function(index, amount, physicalRows, source) {
          for (let i = 0; i < amount; i++) {
            ...
            // Custom logic to decide if the row should be removed
            // if (rowData !== rowData) {
            //   return false; // Prevent row removal if condition is met
            // }
          }
        },
  ...
  const updateDataHarmonizer = (data_harmonizer, [row, col, oldVal, newVal]) =>
    {
      ...
      // const shouldCreate = (oldVal, newVal) =>
      //   isEmptyUnitVal(oldVal) && !isEmptyUnitVal(newVal);
      // const shouldDelete = (oldVal, newVal) =>
      //   !isEmptyUnitVal(oldVal) && isEmptyUnitVal(newVal);
      // const shouldModify = (oldVal, newVal) =>
      //   !(isEmptyUnitVal(oldVal) || isEmptyUnitVal(newVal)) && oldVal !== newVal;
      ...
      hot.batch(() => {
        ...
        hot.getData().every((childRow, rowIndex) => {
          ...
          // TODO: what if a single row already exists?
