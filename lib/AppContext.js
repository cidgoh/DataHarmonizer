  ...
  */
  ...
  crudFilterDependentRows(class_name, dependent_report) {
    ...
    if (!dependent_report.incomplete) {
      ...
      //console.log("rendering",class_name, dependent_report, "show:", oldHiddenRows, "hide:",rowsToHide)
  ...
  crudGetDependentRows(start_name, skippable=false, changes = {}) {
    ...
    family.forEach((class_name) => {
      ...
      let [key_vals, changed_key_vals, incomplete] = this.crudGetDependentKeyVals(class_name, changes);
       ...
       */
      if (!isEmpty(key_vals)) {
        let rows = this.crudFindAllRowsByKeyVals(class_name, key_vals);
        if (rows.length) {
  ...
  crudGetDependentKeyVals(class_name, changes) { 
    ...
    let incomplete = false; // = true if any unique key slot is missing
    ...
    if (class_dh) {
      ...
      // Issue: a change in foreign keys - creates/drops a value.
      // 
      // Verify order of dependents, that isolate is coming before amrtest.
      ...
      for (let [slot_name, slot_link] of Object.entries(dependent_slots)) {
        ...
        let foreign_slots = dependent_f_rows.slots;
        let foreign_value = foreign_slots[slot_link.foreign_slot];
        key_vals[slot_name] = foreign_value; 

        // Now inherit changed_slots values
        let changed_keys = dependent_f_rows?.changed_slots;
        if (changed_keys && slot_link.foreign_slot in changed_keys) {
          changed_key_vals[slot_name] = changed_keys[slot_link.foreign_slot];
        }

        // Left-to-right processing of dependent_rows map should have foreign
        // keys filled in. If they aren't, then they are incomplete.
        if (changed_key_vals[slot_name] === null) {
          incomplete = true;
        }
        else if (key_vals[slot_name] === null) {
          incomplete = true;
        }

      }

      // Here we have a field/slot under user control, so get value from 
      // focused table.
      // Merges unique_key_slot & target_slot list for lookup of values (i.e. no overwrites).
      // Remaining s1) Slots to be filled are class_name's unique keys, or are targets 
      //    of other table's foreign keys.
      let unique_key_slots = this.relations[class_name].unique_key_slots;
      let search_slots = Object.keys(Object.assign(
        unique_key_slots, 
        this.relations[class_name].target_slots
      ));
      for (let slot_name of search_slots) {
        // Slot may have already been processed
        if (!(slot_name in dependent_slots)) {

          key_vals[slot_name] = this.crudGetDHCellValue(class_dh, slot_name, class_row);
          if (changes && slot_name in changes) {
            changed_key_vals[slot_name] = changes[slot_name].value;
          }

          // We have a unique_key slot that needs a value to be complete
          //if (slot_name in unique_key_slots) {
          //}
      
        } 
      }

      //console.log("crudGetDependentKeyVals", class_name, slot_name, this.dependent_rows, foreign_class_keys, foreign_value, incomplete)
