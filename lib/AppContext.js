...
const matchTitle = looseMatchInObject(['title']);
  ...
  const createContextMenuHandler = (shared_key_spec) => {
    return (rowData, source) => {
      schema_tree_node.children.forEach((cls_key) => {
        ...
        const col = getColumnIndexByFieldName(data_harmonizers[cls_key], shared_key_spec.name);
...
export class AppContext {
  ...
  propagateFilter(
    ...
    visitSchemaTree(
      ...
      (schema_tree_node) => {
        const hot = data_harmonizers[schema_tree_node.name].hot;
        const columnIndex = getColumnIndexByFieldName(data_harmonizers[schema_tree_node.name], shared_key_name);
