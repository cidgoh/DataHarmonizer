...
export default class AppContext {
  ...
  */
  ...
  async setupDataHarmonizers({
    ...
    console.log("GETRELATIONS",this.relations)
    this.schema = schema;
    ...
    /*
    for (const dh in this.dhs)
        this.relations[template_name].dependents = this.crudGetDependents(dh.template_name)
    */
    return this;
  ...
  */ 
  ...
  crudGetRelations(schema) {
    ...
    // 2nd pass, since all dependents have been created, AND this must produce
    // a carefully organized MAP {[class_name]: dh...} which functions can 
    // process in left-to-right order to account for primary/foreign key 
    // relations.
    for (const class_name in schema.classes) {
      relations[class_name].dependents = this.crudGetDependents(class_name);
    }
    return relations;
  }
