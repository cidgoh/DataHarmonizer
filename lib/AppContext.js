...
import { wait, tap, isEmptyUnitVal, stripDiv } from '@/lib/utils/general';
import { invert, removeNumericKeys } from '@/lib/utils/objects';
import { dhTabNav, dhRoot } from '@/web';

const looseMatchInObject = (keys) => (obj) => (matchKey) => {
  const returnIf = (obj) => key => key in obj ? [obj[key]] : [];
  return keys.flatMap(returnIf(obj)).includes(matchKey);
}
const looseMatchInField = looseMatchInObject(['name', 'alias', 'title']);

function getColumnIndexByHeaderName(data_harmonizer, headerName) {
  ...
  for (let i = 0; i < fields.length; i++) {
    if (looseMatchInField(fields[i])(headerName)) {
      return i;
    };
  }
...
function makeColumnReadOnly(data_harmonizer, shared_key_name) {
  ...
  const columnIndex = getColumnIndexByHeaderName(data_harmonizer, shared_key_name);
  ...
  const createContextMenuHandler = (shared_key_spec) => {
    return (rowData, source) => {
      schema_tree_node.children.forEach((cls_key) => {
        ...
        const col = getColumnIndexByHeaderName(data_harmonizers[cls_key], shared_key_spec.name);
...
export class AppContext {
  ...
  propagateFilter(
    ...
    visitSchemaTree(
      ...
      (schema_tree_node) => {
        const hot = data_harmonizers[schema_tree_node.name].hot;
        const columnIndex = getColumnIndexByHeaderName(data_harmonizers[schema_tree_node.name], shared_key_name);

        if (columnIndex === -1) {
          console.error('propagateFilter: Column name not found', shared_key_name, schema_tree);
          return;
        }
  ...
  async setupDataHarmonizers({
    ...
    const schema_tree = {
      ...
      "GRDI_Sample": { shared_key: ["sample_collector_sample_ID"], children: ["AMR_Test"] }
