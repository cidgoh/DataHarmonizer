...
export default class AppContext {
  ...
  async tabChange(class_name) {
    ...
    if (this.getCurrentDataHarmonizer() !== class_name) {
      // ── Save slot-type filter for the tab we are leaving ─────────────────
      // The checkboxes still reflect the Slot tab's state at this point, so
      // read them before switching the current DH pointer.
      const leavingDh = this.getCurrentDataHarmonizer();
      if (leavingDh?.template_name === 'Slot') {
        const slotTypeVals = $('#slot_type_optgroup input[type="checkbox"]:checked')
          .map(function () { return this.value; }).get();
        leavingDh._slotTypeFilter =
          slotTypeVals.length > 0 && slotTypeVals.length < 3
            ? new Set(slotTypeVals)
            : new Set();
      }
      ...
      // ── Show/hide the Fields-by-type section in the Display dropdown ──────
      // Only meaningful on the Field / Slot tab; hidden on all other tabs.
      // On the Slot tab, restore checkbox states from the stored filter.
      const isSlotTab = class_name === 'Slot';
      const $slotTypeGroup = $('#slot_type_optgroup');
      if (isSlotTab) {
        $slotTypeGroup.show();
        // Restore which slot types were checked last time this tab was active.
        const filter = dh._slotTypeFilter; // Set or undefined
        $slotTypeGroup.find('input[type="checkbox"]').each(function () {
          // No stored filter (size 0 or undefined) means show all → all checked.
          $(this).prop('checked', !filter || filter.size === 0 || filter.has(this.value));
        });
      } else {
        // Reset checkboxes to "all checked" then hide, ready for next Slot visit.
        $slotTypeGroup.find('input[type="checkbox"]').prop('checked', true);
        $slotTypeGroup.hide();
      }
  ...
  refreshTabDisplay() {
    ...
    // Display menu: read the checked radio for main type, and checkboxes for
    // the optional Fields-by-type filter (Slot tab only).
    let report_type = $('input[name="display-main-type"]:checked').val() ?? '';
    const slotTypeVals = $('#slot_type_optgroup input[type="checkbox"]:checked')
      .map(function () { return this.value; }).get();

    // Keep the Slot tab's row filter in sync whenever refreshTabDisplay runs
    // while that tab is active (catches both menu changes and tab-switch triggers).
    if (class_name === 'Slot' && dh._slotTypeFilter !== undefined) {
      dh._slotTypeFilter =
        slotTypeVals.length > 0 && slotTypeVals.length < 3
          ? new Set(slotTypeVals)
          : new Set(); // empty = no type filter = show all rows
    }
    ...
    switch (report_type) {
      case 'all': {
        ...
        for (const tab_dh of Object.values(this.dhs)) {
          ...
          // Re-apply any slot-type row filter so it survives a show-all reset.
          this.applySlotTypeFilter(tab_dh);
        }
      ...
      */
      ...
      case 'schema': {
        ...
        if (class_name === 'Slot' && schemaOnlyVals.schema_id
            && (!dh._slotTypeFilter || dh._slotTypeFilter.size === 0
                || dh._slotTypeFilter.has('slot'))) {
          this._showMatchingBaseSlots(dh, schemaOnlyVals);
        }
        this.applySlotTypeFilter(dh);
        break;
      }
      ...
      default: {
        ...
        // Special: on the Slot tab, when "Schema field" is checked, also
        // un-hide base-slot ('slot') rows that share the selected schema_id
        // and a slot.name with any slot_usage/attribute row that passed the
        // key filter.  This lets users compare class-level overrides alongside
        // the schema-level slot definitions they inherit from.
        if (class_name === 'Slot' && fkey_vals.schema_id
            && (!dh._slotTypeFilter || dh._slotTypeFilter.size === 0 || dh._slotTypeFilter.has('slot'))) {
          this._showMatchingBaseSlots(dh, fkey_vals);
        }
        // Compose slot-type row filter on top of the key filter.
        this.applySlotTypeFilter(dh);
      }
    }
    ...
    */
    ...
    if (this.schemaEditor !== undefined && dh.template_name === 'Slot') {
      ...
      sortConfig = orderBy === 'label' ? dh.slotSortByLabel
                 : orderBy === 'rank'  ? dh.slotSortByTableRank
                 :                       dh.slotSortByStructure;
    }
    columnSorting.sort(sortConfig); // possibly null

    // Re-apply slot-type filter AFTER sort: sort() may trigger an internal
    // render that shifts visual↔physical row mapping, so we ensure hidden rows
    // are set correctly in the final state before the user-visible render.
    this.applySlotTypeFilter(dh);
  ...
  /**
   * If dh carries an active slot_type row filter (_slotTypeFilter), additionally
   * hide every row whose slot_type column value is not in that Set.
   * Composes with whatever hiddenRows state is already in place (key filter,
   * validation filter) because hideRows() only adds to the hidden set.
   * A missing or size-0 set means "no type filter — show all" (no-op).
   * Callers are responsible for calling dh.render() afterwards.
   */
  applySlotTypeFilter(dh) {
    if (!dh._slotTypeFilter || dh._slotTypeFilter.size === 0) return;
    ...
    for (let physRow = 0; physRow < dh.hot.countSourceRows(); physRow++) {
      ...
      if (slot_type && !dh._slotTypeFilter.has(slot_type)) {
        toHide.push(physRow);
      }
    }
    if (toHide.length > 0) {
      // HOT 15: hideRows takes VISUAL row indices; convert from physical.
      hiddenRowsPlugin.hideRows(toHide.map(r => dh.hot.toVisualRow(r)));
    }
  }
   ...
   * survived the key filter.
   *
   * This lets users see the schema-level slot definition alongside the
   * class-level overrides (slot_usage rows) that reference it, without
   * losing the class-scoped filter.
   *
   * Only called when "Schema field" is checked in the Display dropdown
   * (i.e. dh._slotTypeFilter is empty or contains 'slot').
   *
  ...
  */
  ...
  setDHTabStatus(class_name) {
    ...
    // Exception: in Schema Editor the Slot (Field) tab allows schema-level slot
    // rows that have no class_id, so enable it as soon as schema_id is known
