 ...
 */
function setupSharedColumn(data_harmonizer, shared_key_name, change_callback, contex_menu_callback) {

  const hot = data_harmonizer.hot;

  // Get the index of the column based on the shared_key_name
  const colHeadersHTML = hot.getColHeader();
  const colHeadersText = colHeadersHTML.map(stripDiv);
  const columnIndex = colHeadersText.indexOf(shared_key_name);
      ...
      return {
        beforeRemoveRow: function(index, amount, physicalRows, source) {
          for (let i = 0; i < amount; i++) {
            ...
            console.log('Data of the row about to be removed:', rowData, index, amount, physicalRows, source);
          }
        },
        afterRemoveRow: function(index, amount, physicalRows, source) {
          for (let i = 0; i < amount; i++) {
            ...
            console.log('Data of the removed row:', rowData, index, amount, physicalRows, source);
...
function makeSharedKeyHandler(
  ...
  const updateDataHarmonizer = (data_harmonizer, [row, col, oldVal, newVal]) =>
    {
      const hot = data_harmonizer.hot;
    ...
    return (rowData, source) => {
      schema_tree_node.children.forEach((cls_key) => {
        ...
        hot.batch(() => {
          const CONTINUE_SIGNAL = true;
          hot.getData().every((childRow, rowIndex) => {
            console.log('childRow', childRow, source, childRow[col], colVal, childRow[col] === colVal)
        ...
        // plugin.clearConditions(col); // change newVal per new selection in the column
        // plugin.addCondition(col, 'eq', [newVal]);
        plugin.filter();
        data_harmonizers[cls_key].showAllRows();
        data_harmonizers[cls_key].hideAllButEmpty();
        hot.render();
      });
    }
  };
...
export class AppContext {
  ...
  propagateFilter(data_harmonizers, shared_key_name, schema_tree, child_name, valueToMatch = null) {
    console.log('propagateFilter');
    visitSchemaTree(
      schema_tree,
      (schema_tree_node) => {
        console.log('visitSchemaTree:', schema_tree_node, shared_key_name, schema_tree, child_name, valueToMatch, data_harmonizers[schema_tree_node.name].hot);
        ...
        if (valueToMatch !== null) plugin.addCondition(columnIndex, 'eq', [valueToMatch]);
        plugin.filter();
      ...
      dh.hot.addHook('afterSelection', (row, col) => {
        ...
        if (!(valueToMatch === null || typeof valueToMatch === 'undefined')) {
          ...
          schema_tree[dh.class_assignment].children.forEach((child_name) => {
            ...
            this.propagateFilter(data_harmonizers, shared_key_name, schema_tree, child_name, valueToMatch);
      ...
      dh.hot.addHook('afterDeselect', () => {
        ...
        schema_tree[dh.class_assignment].children.forEach((child_name) => {
          const shared_key_name = schema_tree[
            ...
            .name;
            this.propagateFilter(data_harmonizers, shared_key_name, schema_tree, child_name);
        });
      });
    });
      ...
      Object.entries(schema_tree)
        ...
        .forEach((obj, index) => {
          if (obj.length > 0) {
            ...
            const is_child = spec.shared_keys.some(shared_key_spec => shared_key_spec.relation === 'parent');
