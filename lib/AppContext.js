...
export default class AppContext {
  ...
  async tabChange(class_name) {
    ...
    if (this.getCurrentDataHarmonizer() !== class_name) {
      ...
      if (dh.schema.name === 'DH_LinkML')
        $('#slot_report_control').toggle(class_name === 'Slot');
      ...
      .forEach((obj, index) => {
        if (obj.length > 0) {
          ...
          dhTab.addEventListener('click', (event) => { 
            ...
            // A timing thing causes HandsonTable not to render header row(s) and left sidebar column(s) on tab change unless we do a timed delay on tabChange() call.
