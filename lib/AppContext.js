  ...
  const updateDataHarmonizer = (data_harmonizer, [, col, oldVal, newVal]) => {
    ...
    hot.batch(() => {
      ...
      data.every((childRow, rowIndex) => {
        ...
        const shouldModifyChild = oldChildVal === oldVal && oldChildVal !== newVal && !isEmptyUnitVal(oldVal);
        ...
        console.log('updateDataHarmonizer', oldChildVal, isEmptyUnitVal(oldChildVal))
        ...
        hot.batch(() => {
          ...
          data.every((childRow, rowIndex) => {
            if (source === 'ContextMenu.removeRow') {
              if (childRow[col] === colVal) {  // TODO: revise to shared key name?
                clearRow(hot, rowIndex);
              }
            }
            return CONTINUE_SIGNAL;
    ...
    Object.values(data_harmonizers).forEach((dh) => {
      dh.hot.addHook('afterSelection', (row, col) => {
        ...
        if (!(isEmptyUnitVal(valueToMatch))) {
          console.log('afterSelection', 'first condition')
          // get value at cell
          ...
          schema_tree[dh.class_assignment].children.forEach((child_name) => {
            console.log('afterSelection', 'second condition', data_harmonizers[child_name])
