...
export default class AppContext {
  ...
  constructor(appConfig = new AppConfig(getTemplatePathInScope())) {
    ...
    $(document).on('dhTabChange', (event, class_name) => {
      console.info(
        'Tab change:',
      ...
      // Trigger Toolbar to refresh sections
        ...
        switch (enum_name) {
          ...
          case 'SchemaSlotGroupMenu':
            ...
            if (class_name) {
              ...
              const slot_usage_dh = this.dhs['SlotUsage'];
              const slot_group_ptr = slot_usage_dh.slot_name_to_column['slot_group'];
              const slot_rows = this.crudFindAllRowsByKeyVals('SlotUsage', {
                schema_id: schema_name, 
                class_id: class_name
              })

              for (let row of slot_rows) {
                const slot_group_text = slot_usage_dh.hot.getDataAtCell(row, slot_group_ptr);
              ...
              console.log("DYNAMIC", class_name, permissible_values, slot_usage_dh.slots[slot_group_ptr])
            }
        ...
        if (obj.length > 0) {
          ...
          dhTab.addEventListener('click', (event) => {
            ...
            if (event.srcElement.classList.contains("disabled"))
              return false;
            $(document).trigger('dhTabChange', class_name);
          });
  ...
  */
  ...
  crudGetDependentRows(start_name, skippable=false, changes = {}) {
    ...
    this.relations[start_name].dependents.keys().forEach((class_name) => {
      ...
      Object.entries(this.relations[class_name].dependent_slots).forEach(([slot_name, key_mapping]) => {
