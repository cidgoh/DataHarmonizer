...
function setupSharedColumn(
  ...
  if (columnIndex === -1) {
    ...
    hot.addHook('afterUndo', function(action) {
      const { actionType, changes, source, index } = action;
...
function makeSharedKeyHandler(
  ...
  const updateDataHarmonizer = (data_harmonizer, [, col, oldVal, newVal]) => {
    ...
    hot.batch(() => {
      hot.getData().every((childRow, rowIndex) => {
        ...
        const shouldModifyChild = oldChildVal === oldVal && oldChildVal !== newVal;
  ...
  const createUndoRedoHandler = (shared_key_name) => {
        ...
        .forEach((obj, index) => {
          if (obj.length > 0) {
            ...
            if (!document.getElementById(dhId)) {
              ...
              if (is_child) {
                data_harmonizers[spec.name].hideAll();
              }
            }
          }
        });
