...
import schema_editor_schema from '@/web/templates/schema_editor/schema.json';
import grdi from '@/web/templates/grdi/schema.json';
import { buildAppContext, setup1M } from '@/lib/utils/1m';

console.log(buildAppContext(schema_editor_schema));
console.log(buildAppContext(grdi));
...
export default class AppContext {
  ...
  bindEventListeners() {
    $(document).on('dhTabChange', (event, data) => {
      console.info('dhTabChange', this.getCurrentDataHarmonizer().class_assignment, '->', data.specName);
      const { specName } = data;
    ...
    $(document).on('dhCurrentSelectionChange', (event, data) => {
      console.info('dhCurrentSelectionChange', this.currentSelection, '->', data.currentSelection);
