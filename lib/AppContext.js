...
import { 
  ...
  attachSelectionHandlersToDataHarmonizers, 
  buildSchemaTree
} from '@/lib/utils/1m';
...
export class AppContext {
  ...
  bindEventListeners() {
    ...
    $(document).on('dhTabChange', (event, data) => {
      ...
      console.log('current dataharmonizer', this.getCurrentDataHarmonizer());
    ...
    $(document).on('dhCurrentSelectionChange', (event, data) => {
      ...
      console.log('current selection', this.getCurrentDataHarmonizer());
    });
    ...
    .forEach((obj, index) => {
      if (obj.length > 0) {
        ...
        if (!document.getElementById(dhId)) {
          ...
          console.log('makeDataHarmonizersFromSchemaTree', schema, spec.name, schema_name);
