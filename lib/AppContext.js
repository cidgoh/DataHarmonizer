...
export default class AppContext {
  ...
  */
  ...
  crudUpdateRecordPath() {
    ...
    while (class_name in this.relations) {
      //if (class_name == current_template_name) {} // focused template.
      ...
      hierarchy_text = `<b>${class_name}</b>&nbsp;[${key_string}] ` + (hierarchy_text ? ' &gt;&nbsp;' : '') + hierarchy_text;

      // ISSUE, SEVERAL parents are possible - need them all displayed.
      class_name = Object.keys(this.relations[class_name].parent)?.[0];
    }
    $("#record-hierarchy").html(hierarchy_text);
  }
  ...
  crudGetDependentRows(start_name, skippable=false, changes = {}) {
    ...
    this.relations[start_name].dependents.keys().forEach((class_name) => {
      ...
      */
      ...
      //console.log("CHANGED DEPS", class_name, fkey_vals, changed_dep_keys)
      if (fkey_status >0) {
