...
export default class AppContext {
  ...
  */
  ...
  setDHTabStatus(class_name) {
    ...
    // Exception: in Schema Editor the Slot (Field) and SlotGroup (Table section)
    // tabs both contain schema-level rows that have no class_id, so enable them
    // as soon as schema_id is known (state >= 1 = at least one FK satisfied).
    const slotTabException = this.schemaEditor !== undefined && (class_name === 'Slot' || class_name === 'SlotGroup');
