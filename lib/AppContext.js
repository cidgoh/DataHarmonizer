...
export default class AppContext {
  ...
  async reload(template_path, locale = null, forced_schema= null) {
    ...
    for (const class_name in schema.classes) {
      if (class_name != 'Container' && class_name != 'dh_interface') {
        this.relations[class_name].dependents = this.crudGetDependents(class_name);
      }
    }
  ...
  makeDHsFromRelations(schema, template_name) {
    ...
    Object.entries(schema.classes)
      ...
      .filter(([class_name]) => class_name == template_name || this.relations?.[template_name]?.dependents?.has(class_name))
      .forEach((obj, index) => {
        if (obj.length > 0) {
...
/**
  ...
  useTemplate(dh, template_name) {
    ...
    let attributes = Object.entries(
      this.template.default.schema.classes
    )
      .filter(
        ([cls_key]) =>
          cls_key === dh.template_name
      )
      .reduce((acc, [, spec]) => {
        return {
          ...acc,
    ...
    for (let name in attributes) {
      ...
      if (!sectionIndex.has(section_title)) {
        ...
        const section =
          this.template.current.schema['slots'][section_title];
        if (section) {
   ...
   */
  getSlots(dh) {
    let slots = Array.prototype.concat.apply(
      [],
  ...
  */
    ...
    Object.entries(parents).forEach(([parent_name, parent]) => {
      Object.entries(parent).forEach(([slot_name, foreign_slot_name]) => {
        ...
        if (value == null) {
          ...
          required_selections[slot_name] = {value:value, parent:parent_name, slot:foreign_slot_name};
        }
      });
    });
  ...
  crudGetDependentUpdates(class_name, row, update=false) {
    ...
    // Here we have complete unique_keys search to find relate records by.
    if (change_log > '') {
        change_log = `Your key change on ${class_name} row ${parseInt(row)+1} would also change existing dependent table records! Do you want to continue?  Check:\n ${change_log}`;
    }
    return change_log;
  }
