...
import { dhAlert } from './utils/dialog';
...
// DH terminology dictionary.
// Text containing {{class}}, {{slot}}, … placeholders is rendered using the
// active mode.  Will eventually move to a separate file next to translations.json.
const DH_TERMS = {
  // User-friendly labels (default, non-expert mode)
  // Keys that are SchemaEditor class names (Schema, Class, SlotGroup, …) map to
  // DH-friendly display labels; these are looked up by class name at render time.
  // Keys that are prose tokens (slot, 'schema slot', 'slot group', …) are resolved
  // via applyDhTerms() wherever {{token}} placeholders appear in strings.
  //
  // Naming conventions for keys:
  //   • No dot notation  — use 'schema slots'  not 'schema.slots'
  //   • No underscores   — use 'slot group'     not 'slot_group' / 'Slot_group'
  //   • Space-separated words and standard capitalisation only
  default: {
    annotation:           'annotation',
    attribute:            'attribute',
    attributes:           'attributes',
    'attribute slot':     'custom field',
    Class:                'Table',
    class:                'table',
    'class attributes':   'custom fields',
    'class attribute':    'custom field',
    'class slot':         'derived field',
    'class slot usage':   'table field',
    classes:              'tables',
    enum:                 'picklist',
    Enum:                 'Picklist',
    enumeration:          'picklist',
    enumerations:         'picklists',
    EnumSource:           'Picklist config',
    enums:                'picklists',
    Extension:            'Extension',
    'permissible value':  'picklist choice',
    'permissible values': 'picklist choices',
    PermissibleValue:     'Picklist choices',
    rule:                 'rule',
    Rule:                 'Rule',
    schema:               'schema',
    Schema:               'Schema',
    schemas:              'schemas',
    'schema slot':        'schema field',
    'schema slots':       'schema fields',
    Section:              'Section',
    Setting:              'Setting',
    slot:                 'field',
    'class slot(s)':      'field(s)',
    Slot:                 'Field',
    'slot group':         'section',
    'Slot group':         'Field section',
    'slot groups':        'sections',
    'Slot name':          'Field ID',
    slots:                'fields',
    'unique key':         'table key',
    UniqueKey:            'Table key',
    // Possessive forms (straight apostrophe U+0027)
    "class's":            "table's",
    "slot's":             "field's",
    // Schema Editor column-header title translations (whole-title direct lookup)
    // Keys are the canonical LinkML-phrased titles stored in schema.yaml;
    // values are the common-language display strings for this mode.
    'Class ID':           'Table ID',
    'Section name':       'Section name',
    'Slot ID':            'Field ID',
    'Enum ID':            'Picklist ID',
    'Root Class':         'Root Table',
    'Unique key slots':   'Unique key fields',
    'On class':           'On table',
    'On slot':            'On field',
  },
  // Literal LinkML names (shown when "Use LinkML terminology" is checked)
  linkml: {
    annotation:           'annotation',
    attribute:            'attribute',
    attributes:           'attributes',
    ...
    'class slot usage':   'class.slot_usage',
    classes:              'classes',
    enum:                 'enum',
    Enum:                 'Enum',
    ...
    'schema slots':       'schema.slots',
    Section:              'Slot group',
    Setting:              'Setting',
    slot:                 'slot',
    'class slot(s)':      'class slot(s)',
    Slot:                 'Slot',
    ...
    "slot's":             "slot's",
    // Schema Editor column-header title translations (whole-title direct lookup)
    // In LinkML mode these are identity mappings except for Section-related
    // titles where "Section" maps to the canonical LinkML "Slot group" term.
    'Class ID':           'Class ID',
    'Section name':       'Slot group name',
    'Slot ID':            'Slot ID',
    'Enum ID':            'Enum ID',
    'Root Class':         'Root Class',
    'Unique key slots':   'Unique key slots',
    'On class':           'On class',
    'On slot':            'On slot',
    'Semantic URI':       'Meaning',
  },
};
...
export default class AppContext {
  ...
  async tabChange(class_name) {
    ...
    if (this.getCurrentDataHarmonizer() !== class_name) {
      ...
      // Ensure every DH has a visibility-state record (lazy init).
      dh._visState ??= { colMode: 'all', sections: [], rowMode: 'all' };

      // Refresh SchemaClassMenu when entering the Table (Class) tab so the
      // is_a dropdown always reflects the currently-selected schema.  The
      // schema-scoped menus are updated by a setTimeout in the Schema tab's
      // afterSelection hook, but tab navigation can outrace that callback;
      // refreshing here ensures the source is always current on entry.
      if (class_name === 'Class' && this.schemaEditor) {
        this.schemaEditor.refreshMenus(['SchemaClassMenu']);
      }
      // Swap in this tab's pre-built section menu and restore toolbar state.
      this.toolbar.applySectionMenu(dh);
      this.toolbar._restoreVisibilityUI(dh);
      // Hide any floating validation tooltip left over from the previous tab.
      const bar = document.getElementById('validation-status-bar');
      if (bar) bar.style.display = 'none';

      // ── Show/hide the Field order section in the Display dropdown ──────
      // Only meaningful on the Field / Slot tab; hidden on all other tabs.
      $('#slot_type_optgroup').toggle(class_name === 'Slot');

      // Update next-error button to reflect the incoming tab's validation state
      // without clearing it — validation results persist across tab switches.
      ...
      // Restore the scroll position this tab had when the user last left it.
      // Runs in a fresh task so it fires after HOT's own post-render scroll
      // adjustments (triggered by the tab-pane display:none → block transition).
      if (dh._scrollState) {
        const { scrollLeft, scrollTop } = dh._scrollState;
        setTimeout(() => {
          const wt = dh.hot.rootElement?.querySelector('.wtHolder');
          if (wt) {
            wt.scrollLeft = scrollLeft;
            wt.scrollTop  = scrollTop;
          }
        }, 0);
      }
    }
  ...
  /**
   * Accessed by both a tabChange() and by changes to the Display dropdown menu
   */
  refreshTabDisplay() {
    ...
    // Display menu: read the checked radio for main type.
    let report_type = $('input[name="display-main-type"]:checked').val() ?? '';
    ...
    // Record hierarchy cookie crumbs: shown in "Records by key" mode (full trail)
    // and in "All records" mode when the current tab has FK parent(s), so the
    // user can see what parent context will be written to any inserted rows.
    const hasParents = Object.keys(classRel?.parent ?? {}).length > 0;
    $('#record-hierarchy').toggle(report_type === '' || (report_type === 'all' && hasParents));
    ...
    */
    ...
    // For 'all' mode: save the active tab's selection by physical row (sort-stable).
    // The case 'all' loop (filtersPlugin.filter, scrollViewportTo, render) can
    // inadvertently reset HOT's visual cursor to (0,0).  We capture the physical
    // row now and re-select the correct visual row after sorting completes below.
    const _activeSel = dh.hot.getSelected();
    const _activeVisRow = _activeSel?.[0]?.[0] ?? -1;
    const activePhysRow = _activeVisRow >= 0 ? dh.hot.toPhysicalRow(_activeVisRow) : -1;
    const activeCol = _activeSel?.[0]?.[1] ?? 0;
    ...
    switch (report_type) {
      case 'all': {
        // Snapshot inactive tabs' HOT selection state before the show-all loop.
        // Operations like scrollViewportTo / filtersPlugin.filter / render on
        // inactive HOT instances can inadvertently select cell (0,0) even when
        // the user never clicked that tab.  We restore the prior state after the
        // loop so switching to "All records" does not silently change the cursor.
        const inactiveSels = new Map();
        for (const [name, tabDh] of Object.entries(this.dhs)) {
          if (tabDh !== dh) {
            inactiveSels.set(name, tabDh.hot.getSelected() ?? null);
          }
        }

        this.tabFilter(dh, {}); // gets rid of any filters
        // Clear column filters and hidden-row state on all tabs.
        for (const tab_dh of Object.values(this.dhs)) {
          const filtersPlugin = tab_dh.hot.getPlugin('filters');
          if (filtersPlugin.isEnabled()) {
            filtersPlugin.clearConditions();
            filtersPlugin.filter();
          }
          tab_dh.changeRowVisibility('show-all-rows-dropdown-item');
        }

        // Restore inactive tabs' selection state.  Three outcomes per tab:
        // • Row unchanged — no-op.
        // • Had a selection that moved — put it back.
        // • Had no selection but loop auto-selected (0,0) — deselect.
        for (const [name, priorSel] of inactiveSels) {
          const tabDh = this.dhs[name];
          const curSel = tabDh.hot.getSelected() ?? null;
          const curRow = curSel?.[0]?.[0] ?? -1;
          const priorRow = priorSel?.[0]?.[0] ?? -1;
          if (curRow !== priorRow) {
            if (priorRow >= 0) {
              const [r, c, r2, c2] = priorSel[0];
              tabDh.hot.selectCell(r, Math.max(0, c), r2, Math.max(0, c2), false);
            } else {
              tabDh.hot.deselectCell();
            }
          }
        }

        // Enable every dependent tab so the user can freely browse all tables.
        for (const dep_name of Object.keys(this.dhs)) {
          if (this.dependent_rows.has(dep_name)) {
            const domId = Object.keys(this.dhs).indexOf(dep_name);
      ...
      */
      ...
      // Show all rows for the currently selected schema, without filtering by
      // class.  On the Field (Slot) tab this lets users browse all fields
      // across every table in one schema; for other tabs whose only FK is
      // schema_id this behaves the same as "by key".
      case 'schema': {
        const baseVals = this.dependent_rows.get(class_name)?.fkey_vals ?? {};
        const schemaOnlyVals = 'schema_id' in baseVals
          ? { schema_id: baseVals.schema_id }
          : {};
        this.tabFilter(dh, schemaOnlyVals);
        if (class_name === 'Slot' && schemaOnlyVals.schema_id) {
          this._showMatchingBaseSlots(dh, schemaOnlyVals);
        }
        break;
      }
      ...
      case '':
      default: {
        // Re-apply tab enabled/disabled state from the existing dependent_rows
        // data (set by afterSelectionEnd / afterChange events).
        // Do NOT recalculate from root here: a full root recalculation would
        // cascade with only FK-chain values (no user-selected intermediate keys)
        // and would always disable 3rd-level tabs like AMRTest even when the
        // user has a valid GRDIIsolate row selected.
        if (this.dependent_rows.size > 0) {
          const rootClassName = Object.keys(this.dhs)[0];
          const classDependents = this.relations[rootClassName]?.dependents;
          if (classDependents) {
            for (const [dep_name] of classDependents.entries()) {
              this.setDHTabStatus(dep_name);
            }
          }
        ...
        if (this.schemaEditor !== undefined) {
          // The Slot tab relaxes the class_id FK so schema-level slot rows
          // (slot_type='slot') remain visible when no Class row is selected.
          // All other tabs must satisfy their FK constraints strictly.
          if (dh.template_name === 'Slot' && this.selectedTableRow('Class') < 0) {
            delete fkey_vals['class_id'];
          }
        }

        // Skip tabFilter for root tabs with no FK constraints (e.g. Schema).
        // Calling it with empty key_vals triggers an unnecessary filter+render
        // cycle that resets the horizontal scroll position.
        if (Object.keys(fkey_vals).length > 0) {
          this.tabFilter(dh, fkey_vals); // filter by selected keys.
        }
        // Special: on the Slot tab, also un-hide base-slot ('slot') rows that
        // share the selected schema_id and a slot.name with any slot_usage/attribute
        // row that passed the key filter.  This lets users compare class-level
        // overrides alongside the schema-level slot definitions they inherit from.
        if (class_name === 'Slot' && fkey_vals.schema_id) {
          this._showMatchingBaseSlots(dh, fkey_vals);
        }
      }
    }
    ...
    */
    ...
    // On the Schema Editor's Field (Slot) tab, pick the sort config that matches
    // the "Order by" radio selection; elsewhere use the tab's default.
    let sortConfig = dh.defaultMultiColumnSortConfig;
    if (this.schemaEditor !== undefined && dh.template_name === 'Slot') {
      const orderBy = $('input[name="slot-order-type"]:checked').val() ?? 'rank';
      sortConfig = orderBy === 'label' ? dh.slotSortByLabel : dh.slotSortByTableRank;
    }
    columnSorting.sort(sortConfig); // possibly null

    dh.render();  // Otherwise cells() implements meta and class stuff that isn't reflected

    // Apply concise view (hide FK columns) when enabled.
    this.toolbar?._applyConciseView(dh);

    // Restore cursor after tabFilter / sort may have reset it to (0,0).
    // Applies to all display modes: filtersPlugin.filter() + resumeExecution()
    // inside tabFilter reset HOT's selection regardless of report_type.
    // The physical row captured before filtering is sort-stable, so toVisualRow()
    // gives the correct position after any sort applied inside tabFilter.
    // Skip restore if the row is now hidden by the key filter (filtered out).
    if (activePhysRow >= 0) {
      const newVisRow = dh.hot.toVisualRow(activePhysRow);
      const hiddenRows = new Set(dh.hot.getPlugin('hiddenRows').getHiddenRows());
      if (newVisRow != null && newVisRow >= 0 && !hiddenRows.has(newVisRow)) {
        const curVisRow = dh.hot.getSelected()?.[0]?.[0] ?? -1;
        if (curVisRow !== newVisRow) {
          dh.hot.selectCell(newVisRow, activeCol, newVisRow, activeCol, false);
        }
      }
    } else if (report_type === 'all' && (dh.hot.getSelected()?.length ?? 0) > 0) {
      // 'All records' mode only: clear any spurious (0,0) auto-selection that
      // the show-all loop may have placed on a tab that had no prior selection.
      dh.hot.deselectCell();
    }

    // In "Records by selected key" mode: if filtering leaves exactly one
    // non-empty row visible and it is not already selected, auto-select it.
    // This propagates FK key values into dependent_rows so other tabs filter
    // correctly without requiring the user to manually click the row.
    if (report_type === '') {
      const hiddenPlugin = dh.hot.getPlugin('hiddenRows');
      const hiddenSet = new Set(hiddenPlugin.getHiddenRows());
      const visibleNonEmpty = [];
      for (let vr = 0; vr < dh.hot.countRows(); vr++) {
        if (hiddenSet.has(vr)) continue;
        if (!dh.hot.isEmptyRow(dh.hot.toPhysicalRow(vr))) visibleNonEmpty.push(vr);
      }
      if (visibleNonEmpty.length === 1 && dh.current_selection[0] !== visibleNonEmpty[0]) {
        dh.hot.selectCell(visibleNonEmpty[0], 0);
      }
    }
  ...
  /**
   * After tabFilter has run on the Slot tab, additionally un-hide any base-slot
   * ('slot_type' = 'slot') rows whose schema_id matches the selected schema and
   * whose slot.name appears in at least one slot_usage/attribute row that
   * survived the key filter.  This lets users see the schema-level slot
   * definition alongside the class-level overrides (slot_usage rows) that
   * reference it, without losing the class-scoped filter.
   *
   * hiddenRowsPlugin.getHiddenRows() returns physical row indices.
   * showRows() expects visual row indices.
   */
  _showMatchingBaseSlots(dh, fkey_vals) {
    // If the slot-type filter is active and excludes 'slot', schema-slot rows
    // are being force-hidden by the Slot tab's afterRender hook; skip showing them.
    if (dh._slotTypeFilter && !dh._slotTypeFilter.has('slot')) return;
    const hiddenRowsPlugin = dh.hot.getPlugin('hiddenRows');
    // getHiddenRows() converts internally-stored physical indices to VISUAL
    // before returning (see hiddenRows.js line 332-334), so the result is a
    // set of visual row indices.
    const hiddenVisualRows = new Set(hiddenRowsPlugin.getHiddenRows());

    // Collect slot names from currently-visible slot_usage / attribute rows.
    const shownNames = new Set();
    for (let physRow = 0; physRow < dh.hot.countSourceRows(); physRow++) {
      const visualRow = dh.hot.toVisualRow(physRow);
      if (hiddenVisualRows.has(visualRow)) continue; // skip hidden rows
      const slot_type = dh.hot.getSourceDataAtCell(physRow, dh.slot_type_column);
      if (slot_type === 'slot_usage' || slot_type === 'attribute') {
        const name = dh.hot.getSourceDataAtCell(physRow, dh.slot_name_column);
        if (name) shownNames.add(name);
      }
    }
    if (shownNames.size === 0) return;

    // Un-hide 'slot' rows that match both the schema_id and the collected names.
    const schemaId = fkey_vals.schema_id;
    const toUnhide = [];
    for (let physRow = 0; physRow < dh.hot.countSourceRows(); physRow++) {
      const visualRow = dh.hot.toVisualRow(physRow);
      if (!hiddenVisualRows.has(visualRow)) continue; // already visible
      if (dh.hot.getSourceDataAtCell(physRow, dh.slot_type_column) !== 'slot') continue;
      if (dh.hot.getSourceDataAtCell(physRow, dh.schema_name_column) !== schemaId) continue;
      const name = dh.hot.getSourceDataAtCell(physRow, dh.slot_name_column);
      if (!shownNames.has(name)) continue;
      toUnhide.push(visualRow); // showRows expects visual indices
    }

    if (toUnhide.length > 0) {
      hiddenRowsPlugin.showRows(toUnhide);
    }
  }
  ...
  tabFilter(dh, key_vals) {
    ...
    // Clear and re-apply Filters plugin conditions.  clearConditions() alone
    // removes the conditions from the queue but does NOT update the internal
    // RowIndexMapper that toVisualRow() reads.  filterAll() hides rows via the
    // Filters plugin ("no match" condition + filter()); until filter() is called
    // again after clearConditions(), toVisualRow() returns null for every row,
    // making hiddenRowsPlugin.showRows() a no-op and leaving the table empty.
    let filtersPlugin = dh.hot.getPlugin('filters');
    filtersPlugin.clearConditions();
    // filter() updates the RowIndexMapper synchronously even inside
    // suspendExecution (only the DOM render is deferred), so toVisualRow()
    // returns correct indices for the showRows/hideRows calls below.
    filtersPlugin.filter();
    ...
    for (let row = 0; row < dh.hot.countSourceRows(); row++) {
      ...
      Object.entries(key_vals).forEach(([key_name, value]) => {
        // null means "match any" — the corresponding parent has no selection so
        // do not use this FK to restrict visibility (same semantics as
        // crudFindAllRowsByKeyVals which also skips null entries).
        if (value === null) return;
        let column = dh.slot_name_to_column[key_name];
        // Use getSourceDataAtCell so the loop index is a physical row index,
        // matching the physical-to-visual conversion done below before the
        // showRows/hideRows calls.  getDataAtCell takes visual indices which
        // diverge from physical once sorting is applied.
        if (dh.hot.getSourceDataAtCell(row, column) !== value)
          found = false;
      })
      if (found)
        shown.push(row);
    ...
    // HOT 15: showRows/hideRows take VISUAL (not physical) row indices.
    // After multiColumnSorting is applied, physical ≠ visual, so we must
    // convert before passing to the plugin.
    hiddenRowsPlugin.showRows(shown.map(r => dh.hot.toVisualRow(r)));
    hiddenRowsPlugin.hideRows(hidden.map(r => dh.hot.toVisualRow(r)));
  ...
  async reload(template_path, locale = null, forced_schema = null) {
    ...
    // LinkML allows classes, slots and enums to omit the redundant `name`
    // field (the dict key already serves as the name).  Downstream code relies
    // on .name being present on every object, so backfill from the dict key
    // wherever it is missing.
    Object.entries(schema.classes || {}).forEach(([key, obj]) => {
      if (!obj.name) obj.name = key;
    });
    Object.entries(schema.slots || {}).forEach(([key, obj]) => {
      if (!obj.name) obj.name = key;
    });
    ...
    Object.entries(schema.enums || {}).forEach(([enum_key, enum_obj]) => {
      if (!enum_obj.name) enum_obj.name = enum_key;
      ...
      // Backfill .text (the permissible value code) from the dict key.
      // Standard LinkML schemas omit this DH extension; YAML also parses
      // bare entries like `Combustion:` as null rather than an object.
      if (enum_obj.permissible_values) {
        for (const [pv_key, pv] of Object.entries(enum_obj.permissible_values)) {
          if (pv === null || pv === undefined || typeof pv !== 'object') {
            // Bare keys parse as null; string values (e.g. `B: Brahma`) are
            // also non-objects. Normalise to an object, preserving any string
            // as the title.
            enum_obj.permissible_values[pv_key] = {
              text: pv_key,
              ...(typeof pv === 'string' && pv && { title: pv }),
            };
          } else if (!pv.text) {
            pv.text = pv_key;
          }
    ...
    Object.entries(schema.classes)
      ...
      .forEach((obj, index) => {
        if (obj.length > 0) {
          ...
          // Look up the DH-friendly label by class name; fall back to title or name.
          const raw_title = class_obj.title || class_name;
          const _tabTerms = this.getDhTermVars();
          const tab_label = _tabTerms[class_name] ?? raw_title;
          const dhTab = createDataHarmonizerTab(dhId, tab_label, class_name, tooltip, index === 0);
          // Store class_name (stable) so the LinkML-terms toggle can re-look-up the label.
          dhTab.setAttribute('data-label-template', class_name);

          // Future: Change createDataHarmonizerTab() to return jquery dom element id?
          dhTab.addEventListener('click', (event) => {
            // Disabled tabs must not trigger a tab change.
            if (event.srcElement.classList.contains("disabled"))
              return false;

            // Snapshot the scroll position of the tab we are leaving.
            // Must happen on click (before Bootstrap transitions the pane away).
            const leavingDh = this.getCurrentDataHarmonizer();
            if (leavingDh) {
              const wt = leavingDh.hot.rootElement?.querySelector('.wtHolder');
              leavingDh._scrollState = {
                scrollLeft: wt?.scrollLeft ?? 0,
                scrollTop:  wt?.scrollTop  ?? 0,
              };
            }
          });

          // Bootstrap fires shown.bs.tab on the newly-active tab link after the
          // pane is fully visible (display:block, transition complete). Running
          // tabChange() here guarantees HOT's container has correct dimensions
          // before render() is called — no arbitrary setTimeout delay needed.
          $(dhTab).on('shown.bs.tab', () => {
            this.tabChange(class_name);
          });
          ...
          if (class_name != 'Container' && 'Container' in schema.classes) {
            // Use the dict key (attribute_name) not attribute.name — they must match
            // the keys used in getContainerData / saveTabularDataFile.
            let found_name = Object.entries(schema.classes.Container.attributes)
              .find(([, v]) => v.range && v.range === class_name)?.[0];
            if (found_name)
  ...
  useTemplate(dh, template_name) {
    ...
    // For class-scoped locale translations (slot_usage and attribute fields),
    // we detect translated fields by comparing the locale-merged schema against
    // the default (English) schema: any text field whose value CHANGED is a
    // locale translation, so we apply it as the highest-priority overlay.
    // This avoids accidentally overwriting a global slot's French title with
    // the English class-attribute title (which would happen if we blindly
    // deepMerged the full locale-merged class attribute).
    const LOCALE_TEXT_FIELDS = ['title', 'description', 'comments', 'examples'];
    const current_cls = this.template.current.schema.classes?.[dh.template_name];
    const default_cls = this.template.default.schema.classes?.[dh.template_name];
    ...
    for (let name in attributes) {
      ...
      // Apply class-scoped locale translations as the highest-priority overlay.
      // slot_usage fields: fr.classes.[class].slot_usage.[name].[col]
      // attribute fields:  fr.classes.[class].attributes.[name].[col]
      // Both paths are checked; attribute locale takes precedence if both are set.
      const su_curr = current_cls?.slot_usage?.[name];
      const su_def  = default_cls?.slot_usage?.[name];
      if (su_curr) {
        for (const f of LOCALE_TEXT_FIELDS) {
          if (su_curr[f] != null && su_curr[f] !== su_def?.[f]) slot[f] = su_curr[f];
        }
      }
      const at_curr = current_cls?.attributes?.[name];
      const at_def  = default_cls?.attributes?.[name];
      if (at_curr) {
        for (const f of LOCALE_TEXT_FIELDS) {
          if (at_curr[f] != null && at_curr[f] !== at_def?.[f]) slot[f] = at_curr[f];
        }
      }
      ...
      for (let range of range_array) {
        ...
        if (range in this.template.default.schema.types) {
          ...
          switch (range) {
            ...
            // WhitespaceMinimizedString is Dataharmonizer's obsoletable term for xs:token
  ...
  */
  ...
  crudUpdateRecordPath() {
    const dh = this.getCurrentDataHarmonizer();
    if (!dh) return;
    const schema = this.template.current.schema;
    const terms  = this.getDhTermVars(); // look up display labels by class name
    const current_template_name = dh.template_name;
    ...
    while (stack.length >0) {
      ...
      if (class_name in done)
        continue;
      const raw_title = schema.classes[class_name]?.title || class_name;
      const class_label = terms[class_name] ?? raw_title;
      const dependent = this.dependent_rows.get(class_name);

      // Push parents first so the walk always climbs to root (Schema) even when
      // this tab has no selection and will be skipped from the breadcrumb.
      const parents = Object.keys(this.relations[class_name]?.parent ?? {});
      if (!isEmpty(parents))
        stack.push(...parents);

      let key_string = '';
      let tooltip = '';
      // Skip this class in the breadcrumb when no row is selected (deselected after deletion).
      if (dependent.has_selection === false) continue;
    ...
    // In "Records by selected schema" mode show only the first (root) crumb.
    const _report_type = $('input[name="display-main-type"]:checked').val() ?? '';
    const _display = _report_type === 'schema' ? hierarchy_text.slice(0, 1) : hierarchy_text;
    $("#record-hierarchy").html(_display.join('&nbsp;/&nbsp;'));
  }

  crudCalculateDependentKeys(class_name) {
    // When switching schemas, deselect all dependent tabs BEFORE computing the
    // cascade.  Without this, crudGetDependentRows reads the stale HOT selection
    // from the Class/Slot tabs (e.g. CanCOGeNCovid19) and folds it into key_vals,
    // causing the breadcrumb and row-filter to reference a class that belongs to
    // the previous schema.
    if (class_name === 'Schema') {
      for (const dh of Object.values(this.dhs)) {
        if (dh.template_name !== 'Schema') {
          dh.hot.deselectCell();
        }
      }
    }
    ...
    this.crudUpdateRecordPath();
    if (!this.relations[class_name]) return;
    let class_dependents = this.relations[class_name].dependents;

    // Switching to a different Schema row changes the entire data context for
    // every tab.  Any previously applied row-visibility filter (show valid /
    // show invalid) is stale for the new schema, so reset all tabs to
    // "show all rows" and clear the stored rowMode so the Display dropdown
    // buttons reflect the correct state when the user navigates to each tab.
    // Also remove any red validation-error tab styling — it belonged to the
    // previous schema's data and is no longer meaningful.
    if (class_name === 'Schema') {
      for (const dh of Object.values(this.dhs)) {
        // The Schema tab is the root selector — its row-visibility state is not
        // schema-specific, so skip changeRowVisibility (which calls scrollViewportTo)
        // to avoid resetting the user's horizontal scroll position on row click.
        if (dh.template_name !== 'Schema') {
          dh.changeRowVisibility('show-all-rows-dropdown-item');
          if (dh._visState) dh._visState.rowMode = 'all';
        }
        $(`#tab-bar-${dh.template_name} > a`).removeClass('tab-has-errors');
      }
    }
  ...
  crudFilterDependentRows(class_name, dependent_report) {
    dhAlert("this shouldn't be running");
  ...
  setDHTabStatus(class_name) {
    // Never disable tabs when "Display: All records" is active.
    if ($('input[name="display-main-type"]:checked').val() === 'all') return;
    ...
    // Exception: in Schema Editor the Slot (Field) tab contains schema-level rows
    // that have no class_id, so enable it as soon as schema_id is known
    // (state >= 1 = at least one FK satisfied).
    const slotTabException = this.schemaEditor !== undefined && class_name === 'Slot';
    // Exception: in Schema Editor the UniqueKey (Table Key) tab is only meaningful
    // once a schema is selected AND the Class tab has at least one class for it.
    // We don't require the user to select a specific class first — just that classes
    // exist — so enable as soon as schema_id is known and Class rows are present.
    const uniqueKeyTabException = this.schemaEditor !== undefined && class_name === 'UniqueKey';
    let disabled;
    if (uniqueKeyTabException) {
      if (state < 1) {
        disabled = true; // no schema selected yet
      } else {
        const classDh     = this.dhs['Class'];
        const schemaId    = this.dependent_rows.get('UniqueKey')?.fkey_vals?.schema_id;
        const schemaIdCol = classDh?.slot_name_to_column?.['schema_id'];
        const hasClasses  = schemaId && schemaIdCol !== undefined && classDh?.hot &&
          classDh.hot.getSourceData().some(row => row[schemaIdCol] === schemaId);
        disabled = !hasClasses;
      }
    } else {
      disabled = slotTabException ? state < 1 : state < 2;
    }
    $('#tab-data-harmonizer-grid-' + domId)
  ...
  crudGetDependentRows(start_name, _skippable=false, changes = {}) {
    ...
    this.dependent_rows.set(start_name, {
      ...
      has_selection: this.dhs[start_name]?.hot.getSelected() != null,
    });

    // For each dependent
    this.relations[start_name]?.dependents?.keys().forEach((class_name) => {
      // fkey_vals: built purely from the ancestor cascade chain (FK slots only).
      // Used for this tab's own row filter (tabFilter) so it shows all rows that
      // belong to the selected ancestor — e.g. all Tables for Schema B.
      // key_vals: fkey_vals plus the currently-selected row's own unique-key
      // values, so deeper dependents can be correctly enabled/filtered even when
      // the cascade starts from a distant ancestor.
      let fkey_vals = {};
      let fkey_status;
      ...
      if (this.relations[class_name].foreign_key_count === 0) {
        ...
        for (let [slot_name, slot_link] of Object.entries(this.relations[class_name].dependent_slots)) {
          ...
          if (parent_dep) {
            ...
            // Normal display (not skippable): '' means "no parent selected" →
            // treat as null so the search matches any child row.
            // getChangeReport (skippable): '' is the literal current key value
            // of the row being edited — keep it so the search finds only rows
            // whose FK equals '' (i.e. none), preventing a false cascade warning
            // when a brand-new, still-unnamed row is being given its first key.
            if (foreign_value === '' && !_skippable) foreign_value = null;
          }
      ...
      // key_vals for cascade: FK-chain values plus the currently-selected row's
      // own unique-key / target-slot values (if a selection exists in this tab).
      // Including the selected row's primary key values ensures deeper dependents
      // (e.g. Table Section) are correctly enabled when the cascade starts from
      // an ancestor (e.g. Schema) without losing the Table B selection.
      // Only non-FK slots are added here; FK slots are already set via fkey_vals.
      //
      // Skipped when called from getChangeReport (_skippable=true): in that
      // context we need the broadest possible search (null FK = match any) so
      // we don't miss records in sibling tables, and we avoid narrowing the
      // cascade to only the currently-selected intermediate row.
      let key_vals = {...fkey_vals};
      const _cascadeDh  = this.dhs[class_name];
      const _cascadeSel = _cascadeDh?.hot.getSelected();
      if (_cascadeSel && !_skippable) {
        const _cascadeRow   = _cascadeSel[0][0];
        const _cascadeSlots = Object.keys({
          ...this.relations[class_name].unique_key_slots,
          ...this.relations[class_name].target_slots,
        });
        for (const _slot of _cascadeSlots) {
          if (!(_slot in fkey_vals)) {
            key_vals[_slot] = this.crudGetDHCellValue(_cascadeDh, _slot, _cascadeRow);
          }
        }
      }
      ...
      let dependent_rows_obj = {
        ...
        key_vals: key_vals,
        key_changed_vals: {},
        has_selection: _cascadeSel != null,
      };
      ...
      Object.entries(this.relations[class_name].dependent_slots)
        .forEach(([slot_name, key_mapping]) => {
          // E.g. key_mapping.foreign_class = "Slot"
          let foreign_class = this.dependent_rows.get(key_mapping.foreign_class);
          if (!foreign_class) {
            console.log("WARNING: ",class_name, slot_name, "dependency on", key_mapping.foreign_class, "but that class hasn't been processed yet.  REORDER?")
          }
          else {
            //console.log("DEPENDENT slots for", class_name, slot_name, key_mapping, foreign_class, this.dependent_rows)
            const parent_changes = foreign_class['key_changed_vals'];
      ...
      */
      ...
      if (fkey_status > 0) {
        // In getChangeReport (_skippable=true): if any FK slot is null AND the
        // corresponding parent's key_changed_vals contains that FK's foreign_slot,
        // the parent row is brand-new (its key is being set for the first time) —
        // no existing child records can reference a null/empty key.  Skip the
        // count to prevent a false-positive cascade warning (e.g. naming a new
        // picklist PB must not find the existing choices of picklist PA).
        const skipSearch = _skippable && Object.entries(this.relations[class_name].dependent_slots).some(
          ([slot_name, slot_link]) => {
            if (fkey_vals[slot_name] !== null) return false;
            const parent = this.dependent_rows.get(slot_link.foreign_class);
            return parent && (slot_link.foreign_slot in parent.key_changed_vals);
          }
        );
        if (!skipSearch) {
  ...
  crudGetDependentKeyVals(class_name, _changes) {
    ...
    if (class_dh) { // FUTURE: avoid this case altogether.
      ...
      // Container and dh_interface are excluded from this.relations; guard
      // against any other class that may end up in dhs without a relations entry.
      if (!this.relations[class_name]) {
      ...
      for (let slot_name of search_slots) {
        if (!(slot_name in dependent_slots)) { // FK slot already inherited.
          // When _changes includes this key slot, use its pre-change (old) value so
          // that the cascade search finds records linked to the OLD key.  This is
          // necessary because HOT moves the selection to the clicked row BEFORE
          // firing beforeChange for the just-edited row — so hot.getSelected() would
          // return the wrong row (e.g. the already-selected GRDI row instead of the
          // newly-edited empty row 3), causing false cascade warnings.
          if (_changes && Object.prototype.hasOwnProperty.call(_changes, slot_name)) {
            key_vals[slot_name] = _changes[slot_name].old_value ?? null;
          } else {
  ...
  */
  ...
  /**
   * Returns the active DH terminology variables (class, slot, …).
   * Driven by `this.use_linkml_terms`.
   * @returns {Object} key→display-string mapping for the current mode.
   */
  getDhTermVars() {
    return DH_TERMS[this.use_linkml_terms ? 'linkml' : 'default'];
  }

  /**
   * Replaces {{...}} term placeholders in `text` using the active terminology
   * dictionary.  Placeholders not in the dictionary are left unchanged.
   * Use this wherever SchemaEditor UI text contains {{class}}, {{slot}}, etc.
   * @param {string} text
   * @returns {string}
   */
  applyDhTerms(text) {
    const terms = this.getDhTermVars();
    // [^}]+ matches keys that may contain spaces, dots, or underscores (e.g. "schema.slots")
    return text.replace(/\{\{([^}]+)\}\}/g, (m, k) => (k in terms ? terms[k] : m));
  }

  /**
   * Translates a column/field title by exact whole-string lookup in the active
   * terminology dictionary.  Intended for HOT column headers and column-help
   * popups where the schema.yaml title is the canonical LinkML form and must
   * be rendered in the currently-active display language.
   *
   * Unlike applyDhTerms (which resolves {{...}} placeholders), this method
   * matches the full title string as a dictionary key and returns the mapped
   * value, or the original text if no mapping is found.
   *
   * @param {string} text  The canonical title from schema.yaml.
   * @returns {string}
   */
  applyDhTermsToTitle(text) {
    if (!text) return text;
    const terms = this.getDhTermVars();
