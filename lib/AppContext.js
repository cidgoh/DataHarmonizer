...
*/
  ...
  */
    ...
    if (errors.length == 0) {
      rowsToHide = rowsToHide.filter((row) => {
        for (const [slot_name, value] of Object.entries(required_selections)) {
          const col = this.dhs[class_name].slot_name_to_column[slot_name];
   ...
   *
   * 2) Dependent's primary key values are held in more than one root table
   * record no action in this case (except alert) since operations on remaining
   * root record(s) should determine dependents fates.
   *
   ...
   *
   * INPUT
   * @root_name {String} name of root table to begin looking for cascading effect of foreign key changes.
   ...
   */
  crudGetDependentChanges(root_name, changes = {}) { //, do_action = false 
    let change_report = '';
    let found_dependent_rows = false;
    let dependent_map = new Map();
    // Starts off dependent_map with key_vals mapping of user's selected row.
    let [key_vals, changed_key_vals] = this.crudGetDependentKeyVals(root_name, dependent_map, changes, true);
    dependent_map.set(root_name, {
      slots: key_vals,
    ...
    for (const [dependent_name, dependent_obj] of this.relations[root_name].dependents) {
      ...
      let [dependent_key_vals, changed_key_vals] = this.crudGetDependentKeyVals(dependent_name, dependent_map, changes);
      //console.log("crudGetDependentChanges checking", dependent_name, dependent_key_vals)
      if (!isEmpty(dependent_key_vals)) {
        const dependent_dh = this.dhs[dependent_name];

        let dependent_map_obj = {slots: dependent_key_vals};

        /*
         * 2) Query given dependent to see # of rows of dependent retrieved.  
         * 3) Add rowcount to dependent_map.
         *
        ...
        if (rows.length) {
          dependent_map_obj['changed_slots'] = changed_key_vals;
          dependent_map_obj['count'] = rows.length;   
          dependent_map_obj['rows'] = rows;
        }
        dependent_map.set(dependent_name, dependent_map_obj);
        dependent_map.get(dependent_name);

      } // end if dependent_key_vals
   ...
   * @param {Object} dependent_map dictionary of dependents to get values from
   * @param {Object} changes in root (holds current vs new slot values)
   * @param {Boolean} is_root true if class_name is root of search. (first class in dependent_map)
   ...
   */
  crudGetDependentKeyVals(class_name, dependent_map, changes, is_root = false) { 
    let key_vals = {};
    ...
    for (let slot_name of search_slots) {
      ...
      if (is_root) {
        ...
        if (this.relations[class_name].dependent_slots?.[slot_name]) {
          ...
          let foreign_class_keys = dependent_map.get(slot_link.foreign_class)?.slots;
          //console.log("HERE", class_name, slot_name, foreign_class_keys, dependent_map)
          if (foreign_class_keys)
            key_vals[slot_name] = foreign_class_keys[slot_link.foreign_slot];
            // Now inherit changed_slots values
            let changed_keys = dependent_map.get(slot_link.foreign_class)?.changed_slots;
            if (changed_keys)
    ...
    if (!isEmpty(variable_slots)) {
      ...
      console.log("At crudGetDependentKeyVals, should run query for", class_name, variable_slots)
    }
    return [key_vals, changed_key_vals];
  }
