...
export default class AppContext {
  ...
  refreshTabDisplay() {
    ...
    */
    ...
    // 'All records' mode: restore the active tab's cursor to wherever it was
    // before the show-all operations may have reset it.  The physical row saved
    // above is sort-stable, so toVisualRow() gives the correct position after
    // any sort change.  If there was no prior selection, ensure nothing is
    // auto-selected (deselect anything the loop may have picked).
    if (report_type === 'all') {
      if (activePhysRow >= 0) {
        const newVisRow = dh.hot.toVisualRow(activePhysRow);
        if (newVisRow != null && newVisRow >= 0) {
      ...
      } else if ((dh.hot.getSelected()?.length ?? 0) > 0) {
        dh.hot.deselectCell();
      }
    }
