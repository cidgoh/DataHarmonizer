...
var EXPORT_FORMATS = {
      ...
      for (const inputRow of dh.getTrimmedData(dh.hot)) {
        ...
        for (const [headerName, sources] of ExportHeaders) {
          ...
          */
          ...
          // Otherwise apply source (many to one) to target field transform:
          const value = dh.getMappedField(headerName, inputRow, sources, sourceFields,sourceFieldNameMap, ':', 'NCBI_BIOSAMPLE');
    ...
    method: function (dh) {
      ...
      for (const inputRow of dh.getTrimmedData(dh.hot)) {
        ...
        for (const [headerName, sources] of ExportHeaders) {
          ...
          // Otherwise apply source (many to one) to target field transform:
          const value = dh.getMappedField(headerName, inputRow, sources, sourceFields, sourceFieldNameMap, ':', 'NCBI_Genbank') 
          outputRow.push(value);
        }
        outputMatrix.push(outputRow);
      }
  ...
  NCBI_Genbank_source_modifiers: {
    ...
    method: function (dh) {
      ...
      const sourceFieldNameMap = dh.getFieldNameMap(sourceFields);
      // Fills in the above mapping (or just set manually above) 
      dh.getHeaderMap(ExportHeaders, sourceFields, 'NCBI_Genbank_source_modifiers');
      ...
      for (const inputRow of dh.getTrimmedData(dh.hot)) {
        ...
        for (const [headerName, sources] of ExportHeaders) {
          ...
          // Otherwise apply source (many to one) to target field transform:
          const value = dh.getMappedField(headerName, inputRow, sources, sourceFields,sourceFieldNameMap, ':', 'NCBI_Genbank_source_modifiers') 
          outputRow.push(value);
        }
        outputMatrix.push(outputRow);
      }
